...
class TC_FANCONTROL(MatterBaseTest):
    ...
    async def _read_fan_control_attribute(self, endpoint, attribute):
        return await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=Clusters.Objects.FanControl, attribute=attribute
        )
    ...
    async def test_TC_FANCONTROL(self):
        ...
        await self.send_single_cmd(
            cmd=Clusters.Objects.FanControl.Commands.Step(
                direction=Clusters.Objects.FanControl.Enums.StepDirectionEnum.kIncrease, wrap=False, lowestOff=False
            ),
            endpoint=endpoint,
        )
        ...
        await self.send_single_cmd(
            cmd=Clusters.Objects.FanControl.Commands.Step(
                direction=Clusters.Objects.FanControl.Enums.StepDirectionEnum.kDecrease, wrap=False, lowestOff=False
            ),
            endpoint=endpoint,
        )
