...
class TC_WATERHEATER(MatterBaseTest):
    ...
    async def _read_identify_time(self):
        return await self.read_single_attribute_check_success(
            endpoint=self.ENDPOINT, cluster=Clusters.Objects.Identify, attribute=Clusters.Objects.Identify.Attributes.IdentifyTime
        )
    ...
    async def thermostat_test(self, device):
        ...
        asserts.assert_equal(
            val,
            self.INITIAL_OCCUPIED_HEATING_SETPOINT,
        ...
        asserts.assert_equal(
            val,
            cluster.Bitmaps.RelayStateBitmap.kHeat,
    ...
    async def water_heater_management_test(self, device):
        ...
        expected_heater_types = (
            cluster.Bitmaps.WaterHeaterHeatSourceBitmap.kImmersionElement1 | cluster.Bitmaps.WaterHeaterHeatSourceBitmap.kHeatPump
        )
        val = await self.read_single_attribute_check_success(
        ...
        try:
            await self.send_single_cmd(
                cmd=cluster.Commands.Boost(
                    boostInfo=cluster.Structs.WaterHeaterBoostInfoStruct(duration=self.MIN_BOOST_DURATION - 1)
                ),
                endpoint=self.ENDPOINT,
            )
        ...
        await self.send_single_cmd(cmd=cluster.Commands.CancelBoost(), endpoint=self.ENDPOINT)
        ...
        asserts.assert_equal(
            val, cluster.Enums.BoostStateEnum.kInactive, "BoostState should be kInactive after CancelBoost command"
        )
        ...
        await self.write_single_attribute(
            attribute_value=Clusters.Objects.Thermostat.Attributes.OccupiedHeatingSetpoint(2500), endpoint_id=self.ENDPOINT
        )
    ...
    async def water_heater_mode_test(self):
        ...
        await self.send_single_cmd(cmd=cluster.Commands.ChangeToMode(newMode=1), endpoint=self.ENDPOINT)
        ...
        await self.send_single_cmd(cmd=cluster.Commands.ChangeToMode(newMode=0), endpoint=self.ENDPOINT)
        ...
        response = await self.send_single_cmd(cmd=cluster.Commands.ChangeToMode(newMode=2), endpoint=self.ENDPOINT)
