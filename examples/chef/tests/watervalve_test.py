...
class TC_WATERVALVE(MatterBaseTest):
    ...
    async def valve_configuration_and_control_test(self, device):
        ...
        asserts.assert_equal(
            ...
            NullValue,
            "RemainingDuration should be NULL",
        )
        asserts.assert_equal(
            await self.read_single_attribute_check_success(
        ...
        asserts.assert_equal(
            ...
            NullValue,
            "TargetLevel should be NULL",
        )

        # 4.2.1 (again): Send an Open command with openDuration=15 and targetLevel=50.
        await self.send_single_cmd(cmd=cluster.Commands.Open(openDuration=15, targetLevel=50), endpoint=self.ENDPOINT)
        ...
        await self.send_single_cmd(cmd=cluster.Commands.Close(), endpoint=self.ENDPOINT)
        ...
        asserts.assert_equal(
            ...
            NullValue,
            "OpenDuration should be NULL after Close",
        )
        asserts.assert_equal(
            await self.read_single_attribute_check_success(
                endpoint=self.ENDPOINT, cluster=cluster, attribute=attributes.RemainingDuration
            ...
            NullValue,
            "RemainingDuration should be NULL after Close",
        )
    ...
    async def test_TC_WATERVALVE(self):
        ...
        await self.send_single_cmd(cmd=Clusters.Objects.Identify.Commands.Identify(identifyTime=5), endpoint=self.ENDPOINT)
