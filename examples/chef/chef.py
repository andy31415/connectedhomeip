...
_DEVICE_LIST = [file[:-4] for file in os.listdir(_DEVICE_FOLDER) if file.endswith(".zap") and file != "template.zap"]
...
def load_config() -> None:
    ...
    if os.path.exists(configFile):
        ...
        flush_print(
            "Running for the first time and configuring config.yaml. "
        ...
        config["nrfconnect"]["ZEPHYR_SDK_INSTALL_DIR"] = os.environ.get("ZEPHYR_SDK_INSTALL_DIR")
        ...
        config["telink"]["ZEPHYR_SDK_INSTALL_DIR"] = os.environ.get("TELINK_ZEPHYR_SDK_DIR")
...
def check_python_version() -> None:
    if sys.version_info[0] < 3:
        flush_print("Must use Python 3. Current version is " + str(sys.version_info[0]))
        exit(1)
...
def flush_print(to_print: str, with_border: bool = False) -> None:
...
def bundle(platform: str, device_name: str) -> None:
    ...
    metadata_dest = os.path.join(_CD_STAGING_DIR, os.path.basename(metadata_file))
...
def bundle_linux(device_name: str) -> None:
    linux_root = os.path.join(_CHEF_SCRIPT_PATH, "linux", "out")
...
def bundle_nrfconnect(device_name: str) -> None:
    ...
    script_files = ["firmware_utils.py", "nrfconnect_firmware_utils.py"]
    nrf_root = os.path.join(_CHEF_SCRIPT_PATH, "nrfconnect", "build", "nrfconnect", "zephyr")
    scripts_root = os.path.join(_REPO_BASE_PATH, "scripts", "flashing")
    gen_script_path = os.path.join(scripts_root, "gen_flashing_script.py")
...
def bundle_esp32(device_name: str) -> None:
    ...
    """
    esp_root = os.path.join(_CHEF_SCRIPT_PATH, "esp32", "build")
    manifest_file = os.path.join(esp_root, "chip-shell.flashbundle.txt")
...
def bundle_telink(device_name: str) -> None:
    ...
    telink_root = os.path.join(_CHEF_SCRIPT_PATH, "telink", "build", "zephyr")
    sub_dir = os.path.join(_CD_STAGING_DIR, device_name)
    os.mkdir(sub_dir)
    for zephyr_ext in zephyr_exts:
        input_base = f"zephyr.{zephyr_ext}"
...
def main() -> int:
    ...
    parser.add_option(
        "-u", "--update_toolchain", help="updates toolchain & installs zap", action="store_true", dest="do_update_toolchain"
    )
    parser.add_option("-b", "--build", help="builds", action="store_true", dest="do_build")
    parser.add_option("-c", "--clean", help="clean build. Only valid if also building", action="store_true", dest="do_clean")
    parser.add_option("-f", "--flash", help="flashes device", action="store_true", dest="do_flash")
    parser.add_option(
        "-e", "--erase", help="erases flash before flashing. Only valid if also flashing", action="store_true", dest="do_erase"
    )
    parser.add_option(
        "-i", "--terminal", help="opens terminal to interact with with device", action="store_true", dest="do_interact"
    )
    parser.add_option(
        "-I",
        "--enable_lit_icd",
        help="enable LIT ICD (Long Idle Time Intermittently Connected Device) mode",
        action="store_true",
        default=False,
    )
    parser.add_option(
        "-m", "--menuconfig", help="runs menuconfig on platforms that support it", action="store_true", dest="do_menuconfig"
    )
    parser.add_option(
        "-z", "--zap", help="runs zap to generate data model & interaction model artifacts", action="store_true", dest="do_run_zap"
    )
    parser.add_option(
        "-g", "--zapgui", help="runs zap GUI display to allow editing of data model", action="store_true", dest="do_run_gui"
    )
    parser.add_option(
        "-d",
        "--device",
        dest="sample_device_type_name",
        help="specifies device type. Default is lighting. See info above for supported device types",
        metavar="TARGET",
        choices=_DEVICE_LIST,
    )
    parser.add_option(
        "-t",
        "--target",
        ...
        choices=["nrfconnect", "esp32", "linux", "silabs-thread", "ameba", "telink"],
        metavar="TARGET",
        default="linux",
    )
    parser.add_option(
        ...
        "--rpc",
        help=(
            "enables Pigweed RPC interface. Enabling RPC disables the shell interface. "
    ...
    parser.add_option(
        "-a",
        "--automated_test_stamp",
        help='provide the additional stamp "branch:commit_id" as the software version string for automated tests.',
        action="store_true",
        dest="do_automated_test_stamp",
    )
    parser.add_option(
        "-v", "--vid", dest="vid", type=int, help="specifies the Vendor ID. Default is 0xFFF1", metavar="VID", default=0xFFF1
    )
    parser.add_option(
        "-p", "--pid", dest="pid", type=int, help="specifies the Product ID. Default is 0x8000", metavar="PID", default=0x8000
    )
    parser.add_option(
        "-P",
        "--pname",
        ...
        help="specifies the Product Name. Default is TEST_PRODUCT",
        default="TEST_PRODUCT",
    )
    parser.add_option("", "--rpc_console", help="Opens PW RPC Console", action="store_true", dest="do_rpc_console")
    parser.add_option(
        "-y",
        "--tty",
    ...
    parser.add_option(
        "",
        "--build_all",
        help=("For use in CD only. Builds and bundles all chef examples for the specified platform. Chef exits after completion."),
        dest="build_all",
        action="store_true",
    )
    parser.add_option(
        "",
        "--dry_run",
        help="Display list of target builds of the --build_all command without building them.",
        dest="dry_run",
        action="store_true",
    )
    parser.add_option(
        ...
        "--build_exclude",
        help=(
            "For use with --build_all. Build labels to exclude. Accepts a regex pattern. Mutually exclusive with --build_include."
        ),
        dest="build_exclude",
    )
    parser.add_option(
        ...
        "--build_include",
        help=(
            "For use with --build_all. Build labels to include. Accepts a regex pattern. Mutually exclusive with --build_exclude."
        ),
        dest="build_include",
    )
    parser.add_option(
    ...
    parser.add_option(
        "",
        "--ci",
        help=("Builds Chef examples defined in cicd_config. Uses specified target from -t. Chef exits after completion."),
        dest="ci",
        action="store_true",
    )
    parser.add_option(
        ...
        "--ci_linux",
        help=(
            "Builds Chef Examples defined in cicd_config under ci_allow_list_linux. "
            "Devices are built without -c for faster compilation."
        ),
        dest="ci_linux",
        action="store_true",
    )
    parser.add_option("", "--cpu_type", help="CPU type to compile for. Linux only.", choices=["arm64", "arm", "x64"])
    parser.add_option(
        "",
        "--enable_ipv4",
    ...
    parser.add_option(
        "",
        "--icd_persist_subscription",
        help="Enable ICD persistent subscription and re-establish subscriptions from the server side after reboot",
        action="store_true",
        default=False,
    )
    parser.add_option(
        "", "--icd_subscription_resumption", help="Enable subscription resumption after timeout", action="store_true", default=False
    )
    ...
    if options.build_all:
        if options.build_include and options.build_exclude:
            flush_print("Error. --build_include and --build_exclude are mutually exclusive options.")
            exit(1)
    ...
    global gen_dir
    gen_dir = f"{_CHEF_SCRIPT_PATH}/out/{options.sample_device_type_name}/zap-generated/"
    ...
    if options.build_target == "esp32":
        ...
        if os.path.exists(os.path.join(ncs_toolchain_dir, "manifest.json")):
            ...
            shell.run_cmd(f"export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:{ncs_toolchain_dir}/usr/local/lib")
        ...
        if "SILABS_BOARD" not in config["silabs-thread"] or config["silabs-thread"]["SILABS_BOARD"] is None:
            flush_print("SILABS_BOARD was not configured. Make sure silabs-thread.SILABS_BOARD is set on your config.yaml file")
            exit(1)
        ...
        if config["telink"]["ZEPHYR_BASE"] is None:
            flush_print("Path for Telink SDK was not found. Make sure Telink_SDK is set on your config.yaml file")
            exit(1)
        if config["telink"]["ZEPHYR_SDK_INSTALL_DIR"] is None:
            flush_print("Path for Telink toolchain was not found. Make sure Telink toolchain is set on your config.yaml file")
            exit(1)
        shell.run_cmd("export ZEPHYR_TOOLCHAIN_VARIANT=zephyr")
        shell.run_cmd(f"export ZEPHYR_SDK_INSTALL_DIR={config['telink']['ZEPHYR_SDK_INSTALL_DIR']}")
        shell.run_cmd(f"export ZEPHYR_BASE={config['telink']['ZEPHYR_BASE']}")
        shell.run_cmd(f"source {config['telink']['ZEPHYR_BASE']}/zephyr-env.sh")
    else:
    ...
    if options.do_update_toolchain:
        if options.build_target == "esp32":
            ...
            shell.run_cmd(f"cd {_REPO_BASE_PATH} && python3 scripts/setup/nrfconnect/update_ncs.py --update")
        elif options.build_target == "silabs-thread":
    ...
    if options.do_run_gui:
        ...
        shell.run_cmd(f"{_REPO_BASE_PATH}/scripts/tools/zap/run_zaptool.sh {options.sample_device_type_name}.zap")
    ...
    if options.do_rpc:
        ...
        if options.build_target == "esp32":
            shell.run_cmd(f"export SDKCONFIG_DEFAULTS={_CHEF_SCRIPT_PATH}/esp32/sdkconfig_rpc.defaults")
            shell.run_cmd(
        ...
        if options.build_target == "esp32":
            shell.run_cmd(f"export SDKCONFIG_DEFAULTS={_CHEF_SCRIPT_PATH}/esp32/sdkconfig.defaults")
            shell.run_cmd(
                f"[ -f {_CHEF_SCRIPT_PATH}/esp32/sdkconfig ] || cp "
    ...
    if options.do_build:
        ...
        flush_print(f"Product ID 0x{options.pid:02X} / Vendor ID 0x{options.vid:02X}")
        ...
        if options.build_target == "esp32":
            ...
            if options.enable_ipv4:
                if sys.platform == "darwin":
                    shell.run_cmd("sed -i '' 's/CONFIG_DISABLE_IPV4=y/#\\ CONFIG_DISABLE_IPV4\\ is\\ not\\ set/g' sdkconfig ")
                else:
                    shell.run_cmd("sed -i 's/CONFIG_DISABLE_IPV4=y/#\\ CONFIG_DISABLE_IPV4\\ is\\ not\\ set/g' sdkconfig ")
            else:
                if sys.platform == "darwin":
                    shell.run_cmd("sed -i '' 's/#\\ CONFIG_DISABLE_IPV4\\ is\\ not\\ set/CONFIG_DISABLE_IPV4=y/g' sdkconfig ")
                    shell.run_cmd("sed -i '' 's/CONFIG_LWIP_IPV4=y/#\\ CONFIG_LWIP_IPV4\\ is\\ not\\ set/g' sdkconfig ")
                else:
                    shell.run_cmd("sed -i 's/#\\ CONFIG_DISABLE_IPV4\\ is\\ not\\ set/CONFIG_DISABLE_IPV4=y/g' sdkconfig ")
                    shell.run_cmd("sed -i 's/CONFIG_LWIP_IPV4=y/#\\ CONFIG_LWIP_IPV4\\ is\\ not\\ set/g' sdkconfig ")
            ...
            shell.run_cmd(f"cp build/$(git rev-parse HEAD)-{options.sample_device_type_name}.tar.xz {_CHEF_SCRIPT_PATH}")
        elif options.build_target == "nrfconnect":
            shell.run_cmd(f"cd {_CHEF_SCRIPT_PATH}/nrfconnect")
            ...
            nrf_build_cmds.append(f"-DCONFIG_CHEF_DEVICE_TYPE='\"{options.sample_device_type_name}\"'")
            nrf_build_cmds.append("-DCONFIG_OPENTHREAD_NORDIC_LIBRARY_MTD=y")
            if options.enable_lit_icd or re.search(_ICD_DEVICE_PATTERN, options.sample_device_type_name):
                nrf_build_cmds.append("-DCONFIG_CHIP_ENABLE_ICD_SUPPORT=y")
                nrf_build_cmds.append("-DCONFIG_CHIP_ENABLE_READ_CLIENT=y")
                nrf_build_cmds.append("-DCONFIG_CHIP_ICD_LIT_SUPPORT=y")
                nrf_build_cmds.append("-DCONFIG_CHIP_ICD_CHECK_IN_SUPPORT=y")
                nrf_build_cmds.append("-DCONFIG_CHIP_ICD_UAT_SUPPORT=y")
                nrf_build_cmds.append("-DCONFIG_CHIP_ICD_DSLS_SUPPORT=y")
                nrf_build_cmds.append("-DCONFIG_CHIP_ICD_REPORT_ON_ACTIVE_MODE=y")
                nrf_build_cmds.append("-DCONFIG_CHIP_ICD_SIT_SLOW_POLL_LIMIT=5000")
            else:
                nrf_build_cmds.append("-DCONFIG_CHIP_ENABLE_ICD_SUPPORT=n")
            ...
            efr32_cmd_args = []
            efr32_cmd_args.append(f"{_REPO_BASE_PATH}/scripts/examples/gn_silabs_example.sh")
            ...
            if "aarch" not in uname_resp and "arm" not in uname_resp:
                if options.cpu_type == "arm64":
                    if "SYSROOT_AARCH64" not in shell.env:
                        flush_print("SYSROOT_AARCH64 env variable not set. AARCH64 toolchain needed for cross-compiling for arm64.")
                        exit(1)
                    shell.env["PKG_CONFIG_PATH"] = f"{shell.env['SYSROOT_AARCH64']}/lib/aarch64-linux-gnu/pkgconfig"
                    ...
                    if "SYSROOT_ARMHF" not in shell.env:
                        flush_print("SYSROOT_ARMHF env variable not set. ARMHF toolchain needed for cross-compiling for arm.")
                        exit(1)
                    shell.env["PKG_CONFIG_PATH"] = f"{shell.env['SYSROOT_ARMHF']}/lib/arm-linux-gnueabihf/pkgconfig"
            ...
            if sw_ver_string:
                linux_args.append(f'chip_device_config_device_software_version_string = "{sw_ver_string}"')
    ...
    if options.do_flash:
        ...
        if options.build_target == "esp32":
            ...
            shell.run_cmd(f"cd {_CHEF_SCRIPT_PATH}/esp32")
            if options.do_erase:
                shell.run_cmd(f"idf.py -p {config['esp32']['TTY']} erase-flash")
            ...
            shell.run_cmd(f"python3 out/{options.sample_device_type_name}/{silabs_board}/matter-silabs-chef-example.flash.py")
    ...
    if options.do_interact:
        ...
        if options.build_target == "esp32":
            ...
            flush_print(f"{_CHEF_SCRIPT_PATH}/linux/out/{options.sample_device_type_name}")
    ...
    if options.do_rpc_console:
        if options.build_target == "esp32":
            shell.run_cmd(f"python3 -m chip_rpc.console --device {config['esp32']['TTY']}")
        elif options.build_target == "silabs-thread":
