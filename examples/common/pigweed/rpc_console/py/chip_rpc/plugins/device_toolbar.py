...
class DeviceToolbar(WindowPaneToolbar, PluginMixin):
    ...
    def _format_field(self, name, value):
        return [
            ("class:theme-bg-active class:theme-fg-active", "{}:".format(name)),
    ...
    def _update_toolbar_text(self):
        ...
        if not status.ok():
            self.formatted_text = [self._format_field("ERROR GetDeviceInfo", status.name)]
            return
        ...
        if not status.ok():
            self.formatted_text = [self._format_field("ERROR GetPairingState", status.name)]
            return

        if pairing:
            self.formatted_state = ("class:theme-fg-blue", "PAIRING")
            tokens.extend(self._format_field("Pairing Code", device_info.pairing_info.code))
            tokens.extend(self._format_field("Pairing Discriminator", device_info.pairing_info.discriminator))
        else:
            status, device_state = self.rpcs.chip.rpc.Device.GetDeviceState()
            if not status.ok():
                self.formatted_text = [self._format_field("ERROR GetDeviceState", status.name)]
                return
            ...
            if len(device_state.fabric_info) == 0 or device_state.fabric_info[0].node_id == 0:
                ...
                tokens.extend(self._format_field("Fabric", device_info.fabric_state[0].fabric_id))
                tokens.extend(self._format_field("Node", device_info.fabric_state[0].node_id))
    ...
    def __init__(self, rpcs, *args, **kwargs):
        super().__init__(*args, center_section_align=WindowAlign.RIGHT, **kwargs)

        self.rpcs = rpcs

        self.show_toolbar = True
        self.formatted_text = []
        self.formatted_state = ("class:theme-fg-blue", "MATTER Toolbar")

        # Buttons for display in the center
        self.add_button(ToolbarButton(description="Refresh", mouse_handler=self._update_toolbar_text))
