...
def get_identity():
    ...
    if failure_str in command_result:
        print("No valid identity has been found. Application will run without entitlements.")
        exit(0)
...
def codesign(args):
    command = "codesign --force -d --sign {identity} {target}".format(identity=get_identity(), target=args.target_path)
    command_result = run_command(command)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Codesign the darwin-framework-tool binary")
