...
{
    if (IsEnabled(Attributes::CurrentHeapHighWatermark::Id, mEnabledAttributes) &&
...
CHIP_ERROR SoftwareDiagnosticsLogic::Attributes(ReadOnlyBufferBuilder<DataModel::AttributeEntry> & builder)
{
    ...
    return listBuilder.Append(Span<const DataModel::AttributeEntry>() /* mandatory */, mEnabledAttributes);
}
