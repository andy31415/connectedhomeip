...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "../controller/python"))
...
DIGEST_ALL_ALGORITHMS = hashlib.algorithms_available.intersection(DIGEST_ALGORITHM_ID.keys())
...
def generate_header(header_tlv: bytes, payload_size: int):
    ...
    fixed_header = struct.pack(
        FIXED_HEADER_FORMAT, HEADER_MAGIC, struct.calcsize(FIXED_HEADER_FORMAT) + len(header_tlv) + payload_size, len(header_tlv)
    )
...
def main():
    ...
    create_parser.add_argument("-da", "--digest-algorithm", choices=DIGEST_ALL_ALGORITHMS, required=True, help="Digest algorithm")
    ...
    create_parser.add_argument("input_files", nargs="+", help="Path to input image payload file")
    ...
    change_tool.add_argument("-da", "--digest-algorithm", choices=DIGEST_ALL_ALGORITHMS, help="Digest algorithm")
