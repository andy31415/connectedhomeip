...
qrcode_format = BitStruct(
    ...
    "flow" / Enum(BitsInteger(2), Standard=0, UserIntent=1, Custom=2),
    "pid" / BitsInteger(16),
    "vid" / BitsInteger(16),
    ...
    def from_container(container, is_qrcode):
        ...
        if is_qrcode:
            payload = SetupPayload(
                ...
                container["pincode"],
                container["discovery"],
                CommissioningFlow(container["flow"].__int__()),
                container["vid"],
                container["pid"],
            )
        else:
...
@click.option(
    "--discovery-cap-bitmask",
    "-dm",
    type=click.IntRange(0, 7),
    default=4,
    help="Commissionable device discovery capability bitmask. 0:SoftAP, 1:BLE, 2:OnNetwork. Default: OnNetwork",
)
@click.option(
    "--commissioning-flow",
    "-cf",
    ...
    default=0,
    help="Commissioning flow, 0:Standard, 1:User-Intent, 2:Custom",
)
