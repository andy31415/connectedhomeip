...
def cert_type_as_str(cert_type: CertType) -> str:
    type_strs = {CertType.NOC: "NOC", CertType.ICAC: "ICAC", CertType.RCAC: "RCAC"}
...
class OpCertBuilder:
    ...
    def make_certs_and_keys(self) -> None:
        ...
        cmd = (
            ...
            + format_flag
            + " -o "
            + self.own.cert_file_name
            + " -O "
            + self.own.key_file_name
        )
        subprocess.run(cmd, shell=True)
    ...
    with open(args.outfile + ".h", "w") as hfile, open(args.outfile + ".cpp", "w") as cfile:
        ...
        for test_case in DER_CERT_ERROR_TEST_CASES:
            for cert_type in [CertType.NOC, CertType.ICAC, CertType.RCAC]:
                ...
                if (
                    test_case["error_flag"] == "subject-node-id-invalid"
                    or test_case["error_flag"] == "ext-basic-pathlen-presence-wrong"
        ...
        for test_case in CHIP_TLV_CERT_ERROR_TEST_CASES:
            for cert_type in [CertType.NOC, CertType.ICAC, CertType.RCAC]:
                if not (
                    test_case["is_chip_to_x509_expected_to_fail"]
                    or test_case["is_chip_cert_load_expected_to_fail"]
                    or test_case["is_validate_chip_rcac_expected_to_fail"]
                    or test_case["is_get_cert_type_expected_to_fail"]
                ):
                    break

                # The following error cases are applicable only for NOC
                if (
                    test_case["error_flag"] == "subject-node-id-invalid" or test_case["error_flag"] == "subject-fabric-id-missing"
                ) and cert_type != CertType.NOC:
                    break
                ...
                if test_case["is_get_cert_type_expected_to_fail"] and not (
                    test_case["error_flag"] == "subject-cat-twice" and cert_type == CertType.NOC
                ):
