...
class Names:
    def __init__(self, cert_type: CertType, test_dir, dev_dir, vid, pid):
        ...
        prefixes = {
            CertType.PAA: test_dir + paa_file_prefix,
        ...
        array_names_prefix = {CertType.PAA: "kTestPAA_", CertType.PAI: "kDevelopmentPAI_", CertType.DAC: "kDevelopmentDAC_"}
        array_names_suffix = {CertType.PAA: "", CertType.PAI: "_{:X}".format(vid), CertType.DAC: "_{:X}_{:X}".format(vid, pid)}
        generic_prefix = {CertType.PAA: "kPaa", CertType.PAI: "kPai", CertType.DAC: "kDac"}
...
class DevCertBuilder:
    ...
    def full_arrays(self) -> tuple[str, str]:
        ...
        h_pub = "extern const uint8_t " + self.own.public_key_array_name + "[65];\n"
        h_priv = "extern const uint8_t " + self.own.private_key_array_name + "[32];\n"

        c = cert + make_c_array(pub, self.own.public_key_array_name, 17) + make_c_array(priv, self.own.private_key_array_name, 16)
    ...
    def generic_arrays(self) -> tuple[str, str]:
        ...
        cert = c_start + "Cert       = ByteSpan(" + self.own.cert_array_name + ");\n"
        priv = c_start + "PrivateKey = ByteSpan(" + self.own.private_key_array_name + ");\n"
        publ = c_start + "PublicKey  = ByteSpan(" + self.own.public_key_array_name + ");\n"
        c = cert + priv + publ
        return [h, c]
    ...
    def headers(self, filename) -> tuple[str, str]:
        ...
        c_top = copyrightNotice + '#include "' + filename + '.h"\n\n' + includes + namespaces
        return [h_top, c_top]
...
def main():
    ...
    argparser.add_argument(
        "-t", "--test_dir", dest="testdir", default="credentials/test/attestation/", help="directory holding test PAA"
    )
    argparser.add_argument(
        "-o", "--out_dir", dest="outdir", default="src/credentials/examples/", help="Output directory for array files"
    )
    argparser.add_argument(
        "-c",
        "--chip-cert_dir",
        dest="chipcertdir",
        default="out/debug/standalone/",
        help="Directory where chip-cert tool is located",
    )
    argparser.add_argument(
        "--paiout", dest="paiout", default="ExamplePAI", help="output file name for PAI C arrays (no .h or .cpp extension)"
    )
    argparser.add_argument(
        "--dacout", dest="dacout", default="ExampleDACs", help="output file name for PAI C arrays (no .h or .cpp extension)"
    )

    args = argparser.parse_args()
    builder = DevCertBuilder(CertType.PAI, args.certdir, args.testdir, args.chipcertdir, 0xFFF1, 0x8000)

    builder.make_certs_and_keys()
    [h_generic, c_generic] = builder.generic_arrays()
    [h_top, c_top] = builder.headers(args.paiout)
    footer = builder.footer()
    with open(args.outdir + args.paiout + ".h", "w") as hfile, open(args.outdir + args.paiout + ".cpp", "w") as cfile:
        hfile.write(h_top)
        hfile.write(h_generic)
        cfile.write(c_top)

        for vid in range(0xFFF1, 0xFFF4):
            builder = DevCertBuilder(CertType.PAI, args.certdir, args.testdir, args.chipcertdir, vid, 0x8000)
            builder.make_certs_and_keys()
    ...
    with open(args.outdir + args.dacout + ".h", "w") as hfile, open(args.outdir + args.dacout + ".cpp", "w") as cfile:
        builder = DevCertBuilder(CertType.DAC, args.certdir, args.testdir, args.chipcertdir, 0xFFF1, 0x8000)
        ...
        for vid in range(0xFFF1, 0xFFF4):
            for pid in range(0x8000, 0x8020):
                builder = DevCertBuilder(CertType.DAC, args.certdir, args.testdir, args.chipcertdir, vid, pid)
                builder.make_certs_and_keys()
                [h, c] = builder.full_arrays()
                [h_generic, c_generic] = builder.generic_arrays()

                define = (
                    "#if CHIP_DEVICE_CONFIG_DEVICE_VENDOR_ID == 0x{:X} && CHIP_DEVICE_CONFIG_DEVICE_PRODUCT_ID == 0x{:X}\n".format(
                        vid, pid
                    )
                )
                end = "#endif\n"
