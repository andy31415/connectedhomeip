...
class Names:
    def __init__(self, cert_type: CertType, paa_path, test_case_out_dir):
        prefixes = {CertType.PAA: paa_path, CertType.PAI: test_case_out_dir + "/pai-", CertType.DAC: test_case_out_dir + "/dac-"}
...
class DevCertBuilder:
    def __init__(
        ...
        error_type: str,
        paa_path: str,
        test_case_out_dir: str,
        chip_cert: str,
        vid: int,
        pid: int,
        custom_cn_attribute: str,
        valid_from: str,
    ):
        self.vid = vid
        self.pid = pid
    ...
    def make_certs_and_keys(self) -> None:
        ...
        cmd = "openssl x509 -inform pem -in " + self.own.cert_pem + " -out " + self.own.cert_der + " -outform DER"
        subprocess.run(cmd, shell=True)
        cmd = "openssl ec -inform pem -in " + self.own.key_pem + " -out " + self.own.key_der + " -outform DER"
        subprocess.run(cmd, shell=True)
...
def generate_test_case_vector_json(test_case_out_dir: str, test_cert: str, test_case, basic_info_pid: int):
    ...
    if "is_success_case" in test_case:
        ...
        if (test_cert == "pai") and (test_case["test_folder"] in ["ext_basic_pathlen0", "ext_key_usage_dig_sig_wrong"]):
    ...
    files_to_add = {
        key: os.path.join(test_case_out_dir, filename) for key, filename in files_to_add.items() if filename in files_available
    }
...
def main():
    ...
    argparser.add_argument(
        "-p", "--paa", dest="paapath", default="credentials/test/attestation/Chip-Test-PAA-FFF1-", help="PAA to use"
    )
    argparser.add_argument(
        "--paa_different_origin",
        dest="paapath_different_origin",
    ...
    argparser.add_argument(
        ...
        "--chip-cert-dir",
        dest="chipcertdir",
        default="out/debug/linux_x64_clang/",
        help="Directory where chip-cert tool is located",
    )
    ...
    for test_cert in ["dac", "pai"]:
        for test_case in CERT_STRUCT_TEST_CASES:
            ...
            builder = DevCertBuilder(
                CertType.PAI, error_type_pai, args.paapath, test_case_out_dir, chipcert, vid, PID_NOT_PRESENT, "", pai_valid_from
            )
            ...
            builder = DevCertBuilder(
                CertType.DAC, error_type_dac, args.paapath, test_case_out_dir, chipcert, vid, pid, "", dac_valid_from
            )
            builder.make_certs_and_keys()
    ...
    for test_cert in ["dac", "pai"]:
        for test_case in VIDPID_FALLBACK_ENCODING_TEST_CASES:
            ...
            # Generate PAI Cert/Key
            builder = DevCertBuilder(
                CertType.PAI, "no-error", args.paapath, test_case_out_dir, chipcert, vid_pai, pid_pai, common_name_pai, ""
            )
            builder.make_certs_and_keys()

            # Generate DAC Cert/Key
            builder = DevCertBuilder(
                CertType.DAC, "no-error", args.paapath, test_case_out_dir, chipcert, vid_dac, pid_dac, common_name_dac, ""
            )
            builder.make_certs_and_keys()
    ...
    for test_case in CD_STRUCT_TEST_CASES:
        ...
        builder = DevCertBuilder(CertType.PAI, "no-error", paapath, test_case_out_dir, chipcert, dac_vid, dac_pid, "", "")
        builder.make_certs_and_keys()

        # Generate DAC Cert/Key
        builder = DevCertBuilder(CertType.DAC, "no-error", paapath, test_case_out_dir, chipcert, dac_vid, dac_pid, "", "")
        builder.make_certs_and_keys()
        ...
        if (
            test_case["error_flag"] == "authorized-paa-list-count0"
            or test_case["error_flag"] == "authorized-paa-list-count1-valid"
        ...
        cmd = (
            ...
            + dac_origin_flag
            + authorized_paa_flag
            + ' -d 0x1234 -c "ZIG20141ZB330001-24" -l 0 -i 0 -n 9876 -t '
            + str(certification_type)
        )
        subprocess.run(cmd, shell=True)
    ...
    cmd = (
        chipcert
        + ' gen-att-cert -t a -c "Invalid (Not Registered in the DCL) Matter PAA" -f "'
        + VALID_IN_PAST
        + '" -l 4294967295 -o '
        + paapath
        + "Cert.pem -O "
    ...
    # Generate PAI Cert/Key
    builder = DevCertBuilder(
        CertType.PAI, test_case["error_flag"], paapath, test_case_out_dir, chipcert, vid, PID_NOT_PRESENT, "", VALID_IN_PAST
    )
    builder.make_certs_and_keys()

    # Generate DAC Cert/Key
    builder = DevCertBuilder(
        CertType.DAC, test_case["error_flag"], paapath, test_case_out_dir, chipcert, vid, pid, "", VALID_IN_PAST
    )
    builder.make_certs_and_keys()
