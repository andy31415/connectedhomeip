    ...
    def run_controller_test(self):
        ...
        for server in servers:
            self.execute_device_cmd(
                server["id"],
                'CHIPCirqueDaemon.py -- run gdb -return-child-result -q -ex "set pagination off" -ex run -ex "bt 25" --args {} --thread --discriminator {}'.format(
        ...
        command = "gdb -return-child-result -q -ex run -ex bt --args python3 {} -t 150 -d {} --paa-trust-store-path {} --nodeid {}".format(
            os.path.join(CHIP_REPO, "src/controller/python/tests/scripts/python_commissioning_flow_test.py"),
            TEST_DISCRIMINATOR,
