...
class TestCommissioner(CHIPVirtualHome):
    ...
    def run_controller_test(self):
        servers = [
            {"ip": device["description"]["ipv6_addr"], "id": device["id"]}
        ...
        for server in servers:
            self.execute_device_cmd(
                ...
                ).format(os.path.join(CHIP_REPO, "out/debug/standalone/chip-all-clusters-app"), server["discriminator"]),
            )
        ...
        command = (
            ...
            "{} -t 150 --paa-trust-store-path {} --discriminator {} --setup-payload {} --nodeid {} --discovery-type {}"
        ).format(
            os.path.join(CHIP_REPO, "src/controller/python/tests/scripts/commissioning_test.py"),
