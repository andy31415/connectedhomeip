...
class TestSplitCommissioning(CHIPVirtualHome):
    ...
    def run_controller_test(self):
        ethernet_ips = [device["description"]["ipv6_addr"] for device in self.non_ap_devices if device["type"] == "CHIPEndDevice"]
        ...
        for server in server_ids:
            self.execute_device_cmd(
                ...
                (
                    ...
                    '"set pagination off" -ex run -ex "bt 25" --args {} --thread'
                ).format(os.path.join(CHIP_REPO, "out/debug/standalone/chip-all-clusters-app")),
            )
        ...
        command = (
            "gdb -return-child-result -q -ex run -ex bt --args python3 "
            "{} -t 150 --address1 {} --address2 {} --paa-trust-store-path {}"
        ).format(
            os.path.join(CHIP_REPO, "src/controller/python/tests/scripts/split_commissioning_test.py"),
            ethernet_ips[0],
            ethernet_ips[1],
