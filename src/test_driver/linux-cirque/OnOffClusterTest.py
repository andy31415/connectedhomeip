...
class TestOnOffCluster(CHIPVirtualHome):
    ...
    def run_data_model_test(self):
        ...
        server_ids = [device["id"] for device in self.non_ap_devices if device["type"] == "CHIP-Server"]
        tool_ids = [device["id"] for device in self.non_ap_devices if device["type"] == "CHIP-Tool"]
        ...
        for device_id in server_ids:
            self.execute_device_cmd(
                device_id,
                "CHIPCirqueDaemon.py -- run {} --thread".format(os.path.join(CHIP_REPO, "out/debug/standalone/chip-lighting-app")),
            )
        ...
        for ip in server_ip_address:
            ...
            self.assertEqual(ret["return_code"], "0", "{} command failure: {}".format("pairing softap", ret["output"]))
        ...
        for device_id in server_ids:
            ...
            self.assertTrue(
                self.sequenceMatch(
                    self.get_device_log(device_id).decode("utf-8"),
                    [
                        "Thread initialized.",
