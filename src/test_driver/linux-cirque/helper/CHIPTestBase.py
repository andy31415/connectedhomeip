...
class CHIPVirtualHome:
    ...
    def execute_device_cmd(self, device_id, cmd, stream=False):
        self.logger.info("device: {} exec: {}".format(self.get_device_pretty_id(device_id), cmd))
        ret = requests.get(
            self._build_request_url("device_cmd", [self.home_id, device_id, cmd]), params={"stream": stream}, stream=stream
        )
        if stream:
            return ret
        ...
        self.logger.info("command return code: {}".format(ret_struct.get("return_code", "Unknown")))
    ...
    def reset_thread_devices(self, devices: Union[List[str], str]):
        ...
        for device_id in devices:
            ...
            self.assertTrue(self.wait_for_device_output(device_id, "Thread interface: wpan0", 10))
            self.assertTrue(self.wait_for_device_output(device_id, "[SVR] Server Listening...", 15))
            # Clear default Thread network commissioning data
            self.logger.info("Resetting thread network on {}".format(self.get_device_pretty_id(device_id)))
    ...
    def form_thread_network(self, device_id: str, expected_role: Union[str, List[str]], timeout: int = 15, dataset: str = ""):
        """
    ...
    def connect_to_thread_network(self):
        ...
        for i in range(30):
            ...
            threadNetworkFormed = (roles.count("leader") == 1) and (
                roles.count("leader") + roles.count("router") + roles.count("child") == len(self.thread_devices)
            )
    ...
    def enable_wifi_on_device(self):
        ...
        for device in self.non_ap_devices:
            self.logger.info("device: {} connecting to desired ssid: {}".format(self.get_device_pretty_id(device["id"]), ssid))
    ...
    def get_device_thread_ip(self, device_id):
        ...
        for ipstr in ipaddr_list:
            try:
                self.logger.info("device: {} thread ip: {}".format(self.get_device_pretty_id(device_id), ipstr))
                ...
                if not ipaddr.is_private:
                    continue
                if (
                    re.match(("fd[0-9a-f]{2}:[0-9a-f]{4}:[0-9a-f]{4}:[0-9a-f]{4}:0000:00ff:fe00:[0-9a-f]{4}"), ipaddr.exploded)
                    is not None
                ):
                    continue
    ...
    def initialize_home(self):
        home_id = requests.post(self._build_request_url("create_home"), json=self.device_config).json()

        self.logger.info("home id: {} created!".format(home_id))

        self.assertTrue(home_id in list(self.query_api("get_homes")), "created home_id did not match id from get_homes!!")
        ...
        self.logger.info("home id: {} devices: {}".format(home_id, json.dumps(created_devices, indent=4, sort_keys=True)))
        ...
        self.assertEqual(device_types, wanted_device_types, "created device does not match to device config!!")

        self.device_config = created_devices

        self.device_ids = list(self.device_config)
        self.non_ap_devices = [device for device in self.device_config.values() if device["type"] != "wifi_ap"]
        self.thread_devices = [
            device for device in self.device_config.values() if device["capability"].get("Thread", None) is not None
        ]
        self.ap_devices = [device for device in self.device_config.values() if device["type"] == "wifi_ap"]
    ...
    def save_device_logs(self):
        ...
        for device in self.non_ap_devices:
            ...
            f_name = "{}-{}-{}.log".format(device["type"], timestamp, device["id"][:8])
            self.logger.debug("device log name: \n{}".format(f_name))
            with open(os.path.join(log_dir, f_name), "wb") as fp:
                fp.write(ret_log)

    def start_wpa_supplicant(self, device_id):
        self.logger.info("device: {}: starting wpa_supplicant on device".format(self.get_device_pretty_id(device_id)))

        start_wpa_supplicant_command = "".join(
            ["wpa_supplicant -B -i wlan0 ", "-c /etc/wpa_supplicant/wpa_supplicant.conf ", "-f /var/log/wpa_supplicant.log -t -dd"]
        )
    ...
    def write_psk_to_wpa_supplicant_config(self, device_id, ssid, psk):
        self.logger.info("device: {}: writing ssid, psk to wpa_supplicant config".format(self.get_device_pretty_id(device_id)))

        write_psk_command = "".join(
            ["sh -c 'wpa_passphrase {} {} >> ".format(ssid, psk), "/etc/wpa_supplicant/wpa_supplicant.conf'"]
        )

        return self.execute_device_cmd(device_id, write_psk_command)

    def kill_existing_wpa_supplicant(self, device_id):
        self.logger.info("device: {}: kill existing wpa_supplicant".format(self.get_device_pretty_id(device_id)))
