...
class TestCommissioningWindow(CHIPVirtualHome):
    ...
    def run_controller_test(self):
        ...
        command = (
            "gdb -return-child-result -q -ex run -ex bt --args python3 {} -t 150 --address {} --paa-trust-store-path {}"
        ).format(
            os.path.join(CHIP_REPO, "src/controller/python/tests/scripts/commissioning_window_test.py"),
            servers[0]["ip"],
