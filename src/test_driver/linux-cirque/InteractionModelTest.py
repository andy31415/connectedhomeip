...
class TestInteractionModel(CHIPVirtualHome):
    ...
    def run_data_model_test(self):
        resp_ips = [device["description"]["ipv6_addr"] for device in self.non_ap_devices if device["type"] == "CHIP-IM-Responder"]
        resp_ids = [device["id"] for device in self.non_ap_devices if device["type"] == "CHIP-IM-Responder"]
        req_ids = [device["id"] for device in self.non_ap_devices if device["type"] == "CHIP-IM-Initiator"]
        ...
        for id in resp_ids:
            self.execute_device_cmd(
                id,
                "CHIPCirqueDaemon.py -- run gdb -batch -return-child-result -q -ex run -ex bt {}".format(
        ...
        command = (
            ...
            + os.path.join(CHIP_REPO, "out/debug/linux_x64_gcc/chip-im-initiator")
            + " {}"
        )

        for ip in resp_ips:
            ret = self.execute_device_cmd(req_device_id, command.format(ip))
            self.assertEqual(ret["return_code"], "0", "{} failure: {}".format("IM", ret["output"]))
