...
class TestCommissioningFailure(CHIPVirtualHome):
    ...
    def run_controller_test(self):
        ...
        for server in server_ids:
            self.execute_device_cmd(
                ...
                (
                    ...
                    '-ex run -ex "bt 25" --args {} --thread --discriminator {}'
                ).format(os.path.join(CHIP_REPO, "out/debug/standalone/chip-all-clusters-app"), TEST_DISCRIMINATOR),
            )
        ...
        command = (
            "gdb -return-child-result -q -ex run -ex bt --args python3 {} -t 150 -a {} --paa-trust-store-path {} --fail-on-report"
        ).format(
            os.path.join(CHIP_REPO, "src/controller/python/tests/scripts/commissioning_failure_test.py"),
            ethernet_ip,
