...
@click.option("--image", default=[], multiple=True, help="What images to execute (will be executed one after another)")
@click.option("--file-image-list", default=None, help="Read the images from the given file (contains images one per line)")
@click.option(
    "--qemu",
...
def main(log_level, log_timestamps, image, file_image_list, qemu, verbose):
    ...
    for path in image:
        ...
        status = subprocess.run(
            [qemu, "-nographic", "-no-reboot", "-machine", "esp32", "-drive", "file=%s,if=mtd,format=raw" % path],
            capture_output=True,
        )
