...
from TC_GC_common import (
    ...
    get_auxiliary_acl_equivalence_set,
    get_feature_map,
...
class TC_GC_2_4(MatterBaseTest):
    ...
    def steps_TC_GC_2_4(self):
        return [
            ...
            TestStep(
                "1c",
                "TH subscribes to Membership attribute and to the AuxiliaryAccessUpdated event of the AccessControl cluster with a min interval of 0s and max interval of 30s. Accumulate all reports seen.",
            ),
            ...
            TestStep(
                "1f", "TH awaits subscription report of AuxiliaryAccessUpdated event from AccessControl cluster (expect no report)"
            ),
            ...
            TestStep(
                "2a",
                "Completely Leave Group G2 by omitting the endpoint list parameters. LeaveGroup (GroupID=G2, Endpoints omitted)",
            ),
            ...
            TestStep(
                "4f",
                "TH awaits subscription report of new Membership within max interval. (If SD supported: G3 entry with empty endpoints list, else G3 entry removed)",
            ),
            ...
            TestStep(
                7, "TH awaits subscription report of AuxiliaryAccessUpdated event from AccessControl cluster (expect no report)"
            ),
    ...
    async def test_TC_GC_2_4(self):
        ...
        for endpoint in endpoints_list:
            asserts.assert_false(
                (fabric_index, groupID1, endpoint) in actual_set,
        ...
        await self.send_single_cmd(
            Clusters.Groupcast.Commands.JoinGroup(
                groupID=groupID2, endpoints=endpoints_list, keySetID=keySetID1, useAuxiliaryACL=True
            )
        )
        ...
        for endpoint in endpoints_list:
            asserts.assert_true(
                (fabric_index, groupID2, endpoint) in actual_set,
        ...
        if not ln_enabled:
            ...
            groupID3 = 3
            await self.send_single_cmd(
                Clusters.Groupcast.Commands.JoinGroup(groupID=groupID3, endpoints=endpoints_list, keySetID=keySetID1)
            )
            ...
            if len(endpoints_list) == 1:
                ...
                resp: Clusters.Groupcast.Commands.LeaveGroupResponse = await self.send_single_cmd(
                    Clusters.Groupcast.Commands.LeaveGroup(groupID=groupID3, endpoints=endpoint_2)
                )
                asserts.assert_is_not_none(resp.endpoints, "LeaveGroupResponse endpoints should not be None")
                asserts.assert_equal(
                    resp.endpoints,
            ...
            resp: Clusters.Groupcast.Commands.LeaveGroupResponse = await self.send_single_cmd(
                Clusters.Groupcast.Commands.LeaveGroup(groupID=groupID3, endpoints=endpoint_1)
            )
            asserts.assert_is_not_none(resp.endpoints, "LeaveGroupResponse endpoints should not be None")
            asserts.assert_equal(
                resp.endpoints, endpoint_1, f"LeaveGroupResponse cmd endpoints list {resp.endpoints} is not equal to {endpoint_1}"
            )
        ...
        try:
            ...
            asserts.assert_equal(
                e.status, Status.NotFound, f"Send LeaveGroup command error should be {Status.NotFound} instead of {e.status}"
            )
        ...
        asserts.assert_is_not_none(resp.endpoints, "LeaveGroupResponse endpoints should not be None")
        asserts.assert_equal(
            resp.endpoints, [], f"LeaveGroupResponse cmd endpoints list {resp.endpoints} is not equal to an empty list"
        )
