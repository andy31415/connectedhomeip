...
class TC_ICDM_3_2(MatterBaseTest):
    ...
    def steps_TC_ICDM_3_2(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "3a", "TH sends RegisterClient command with same CheckInNodeID1 as in Step 1a and different MonitorSubID2 and Key2."
            ),
            ...
            TestStep(
                "5a",
                "TH sends RegisterClient command with same CheckInNodeID1 as in Step 1a and different MonitorSubID4 and Key4, and a valid wrong VerificationKey4.",
            ),
            ...
            TestStep(
                "8b",
                "TH sends RegisterClient command with same CheckInNodeID5 as in Step 6a and different MonitorSubID6 and Key6, and an invalid VerificationKey6.",
            ),
            TestStep(
                "8c",
                "TH sends RegisterClient command with same CheckInNodeID5 as in Step 6a and different MonitorSubID7 and Key7, and an valid wrong VerificationKey7.",
            ),
            TestStep(
                "8d",
                "TH sends RegisterClient command with same CheckInNodeID5 and VerificationKey5 as in Step 6a and different MonitorSubID9 and Key9.",
            ),
    ...
    async def test_TC_ICDM_3_2(self):
        ...
        try:
            ...
            try:
                await self._send_single_icdm_command(
                    commands.RegisterClient(
                        checkInNodeID=client3.checkInNodeID,
                        monitoredSubject=client3.subjectId,
                        key=client3.key,
                        clientType=client3.clientType,
                        verificationKey=kInvalidKey,
                    )
                )
            except InteractionModelError as e:
                asserts.assert_equal(e.status, Status.Success, "Unexpected error returned")
            ...
            try:
                await self._send_single_icdm_command(
                    commands.RegisterClient(
                        checkInNodeID=client5.checkInNodeID,
                        monitoredSubject=client5.subjectId,
                        key=client5.key,
                        clientType=client5.clientType,
                    )
                )
            except InteractionModelError as e:
                asserts.assert_equal(e.status, Status.Success, "Unexpected error returned")
            ...
            try:
                await self._send_single_icdm_command(
                    commands.RegisterClient(
                        checkInNodeID=client7.checkInNodeID,
                        monitoredSubject=client7.subjectId,
                        key=client7.key,
                        clientType=client7.clientType,
                        verificationKey=kIncorrectKey,
                    )
                )
            except InteractionModelError as e:
