...
class TC_TSTAT_4_2(MatterBaseTest):
    def check_atomic_response(
        ...
        for attrStatus in response.attributeStatus:
            if attrStatus.attributeID == cluster.Attributes.Presets.attribute_id:
                asserts.assert_equal(attrStatus.statusCode, expected_preset_status, "Preset attribute should have the right status")
                found_preset_status = True
            if attrStatus.attributeID == cluster.Attributes.Schedules.attribute_id:
                asserts.assert_equal(
                    attrStatus.statusCode, expected_schedules_status, "Schedules attribute should have the right status"
                )
    ...
    def check_returned_presets(self, sent_presets: list, returned_presets: list):
        ...
        for i, sent_preset in enumerate(sent_presets):
            ...
            asserts.assert_equal(sent_preset, returned_preset, "Returned preset is not the same as sent preset")
    ...
    def get_available_scenario(self, presetTypes: list, presetScenarioCounts: map):
        availableScenarios = [
            presetType.presetScenario
            for presetType in presetTypes
            if presetScenarioCounts.get(presetType.presetScenario, 0) < presetType.numberOfPresets
        ]
    ...
    async def write_presets(
        self, endpoint, presets, dev_ctrl: ChipDeviceCtrl = None, expected_status: Status = Status.Success
    ) -> Status:
        if dev_ctrl is None:
    ...
    async def send_atomic_request_begin_command(
        self,
        ...
        try:
            response = await self.send_single_cmd(
                cmd=cluster.Commands.AtomicRequest(
                    requestType=Globals.Enums.AtomicRequestTypeEnum.kBeginWrite,
            ...
            self.check_atomic_response(
                response,
                expected_status,
                expected_overall_status,
                expected_preset_status,
                expected_schedules_status,
                expected_timeout,
            )
    ...
    async def send_atomic_request_commit_command(
        self,
        dev_ctrl: ChipDeviceCtrl = None,
        endpoint: int = None,
        expected_status: Status = Status.Success,
        ...
        try:
            response = await self.send_single_cmd(
                cmd=cluster.Commands.AtomicRequest(
                    requestType=Globals.Enums.AtomicRequestTypeEnum.kCommitWrite,
                    attributeRequests=[cluster.Attributes.Presets.attribute_id],
                ),
                dev_ctrl=dev_ctrl,
                endpoint=endpoint,
            ...
            self.check_atomic_response(
                response, expected_status, expected_overall_status, expected_preset_status, expected_schedules_status
            )
        except InteractionModelError as e:
            asserts.assert_equal(e.status, expected_status, "Unexpected error returned")

    async def send_atomic_request_rollback_command(
        self,
        dev_ctrl: ChipDeviceCtrl = None,
        endpoint: int = None,
        expected_status: Status = Status.Success,
        ...
        try:
            response = await self.send_single_cmd(
                cmd=cluster.Commands.AtomicRequest(
                    requestType=Globals.Enums.AtomicRequestTypeEnum.kRollbackWrite,
                    attributeRequests=[cluster.Attributes.Presets.attribute_id],
                ),
                dev_ctrl=dev_ctrl,
                endpoint=endpoint,
    ...
    async def send_set_active_preset_handle_request_command(
        self, endpoint: int = None, value: bytes = None, expected_status: Status = Status.Success
    ):
        try:
            await self.send_single_cmd(cmd=cluster.Commands.SetActivePresetRequest(value), endpoint=endpoint)
    ...
    def steps_TC_TSTAT_4_2(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "2",
                "TH writes to the Presets attribute without calling the AtomicRequest command",
            ...
            TestStep(
                "3",
                "TH writes to the Presets attribute after calling the AtomicRequest begin command but doesn't call AtomicRequest commit",
                "Verify that the Presets attribute was not updated since AtomicRequest commit command was not called.",
            ),
            TestStep(
                "4",
                "TH writes to the Presets attribute after calling the AtomicRequest begin command and calls AtomicRequest commit",
                "Verify that the Presets attribute was updated with new presets.",
            ),
            TestStep(
                "5",
                "TH writes to the Presets attribute with a built-in preset removed",
                "Verify that the AtomicRequest commit returned CONSTRAINT_ERROR (0x87).",
            ),
            TestStep(
                "6",
                "TH writes to the Presets attribute with a preset removed whose handle matches the value in the ActivePresetHandle attribute",
                "Verify that the AtomicRequest commit returned INVALID_IN_STATE (0xcb).",
            ),
            TestStep(
                "7",
                "TH writes to the Presets attribute with a built-in preset modified to be not built-in",
                "Verify that the AtomicRequest commit returned CONSTRAINT_ERROR (0x87).",
            ),
            TestStep(
                "8",
                "TH writes to the Presets attribute with a new preset having builtIn set to true",
                "Verify that the AtomicRequest commit returned CONSTRAINT_ERROR (0x87).",
            ),
            TestStep(
                "9",
                "TH writes to the Presets attribute with a new preset having a preset handle that doesn't exist in the Presets attribute",
                "Verify that the AtomicRequest commit returned NOT_FOUND (0x8b).",
            ),
            TestStep(
                "10",
                "TH writes to the Presets attribute with duplicate presets",
                "Verify that the AtomicRequest commit returned CONSTRAINT_ERROR (0x87).",
            ),
            TestStep(
                "11",
                "TH writes to the Presets attribute with a non built-in preset modified to be built-in",
                "Verify that the AtomicRequest commit returned CONSTRAINT_ERROR (0x87).",
            ),
            TestStep(
                "12",
                "TH writes to the Presets attribute with a preset that doesn't support names in the PresetTypeFeatures bitmap but has a name",
                "Verify that the AtomicRequest commit returned CONSTRAINT_ERROR (0x87).",
            ),
            TestStep(
                "13",
                "TH writes to the Presets attribute but calls the AtomicRequest rollback command to cancel the edit request",
                "Verify that the edit request was rolled back",
            ),
            TestStep(
                "14",
                "TH starts an atomic write, and TH2 attempts to open an atomic write before TH is complete",
                "Verify that the atomic request is rejected",
            ),
            TestStep(
                "15", "TH starts an atomic write, and TH2 attempts to write to presets", "Verify that the write request is rejected"
            ),
            TestStep(
                "16",
                "TH starts an atomic write, and before it's complete, TH2 removes TH's fabric; TH2 then opens an atomic write",
                "Verify that the atomic request is successful",
            ),
            TestStep(
                "17",
                "TH writes to the Presets attribute with a preset that has a presetScenario not present in PresetTypes attribute",
                "Verify that the write request returned CONSTRAINT_ERROR (0x87).",
            ),
            TestStep(
                "18",
                "TH writes to the Presets attribute such that the total number of presets is greater than the number of presets supported",
    ...
    async def test_TC_TSTAT_4_2(self):
        ...
        if supportsHeat:
            ...
            if self.check_pics("TSTAT.S.A0015") and self.check_pics("TSTAT.S.A0016"):
                minHeatSetpointLimit = await self.read_single_attribute_check_success(
                    endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.MinHeatSetpointLimit
                )
                maxHeatSetpointLimit = await self.read_single_attribute_check_success(
        ...
        if supportsCool:
            ...
            if self.check_pics("TSTAT.S.A0017") and self.check_pics("TSTAT.S.A0018"):
                minCoolSetpointLimit = await self.read_single_attribute_check_success(
                    endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.MinCoolSetpointLimit
                )
                maxCoolSetpointLimit = await self.read_single_attribute_check_success(
        ...
        if supportsOccupancy:
            occupied = (
                await self.read_single_attribute_check_success(
                    endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.Occupancy
                )
                & 1
            )
        ...
        if occupied:
            if supportsHeat:
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.OccupiedHeatingSetpoint(heatSetpoint - 1), endpoint_id=endpoint
                )
            if supportsCool:
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.OccupiedCoolingSetpoint(coolSetpoint - 1), endpoint_id=endpoint
                )
        else:
            if supportsHeat:
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.UnoccupiedHeatingSetpoint(heatSetpoint - 1), endpoint_id=endpoint
                )
            if supportsCool:
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.UnoccupiedCoolingSetpoint(coolSetpoint - 1), endpoint_id=endpoint
                )

        self.step("2")
        if self.pics_guard(self.check_pics("TSTAT.S.F08") and self.check_pics("TSTAT.S.A0050")):
            # Read the numberOfPresets supported.
            numberOfPresetsSupported = await self.read_single_attribute_check_success(
            ...
            presetTypes = await self.read_single_attribute_check_success(
        ...
        self.step("3")
        if self.pics_guard(
            self.check_pics("TSTAT.S.F08") and self.check_pics("TSTAT.S.A0050") and self.check_pics("TSTAT.S.Cfe.Rsp")
        ):
            ...
            if availableScenario is not None and len(current_presets) < numberOfPresetsSupported:
                ...
                saved_presets = await self.read_single_attribute_check_success(
                    endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.Presets
                )
                ...
                log.info("Couldn't run test step 3 since there was no available preset scenario to append")
        ...
        if self.pics_guard(
            ...
            if availableScenario is not None:
                ...
                log.info("Couldn't run test step 4 since there were no built-in presets")
        ...
        if self.pics_guard(
            ...
            if len(builtInPresets) > 0:
                ...
                await self.send_atomic_request_commit_command(
                    expected_overall_status=Status.Failure, expected_preset_status=Status.ConstraintError
                )
            else:
                log.info("Couldn't run test step 5 since there were no built-in presets")

        self.step("6")
        if self.pics_guard(
            self.check_pics("TSTAT.S.F08")
            and self.check_pics("TSTAT.S.A0050")
            and self.check_pics("TSTAT.S.C06.Rsp")
            and self.check_pics("TSTAT.S.Cfe.Rsp")
        ):
            ...
            if len(notBuiltInPresets) > 0:
                ...
                activePresetHandle = await self.read_single_attribute_check_success(
                ...
                asserts.assert_equal(
                    activePresetHandle, activePreset.presetHandle, "Active preset handle was not updated as expected"
                )
                ...
                await self.send_atomic_request_commit_command(
                    expected_overall_status=Status.Failure, expected_preset_status=Status.InvalidInState
                )
            else:
                log.info("Couldn't run test step 6 since there were no non-built-in presets to activate and delete")
        ...
        if self.pics_guard(
            ...
            if len(builtInPresets) > 0:
                ...
                log.info("Couldn't run test step 7 since there was no built-in presets")
        ...
        if self.pics_guard(
            ...
            if len(current_presets) < numberOfPresetsSupported and availableScenario is not None:
                ...
                log.info("Couldn't run test step 8 since there was no available preset scenario to append")
        ...
        if self.pics_guard(
            ...
            test_presets.append(
                self.make_preset(
                    cluster.Enums.PresetScenarioEnum.kWake,
                    coolSetpoint,
                    heatSetpoint,
                    presetHandle=random.randbytes(16),
                    name="Wake",
                    builtIn=True,
                )
            )
        ...
        if self.pics_guard(
            ...
            if len(current_presets) < numberOfPresetsSupported and availableScenario is not None:
                ...
                test_presets.append(
                    self.make_preset(availableScenario, coolSetpoint, heatSetpoint, presetHandle=duplicatePreset.presetHandle)
                )
                ...
                log.info("Couldn't run test step 10 since there was no available preset scenario to duplicate")
        ...
        if self.pics_guard(
            ...
            if len(notBuiltInPresets) > 0:
                ...
                log.info("Couldn't run test step 11 since there were no presets that were not built-in")
        ...
        if self.pics_guard(
            ...
            availableScenarios = [
                presetType.presetScenario
                for presetType in presetTypes
                if (presetType.presetTypeFeatures & cluster.Bitmaps.PresetTypeFeaturesBitmap.kSupportsNames) == 0
                and presetScenarioCounts.get(presetType.presetScenario, 0) <= presetType.numberOfPresets
            ]
            ...
            if len(availableScenarios) > 0:
                ...
                log.info("Couldn't run test step 12 since there was no available preset scenario without name support")
        ...
        if self.pics_guard(
            ...
            if len(current_presets) < numberOfPresetsSupported and availableScenario is not None:
                ...
                log.info("Couldn't run test step 13 since there was no available preset scenario to add")
        ...
        if self.pics_guard(
            ...
            status = await self.send_atomic_request_begin_command(
                dev_ctrl=secondary_controller, expected_overall_status=Status.Failure, expected_preset_status=Status.Busy
            )
        ...
        if self.pics_guard(
            ...
            await self.write_presets(
                endpoint=endpoint, presets=current_presets, dev_ctrl=secondary_controller, expected_status=Status.Busy
            )
        ...
        if self.pics_guard(
            ...
            await self.send_single_cmd(
                Clusters.OperationalCredentials.Commands.RemoveFabric(fabricIndex=secondary_fabric_index), endpoint=0
            )
        ...
        if self.pics_guard(
            ...
            presetScenarioHeadrooms = [
                ScenarioHeadroom(
                    presetType.presetScenario, presetType.numberOfPresets - presetScenarioCounts.get(presetType.presetScenario, 0)
                )
                for presetType in presetTypes
            ]
            ...
            if presetScenarioHeadrooms:
                ...
                test_presets.extend(
                    [
                        cluster.Structs.PresetStruct(
                            presetHandle=NullValue,
                            presetScenario=presetScenarioHeadroom.presetScenario,
                            coolingSetpoint=coolSetpoint,
                            heatingSetpoint=heatSetpoint,
                            builtIn=False,
                        )
                    ]
                    * (presetScenarioHeadroom.remaining + 1)
                )
