...
class TC_CC_2_1(MatterBaseTest):
    ...
    def steps_TC_CC_2_1(self) -> list[TestStep]:
        ...
        return [
            ...
            TestStep(
                24,
                "TH reads from the DUT the (0x0010) NumberOfPrimaries attribute and verify that the DUT response contains an uint8 and value SHALL be in range from 0..6. NumberOfPrimaries attribute is consistent with the PrimaryN<X,Y,Intensity> attributes provided",
            ),
            TestStep(
                25,
                "TH reads Primary1X attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 1 or more",
            ),
            TestStep(
                26,
                "TH reads Primary1Y attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 1 or more",
            ),
            TestStep(
                27,
                "TH reads Primary1Intensity attribute from DUT and Verify that the DUT response contains an uint8 if NumberOfPrimaries is 1 or more",
            ),
            TestStep(
                28,
                "TH reads Primary2X attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 2 or more",
            ),
            TestStep(
                29,
                "TH reads Primary2Y attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 2 or more",
            ),
            TestStep(
                30,
                "TH reads Primary2Intensity attribute from DUT and Verify that the DUT response contains an uint8 if NumberOfPrimaries is 2 or more",
            ),
            TestStep(
                31,
                "TH reads Primary3X attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 3 or more",
            ),
            TestStep(
                32,
                "TH reads Primary3Y attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 3 or more",
            ),
            TestStep(
                33,
                "TH reads Primary3Intensity attribute from DUT and Verify that the DUT response contains an uint8 if NumberOfPrimaries is 3 or more",
            ),
            TestStep(
                34,
                "TH reads Primary4X attribute from DUT Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 4 or more",
            ),
            TestStep(
                35,
                "TH reads Primary4Y attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 4 or more",
            ),
            TestStep(
                36,
                "TH reads Primary4Intensity attribute from DUT and Verify that the DUT response contains an uint8 if NumberOfPrimaries is 4 or more",
            ),
            TestStep(
                37,
                "TH reads Primary5X attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 5 or more",
            ),
            TestStep(
                38,
                "TH reads Primary5Y attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 5 or more",
            ),
            TestStep(
                39,
                "TH reads Primary5Intensity attribute from DUT and Verify that the DUT response contains an uint8 if NumberOfPrimaries is 5 or more",
            ),
            TestStep(
                40,
                "TH reads Primary6X attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 6 or more",
            ),
            TestStep(
                41,
                "TH reads Primary6Y attribute from DUT and Verify that the DUT response contains an uint16 [Min:0 Max:0xfeff] if NumberOfPrimaries is 6 or more",
            ),
            TestStep(
                42,
                "TH reads Primary6Intensity attribute from DUT and Verify that the DUT response contains an uint8 if NumberOfPrimaries is 6 or more",
            ),
    ...
    async def _verify_attribute(
        self,
        attribute: Attribute,
        data_type: ValueTypesEnum,
        enum_range: Optional[list] = None,
        ...
        if await self.attribute_guard(endpoint=self.endpoint, attribute=attribute):
            ...
            attr_val = await self.read_single_attribute_check_success(
                cluster=self.cluster, attribute=attribute, endpoint=self.endpoint
            )
            ...
            if "uint" in data_type.name.lower() and (max_len is not None or min_len is not None):
                if isinstance(min_len, int):
                    ...
                    asserts.assert_true(
                        (attr_val >= min_len), f"Attribute {attribute} with value {attr_val} is out of range (min): {min_len}"
                    )
                if isinstance(max_len, int):
                    log.info(f"Max len defined validation max range for uint {max_len}")
                    asserts.assert_true(
                        (attr_val <= max_len), f"Attribute {attribute} with value {attr_val} is out of range (max): {max_len}"
                    )
            return attr_val
        return None
    ...
    async def test_TC_CC_2_1(self):
        ...
        feature_map_value = await self.read_single_attribute_check_success(
            cluster=self.cluster, endpoint=self.endpoint, attribute=self.attributes.FeatureMap
        )

        self.step(19)
        color_capabilities_value = await self._verify_attribute(
            self.attributes.ColorCapabilities, ValueTypesEnum.UINT16, max_len=0x001F
        )
        ...
        self.step(20)
        colortempphysicalminmireds_val = await self._verify_attribute(
            self.attributes.ColorTempPhysicalMinMireds, ValueTypesEnum.UINT16, min_len=1, max_len=65279
        )

        self.step(21)
        colortempphysicalmaxmireds_val = await self._verify_attribute(
            self.attributes.ColorTempPhysicalMaxMireds, ValueTypesEnum.UINT16, min_len=1, max_len=65279
        )

        self.step(22)
        await self._verify_attribute(
            self.attributes.CoupleColorTempToLevelMinMireds,
            ValueTypesEnum.UINT16,
            min_len=colortempphysicalminmireds_val,
            max_len=colortempphysicalmaxmireds_val,
        )
        ...
        if await self.attribute_guard(endpoint=self.endpoint, attribute=self.attributes.StartUpColorTemperatureMireds):
            sctmr_val = await self.read_single_attribute_check_success(
                cluster=self.cluster, endpoint=self.endpoint, attribute=self.attributes.StartUpColorTemperatureMireds
            )
        ...
        number_of_primaries_value = await self._verify_attribute(
            self.attributes.NumberOfPrimaries, ValueTypesEnum.UINT8, min_len=0, max_len=6, nullable=True
        )
        if number_of_primaries_value is NullValue or number_of_primaries_value == 0:
            ...
            for primariesindex in range(1, 7):
                log.info(f"Skip if the test index {primariesindex} is graeter than NumberOfPrimaries {number_of_primaries_value} ?")
                ...
                await self._verify_attribute(
                    getattr(self.attributes, f"Primary{primariesindex}X"), ValueTypesEnum.UINT16, max_len=65279
                )
                current_step += 1
                self.step(current_step)
                await self._verify_attribute(
                    getattr(self.attributes, f"Primary{primariesindex}Y"), ValueTypesEnum.UINT16, max_len=65279
                )
                current_step += 1
                self.step(current_step)
                await self._verify_attribute(
                    getattr(self.attributes, f"Primary{primariesindex}Intensity"), ValueTypesEnum.UINT8, nullable=True
                )
