...
class TC_ACL_2_5(MatterBaseTest):
    ...
    async def internal_test_TC_ACL_2_5(self, force_legacy_encoding: bool):
        ...
        events_response = await self.th1.ReadEvent(self.dut_node_id, events=[(0, acec_event)], fabricFiltered=True)
        ...
        extension = Clusters.AccessControl.Structs.AccessControlExtensionStruct(data=D_OK_EMPTY)
        ...
        direct_events = await self.default_controller.ReadEvent(
            self.dut_node_id, events=[(0, acec_event)], fabricFiltered=True, eventNumberFilter=latest_event_num
        )
        ...
        asserts.assert_equal(
            subscription_event.changeType, Clusters.AccessControl.Enums.ChangeTypeEnum.kAdded, "Expected Added change type"
        )

        asserts.assert_equal(subscription_event.latestValue.data, D_OK_EMPTY, "LatestValue.Data should be D_OK_EMPTY")
        asserts.assert_in(
            "matter.clusters.Types.Nullable", str(type(subscription_event.adminPasscodeID)), "AdminPasscodeID should be Null"
        )
        asserts.assert_equal(
            subscription_event.adminNodeID, self.default_controller.nodeId, "AdminNodeID should be the controller node ID"
        )
        asserts.assert_equal(
            subscription_event.latestValue.fabricIndex, f1, "LatestValue.FabricIndex should be the current fabric index"
        )
        asserts.assert_equal(subscription_event.fabricIndex, f1, "FabricIndex should be the current fabric index")
        ...
        new_extension = Clusters.AccessControl.Structs.AccessControlExtensionStruct(data=D_OK_SINGLE)
        ...
        if not force_legacy_encoding:
            # Verify event data
            asserts.assert_equal(
                event_data1.changeType, Clusters.AccessControl.Enums.ChangeTypeEnum.kChanged, "Expected Changed change type"
            )
            asserts.assert_in(
                "matter.clusters.Types.Nullable", str(type(event_data1.adminPasscodeID)), "AdminPasscodeID should be Null"
            )
            asserts.assert_equal(
                event_data1.adminNodeID, self.default_controller.nodeId, "AdminNodeID should be the controller node ID"
            )
            asserts.assert_equal(event_data1.latestValue.data, D_OK_SINGLE, "LatestValue.Data should match D_OK_SINGLE")
            asserts.assert_equal(
                event_data1.latestValue.fabricIndex, f1, "LatestValue.FabricIndex should be the current fabric index"
            )
            asserts.assert_equal(event_data1.fabricIndex, f1, "FabricIndex should be the current fabric index")

        if force_legacy_encoding:
            # Verify event data 1 struct
            asserts.assert_equal(
                event_data1.changeType, Clusters.AccessControl.Enums.ChangeTypeEnum.kRemoved, "Expected Removed change type"
            )
            asserts.assert_in(
                "matter.clusters.Types.Nullable", str(type(event_data1.adminPasscodeID)), "AdminPasscodeID should be Null"
            )
            ...
            asserts.assert_equal(event_data1.latestValue.data, D_OK_EMPTY, "LatestValue.Data should match D_OK_EMPTY")
            asserts.assert_equal(
                event_data1.latestValue.fabricIndex, f1, "LatestValue.FabricIndex should be the current fabric index"
            )
            asserts.assert_equal(event_data1.fabricIndex, f1, "FabricIndex should be the current fabric index")
            ...
            asserts.assert_equal(
                event_data2.changeType, Clusters.AccessControl.Enums.ChangeTypeEnum.kAdded, "Expected Added change type"
            )
            asserts.assert_in(
                "matter.clusters.Types.Nullable", str(type(event_data2.adminPasscodeID)), "AdminPasscodeID should be Null"
            )
            asserts.assert_equal(
                event_data2.adminNodeID, self.default_controller.nodeId, "AdminNodeID should be the controller node ID"
            )
            asserts.assert_equal(event_data2.latestValue.data, D_OK_SINGLE, "LatestValue.Data should match D_OK_SINGLE")
            asserts.assert_equal(
                event_data2.latestValue.fabricIndex, f1, "LatestValue.FabricIndex should be the current fabric index"
            )
            asserts.assert_equal(event_data2.fabricIndex, f1, "FabricIndex should be the current fabric index")
        ...
        too_long_extension = Clusters.AccessControl.Structs.AccessControlExtensionStruct(data=too_long_data)
        ...
        asserts.assert_equal(a[0].Status, Status.ConstraintError, "Write should have failed with CONSTRAINT_ERROR 135")
        ...
        events_response2 = await self.default_controller.ReadEvent(
            self.dut_node_id, events=[(0, acec_event)], fabricFiltered=True, eventNumberFilter=latest_event_num + 1
        )
        ...
        if force_legacy_encoding:
            ...
            # Verify event data 1 struct
            asserts.assert_equal(
                event_data3.changeType, Clusters.AccessControl.Enums.ChangeTypeEnum.kRemoved, "Expected Removed change type"
            )
            asserts.assert_in(
                "matter.clusters.Types.Nullable", str(type(event_data3.adminPasscodeID)), "AdminPasscodeID should be Null"
            )
            asserts.assert_equal(
                event_data3.adminNodeID, self.default_controller.nodeId, "AdminNodeID should be the controller node ID"
            )
            asserts.assert_equal(event_data3.latestValue.data, D_OK_SINGLE, "LatestValue.Data should match D_OK_SINGLE")
            asserts.assert_equal(
                event_data3.latestValue.fabricIndex, f1, "LatestValue.FabricIndex should be the current fabric index"
            )
            asserts.assert_equal(event_data3.fabricIndex, f1, "FabricIndex should be the current fabric index")
        ...
        asserts.assert_equal(b[0].Status, Status.ConstraintError, "Write should have failed with CONSTRAINT_ERROR")
        ...
        events_response3 = await self.default_controller.ReadEvent(
            self.dut_node_id, events=[(0, acec_event)], fabricFiltered=True, eventNumberFilter=latest_event_num2 + 1
        )
        ...
        if not force_legacy_encoding:
            ...
            # Verify event data 2 struct
            asserts.assert_equal(
                event_data4.changeType, Clusters.AccessControl.Enums.ChangeTypeEnum.kAdded, "Expected Added change type"
            )
            asserts.assert_in(
                "matter.clusters.Types.Nullable", str(type(event_data4.adminPasscodeID)), "AdminPasscodeID should be Null"
            )
            asserts.assert_equal(
                event_data4.adminNodeID, self.default_controller.nodeId, "AdminNodeID should be the controller node ID"
            )
            asserts.assert_equal(event_data4.latestValue.data, D_OK_EMPTY, "LatestValue.Data should match D_OK_EMPTY")
            asserts.assert_equal(
                event_data4.latestValue.fabricIndex, f1, "LatestValue.FabricIndex should be the current fabric index"
            )
            asserts.assert_equal(event_data4.fabricIndex, f1, "FabricIndex should be the current fabric index")
        ...
        if not force_legacy_encoding:
            # Verify event data
            asserts.assert_equal(event_data5.latestValue.data, D_OK_SINGLE, "LatestValue.Data should match D_OK_SINGLE")

        if force_legacy_encoding:
            # Verify event data 1 struct
            asserts.assert_equal(event_data5.latestValue.data, D_OK_EMPTY, "LatestValue.Data should match D_OK_EMPTY")

        asserts.assert_in(
            "matter.clusters.Types.Nullable", str(type(event_data5.adminPasscodeID)), "AdminPasscodeID should be Null"
        )

        asserts.assert_equal(
            event_data5.changeType, Clusters.AccessControl.Enums.ChangeTypeEnum.kRemoved, "Expected Removed change type"
        )

        asserts.assert_equal(
            event_data5.adminNodeID, self.default_controller.nodeId, "AdminNodeID should be the controller node ID"
        )

        asserts.assert_equal(event_data5.latestValue.fabricIndex, f1, "LatestValue.FabricIndex should be the current fabric index")

        asserts.assert_equal(event_data5.fabricIndex, f1, "FabricIndex should be the current fabric index")
    ...
    def steps_TC_ACL_2_5(self) -> list[TestStep]:
        return [
            TestStep(1, "TH1 commissions DUT using admin node ID", is_commissioning=True),
            TestStep(
                2,
            ...
            TestStep(
                3,
                "TH1 reads DUT Endpoint 0 AccessControl cluster AccessControlExtensionChanged events and create subscription for new events",
                "Result is SUCCESS, value is empty list, new event subscription is created",
            ),
            TestStep(
                4,
                "TH1 writes DUT Endpoint 0 AccessControl cluster Extension attribute, value is list of AccessControlExtensionStruct containing 1 element",
                "Result is SUCCESS",
            ),
            TestStep(
                5,
                "TH1 reads DUT Endpoint 0 AccessControl cluster AccessControlExtensionChanged events",
            ...
            TestStep(
                10,
                "TH1 writes DUT Endpoint 0 AccessControl cluster Extension attribute, value is list of AccessControlExtensionStruct containing 2 elements",
