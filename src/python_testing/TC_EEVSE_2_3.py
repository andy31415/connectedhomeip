...
class TC_EEVSE_2_3(MatterBaseTest, EEVSEBaseTestHelper):
    def desc_TC_EEVSE_2_3(self) -> str:
        ...
        return (
            "5.1.4. [TC-EEVSE-2.3] Optional ChargingPreferences feature functionality with DUT as Server\n"
    ...
    def steps_TC_EEVSE_2_3(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "1a",
                "TH reads from the DUT theFeatureMap_",
            ...
            TestStep(
                "4",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.EEVSE.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.EEVSE.TEST_EVENT_TRIGGER for EVSE TimeOfUse Mode Test Event",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep(
                "5",
            ...
            TestStep("6", "TH sends command ClearTargets", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep("6a", "TH reads from the DUT the NextChargeStartTime", "Value has to be null."),
            TestStep("6b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be null."),
            TestStep("6c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be null."),
            TestStep("6d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be null."),
            TestStep(
                "7", "TH sends command GetTargets", "Response EEVSE.S.C00.Tx(GetTargetsResponse) sent with no targets defined."
            ),
            TestStep(
                "8",
                "TH sends command SetTargets with DayOfTheWeekforSequence=0x7F (i.e. having all days set) and a single ChargingTargets={TargetTimeMinutesPastMidnight=1439, TargetSoC=null, AddedEnergy=25000000}. Store TargetTimeMinutesPastMidnight converted from local timezone into Matter EPOCH in UTC as TargetTime, TargetSoC as TargetSoC, and AddedEnergy as AddedEnergy.",
            ...
            TestStep("8a", "TH reads from the DUT the NextChargeStartTime", "Value has to be null."),
            TestStep("8b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be null."),
            TestStep("8c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be null."),
            TestStep("8d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be null."),
            TestStep(
                "9",
            ...
            TestStep(
                "9a",
                "If SoCReporting feature is in FeatureMap skip to step 11, otherwise TH reads from the DUT the NextChargeStartTime",
                "Value has to be before the next TargetTime above.",
            ),
            TestStep("9b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be TargetTime above."),
            TestStep("9c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be AddedEnergy above."),
            TestStep("9d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be null."),
            TestStep(
                "10",
                "TH sends command GetTargets",
            ...
            TestStep("11a", "TH reads from the DUT the NextChargeStartTime", "Value has to be before the next TargetTime above."),
            TestStep("11b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be TargetTime above."),
            TestStep("11c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be null."),
            TestStep("11d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be 100."),
            TestStep(
                "12",
                "TH sends command GetTargets",
                "Response EEVSE.S.C00.Tx(GetTargetsResponse) sent with targets equivalent to the above (Note 1).",
            ...
            TestStep(
                "13",
                "TH sends command SetTargets with DayOfTheWeekforSequence=0x40 (i.e. having Saturday set) and 10 ChargingTargets with TargetTimeMinutesPastMidnight=60,180,300,420,540,660,780,900,1020,1140 and all with TargetSoC=100, AddedEnergy=2500000",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep(
                "14",
                "TH sends command SetTargets with DayOfTheWeekforSequence=0x01 (i.e. having Sunday set) and no ChargingTargets",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep(
                "15",
            ...
            TestStep("16", "TH sends command ClearTargets", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep("16a", "TH reads from the DUT the NextChargeStartTime", "Value has to be null."),
            TestStep("16b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be null."),
            TestStep("16c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be null."),
            TestStep("16d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be null."),
            TestStep(
                "17", "TH sends command GetTargets", "Response EEVSE.S.C00.Tx(GetTargetsResponse) sent with no targets defined."
            ),
            TestStep(
                "18",
                "TH sends command SetTargets with two identical ChargingTargetSchedules={DayOfTheWeekforSequence=0x01,ChargingTarget[0]={TargetTimeMinutesPastMidnight=60,TargetSoC=100,AddedEnergy=2500000}}",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep(
                "19",
                "TH sends command SetTargets with DayOfTheWeekforSequence=0x40 and 11 ChargingTargets with TargetTimeMinutesPastMidnight=60,180,300,420,540,660,780,900,1020,1140,1260 and all with TargetSoC=100, AddedEnergy=2500000",
            ...
            TestStep(
                "22",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.EEVSE.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.EEVSE.TEST_EVENT_TRIGGER for EVSE TimeOfUse Mode Test Event Clear",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
        ]
    ...
    async def test_TC_EEVSE_2_3(self):
        ...
        dailyTargets = [
            Clusters.EnergyEvse.Structs.ChargingTargetStruct(
                targetTimeMinutesPastMidnight=minutes_past_midnight,
                # targetSoc not sent
                addedEnergy=added_energy,
            )
        ]
        targets = [
            Clusters.EnergyEvse.Structs.ChargingTargetScheduleStruct(dayOfWeekForSequence=0x7F, chargingTargets=dailyTargets)
        ]
        ...
        if soc_reporting_supported:
            log.info("Skipping steps 9a-10 as SoCReporting is supported by the DUT and step 8 above will have failed.")
            ...
            expected_next_target_time_epoch_s = self.compute_expected_target_time_as_epoch_s(minutes_past_midnight)
            asserts.assert_less(next_start_time_epoch_s, expected_next_target_time_epoch_s)
            ...
            self.log_get_targets_response(get_targets_response)
            asserts.assert_equal(
                get_targets_response.chargingTargetSchedules,
                targets,
        ...
        daily_targets_step_11 = [
            Clusters.EnergyEvse.Structs.ChargingTargetStruct(targetTimeMinutesPastMidnight=minutes_past_midnight, targetSoC=100)
        ]
        targets_step_11 = [
            Clusters.EnergyEvse.Structs.ChargingTargetScheduleStruct(
        ...
        expected_target_time_epoch_s = self.compute_expected_target_time_as_epoch_s(minutes_past_midnight)

        asserts.assert_less(
            next_start_time_epoch_s,
            next_target_time_epoch_s,
            f"Unexpected 'NextChargeStartTime' response value - expected this to be < {next_target_time_epoch_s}, was {next_start_time_epoch_s}",
        )

        asserts.assert_equal(
            next_target_time_epoch_s,
            expected_target_time_epoch_s,
        ...
        daily_targets_step_13 = [
            Clusters.EnergyEvse.Structs.ChargingTargetStruct(targetTimeMinutesPastMidnight=60, targetSoC=100, addedEnergy=25000000),
            Clusters.EnergyEvse.Structs.ChargingTargetStruct(
        ...
        asserts.assert_equal(len(get_targets_response.chargingTargetSchedules), 3, "'GetTargets' response should have 3 entries")
        asserts.assert_equal(
            get_targets_response.chargingTargetSchedules[0].dayOfWeekForSequence,
            0x3E,
            "'GetTargets' response entry 0 should have DayOfWeekForSequence = 0x3e (62)",
        )
        asserts.assert_equal(
            get_targets_response.chargingTargetSchedules[0].chargingTargets,
            daily_targets_step_11,
            f"'GetTargets' response entry 0 should have chargingTargets = {daily_targets_step_11})",
        )
        asserts.assert_equal(
            get_targets_response.chargingTargetSchedules[1],
            targets_step_13[0],
            f"'GetTargets' response entry 1 should be {targets_step_13})",
        )
        asserts.assert_equal(
            get_targets_response.chargingTargetSchedules[2],
            targets_step_14[0],
        ...
        targets_step_18 = [
            Clusters.EnergyEvse.Structs.ChargingTargetScheduleStruct(
                dayOfWeekForSequence=0x1, chargingTargets=daily_targets_step_18
            ),
            Clusters.EnergyEvse.Structs.ChargingTargetScheduleStruct(
                dayOfWeekForSequence=0x1, chargingTargets=daily_targets_step_18
            ),
