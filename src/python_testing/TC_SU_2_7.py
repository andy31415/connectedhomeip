...
class TC_SU_2_7(SoftwareUpdateBaseTest):
    ...
    async def setup_test(self):
        ...
        if self.ota_image_download_timeout <= 0:
            asserts.fail(
                "Invalid value for --int-arg ota_image_download_timeout:<seconds> value provided, must be equal or greater than 1."
            )

        if not self.expected_software_version:
            asserts.fail(
                "Missing OTA image software version. Speficy using --int-arg ota_image_expected_version:<ota_image_expected_version>"
            )
        ...
        await self.set_default_ota_providers_list(
            controller=self.controller, provider_node_id=self.provider_node_id, endpoint=0, requestor_node_id=self.requestor_node_id
        )
        logger.info("About to write acl entries")
        await self.create_acl_entry(
            dev_ctrl=self.controller, provider_node_id=self.provider_node_id, requestor_node_id=self.requestor_node_id
        )
    ...
    def steps_TC_SU_2_7(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                2,
                'DUT sends a QueryImage command to the TH/OTA-P. TH/OTA-P sends a QueryImageResponse back to DUT. QueryStatus is set to "Busy"',
                "Verify that the OTA-Subscriber receives a StateTransition event notification for the state change to DelayedOnQuery.",
            ),
            TestStep(
                3,
                "DUT sends a QueryImage command to the TH/OTA-P. TH/OTA-P does not respond back to DUT.",
            ...
            TestStep(
                6,
                'After the OTA image is transferred, DUT sends ApplyUpdateRequest to the OTA-P. OTA-P/TH sends the ApplyUpdateResponse Command to the DUT. Action field is set to "AwaitNextAction".',
    ...
    async def test_TC_SU_2_7(self):
        ...
        await state_transition_event_handler.start(
            controller, self.requestor_node_id, endpoint=0, min_interval_sec=1, max_interval_sec=20, autoResubscribe=True
        )
        # Create AttributeSubscriptionHandler
        update_state_attr_handler = AttributeSubscriptionHandler(
            expected_cluster=Clusters.OtaSoftwareUpdateRequestor,
            expected_attribute=Clusters.OtaSoftwareUpdateRequestor.Attributes.UpdateState,
        )
        await update_state_attr_handler.start(
            dev_ctrl=controller,
        ...
        event_report = state_transition_event_handler.wait_for_event_report(self.ota_req.Events.StateTransition, timeout_sec=70)
        ...
        if self.is_pics_sdk_ci_only:
            attribute_idle = AttributeValue(
                ...
                value=Clusters.OtaSoftwareUpdateRequestor.Enums.UpdateStateEnum.kIdle,
            )
        ...
        await self.verify_version_applied_basic_information(
            controller=controller, node_id=self.requestor_node_id, target_version=self.expected_software_version
        )
        ...
        events_response = await self.controller.ReadEvent(
            self.requestor_node_id, events=[(0, version_applied_event, urgent)], fabricFiltered=True
        )
        ...
        asserts.assert_equal(
            ...
            version_applied_event_data.softwareVersion,
        ...
        self.start_provider(
            ...
            discriminator=self.provider_discriminator,
            port=self.provider_port,
            extra_args=["--delayedQueryActionTimeSec", "10", "--queryImageStatus", "busy"],
            kvs_path=self.provider_kvs_path,
            log_file=self.provider_log,
            timeout=20,
        )
        ...
        await state_transition_event_handler.start(
            controller, self.requestor_node_id, endpoint=0, min_interval_sec=0, max_interval_sec=20, autoResubscribe=True
        )
        await self.announce_ota_provider(controller, self.provider_node_id, self.requestor_node_id)
        event_report = state_transition_event_handler.wait_for_event_report(self.ota_req.Events.StateTransition, timeout_sec=600)
        ...
        self.verify_state_transition_event(
            event_report=event_report,
            expected_previous_state=self.ota_req.Enums.UpdateStateEnum.kQuerying,
            expected_new_state=self.ota_req.Enums.UpdateStateEnum.kDelayedOnQuery,
            expected_reason=self.ota_req.Enums.ChangeReasonEnum.kDelayByProvider,
        )
        state_transition_event_handler.cancel()
        ...
        self.verify_state_transition_event(
            ...
            expected_reason=self.ota_req.Enums.ChangeReasonEnum.kFailure,
        )
        state_transition_event_handler.cancel()
        self.terminate_provider()
        ...
        asserts.assert_equal(
            download_event_report.softwareVersion,
            self.expected_software_version,
        ...
        asserts.assert_equal(
            download_event_report.platformCode,
            NullValue,
        ...
        self.start_provider(
            ...
            discriminator=self.provider_discriminator,
            port=self.provider_port,
            extra_args=["--applyUpdateAction", "awaitNextAction", "--delayedApplyActionTimeSec", "3"],
            kvs_path=self.provider_kvs_path,
            log_file=self.provider_log,
            timeout=10,
        )
        # AttributeSubscriptionHandler
        update_state_attr_handler = AttributeSubscriptionHandler(expected_cluster=Clusters.OtaSoftwareUpdateRequestor)
        await update_state_attr_handler.start(
            dev_ctrl=controller,
            node_id=self.requestor_node_id,
            endpoint=0,
        ...
        attribute_idle = AttributeValue(
            endpoint_id=self.get_endpoint(),
            attribute=Clusters.OtaSoftwareUpdateRequestor.Attributes.UpdateState,
            value=Clusters.OtaSoftwareUpdateRequestor.Enums.UpdateStateEnum.kDownloading,
        )
        update_state_attr_handler.await_all_final_values_reported(expected_final_values=[attribute_idle], timeout_sec=600)
        ...
        self.verify_state_transition_event(
            event_report=event_report,
            expected_previous_state=self.ota_req.Enums.UpdateStateEnum.kApplying,
            expected_new_state=self.ota_req.Enums.UpdateStateEnum.kDelayedOnApply,
            expected_reason=self.ota_req.Enums.ChangeReasonEnum.kDelayByProvider,
            expected_target_version=NullValue,
        )
        update_state_attr_handler.cancel()
