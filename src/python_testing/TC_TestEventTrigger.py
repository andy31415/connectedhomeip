...
class TestEventTrigger(MatterBaseTest):
    ...
    async def test_all_zeros_key(self):
        ...
        with asserts.assert_raises_regex(
            InteractionModelError, "ConstraintError", "All-zero TestEventTrigger key must return ConstraintError"
        ):
            await dev_ctrl.SendCommand(
                self.dut_node_id,
                endpoint=0,
                payload=Clusters.GeneralDiagnostics.Commands.TestEventTrigger(
                    enableKey=kAllZerosKey, eventTrigger=kValidEventTrigger
                ),
            )
    ...
    async def test_incorrect_key(self):
        ...
        test_event_triggers_enabled = await self.read_single_attribute(
            dev_ctrl, self.dut_node_id, endpoint=0, attribute=Clusters.GeneralDiagnostics.Attributes.TestEventTriggersEnabled
        )
        asserts.assert_true(test_event_triggers_enabled, "This test expects Test Event Triggers are Enabled")

        with asserts.assert_raises_regex(
            InteractionModelError, "ConstraintError", "Bad TestEventTrigger key must return ConstraintError"
        ):
            await dev_ctrl.SendCommand(
                self.dut_node_id,
                endpoint=0,
                payload=Clusters.GeneralDiagnostics.Commands.TestEventTrigger(enableKey=kBadKey, eventTrigger=kValidEventTrigger),
            )
    ...
    async def test_correct_key_valid_code(self):
        ...
        asserts.assert_is_none(
            await dev_ctrl.SendCommand(
                ...
                endpoint=0,
                payload=Clusters.GeneralDiagnostics.Commands.TestEventTrigger(
                    enableKey=kExpectedKey, eventTrigger=kValidEventTrigger
                ),
            )
        )
    ...
    async def test_correct_key_invalid_code(self):
        ...
        with asserts.assert_raises_regex(
            InteractionModelError, "InvalidCommand", "Unsupported EventTrigger must return InvalidCommand"
        ):
            await dev_ctrl.SendCommand(
                self.dut_node_id,
                endpoint=0,
                payload=Clusters.GeneralDiagnostics.Commands.TestEventTrigger(
                    enableKey=kExpectedKey, eventTrigger=kInvalidEventTrigger
                ),
            )
    ...
    async def test_multiple_command_request_feature_present_if_needed(self):
        ...
        max_paths_per_invoke = await self.read_single_attribute(
            self.default_controller,
            node_id=self.dut_node_id,
            endpoint=0,
            attribute=Clusters.BasicInformation.Attributes.MaxPathsPerInvoke,
        )
        ...
        accepted_commands = await self.read_single_attribute(
            self.default_controller,
            node_id=self.dut_node_id,
            endpoint=0,
            attribute=Clusters.GeneralDiagnostics.Attributes.AcceptedCommandList,
        )
        generated_commands = await self.read_single_attribute(
            self.default_controller,
            node_id=self.dut_node_id,
            endpoint=0,
            attribute=Clusters.GeneralDiagnostics.Attributes.GeneratedCommandList,
        )
        ...
        if has_data_model_test_feature:
            ...
            asserts.assert_true(
                has_payload_test_response_command, "Must generate PayloadTestResponse command when DMTEST supported"
            )
        ...
        if has_data_model_test_feature:
            ...
            with asserts.assert_raises_regex(
                ...
                await dev_ctrl.SendCommand(
                    self.dut_node_id,
                    endpoint=0,
                    payload=Clusters.GeneralDiagnostics.Commands.PayloadTestRequest(enableKey=kExpectedKey, value=0x55, count=2049),
                )
            ...
            with asserts.assert_raises_regex(
                ...
                await dev_ctrl.SendCommand(
                    ...
                    payload=Clusters.GeneralDiagnostics.Commands.PayloadTestRequest(
                        enableKey=kExpectedKey, value=0x55, count=1280 + 1
                    ),
                )
            ...
            response = await dev_ctrl.SendCommand(
                self.dut_node_id,
                endpoint=0,
                payload=Clusters.GeneralDiagnostics.Commands.PayloadTestRequest(enableKey=kExpectedKey, value=ord("B"), count=0),
            )
            ...
            response = await dev_ctrl.SendCommand(
                self.dut_node_id,
                endpoint=0,
                payload=Clusters.GeneralDiagnostics.Commands.PayloadTestRequest(enableKey=kExpectedKey, value=ord("A"), count=800),
            )
            asserts.assert_equal(response.payload, b"A" * 800, "Expect response to match for count == 800")

        if not has_data_model_test_feature:
            with asserts.assert_raises_regex(
                InteractionModelError, "UnsupportedCommand", "PayloadTestRequest must not be supported if not reported"
            ):
                await dev_ctrl.SendCommand(
                    self.dut_node_id,
                    endpoint=0,
                    payload=Clusters.GeneralDiagnostics.Commands.PayloadTestRequest(enableKey=kExpectedKey, value=0x55, count=1),
                )
