...
class TC_CNET_4_12(MatterBaseTest):
    ...
    async def verify_thread_network_connected(self, networks, expected_thread_network_id, step_number, thread_network_name):
        ...
        for network in networks:
            if network.networkID == expected_thread_network_id:
                asserts.assert_true(network.connected, f"{thread_network_name} Thread network is not connected.")
    ...
    async def verify_thread_network_response(self, resp):
        ...
        asserts.assert_true(debug_text is None or debug_text == "", "debugText must be None or empty if status is success.")

        # If debugText is present and not empty, validate len does not exceed 512 char
        if debug_text:
            asserts.assert_true(len(debug_text) <= 512, "debugText must be a string with a max length of 512 characters.")
    ...
    def steps_TC_CNET_4_12(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                4,
                "TH sends RemoveNetwork Command to the DUT with NetworkID field set to `th_xpan_1` and Breadcrumb field set to 1",
            ...
            TestStep(
                11,
                "TH ensures it can communicate with THREAD_1ST_OPERATIONALDATASET operational network (from --thread-dataset-hex)",
                "",
            ),
            TestStep(
                12,
                "TH discovers and connects to DUT on the THREAD_1ST_OPERATIONALDATASET operational network (from --thread-dataset-hex)",
                "Verify that the TH successfully connects to the DUT",
            ),
            TestStep(
                13,
            ...
            TestStep(
                21,
                "TH switches back to THREAD_1ST_OPERATIONALDATASET operational network (from --thread-dataset-hex)",
                "* Verify that the Networks attribute list has an entry with the following values:\n"
                "  . NetworkID field value as the extended PAN ID of PIXIT.CNET.THREAD_2ND_OPERATIONALDATASET\n"
                "  . Connected field value is of type bool and is TRUE",
            ),
    ...
    async def test_TC_CNET_4_12(self):
        ...
        log.info(
            "Precondition: All required arguments are set and assigned, Thread dataset as str: "
        ...
        log.info(
            "Precondition: All required arguments are set and assigned, Thread dataset as bytes: "
        ...
        connect_max_time_seconds = await self.read_single_attribute_check_success(
            cluster=Clusters.NetworkCommissioning, attribute=Clusters.NetworkCommissioning.Attributes.ConnectMaxTimeSeconds
        )
        ...
        log.info("Step #7: Sleep completed for Thread network connection and SRP record propagation")
        ...
        asserts.assert_equal(breadcrumb_info, 2, "The Breadcrumb attribute is not 2")
        ...
        log.info("Step #11: DUT automatically return to THREAD_1ST")
        ...
        # Includes a fudge factor for SRP record propagation.
        await asyncio.sleep(connect_max_time_seconds + fudge_factor_seconds)
        log.info("Step #16: Sleep completed for Thread network connection and SRP record propagation")
        ...
        asserts.assert_equal(breadcrumb_info, 3, "The Breadcrumb attribute is not 3")

        # Wait for the device to establish connection with the new Thread network
        # Includes a fudge factor for SRP record propagation.
        await asyncio.sleep(connect_max_time_seconds + fudge_factor_seconds)
        log.info("Step #18: Sleep completed for Thread network connection and SRP record propagation")
        ...
        cmd = Clusters.NetworkCommissioning.Commands.AddOrUpdateThreadNetwork(operationalDataset=thread_dataset_1_bytes)
        resp = await self.send_single_cmd(dev_ctrl=self.default_controller, node_id=self.dut_node_id, cmd=cmd)
        ...
        cmd = Clusters.NetworkCommissioning.Commands.ConnectNetwork(networkID=thread_network_id_bytes_th1)
        ...
        # Includes a fudge factor for SRP record propagation.
        await asyncio.sleep(connect_max_time_seconds + fudge_factor_seconds)
        log.info("Step #21: Sleep completed for Thread network connection and SRP record propagation")
