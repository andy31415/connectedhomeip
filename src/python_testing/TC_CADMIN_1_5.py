...
class TC_CADMIN_1_5(CADMINBaseTest):
    async def commission_on_network_expect_error(self, setup_code: int, discriminator: int):
        ...
        try:
            ...
            asserts.assert_equal(
                cm.err, ChipDeviceCtrl.CHIP_ERROR_TIMEOUT, "Unexpected error code returned from Commissioning Attempt"
            )
    ...
    async def test_TC_CADMIN_1_5(self):
        ...
        service = await self.wait_for_correct_cm_value(expected_cm_value=2, expected_discriminator=params.randomDiscriminator)
        ...
        self.step(4)
        await self.commission_on_network_expect_error(
            setup_code=params.commissioningParameters.setupPinCode, discriminator=params.randomDiscriminator
        )
        ...
        self.step(7)
        await self.commission_on_network_expect_error(
            setup_code=params2.commissioningParameters.setupPinCode, discriminator=params2.randomDiscriminator
        )
        ...
        try:
            ...
            asserts.assert_true(
                e.clusterStatus,
                Clusters.AdministratorCommissioning.Enums.StatusCode.kWindowNotOpen,
        ...
        try:
            ...
            asserts.assert_equal(
                e.clusterStatus,
                Clusters.AdministratorCommissioning.Enums.StatusCode.kPAKEParameterError,
        ...
        try:
            cmd = Clusters.AdministratorCommissioning.Commands.OpenCommissioningWindow(
                iterations=100001,
                discriminator=3045,
                PAKEPasscodeVerifier=iter100001,
                commissioningTimeout=180,
                salt=b"SPAKE2P_Key_Salt",
            )
            await self.th1.SendCommand(nodeId=self.dut_node_id, endpoint=0, payload=cmd, timedRequestTimeoutMs=2000000)
        except IME as e:
            asserts.assert_equal(
        ...
        try:
            cmd = Clusters.AdministratorCommissioning.Commands.OpenCommissioningWindow(
                iterations=10000,
                discriminator=3045,
                PAKEPasscodeVerifier=long_salt,
                commissioningTimeout=180,
                salt=b"'this pake salt very very very long'",
            )
            await self.th1.SendCommand(nodeId=self.dut_node_id, endpoint=0, payload=cmd, timedRequestTimeoutMs=2000000)
        except IME as e:
            asserts.assert_equal(
        ...
        params3 = await self.open_commissioning_window(
        ...
        with asserts.assert_raises(ChipStackError) as cm:
            await self.open_commissioning_window(
                dev_ctrl=self.th1, timeout=duration.maxCumulativeFailsafeSeconds, node_id=self.dut_node_id
            )
        ...
        asserts.assert_equal(
            code,
            Clusters.AdministratorCommissioning.Enums.StatusCode.kBusy,
        ...
        await self.th2.CommissionOnNetwork(
            nodeId=self.dut_node_id,
            setupPinCode=params3.commissioningParameters.setupPinCode,
            filterType=ChipDeviceCtrl.DiscoveryFilterType.LONG_DISCRIMINATOR,
            filter=params3.randomDiscriminator,
        )
