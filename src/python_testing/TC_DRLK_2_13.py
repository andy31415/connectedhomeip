...
class TC_DRLK_2_13(MatterBaseTest):
    def steps_TC_DRLK_2_13(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "1a",
                "TH reads OperationalCredentials cluster's CurrentFabricIndex and save the attribute",
                "TH Reads Attribute Successfully",
            ),
            TestStep(
                "1b",
                "TH sends ClearUser Command to DUT with the UserIndex as 0xFFFE to clear all the users",
            ...
            TestStep("2b", ",TH sends SetAliroReaderConfig Command to DUT without GroupResolvingKey", "DUT sends success response"),
            TestStep("2c", "TH sends SetAliroReaderConfig Command to DUT with GroupResolvingKey", "DUT sends success response"),
            TestStep(
                "3",
                "TH reads AliroReaderVerificationKey attribute from DUT",
                "Verify that AliroReaderVerificationKey value is same as 'verificationkey'",
            ),
            TestStep(
                "4",
                "TH reads AliroReaderGroupIdentifier attribute from DUT",
                "Verify that AliroReaderGroupIdentifier value is same as 'groupidentifier'",
            ),
            TestStep(
                "5",
                "TH reads AliroGroupResolvingKey attribute from DUT",
            ...
            TestStep("7", "TH sends ClearAliroReaderConfig Command to DUT", "Verify that the DUT sends SUCCESS response"),
            TestStep(
                "8",
                "TH reads AliroReaderVerificationKey attribute from DUT",
            ...
            TestStep(
                "10", "TH reads AliroGroupResolvingKey attribute from DUT", "Verify that AliroGroupResolvingKey value is null"
            ),
            TestStep(
                "11a", ",TH sends SetAliroReaderConfig Command to DUT without GroupResolvingKey", "DUT sends success response"
            ),
            TestStep("11b", "TH sends SetAliroReaderConfig Command to DUT with GroupResolvingKey", "DUT sends success response"),
            TestStep(
                "12a",
            ...
            TestStep(
                "13a",
                "TH sends SetCredential Command Credential as (6, 2)",
            ...
            TestStep(
                "13b",
                "TH sends SetCredential Command with Credential as (6, 1)",
                "Verify that the DUT responds with SetCredentialResponse command, DUT responds with status success in SetCredentialResponse",
            ),
            TestStep(
                "14",
                "TH reads the LockUserChange event list from DUT",
            ...
            TestStep(
                "15",
                "TH sends GetCredentialStatus Command with Credential as (6,1)",
            ...
            TestStep(
                "18",
                "TH sends SetCredential Command to DUT with CredentialType as AliroEvictableEndpointKey",
                "Verify that the DUT responds with SetCredentialResponse command, DUT responds with status success in SetCredentialResponse",
            ),
            TestStep(
                "19",
            ...
            TestStep(
                "20",
                "TH sends GetCredentialStatus Command with Credential as 7 1",
                "Verify DUT responds with GetCredentialStatusResponse having CredentialExists is true and UserIndex as 1",
            ),
            TestStep(
                "21",
            ...
            TestStep(
                "23",
                "TH sends SetCredential Command to DUT with CredentialType as AliroNonEvictableEndpointKey ",
            ...
            TestStep(
                "25",
                "TH sends GetCredentialStatus Command with Credential as 8 1",
                "Verify DUT responds with GetCredentialStatusResponse having CredentialExists is true and UserIndex as 1",
            ),
            TestStep(
                "26",
            ...
            TestStep(
                "28a",
                "Th Reads NumberOfCredentialsSupportedPerUser saves as numberofcredentialsupportedperuser,"
                "Read operation is successful",
            ),
            TestStep(
                "28b",
                "TH sends ClearCredential Command to DUT to clear all the credentials of Aliro type "
            ...
            TestStep("29a", "TH sends SetUser Command to DUT to create an Aliro user", "DUT sends SUCCESS response"),
            TestStep(
                "29b",
            ...
            TestStep(
                "33",
                "TH sends GetCredentialStatus Command with Credential as 8 max_aliro_keys_supported",
                "Verify DUT responds with GetCredentialStatusResponse having CredentialExists is false and UserIndex as 1",
            ),
            TestStep(
                "34",
                "TH sends ClearCredential Command to DUT to clear the ALIRO CredentialType",
                "Verify that the DUT sends SUCCESS response",
            ),
            TestStep(
                "35", "TH sends ClearUser Command to DUT with the UserIndex as 1", "Verify that the DUT sends SUCCESS response"
            ),
        ]
    ...
    async def aliro_attribute_verifiers(self, aliro_attribute_verify_steps: list[AliroAttributeVerify]):
        for aliro_attribute_step in aliro_attribute_verify_steps:
            ...
            if pics_condition:
                dut_aliro_key = await self.read_attributes_from_dut(
                    endpoint=self.app_cluster_endpoint, cluster=Clusters.Objects.DoorLock, attribute=aliro_attribute_step.attribute
                )
                asserts.assert_equal(
                    dut_aliro_key,
                    aliro_attribute_step.attribute_value,
            ...
            if not use_group_resolving_key and pics_check:
                await self.send_single_cmd(
                    cmd=Clusters.DoorLock.Commands.SetAliroReaderConfig(
                        signingKey=self.signingKey, verificationKey=self.verificationKey, groupIdentifier=self.groupIdentifier
                    ),
                    endpoint=self.app_cluster_endpoint,
                    timedRequestTimeoutMs=1000,
                )
    ...
    async def get_credentials_status(
        self, credentialIndex: int, credentialType: cluster.Enums.CredentialTypeEnum, step, userIndex, credential_exists=True
    ):
        if step:
            self.step(step)
        try:
            ...
            if self.pics_guard(all(self.check_pics(p) for p in flags)):
                credentials_struct = cluster.Structs.CredentialStruct(
                    credentialIndex=credentialIndex, credentialType=credentialType
                )
                response = await self.send_single_cmd(
                    endpoint=self.app_cluster_endpoint,
                    timedRequestTimeoutMs=1000,
                    cmd=cluster.Commands.GetCredentialStatus(credential=credentials_struct),
                )
                asserts.assert_true(
                ...
                asserts.assert_equal(userIndex, response.userIndex, f"User Index is not matching, UserIndex={response.userIndex}")
                return response
        except InteractionModelError as e:
            log.error(e)
    ...
    async def set_credential_cmd(
        self,
        credential_enum: Clusters.DoorLock.Enums.CredentialTypeEnum,
        credentialIndex,
        operationType,
        userIndex,
        credentialData,
        ...
        if step:
            self.step(step)
        credentials = cluster.Structs.CredentialStruct(credentialType=credential_enum, credentialIndex=credentialIndex)
        if self.pics_guard(self.check_pics("DRLK.S.F0d") and self.check_pics("DRLK.S.C22.Rsp")):
            try:
                ...
                asserts.assert_true(
                    matchers.is_type(response, Clusters.Objects.DoorLock.Commands.SetCredentialResponse),
    ...
    async def read_and_validate_lock_event_change(
        self, verify_lock_data_type, priority, operation_source, data_type_operation, fabric_index, node_id, data_index, user_index
    ):
        response_event_list = await self.default_controller.ReadEvent(self.dut_node_id, [1, cluster.Events.LockUserChange])
        for event_read_result in reversed(response_event_list):
            ...
            if event_read_result.Data.lockDataType == verify_lock_data_type:
                asserts.assert_equal(
                    event_read_result.Header.Priority, priority, f"Priority mismatch, Priority={event_read_result.Header.Priority}"
                )
                asserts.assert_equal(
                    event_read_result.Data.operationSource,
                    operation_source,
                    f"OperationSource not matching, operationSource={operation_source}",
                )
                asserts.assert_equal(
                    event_read_result.Data.fabricIndex, fabric_index, f"FabricIndex not matching, fabricIndex={fabric_index}"
                )
                asserts.assert_equal(
                    event_read_result.Data.dataIndex, data_index, f"DataIndex not matching, dataIndex={data_index}"
                )
                asserts.assert_equal(event_read_result.Data.sourceNode, node_id, f"SourceNode not matching, sourceNode={node_id}")
                asserts.assert_equal(
                    event_read_result.Data.userIndex, user_index, f"UserIndex not matching, userIndex={user_index}"
                )
                asserts.assert_equal(
                    event_read_result.Data.dataOperationType,
                    data_type_operation,
    ...
    async def clear_all_aliro_credential(self):
        all_aliro_cred_list = []
        aliro_cred_issuer_credentials = cluster.Structs.CredentialStruct(
            credentialIndex=1, credentialType=Clusters.DoorLock.Enums.CredentialTypeEnum.kAliroCredentialIssuerKey
        )
        all_aliro_cred_list.append(aliro_cred_issuer_credentials)
        aliro_evict_endpoint_credentials = cluster.Structs.CredentialStruct(
        ...
        aliro_non_evict_endpoint_credentials = cluster.Structs.CredentialStruct(
            credentialIndex=1, credentialType=Clusters.DoorLock.Enums.CredentialTypeEnum.kAliroNonEvictableEndpointKey
        )
    ...
    async def test_TC_DRLK_2_13(self):
        ...
        self.step("0")
        self.step("1a")
        self.fabric_idx1 = await self.read_attributes_from_dut(
            endpoint=self.common_cluster_endpoint,
        ...
        aliro_attribute_verify_steps = [
            AliroAttributeVerify(
                th_step="3",
                attribute=Clusters.Objects.DoorLock.Attributes.AliroReaderVerificationKey,
                attribute_value=self.verificationKey,
            ),
            AliroAttributeVerify(
                th_step="4",
                attribute=Clusters.Objects.DoorLock.Attributes.AliroReaderGroupIdentifier,
                attribute_value=self.groupIdentifier,
            ),
            AliroAttributeVerify(
                th_step="5",
                attribute=Clusters.Objects.DoorLock.Attributes.AliroGroupResolvingKey,
        ...
        #  step 6
        self.step("6a")
        await self.send_set_aliro_reader_config_cmd(use_group_resolving_key=False, expected_status=Status.InvalidInState)
        self.step("6b")
        ...
        aliro_attribute_verify_steps = [
            AliroAttributeVerify(
                th_step="8", attribute=Clusters.Objects.DoorLock.Attributes.AliroReaderVerificationKey, attribute_value=NullValue
            ),
            AliroAttributeVerify(
                th_step="9", attribute=Clusters.Objects.DoorLock.Attributes.AliroReaderGroupIdentifier, attribute_value=NullValue
            ),
            AliroAttributeVerify(
                th_step="10", attribute=Clusters.Objects.DoorLock.Attributes.AliroGroupResolvingKey, attribute_value=NullValue
            ),
        ]
        await self.aliro_attribute_verifiers(aliro_attribute_verify_steps=aliro_attribute_verify_steps)
        ...
        if self.pics_guard(self.check_pics("DRLK.S.A0088")):
            self.max_aliro_keys_supported = await self.read_attributes_from_dut(
                endpoint=self.app_cluster_endpoint,
        ...
        await self.set_credential_cmd(
            credentialData=self.alirononevictableendpointkey1,
        ...
        self.step("13b")
        await self.set_credential_cmd(
            credentialData=self.alirocredentialissuerkey,
            operationType=cluster.Enums.DataOperationTypeEnum.kAdd,
        ...
        await self.read_and_validate_lock_event_change(
            ...
            user_index=1,
            data_index=1,
            fabric_index=self.fabric_idx1,
            node_id=self.matter_test_config.controller_node_id,
            data_type_operation=Clusters.DoorLock.Enums.DataOperationTypeEnum.kAdd,
        ...
        await self.get_credentials_status(
            credentialIndex=1, credentialType=cluster.Enums.CredentialTypeEnum.kAliroCredentialIssuerKey, step=None, userIndex=1
        )

        # step 16
        credentials = cluster.Structs.CredentialStruct(
            credentialIndex=1, credentialType=cluster.Enums.CredentialTypeEnum.kAliroCredentialIssuerKey
        )
        ...
        await self.read_and_validate_lock_event_change(
            ...
            fabric_index=self.fabric_idx1,
            node_id=self.matter_test_config.controller_node_id,
            data_type_operation=Clusters.DoorLock.Enums.DataOperationTypeEnum.kClear,
            priority=EventPriority.INFO,
        )
        ...
        await self.set_credential_cmd(
            step="18",
            credentialData=self.aliroevictableendpointkey,
            operationType=cluster.Enums.DataOperationTypeEnum.kAdd,
        ...
        # step 20
        await self.get_credentials_status(
            credentialIndex=1, credentialType=cluster.Enums.CredentialTypeEnum.kAliroEvictableEndpointKey, step="20", userIndex=1
        )
        # step 21
        evictable_credentials = cluster.Structs.CredentialStruct(
        ...
        await self.set_credential_cmd(
            step="23",
            credentialData=self.alirononevictableendpointkey,
            operationType=cluster.Enums.DataOperationTypeEnum.kAdd,
            credential_enum=cluster.Enums.CredentialTypeEnum.kAliroNonEvictableEndpointKey,
            credentialIndex=1,
            userIndex=1,
        ...
        # step 25
        await self.get_credentials_status(
            step="25", credentialIndex=1, credentialType=cluster.Enums.CredentialTypeEnum.kAliroNonEvictableEndpointKey, userIndex=1
        )

        # step 26
        non_evictable_credentials = cluster.Structs.CredentialStruct(
            credentialIndex=1, credentialType=cluster.Enums.CredentialTypeEnum.kAliroNonEvictableEndpointKey
        )
        ...
        log.info(
            f"After reading  attribute NumberOfCredentialsSupportedPerUser we get value {self.numberofcredentialsupportedperuser} "
        )
        self.step("28b")
        ...
        if self.pics_guard(self.check_pics("DRLK.S.A001c") and self.check_pics("DRLK.S.A0088")):
            ...
            while 1:
                if start_credential_index <= (self.max_aliro_keys_supported - 1):
                    ...
                    await self.set_credential_cmd(
                        credentialData=credentials_data,
                        operationType=cluster.Enums.DataOperationTypeEnum.kAdd,
                        credential_enum=cluster.Enums.CredentialTypeEnum.kAliroNonEvictableEndpointKey,
            ...
            await self.set_credential_cmd(
                ...
                operationType=cluster.Enums.DataOperationTypeEnum.kAdd,
                credential_enum=cluster.Enums.CredentialTypeEnum.kAliroNonEvictableEndpointKey,
                credentialIndex=self.max_aliro_keys_supported,
                userIndex=1,
                userStatus=NullValue,
                userType=NullValue,
