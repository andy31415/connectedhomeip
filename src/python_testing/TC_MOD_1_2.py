...
class TC_MOD_1_2(MatterBaseTest):
    ...
    def _verify_supported_mode(self, supported_mode):
        ...
        if isinstance(supported_mode.semanticTags, list) and len(supported_mode.semanticTags) > 0:
            ...
            for semantictag in supported_mode.semanticTags:
                asserts.assert_true(
                    semantictag.mfgCode >= 0 and semantictag.mfgCode <= 0xFFFF,
                    "Element semantictag.Mfgcode is greater than 16 bits",
                )
                asserts.assert_true(
                    semantictag.value >= 0 and semantictag.value <= 0xFFFF, "Element semantictag.Value is greater than 16 bits"
                )
    ...
    async def test_TC_MOD_1_2(self):
        ...
        supported_modes = await self.read_single_attribute_check_success(
            endpoint=self.endpoint, cluster=self.cluster, attribute=self.cluster.Attributes.SupportedModes
        )
        ...
        asserts.assert_equal(
            len(supported_modes_values),
            len(set(supported_modes_values)),
        ...
        current_mode = await self.read_single_attribute_check_success(
            endpoint=self.endpoint, cluster=self.cluster, attribute=self.cluster.Attributes.CurrentMode
        )
        ...
        if await self.attribute_guard(endpoint=self.endpoint, attribute=self.cluster.Attributes.OnMode):
            on_mode = await self.read_single_attribute_check_success(
                endpoint=self.endpoint, cluster=self.cluster, attribute=self.cluster.Attributes.OnMode
            )
            # On mode can be Nullvalue
            self._log_attribute("OnMode", on_mode)
            asserts.assert_true((isinstance(on_mode, int) or on_mode is NullValue), "Onmode is not int or is not Nullvalue")
        ...
        if await self.attribute_guard(endpoint=self.endpoint, attribute=self.cluster.Attributes.StartUpMode):
            startup_mode = await self.read_single_attribute_check_success(
                endpoint=self.endpoint, cluster=self.cluster, attribute=self.cluster.Attributes.StartUpMode
            )
            self._log_attribute("StartupMode", startup_mode)
            asserts.assert_true(isinstance(startup_mode, int), "Startupmode is not int")
            asserts.assert_in(
                startup_mode, supported_modes_values, f"Startupmode {startup_mode} is not in {supported_modes_values}"
            )

        # Verify the string  is str and larger that 1 char.
        self.step(6)
        description = await self.read_single_attribute_check_success(
            endpoint=self.endpoint, cluster=self.cluster, attribute=self.cluster.Attributes.Description
        )
        ...
        self.step(7)
        standard_namepace = await self.read_single_attribute_check_success(
            endpoint=self.endpoint, cluster=self.cluster, attribute=self.cluster.Attributes.StandardNamespace
        )
        self._log_attribute("StandardNamespace", standard_namepace)
        asserts.assert_true(
            (
                standard_namepace is NullValue
                or (isinstance(standard_namepace, int) and (standard_namepace >= 0 and standard_namepace <= 0xFFFF))
            ),
