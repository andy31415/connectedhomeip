...
class TC_EEM_2_1(MatterBaseTest, EnergyReportingBaseTestHelper):
    ...
    def steps_TC_EEM_2_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "2", "TH reads Accuracy attribute", "Verify that the DUT response contains a MeasurementAccuracyStruct value."
            ),
            TestStep(
                "3",
                "TH reads CumulativeEnergyImported attribute",
                "Verify that the DUT response contains either null or an EnergyMeasurementStruct value.",
            ),
            TestStep(
                "4",
                "TH reads CumulativeEnergyExported attribute",
                "Verify that the DUT response contains either null or an EnergyMeasurementStruct value.",
            ),
            TestStep(
                "5",
                "TH reads PeriodicEnergyImported attribute",
                "Verify that the DUT response contains either null or an EnergyMeasurementStruct value.",
            ),
            TestStep(
                "6",
                "TH reads PeriodicEnergyExported attribute",
                "Verify that the DUT response contains either null or an EnergyMeasurementStruct value.",
            ),
            TestStep(
                "7",
                "TH reads CumulativeEnergyReset attribute",
    ...
    async def test_TC_EEM_2_1(self):
        ...
        asserts.assert_equal(
            accuracy.measurementType,
            Clusters.ElectricalEnergyMeasurement.Enums.MeasurementTypeEnum.kElectricalEnergy,
        ...
        if self.pics_guard(self.check_pics("EEM.S.A0001")):
            ...
            log.info(f"Rx'd CumulativeEnergyImported: {cumulativeEnergyImported}")
        ...
        if self.pics_guard(self.check_pics("EEM.S.A0002")):
            ...
            log.info(f"Rx'd CumulativeEnergyExported: {cumulativeEnergyExported}")
        ...
        if self.pics_guard(self.check_pics("EEM.S.A0003")):
            ...
            log.info(f"Rx'd PeriodicEnergyImported: {periodicEnergyImported}")
        ...
        if self.pics_guard(self.check_pics("EEM.S.A0004")):
            ...
            log.info(f"Rx'd PeriodicEnergyExported: {periodicEnergyExported}")
