...
class TC_EPM_2_1(MatterBaseTest, EnergyReportingBaseTestHelper):
    ...
    async def test_TC_EPM_2_1(self):
        ...
        asserts.assert_not_equal(
            power_mode, Clusters.ElectricalPowerMeasurement.Enums.PowerModeEnum.kUnknown, "PowerMode must not be Unknown"
        )
        ...
        asserts.assert_greater_equal(number_of_measurements, 1, "NumberOfMeasurementTypes must be >= 1")
        ...
        for measurement in accuracy:
            ...
            asserts.assert_equal(
                measurement.minMeasuredValue,
                measurement.accuracyRanges[0].rangeMin,
            ...
            for index, range_entry in enumerate(measurement.accuracyRanges):
                log.info(
                    f"   [{index}] rangeMin:{range_entry.rangeMin} rangeMax:{range_entry.rangeMax} percentMax:{range_entry.percentMax} percentMin:{range_entry.percentMin} percentTypical:{range_entry.percentTypical} fixedMax:{range_entry.fixedMax} fixedMin:{range_entry.fixedMin} fixedTypical:{range_entry.fixedTypical}"
                )
            ...
            asserts.assert_equal(
                measurement.maxMeasuredValue, prev_range_max, "maxMeasuredValue must be the same as the last accuracyRange rangeMax"
            )
            asserts.assert_equal(
                maximum_range,
        ...
        asserts.assert_equal(
            len(accuracy), number_of_measurements, "The number of accuracy entries should match the NumberOfMeasurementTypes"
        )
        ...
        if self.pics_guard(Clusters.ElectricalPowerMeasurement.Attributes.ActiveCurrent.attribute_id in supported_attributes):
            active_current = await self.check_epm_attribute_in_range(
                "ActiveCurrent", MIN_INT64_ALLOWED, MAX_INT64_ALLOWED, allow_null=True
            )
        ...
        if self.pics_guard(Clusters.ElectricalPowerMeasurement.Attributes.ReactiveCurrent.attribute_id in supported_attributes):
            reactive_current = await self.check_epm_attribute_in_range(
                "ReactiveCurrent", MIN_INT64_ALLOWED, MAX_INT64_ALLOWED, allow_null=True
            )
        ...
        if self.pics_guard(Clusters.ElectricalPowerMeasurement.Attributes.ActivePower.attribute_id in supported_attributes):
            active_power = await self.check_epm_attribute_in_range(
                "ActivePower", MIN_INT64_ALLOWED, MAX_INT64_ALLOWED, allow_null=True
            )
        ...
        if self.pics_guard(Clusters.ElectricalPowerMeasurement.Attributes.ReactivePower.attribute_id in supported_attributes):
            reactive_power = await self.check_epm_attribute_in_range(
                "ReactivePower", MIN_INT64_ALLOWED, MAX_INT64_ALLOWED, allow_null=True
            )
        ...
        if self.pics_guard(Clusters.ElectricalPowerMeasurement.Attributes.ApparentPower.attribute_id in supported_attributes):
            apparent_power = await self.check_epm_attribute_in_range(
                "ApparentPower", MIN_INT64_ALLOWED, MAX_INT64_ALLOWED, allow_null=True
            )
        ...
        if self.pics_guard(Clusters.ElectricalPowerMeasurement.Attributes.RMSVoltage.attribute_id in supported_attributes):
            rms_voltage = await self.check_epm_attribute_in_range(
                "RMSVoltage", MIN_INT64_ALLOWED, MAX_INT64_ALLOWED, allow_null=True
            )
        ...
        if self.pics_guard(Clusters.ElectricalPowerMeasurement.Attributes.RMSCurrent.attribute_id in supported_attributes):
            rms_current = await self.check_epm_attribute_in_range(
                "RMSCurrent", MIN_INT64_ALLOWED, MAX_INT64_ALLOWED, allow_null=True
            )
        ...
        if self.pics_guard(Clusters.ElectricalPowerMeasurement.Attributes.HarmonicCurrents.attribute_id in supported_attributes):
            ...
            for index, entry in enumerate(harmonic_currents):
                log.info(f"   [{index}] order:{entry.order} measurement:{entry.measurement}")
        ...
        if self.pics_guard(Clusters.ElectricalPowerMeasurement.Attributes.NeutralCurrent.attribute_id in supported_attributes):
            neutral_current = await self.check_epm_attribute_in_range(
                "NeutralCurrent", MIN_INT64_ALLOWED, MAX_INT64_ALLOWED, allow_null=True
            )
