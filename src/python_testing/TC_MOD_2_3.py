...
class TC_MOD_2_3(MatterBaseTest):
    ...
    def steps_TC_MOD_2_3(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "0a",
                "TH sends KeySetWrite command in the GroupKeyManagement cluster to DUT using a key that is pre-installed on the TH. EpochKey0 only.",
            ),
            TestStep(
                "0b",
            ...
            TestStep(
                "1a",
                "If the Groupcast cluster is enabled on the RootNode endpoint, the TH sends Groupcast JoinGroup command with GroupID 1, Endpoints set with the EndpointId where ModeSelect cluster is enabled and KeySetID 0x01a1 to DUT. Otherwise, TH sends AddGroup command to DUT with the GroupID field set to 1.",
            ),
            TestStep(
                "1b",
                "TH sends a _RemoveAllScenes_ command to DUT with the _GroupID_ field set to _G~1~_. DUT sends a _RemoveAllScenes_ command to TH with the _Status_ field set to 0x00 (SUCCESS) and _GroupID_ field set to _G~1~_.",
            ),
            TestStep(
                "1c",
                "TH sends a _GetSceneMembership_ command to DUT with the _GroupID_ field set to _G~1~_. DUT sends a _GetSceneMembershipResponse_ command to TH with the _Status_ field set to 0x00 (SUCCESS), the _Capacity_ field recorded into _SC~0~_ for later use, the _GroupID_ field set to _G~1~_ and the _SceneList_ field containing 0 entry",
            ),
            TestStep(
                "2",
                "TH reads the _SupportedModes attribute_ from the DUT. Verify that the DUT response provides a list of supported modes. Record the list of supported modes under _L~1~_ for usage in step 5a. Skip tests 3 to 7 if less than 1 mode is supported.",
            ),
            TestStep(
                "3",
                "TH reads the _CurrentMode attribute_ from the DUT. Record the mode value as _M~Initial~_ for usage in steps 5a.",
            ),
            TestStep(
                "4a",
                "TH sends a _StoreScene_ command to DUT with the _GroupID_ field set to _G~1~_ and the _SceneID_ field set to 0x01. DUT sends a _StoreSceneResponse_ command to TH with the _Status_ field set to 0x00 (SUCCESS), the _GroupID_ field set to _G~1~_ and the _SceneID_ field set to 0x01.",
            ),
            TestStep(
                "4b",
                "TH sends a _ViewScene_ command to DUT with the _GroupID_ field set to _G~1~_ and the _SceneID_ field set to 0x01. DUT sends a _ViewSceneResponse_ command to TH with the _Status_ field set to 0x00 (SUCCESS), the _GroupID_ field set to _G~1~_, the _SceneID_ field set to 0x01, the _TransitionTime_ field set to 0 and a set of extension fields appropriate to the values set in step 4a.",
            ),
            TestStep(
                "5a",
            ...
            TestStep(
                "6b",
                "After a few seconds, TH reads the _CurrentMode attribute_ from DUT. Value has to be the same as _M~1~_ from step 5a.",
            ),
            TestStep(
                "7a",
                "TH sends a _RecallScene_ command to DUT with the _GroupID_ field set to _G~1~_, the _SceneID_ field set to 0x01 and the _TransitionTime_ omitted.",
            ),
            TestStep(
                "7b",
                "After a few seconds, TH reads the _CurrentMode attribute_ from DUT. Value has to be the same as _M~Initial~_ from step 3.",
            ),
        ]
    ...
    async def test_TC_MOD_2_3(self):
        ...
        self.step("5b")
        result = await self.TH1.SendCommand(
            self.dut_node_id, self.matter_test_config.endpoint, Clusters.ScenesManagement.Commands.ViewScene(self.kGroup1, 0x02)
        )
        asserts.assert_equal(result.status, Status.Success, "View Scene failed on status")
        asserts.assert_equal(result.groupID, self.kGroup1, "View Scene failed on groupID")
        ...
        for EFS in result.extensionFieldSetStructs:
            ...
            for AV in EFS.attributeValueList:
                if AV.attributeID == current_mode_attribute_id:
                    asserts.assert_equal(AV.valueUnsigned8, mode_select_m1_mode, "View Scene failed on CurrentMode value")
                    found_var = True
        asserts.assert_true(found_var, "View Scene failed to find CurrentMode attribute in ExtensionFieldSets")
        ...
        asserts.assert_equal(
            current_mode_after_recall, mode_select_m1_mode, "CurrentMode is not equal to expected value after Recall Scene"
        )

        self.step("7a")
        result = await self.TH1.SendCommand(
            self.dut_node_id, self.matter_test_config.endpoint, Clusters.ScenesManagement.Commands.RecallScene(self.kGroup1, 0x01)
        )
        ...
        current_mode_after_recall = await self.read_single_attribute_check_success(cluster, attributes.CurrentMode)
        asserts.assert_equal(
            current_mode_after_recall, mode_select_initial_mode, "CurrentMode is not equal to expected value after Recall Scene"
        )
