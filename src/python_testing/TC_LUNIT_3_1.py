...
class TC_LUNIT_3_1(MatterBaseTest):
    ...
    async def write_lunit_temp_unit(
        self, endpoint, temp_unit, dev_ctrl: ChipDeviceCtrl = None, expected_status: list[Status] = [Status.Success]
    ) -> Status:
        if dev_ctrl is None:
        ...
        asserts.assert_in(
            status, expected_status, f"TemperatureUnit write returned {status.name}; expected {[e.name for e in expected_status]}"
        )
        return status
    ...
    def steps_TC_LUNIT_3_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                2,
                "If supported, TH reads from the DUT the SupportedTemperatureUnits attribute",
            ...
            TestStep(
                3,
                "If the SupportedTemperatureUnits is supported, TH creates a `supported` list With each entry in SupportedUnitsList. If SupportedTemperatureUnits is not supported, TH creates a `supported` list with Celsius and Fahrenheit. TH writes each entry in the `supported` list to the DUT the TemperatureUnit attribute",
                "Write is successful",
            ),
            TestStep(
                4,
                "If the SupportedTemperatureUnits is supported, TH construct a list with valid TempUnitEnum values that are not contained in SupportedUnitsList",
            ),
            TestStep(
                5,
                "if the SupportedTemperatureUnits is supported, With each entry in UnsupportedUnitsList, TH writes to the DUT the TemperatureUnit attribute",
                "Write returns CONSTRAINT_ERROR",
            ),
            TestStep(
                6,
                "If SupportedTemperatureUnits is not supported, TH writes Kelvin",
    ...
    @run_if_endpoint_matches(
        has_cluster(Clusters.UnitLocalization) and has_attribute(Clusters.UnitLocalization.Attributes.TemperatureUnit)
    )
    async def test_TC_LUNIT_3_1(self):
        ...
        if self.supports_TEMP:
            ...
            self.step(1)
            temperature_unit = await self.read_lunit_attribute_expect_success(
                endpoint=endpoint, attribute=attributes.TemperatureUnit
            )

            asserts.assert_greater_equal(
                temperature_unit,
                Clusters.Objects.UnitLocalization.Enums.TempUnitEnum.kFahrenheit,
            ...
            if has_supported_temperature_units:
                supported_temperature_units = await self.read_lunit_attribute_expect_success(
                    endpoint=endpoint, attribute=attributes.SupportedTemperatureUnits
                )
                asserts.assert_greater_equal(
                    len(supported_temperature_units), 2, "SupportedTemperatureUnits must have at least two entry in the list"
                )
                asserts.assert_less_equal(
                    len(supported_temperature_units), 3, "SupportedTemperatureUnits may have a maximum of three entries in the list"
                )
            else:
                ...
                supported_temperature_units = [
                    Clusters.UnitLocalization.Enums.TempUnitEnum.kCelsius,
            ...
            if not has_supported_temperature_units:
                await self.write_lunit_temp_unit(
                    endpoint=endpoint, temp_unit=unit, expected_status=[Status.Success, Status.ConstraintError]
                )
            else:
                self.mark_current_step_skipped()
