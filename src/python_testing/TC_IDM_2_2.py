...
class TC_IDM_2_2(IDMBaseTest, BasicCompositionTests):
    ...
    def steps_TC_IDM_2_2(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                2,
                "TH sends the Read Request Message to the DUT to read all attributes on a given cluster and Endpoint, AttributePath = [[Endpoint = Specific Endpoint, Cluster = Specific ClusterID]], On receipt of this message, DUT should send a report data action with the attribute value to the DUT.",
                "On the TH verify the received report data message has only the attributes that the TH has privilege to.",
            ),
            TestStep(
                3,
                "TH sends the Read Request Message to the DUT to read an attribute from a cluster at all Endpoints, AttributePath = [[Cluster = Specific ClusterID, Attribute = Specific Attribute]], On receipt of this message, DUT should send a report data action with the attribute value from all the Endpoints to the DUT.",
            ...
            TestStep(
                5,
                "TH sends the Read Request Message to the DUT to read all attributes from all clusters on all Endpoints, AttributePath = [[]], On receipt of this message, DUT should send a report data action with the attribute value from all the clusters to the DUT.",
                "On the TH verify the received report data message has all the right attribute values.",
            ),
            TestStep(
                6,
                "TH sends the Read Request Message to the DUT to read a global attribute from all clusters at all Endpoints, AttributePath = [[Attribute = Specific Global Attribute]], On receipt of this message, DUT should send a report data action with the attribute value from all the clusters to the DUT.",
                "On the TH verify the received report data message has all the right attribute values.",
            ),
            TestStep(
                7,
                "TH sends the Read Request Message to the DUT to read all attributes from a cluster at all Endpoints, AttributePath = [[Cluster = Specific ClusterID]], On receipt of this message, DUT should send a report data action with the attribute value from all the Endpoints to the DUT.",
                "On the TH verify the received report data message has all the right attribute values.",
            ),
            TestStep(
                8,
                "TH sends the Read Request Message to the DUT to read all attributes from all clusters at one Endpoint, AttributePath = [[Endpoint = Specific Endpoint]], On receipt of this message, DUT should send a report data action with the attribute value from all the Endpoints to the DUT.",
                "On the TH verify the received report data message has all the right attribute values.",
            ),
            TestStep(
                9,
            ...
            TestStep(
                11,
                "TH sends the Read Request Message to the DUT to read an unsupported attribute, DUT responds with the report data action.",
                "Verify on the TH that the DUT sends the status code UNSUPPORTED_ATTRIBUTE",
            ),
            TestStep(
                12,
                "TH sends the Read Request Message to the DUT to read an attribute. Repeat the above steps 3 times.",
            ...
            TestStep(
                18,
                "TH sends a Read Request Message to the DUT to read a non global attribute from all clusters at that Endpoint, AttributePath = [[Endpoint = Specific Endpoint, Attribute = Specific Non Global Attribute]] +",
                "On the TH verify that the DUT sends an error message and not the value of the attribute.",
            ),
            TestStep(
                19,
                "TH sends a Read Request Message to the DUT to read a non global attribute from all clusters at all Endpoints, AttributePath = [[Attribute = Specific Non Global Attribute]] +",
                "On the TH verify that the DUT sends an error message and not the value of the attribute.",
            ),
            TestStep(
                20,
                "TH should have access to only a single cluster at one Endpoint1. TH sends a Read Request Message to the DUT to read all attributes from all clusters at Endpoint1, AttributePath = [[Endpoint = Specific Endpoint]] +",
    ...
    async def test_TC_IDM_2_2(self):
        ...
        path = AttributePath(
            ...
            ClusterId=Clusters.Descriptor.id,
        ...
        path = AttributePath(
            EndpointId=None, ClusterId=Clusters.Descriptor.id, AttributeId=Clusters.Descriptor.Attributes.ServerList.attribute_id
        )
        await self.verify_attribute_read([path])
        ...
        path = AttributePath(
            EndpointId=self.endpoint, ClusterId=None, AttributeId=global_attribute_ids.GlobalAttributeIds.ATTRIBUTE_LIST_ID
        )
        await self.verify_attribute_read([path])

        self.step(5)
        read_request = await asyncio.wait_for(self.default_controller.Read(self.dut_node_id, [()]), timeout=120.0)
        self.verify_empty_wildcard([()], read_request)

        self.step(6)
        await self.read_global_attribute_all_endpoints(attribute_id=global_attribute_ids.GlobalAttributeIds.ATTRIBUTE_LIST_ID)

        self.step(7)
        await self.read_cluster_all_endpoints(cluster=Clusters.Descriptor)

        self.step(8)
        await self.read_endpoint_all_clusters(endpoint=self.endpoint)
        ...
        await self.read_repeat_attribute(
            endpoint=self.endpoint, cluster=Clusters.Descriptor, attribute=Clusters.Descriptor.Attributes.ServerList, repeat_count=3
        )

        self.step(13)
        read_request13, filtered_read13 = await self.read_data_version_filter(
            endpoint=self.endpoint, cluster=Clusters.Descriptor, attribute=Clusters.Descriptor.Attributes.ServerList
        )
        ...
        if Clusters.BasicInformation not in self.endpoints[self.endpoint]:
            ...
            if filtered_read14 and 0 in filtered_read14:
                ...
                asserts.assert_equal(
                    filtered_read14[0][Clusters.BasicInformation][Clusters.BasicInformation.Attributes.NodeLabel],
                    "Hello World",
                    "Data version does not match expected value",
                )
                asserts.assert_equal(
                    (read_request14[0][Clusters.BasicInformation][Clusters.Attribute.DataVersion] + 1),
                    data_version14,
            ...
            await self.default_controller.WriteAttribute(
                self.dut_node_id, [(self.endpoint, Clusters.BasicInformation.Attributes.NodeLabel("Goodbye World"))]
            )
            ...
            filtered_read15 = await self.default_controller.ReadAttribute(
                self.dut_node_id, [(self.endpoint, Clusters.BasicInformation)], dataVersionFilters=data_version_filter15
            )
            ...
            asserts.assert_equal(
                ...
                "Goodbye World",
                "NodeLabel value does not match expected value",
            )
            ...
            asserts.assert_equal(
                returned_attrs15, expected_attrs15, "All cluster attributes should be returned when data version changed"
            )
            ...
            if filtered_read16 and 0 in filtered_read16:
                asserts.assert_equal(
                    filtered_read16[0][Clusters.BasicInformation][Clusters.BasicInformation.Attributes.NodeLabel],
                    "Hello World Again",
                    "Data version does not match expected value",
                )
        ...
        await self.read_non_global_attribute_across_all_clusters(
            endpoint=self.endpoint, attribute=Clusters.Descriptor.Attributes.ServerList
        )

        self.step(19)
        await self.read_non_global_attribute_across_all_clusters(attribute=Clusters.Descriptor.Attributes.ServerList)
        ...
        if Clusters.BasicInformation not in self.endpoints[self.endpoint]:
            ...
            original_acl21, read_request21 = await self.read_limited_access(
                endpoint=self.endpoint, cluster_id=Clusters.BasicInformation.id
            )

            # Verify only BasicInformation cluster is returned (the one we granted access to)
            asserts.assert_true(
                Clusters.BasicInformation in read_request21.attributes[self.endpoint],
                "BasicInformation cluster should be present (granted View access)",
            )

            # Verify we got attributes from BasicInformation
            asserts.assert_true(
                len(read_request21.attributes[self.endpoint][Clusters.BasicInformation]) > 0,
