...
class TC_EEVSE_2_7(MatterBaseTest, EEVSEBaseTestHelper):
    ...
    def steps_TC_EEVSE_2_7(self) -> list[TestStep]:
        return [
            ...
            TestStep("4", "TH sends command Disable", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep(
                "5",
            ...
            TestStep("6", "TH reads from the DUT the State", "Value has to be 0x00 (NotPluggedIn)"),
            TestStep(
                "7",
            ...
            TestStep(
                "8",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.EEVSE.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.EEVSE.TEST_EVENT_TRIGGER for EVSE Set SoC Low Test Event",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep("8a", "TH reads from the DUT the StateOfCharge", "Value has to be 20 % state of charge."),
            TestStep("8b", "TH reads from the DUT the BatteryCapacity", "Value has to be 70,000,000 (70kWh)."),
            TestStep("9", "If ChargingPreferences feature is not supported, skip all remaining test steps."),
            TestStep("10", "TH sends command ClearTargets", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep("10a", "TH reads from the DUT the NextChargeStartTime", "Value has to be null."),
            TestStep("10b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be null."),
            TestStep("10c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be null."),
            TestStep("10d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be null."),
            TestStep(
                "11",
            ...
            TestStep("11a", "TH reads from the DUT the NextChargeStartTime", "Value has to be null."),
            TestStep("11b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be null."),
            TestStep("11c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be null."),
            TestStep("11d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be null."),
            TestStep(
                "12",
            ...
            TestStep("12a", "TH reads from the DUT the NextChargeStartTime", "Value has to be less than the value of TargetTime."),
            TestStep("12b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be equal to the value of TargetTime."),
            TestStep("12c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be null."),
            TestStep("12d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be equal to the value of TargetSoC."),
            TestStep(
                "13",
                "TH sends command Disable",
                "Verify DUT responds w/ status SUCCESS(0x00)\nNOTE: in this step, the EVSE is disabled so all information about future charging should be null.",
            ),
            TestStep("13a", "TH reads from the DUT the NextChargeStartTime", "Value has to be null."),
            TestStep("13b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be null."),
            TestStep("13c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be null."),
            TestStep("13d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be null."),
            TestStep(
                "14",
            ...
            TestStep("14a", "TH reads from the DUT the StateOfCharge", "Value has to be 95% state of charge."),
            TestStep("14b", "TH reads from the DUT the BatteryCapacity", "Value has to be 70,000,000 (70kWh)."),
            TestStep(
                "15",
                "TH sends command EnableCharging with ChargingEnabledUntil=null, minimumChargeCurrent=6000, maximumChargeCurrent=60000",
            ...
            TestStep("15a", "TH reads from the DUT the NextChargeStartTime", "Value has to be null."),
            TestStep("15b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be equal to the value of TargetTime."),
            TestStep("15c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be null."),
            TestStep("15d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be equal to the value of TargetSoC."),
            TestStep(
                "16",
                "TH sends command Disable",
                "Verify DUT responds w/ status SUCCESS(0x00)\nNOTE: in this step, the EVSE is disabled so all information about future charging should be null.",
            ...
            TestStep("17a", "TH reads from the DUT the StateOfCharge", "Value has to be null."),
            TestStep("17b", "TH reads from the DUT the BatteryCapacity", "Value has to be null."),
            TestStep(
                "18",
                "TH sends command EnableCharging with ChargingEnabledUntil=null, minimumChargeCurrent=6000, maximumChargeCurrent=60000",
            ...
            TestStep("18a", "TH reads from the DUT the NextChargeStartTime", "Value has to be less than the value of TargetTime."),
            TestStep("18b", "TH reads from the DUT the NextChargeTargetTime", "Value has to be equal to the value of TargetTime."),
            TestStep(
                "18c", "TH reads from the DUT the NextChargeRequiredEnergy", "Value has to be equal to the value of AddedEnergy."
            ),
            TestStep("18d", "TH reads from the DUT the NextChargeTargetSoC", "Value has to be null."),
        ]
    ...
    async def test_TC_EEVSE_2_7(self):
        ...
        self.step("9")
        has_pref = self.feature_guard(
            endpoint=endpoint, cluster=cluster, feature_int=Clusters.EnergyEvse.Bitmaps.Feature.kChargingPreferences
        )
        ...
        if not has_pref:
            ...
            dailyTargets = [
                Clusters.EnergyEvse.Structs.ChargingTargetStruct(
                    targetTimeMinutesPastMidnight=minutes_past_midnight, targetSoC=target_soc, addedEnergy=added_energy
                )
            ]
            targets = [
