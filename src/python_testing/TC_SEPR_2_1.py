...
class TC_SEPR_2_1(CommodityPriceTestBaseHelper, MatterBaseTest):
    ...
    async def test_TC_SEPR_2_1(self):
        ...
        self.step("3")
        val = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.Currency
        )
        if val is not NullValue:
            asserts.assert_true(isinstance(val, Globals.Structs.CurrencyStruct), "val must be of type CurrencyStruct")
        ...
        if await self.attribute_guard(endpoint=endpoint, attribute=attributes.PriceForecast):
            ...
            matter_asserts.assert_list_element_type(
                val,
                "PriceForecast attribute must contain CommodityPriceStruct elements",
            ...
            for item in val:
                ...
                self.check_CommodityPriceStruct(
                    cluster=cluster, struct=item, details=Clusters.CommodityPrice.Bitmaps.CommodityPriceDetailBitmap(0)
                )
