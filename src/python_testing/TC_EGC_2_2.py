...
class TC_EGC_2_2(ElectricalGridConditionsTestBaseHelper, MatterBaseTest):
    ...
    async def test_TC_EGC_2_2(self):
        ...
        event_data = events_callback.wait_for_event_report(Clusters.ElectricalGridConditions.Events.CurrentConditionsChanged)

        newCurrentConditions = event_data.currentConditions
        if newCurrentConditions is not NullValue:
            asserts.assert_true(
                isinstance(newCurrentConditions, cluster.Structs.ElectricalGridConditionsStruct),
                "val must be of type ElectricalGridConditionsStruct",
            )

            # The other aspects of this verification are handled by the helper
            self.check_ElectricalGridConditionsStruct(cluster=cluster, struct=newCurrentConditions)
        ...
        val = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=attributes.CurrentConditions
        )
