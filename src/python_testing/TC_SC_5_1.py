...
class TC_SC_5_1(MatterBaseTest):
    ...
    def steps_TC_SC_5_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "1",
                "TH writes the ACL attribute in the Access Control cluster to add Operate privileges for group 0x0103 and maintain the current administrative privileges for the TH.",
            ),
            TestStep(
                "2a", "TH sends KeySetWrite command with a key that is NOT installed on the TH (to test key update in next step)."
            ),
            TestStep("2b", "TH sends KeySetWrite command with a key that is pre-installed on the TH."),
            TestStep(
                "3",
                "If Groupcast cluster is enabled on the RootNode endpoint, skip to step 7. Otherwise, TH binds GroupId 0x0103 with GroupKeySetID 0x01a3 in the GroupKeyMap attribute.",
            ),
            ...
            TestStep(
                "8", "TH sends JoinGroup command to Groupcast cluster on EP0 with GroupID 0x0103, Endpoints, KeySetID 0x01a3."
            ),
    ...
    async def run_test_against_endpoint(self, groups_endpoint: int):
        ...
        if groupcast_enabled:
            ...
            self.step("3")
            mapping = [Clusters.GroupKeyManagement.Structs.GroupKeyMapStruct(groupId=0x0103, groupKeySetID=0x01A3, fabricIndex=1)]
            result = await dev_ctrl.WriteAttribute(
                commissioner_node_id, [(0, Clusters.GroupKeyManagement.Attributes.GroupKeyMap(mapping))]
            )
            ...
            result = await dev_ctrl.SendCommand(
                commissioner_node_id, groups_endpoint, Clusters.Groups.Commands.AddGroup(0x0103, "Test Group")
            )
            ...
            if group_names_supported:
                self.step("6a")
                result = await dev_ctrl.SendCommand(
                    commissioner_node_id, groups_endpoint, Clusters.Groups.Commands.ViewGroup(0x0103)
                )
        ...
        if not groupcast_enabled:
            ...
            await dev_ctrl.SendCommand(
                ...
                0,
                Clusters.Groupcast.Commands.JoinGroup(groupID=0x0103, endpoints=join_endpoints, keySetID=0x01A3),
            )
        ...
        # Step 10: KeySetRead
        self.step("10")
        result = await dev_ctrl.SendCommand(commissioner_node_id, 0, Clusters.GroupKeyManagement.Commands.KeySetRead(0x01A3))
