...
class TC_GC_2_6(MatterBaseTest):
    ...
    def steps_TC_GC_2_6(self):
        return [
            ...
            TestStep(
                2, "Join group G1 generating a new key. JoinGroup (GroupID=G1, Endpoints='see notes', KeySetID=K1, Key=InputKey1)"
            ),
            TestStep(
                3,
                "Iteratively Join groups, using a new GroupId and assigning KeySetID K1 every time until group count on fabric F1 = floor(M_max/2). JoinGroup (GroupID=Gn, Endpoints='see notes', KeySetID=K1)",
            ),
            ...
            TestStep(
                7,
                "On F2, join group G1 generating a new key. TH2 sends command JoinGroup (GroupID=G1, Endpoints='see notes', KeySetID=K1, Key=InputKey1)",
            ),
            TestStep(
                8,
                "On F2, iteratively Join groups using a new GroupId and assigning KeySetID K1 every time until the total group count combining all DUT groups on both fabrics = M_max. TH2 sends command JoinGroup (GroupID=Gi, Endpoints='see notes', KeySetID=K1)",
            ),
            TestStep(
                9,
                "On F2, attempt to join 1 additional group. TH2 sends command JoinGroup (GroupID=Gi+1, Endpoints='see notes', KeySetID=K1)",
            ),
        ]
    ...
    async def test_TC_GC_2_6(self):
        ...
        for membership in range(1, f1MaxMembershipCount):
            ...
            await self.send_single_cmd(
                Clusters.Groupcast.Commands.JoinGroup(groupID=groupID, endpoints=endpoints_list, keySetID=keySetID1)
            )
        ...
        try:
            await self.send_single_cmd(
                Clusters.Groupcast.Commands.JoinGroup(groupID=groupIDExhausted, endpoints=endpoints_list, keySetID=keySetID1)
            )
        ...
        self.step(6)
        await self.send_single_cmd(
            Clusters.Groupcast.Commands.JoinGroup(groupID=f1MaxMembershipCount, endpoints=endpoints_list, keySetID=keySetID1)
        )
        ...
        await self.send_single_cmd(
            ...
            cmd=Clusters.Groupcast.Commands.JoinGroup(
                groupID=groupID1, endpoints=endpoints_list, keySetID=keySetID1, key=inputKey1
            ),
        )
        ...
        for membership in range(f1MaxMembershipCount + 1, M_max):
            ...
            await self.send_single_cmd(
                dev_ctrl=self.th2,
                cmd=Clusters.Groupcast.Commands.JoinGroup(groupID=groupID, endpoints=endpoints_list, keySetID=keySetID1),
            )
        ...
        try:
            await self.send_single_cmd(
                dev_ctrl=self.th2,
                cmd=Clusters.Groupcast.Commands.JoinGroup(groupID=groupIDExhausted, endpoints=endpoints_list, keySetID=keySetID1),
            )
            asserts.fail("JoinGroup command should have failed with ResourceExhausted, but it succeeded")
        except InteractionModelError as e:
            asserts.assert_equal(
