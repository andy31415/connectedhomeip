...
class TC_ACL_2_10(MatterBaseTest):
    ...
    def steps_TC_ACL_2_10(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                5,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute, value is list of AccessControlExtensionStruct containing 2 elements",
                "Result is SUCCESS",
            ),
            TestStep(
                6,
                "TH2 writes DUT Endpoint 0 AccessControl cluster ACL attribute, value is list of AccessControlExtensionStruct containing 2 elements",
                "Result is SUCCESS",
            ),
            TestStep(
                7,
                "TH1 writes DUT Endpoint 0 AccessControl cluster Extension attribute, value is list of AccessControlExtensionStruct containing 1 elements; element data field is D_OK_EMPTY",
                "Result is SUCCESS",
            ),
            TestStep(
                8,
                "TH2 writes DUT Endpoint 0 AccessControl cluster Extension attribute, value is list of AccessControlExtensionStruct containing 1 element; element data field is D_OK_SINGLE",
                "Result is SUCCESS",
            ),
            TestStep(9, "Reboot the DUT", "DUT is rebooted"),
            TestStep(
                10,
            ...
            TestStep(14, "TH1 removes fabric `F2` from DUT", "Result is SUCCESS"),
            TestStep(
                15,
                "TH1 reads DUT Endpoint 0 AccessControl cluster ACL attribute",
            ...
            TestStep(17, "TH1 writes DUT Endpoint 0 AccessControl cluster back to admin-only ACL entry", "Result is SUCCESS"),
        ]
    ...
    async def test_TC_ACL_2_10(self):
        ...
        original_acl = await self.read_single_attribute_check_success(
            dev_ctrl=self.th1, endpoint=0, cluster=acl_cluster, attribute=acl_attribute
        )
        ...
        result = await self.th1.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=two_element_acl_th1))])
        ...
        result = await self.th2.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=two_element_acl_th2))])
        log.info("Write result: %s", str(result))
        asserts.assert_equal(result[0].Status, Status.Success, "Write should have succeeded")
        ...
        if await self.attribute_guard(endpoint=self.endpoint, attribute=extension_attr):
            ...
            result = await self.th1.WriteAttribute(self.dut_node_id, [(0, extension_attr(value=extensions_list1))])
        ...
        if await self.attribute_guard(endpoint=self.endpoint, attribute=extension_attr):
            ...
            result = await self.th2.WriteAttribute(self.dut_node_id, [(0, extension_attr(value=extensions_list2))])
        ...
        # acl_cluster = Clusters.AccessControl
        result1 = await self.read_single_attribute_check_success(
            dev_ctrl=self.th1, endpoint=0, cluster=acl_cluster, attribute=acl_attribute
        )
        log.info("TH1 read result: %s", str(result1))
        asserts.assert_equal(len(result1), 2, "Should have exactly two ACL entries")
        ...
        # or Data `D_OK_SINGLE`
        if await self.attribute_guard(endpoint=self.endpoint, attribute=extension_attr):
            th1_extension_attr = await self.read_single_attribute_check_success(
                dev_ctrl=self.th1, endpoint=0, cluster=acl_cluster, attribute=extension_attr
            )
        ...
        result2 = await self.read_single_attribute_check_success(
            dev_ctrl=self.th2, endpoint=0, cluster=acl_cluster, attribute=acl_attribute
        )
        log.info("TH2 read result: %s", str(result2))
        asserts.assert_equal(len(result2), 2, "Should have exactly two ACL entries")
        ...
        # or Data `D_OK_EMPTY`
        if await self.attribute_guard(endpoint=self.endpoint, attribute=extension_attr):
            th2_extension_attr = await self.read_single_attribute_check_success(
                dev_ctrl=self.th2, endpoint=0, cluster=acl_cluster, attribute=extension_attr
            )
        ...
        result3 = await self.read_single_attribute_check_success(
            dev_ctrl=self.th1, endpoint=0, cluster=acl_cluster, attribute=acl_attribute
        )
        log.info("TH1 read result: %s", str(result3))
        asserts.assert_equal(len(result3), 2, "Should have exactly two ACL entries")
        ...
        if await self.attribute_guard(endpoint=self.endpoint, attribute=extension_attr):
            ...
            # containing 1 element; MUST NOT contain an element with FabricIndex `F2`
            # or Data `D_OK_SINGLE`
            th1_extension_attr2 = await self.read_single_attribute_check_success(
                dev_ctrl=self.th1, endpoint=0, cluster=acl_cluster, attribute=extension_attr
            )
        ...
        self.step(17)
        result = await self.th1.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=original_acl))])
