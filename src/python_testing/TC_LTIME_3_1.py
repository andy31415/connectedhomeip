...
class TC_LTIME_3_1(MatterBaseTest):
    ...
    def steps_TC_LTIME_3_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                7,
                "If the prior write was successful, TH reads HourFormat attribute",
            ...
            TestStep(
                10,
                "TH reads SupportedCalendarTypes attribute from DUT and saves as `cluster_supported_calendar_types`",
            ...
            TestStep(
                12,
                "For each entry in `cluster_supported_calendar_types`, TH writes that value to ActiveCalendarType, verifies that the write was successful and confirms the value via a read",
                "Write is successful and read value matches for every entry",
            ),
            TestStep(
                13,
                "TH finds the set of CalendarTypesEnum value that do not appear in `cluster_supported_calendar_types` and saves as `cluster_not_supported_calendar_types`",
            ),
            TestStep(
                14,
                "For each value in `cluster_unsupported_calendar_types`, TH writes the value to the ActiveCalendarType attributes",
                "DUT returns CONSTRAINT_ERROR",
            ),
            TestStep(
                15, "TH writes 50 to ActiveCalendarType attribute", "Verify that the write request shows 0x87 (Constraint Error)."
            ),
        ]
    ...
    async def test_TC_LTIME_3_1(self):
        ...
        self.step(8)
        hourformat_status = await self.write_single_attribute(
            self.cluster.Attributes.HourFormat(5), self.endpoint, expect_success=False
        )
        ...
        cluster_supported_calendar_types = await self.read_single_attribute_check_success(
            self.cluster, self.cluster.Attributes.SupportedCalendarTypes
        )
        asserts.assert_true(
            set(cluster_supported_calendar_types).issubset(set(calendar_type_values)),
            "SupportedCalendarTypes contains unknown values",
        )

        self.step(11)
        active_calendar_type = await self.read_single_attribute_check_success(
            self.cluster, self.cluster.Attributes.ActiveCalendarType
        )
        log.info(f"Value for {active_calendar_type}")
        matter_asserts.assert_valid_uint8(active_calendar_type, "ActiveCalendarType")
        asserts.assert_is_instance(
            active_calendar_type, self.cluster.Enums.CalendarTypeEnum, "ActiveCalendarType  is not type of CalendarTypeEnum"
        )
        asserts.assert_in(
            active_calendar_type, cluster_supported_calendar_types, "ActiveCalendarType is not listed in SupportedCalendarTypes"
        )
        ...
        for unsupported in cluster_not_supported_calendar_types:
            status = await self.write_single_attribute(
                self.cluster.Attributes.ActiveCalendarType(unsupported), self.endpoint, expect_success=False
            )
            asserts.assert_equal(
                status, Status.ConstraintError, f"ConstraintError, unable to write value {unsupported} into ActiveCalendarType"
            )

        self.step(15)
        activecalendartype_status = await self.write_single_attribute(
            self.cluster.Attributes.ActiveCalendarType(50), self.endpoint, expect_success=False
        )
