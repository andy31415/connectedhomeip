...
try:
    ...
    sys.path.append(os.path.abspath(os.path.join(DEFAULT_CHIP_ROOT, "credentials")))
    import fetch_paa_certs_from_dcl
...
class DclCheck(BasicCompositionTests):
    def get_versions_and_assert_all_keys_exist(self):
        ...
        asserts.assert_true(
            key_model_versions in entry, f"Unable to find {key_model_versions} in software versions schema for {self.vid_pid_str}"
        )
        ...
        asserts.assert_true(
            key_software_versions in entry[key_model_versions],
    ...
    def steps_Compliance(self):
        return [
            TestStep(1, "Query the information about all software versions for this PID/VID", "DCL entry exists"),
            TestStep(
                2,
                "Query the compliance information for the specified software versions",
    ...
    def test_Compliance(self):
        ...
        for software_version in software_versions:
            ...
            entry = requests.get(
                f"{self.url}/dcl/compliance/compliance-info/{self.vid}/{self.pid}/{software_version}/matter"
            ).json()
            if key in entry and entry[key][sub_key] == software_version:
                found_versions.append(software_version)
                log.info(f"Found compliance info for {vid_pid_sv_str} in the DCL:")
                log.info(f"{entry[key]}")
                certified_model_entry = requests.get(
                    f"{self.url}/dcl/compliance/certified-models/{self.vid}/{self.pid}/{software_version}/matter"
                ).json()
                asserts.assert_true(
                    cert_model_key in certified_model_entry, f"Unable to find certified model entry for {vid_pid_sv_str}"
                )
        asserts.assert_true(found_versions, f"Unable to find at least one compliance entry for the versions {software_versions}")
    ...
    def steps_CertifiedModel(self):
        return [
            ...
            TestStep(
                2,
                "Query the certified version information for the specified software versions",
    ...
    def test_CertifiedModel(self):
        ...
        for software_version in software_versions:
            ...
            if key in entry and entry[key][sub_key] == software_version:
                is_found = True
                log.info(f"Found certified model for {vid_pid_sv_str} in the DCL:")
                log.info(f"{entry[key]}")
                break
        asserts.assert_true(is_found, f"Unable to find at least one certified model entry for the versions {software_versions}")

    def steps_AllSoftwareVersions(self):
        return [
            TestStep(1, "Query the version information for this software version", "DCL entry exists"),
            TestStep(
                2,
                "For each valid software version with an OtaUrl, verify the OtaChecksumType is in the valid range and the OtaChecksum is a base64. If the softwareVersion matches the current softwareVersion on the device, ensure the entry is valid.",
                "OtaChecksum is base64 and OtaChecksumType is in the valid set",
            ),
        ]
...
class TestConfig(object):
    def __init__(self, code: str, code_type: SetupCodeType):
        ...
        fetch_paa_certs_from_dcl.fetch_paa_certs(
            use_main_net_dcld="",
            use_test_net_dcld="",
            use_main_net_http=True,
            use_test_net_http=False,
            paa_trust_store_path=tmpdir_paa,
        )
        ...
        self.config = MatterTestConfig(
            endpoint=0, dut_node_ids=[1], global_test_params=global_test_params, storage_path=self.admin_storage
        )
        ...
        self.default_controller = (
            ...
            .adminList[0]
            .NewController(nodeId=112233, paaTrustStorePath=str(self.config.paa_trust_store_path))
        )
