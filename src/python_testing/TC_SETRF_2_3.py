...
class TC_SETRF_2_3(CommodityTariffTestBaseHelper):
    ...
    def steps_TC_SETRF_2_3(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "3",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.SETRF.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.SETRF.TEST_EVENT_TRIGGER for Test Event Attributes Value Set Test Event.",
            ...
            TestStep(
                "6",
                "If dayTypeCurrentValue equals 3 (Event), TH reads from the DUT the IndividualDays attribute.",
                """
            ...
            TestStep(
                "7a",
                "If dayTypeCurrentValue does NOT equal 3 (Event), TH reads from the DUT the CalendarPeriods attribute.",
                """
            ...
            TestStep(
                "12",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.SETRF.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.SETRF.TEST_EVENT_TRIGGER for Change Time Test Event.",
                """
                     Verify DUT responds w/ status SUCCESS(0x00).""",
            ),
            TestStep(
            ...
            TestStep(
                "17",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.SETRF.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.SETRF.TEST_EVENT_TRIGGER for Change Day Test Event.",
                """
                     Verify DUT responds w/ status SUCCESS(0x00).""",
            ),
            TestStep(
    ...
    async def test_TC_SETRF_2_3(self):
        ...
        if dayTypeCurrentValue == 3:  # If CurrentDay has Event type.
            ...
            for day in self.individualDaysValue:
                if day.date == dateCurrentValue:
                    asserts.assert_equal(
                        day.dayType, dayTypeCurrentValue, "DayType from CurrentDay must be equal to DayType from IndividualDays."
                    )
                    asserts.assert_equal(
                        day.dayEntryIDs,
                        dayEntryIDsCurrentValue,
            ...
            await self.search_day_pattern_for_given_day_of_week_check_day_entry_IDs(
                list_of_day_pattern_IDs=dayPatternIDsCurrentValue,
        ...
        asserts.assert_equal(
            sorted(tariffComponentsCurrentValue),
            sorted(self.currentTariffComponentsValue),
        ...
        asserts.assert_not_equal(
            ...
            currentDayEntryDateValue_previous,
        ...
        await self.check_current_day_attribute(endpoint, self.currentDayValue)
        asserts.assert_equal(
            self.currentDayValue,
            currentDayValue_previous,
        ...
        asserts.assert_not_equal(
            self.nextDayEntryDateValue,
            nextDayEntryDateValue_previous,
        ...
        await self.check_next_day_attribute(endpoint, self.nextDayValue)
        asserts.assert_equal(self.nextDayValue, nextDayValue_previous, "NextDay must not change after TestEventTrigger.")
        asserts.assert_not_equal(
            self.nextDayValue,
            self.currentDayValue,
        ...
        await self.check_next_day_attribute(endpoint, self.nextDayValue)
        asserts.assert_not_equal(
            self.nextDayValue, nextDayValue_previous, "NextDay must change after TestEventTrigger for Change Day Test Event."
        )
        asserts.assert_not_equal(
            self.nextDayValue,
            self.currentDayValue,
