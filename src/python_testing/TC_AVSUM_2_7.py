...
class TC_AVSUM_2_7(MatterBaseTest, AVSUMTestBase):
    ...
    def steps_TC_AVSUM_2_7(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                2, "Read the viewport and sensorparms from the device. Ensure the viewport is less than or equal to the sensor size"
            ),
            ...
            TestStep(
                5, "Send a VideoStreamAllocate command to AVStreamManagement to allocate a video stream ID. Record the returned ID"
            ),
            TestStep(
                6,
                "Read the DPTZStreams attribute, verify that the response contains an entry with the allocated stream ID and the device viewport",
            ),
            ...
            TestStep(
                12,
                "Read the DPTZStreams attribute, verify that the response contains an entry with the allocated stream ID and the viewport from step 10",
            ),
            ...
            TestStep(
                15,
                "Send a VideoStreamDeallocate command to AVStreamManagement to de-allocate the video stream allocated in step 5. Verify success",
            ),
            TestStep(
                16,
                "Read the DPTZStreams attribute, verify that the response does not contain any entry with the allocated stream ID from step 5",
            ),
        ]

    def pics_TC_AVSUM_2_7(self) -> list[str]:
        return ["AVSUM.S", "AVSUM.S.F00", "AVSM.S"]

    @run_if_endpoint_matches(
        has_feature(
            Clusters.CameraAvSettingsUserLevelManagement, Clusters.CameraAvSettingsUserLevelManagement.Bitmaps.Feature.kDigitalPTZ
        )
    ...
    async def test_TC_AVSUM_2_7(self):
        ...
        await self.send_dptz_set_viewport_command(
            endpoint, failingvideostreamID, smallfailingviewport, expected_status=Status.NotFound
        )
        ...
        await self.send_dptz_set_viewport_command(
            endpoint, videoStreamID, smallfailingviewport, expected_status=Status.ConstraintError
        )
        ...
        self.step(9)
        await self.send_dptz_set_viewport_command(
            endpoint, videoStreamID, largefailingviewport, expected_status=Status.ConstraintError
        )
        ...
        await self.send_single_cmd(
            endpoint=endpoint, cmd=Clusters.CameraAvStreamManagement.Commands.VideoStreamDeallocate(videoStreamID=videoStreamID)
        )

        self.step(16)
        dptz_streams = await self.read_avsum_attribute_expect_success(
