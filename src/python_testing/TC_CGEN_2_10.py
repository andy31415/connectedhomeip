...
class TC_CGEN_2_10(MatterBaseTest):
    ...
    def steps_TC_CGEN_2_10(self) -> list[TestStep]:
        return [
            TestStep(
                1,
                "TH reads from the DUT the attribute TCAcceptedVersion. Store the value as acceptedVersion.",
    ...
    async def test_TC_CGEN_2_10(self):
        ...
        user_acknowledgements = response[ROOT_ENDPOINT_ID][Clusters.GeneralCommissioning][
        ...
        response = await commissioner.SendCommand(
            nodeId=self.dut_node_id,
            endpoint=ROOT_ENDPOINT_ID,
            payload=Clusters.GeneralCommissioning.Commands.SetTCAcknowledgements(TCVersion=accepted_version + 1, TCUserResponse=0),
        )
