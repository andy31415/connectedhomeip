...
class TC_WEBRTCP_2_25(MatterBaseTest, WEBRTCPTestBase):
    ...
    def steps_TC_WEBRTCP_2_25(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                2,
                "TH sends the ProvideOffer command with SFrameConfig containing unsupported cipher suite",
                "DUT responds with DynamicConstraintError indicating unsupported configuration",
            ),
            TestStep(
                3,
                "TH sends the ProvideOffer command with SFrameConfig containing incorrect key length for AES-128-GCM",
                "DUT responds with DynamicConstraintError indicating invalid key length",
            ),
            TestStep(
                4,
                "TH sends the ProvideOffer command with valid SFrameConfig (AES-256-GCM with 32-byte key)",
    ...
    async def test_TC_WEBRTCP_2_25(self):
        ...
        asserts.assert_equal(
            type(resp_offer), Clusters.WebRTCTransportProvider.Commands.ProvideOfferResponse, "Incorrect response type"
        )
