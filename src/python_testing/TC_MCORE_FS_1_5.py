...
class TC_MCORE_FS_1_5(MatterBaseTest):
    ...
    def steps_TC_MCORE_FS_1_5(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                6,
                "TH subscribes to AdministratorCommissioning attributes on DUT_FSA for the newly added endpoint identified in step 3",
            ),
            TestStep(7, "TH opens commissioning window to TH_SERVER directly (not using DUT)"),
            TestStep(
                8,
                "TH reads CurrentFabricIndex attributes on OperationalCredentials cluster from TH_SERVER directly (not using DUT_FSA)",
            ),
            TestStep(9, "TH reads AdministratorCommissioning from TH_SERVER directly (not using DUT)"),
            TestStep(
                10,
                "TH waits up to 10 seconds for subscription report from the AdministratorCommissioning attribute (from step 6) to reflect values from previous step",
            ),
        ]
    ...
    async def test_TC_MCORE_FS_1_5(self):
        ...
        root_endpoint = 0
        parts_list_subscription_contents = [(root_endpoint, Clusters.Descriptor.Attributes.PartsList)]
        ...
        step_1_dut_parts_list = parts_list_cached_attributes[root_endpoint][Clusters.Descriptor][
        ...
        cadmin_subscription_contents = [(newly_added_endpoint, Clusters.AdministratorCommissioning)]
        ...
        cadmin_attribute_handler = AttributeChangeAccumulator(
            name=self.default_controller.name,
        ...
        await self.default_controller.OpenCommissioningWindow(
            nodeId=self.th_server_local_nodeid, timeout=180, iteration=1000, discriminator=3840, option=1
        )
        ...
        th_server_directly_read_result = await self.default_controller.ReadAttribute(
            self.th_server_local_nodeid, [(root_endpoint, Clusters.AdministratorCommissioning)]
        )
        ...
        asserts.assert_equal(
            ...
            "WindowStatus is expected to be EnhancedWindowOpen",
        )
        asserts.assert_equal(
            th_server_direct_cadmin[cadmin_attr.AdminFabricIndex], current_fabric_index, "AdminFabricIndex is unexpected"
        )
            ...
            try:
                ...
                if endpoint == newly_added_endpoint and attribute == cadmin_attr.WindowStatus:
                    if value != th_server_direct_cadmin[cadmin_attr.WindowStatus]:
                        log.info("Window status is %r, waiting for %r", value, th_server_direct_cadmin[cadmin_attr.WindowStatus])
                        continue
        ...
        asserts.assert_true(
            cadmin_sub_new_data, "Timed out waiting for DUT to reflect AdministratorCommissioning attributes for bridged device"
        )

        dut_read = await self.default_controller.ReadAttribute(
            self.dut_node_id, [(newly_added_endpoint, Clusters.AdministratorCommissioning)]
        )
        dut_cadmin_for_th_server = dut_read[newly_added_endpoint][Clusters.AdministratorCommissioning]
        asserts.assert_equal(
            th_server_direct_cadmin[cadmin_attr.WindowStatus],
            dut_cadmin_for_th_server[cadmin_attr.WindowStatus],
            "WindowStatus incorrectly reported by DUT",
        )
        asserts.assert_equal(
            th_server_direct_cadmin[cadmin_attr.AdminFabricIndex],
            dut_cadmin_for_th_server[cadmin_attr.AdminFabricIndex],
            "AdminFabricIndex incorrectly reported by DUT",
        )
        asserts.assert_equal(
            th_server_direct_cadmin[cadmin_attr.AdminVendorId],
            dut_cadmin_for_th_server[cadmin_attr.AdminVendorId],
            "AdminVendorId incorrectly reported by DUT",
        )
