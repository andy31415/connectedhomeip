...
class DEMTestBase:
    ...
    async def validate_feature_map(self, must_have_features, must_not_have_features):
        ...
        for must_have_feature in must_have_features:
            asserts.assert_true(
                feature_map & must_have_feature, f"{must_have_feature.name} must be set but is not. feature_map 0x{feature_map:x}"
            )

        for must_not_have_feature in must_not_have_features:
            asserts.assert_false(
                feature_map & must_not_have_feature,
    ...
    async def validate_pfr_or_sfr_in_feature_map(self):
        ...
        asserts.assert_not_equal(
            feature_map & illegal_combination,
            illegal_combination,
    ...
    async def send_power_adjustment_command(
        self,
        power: int,
        duration: int,
        ...
        try:
            await self.send_single_cmd(
                cmd=Clusters.DeviceEnergyManagement.Commands.PowerAdjustRequest(power=power, duration=duration, cause=cause),
                endpoint=endpoint,
    ...
    async def send_cancel_power_adjustment_command(
        self, endpoint: int = None, timedRequestTimeoutMs: int = 3000, expected_status: Status = Status.Success
    ):
        try:
            await self.send_single_cmd(
                cmd=Clusters.DeviceEnergyManagement.Commands.CancelPowerAdjustRequest(),
                endpoint=endpoint,
                timedRequestTimeoutMs=timedRequestTimeoutMs,
            )
    ...
    async def send_start_time_adjust_request_command(
        self,
        ...
        try:
            await self.send_single_cmd(
                cmd=Clusters.DeviceEnergyManagement.Commands.StartTimeAdjustRequest(
                    requestedStartTime=requestedStartTime, cause=cause
                ),
                endpoint=endpoint,
                timedRequestTimeoutMs=timedRequestTimeoutMs,
            )
    ...
    async def send_start_time_adjust_clear_command(
        self, endpoint: int = None, timedRequestTimeoutMs: int = 3000, expected_status: Status = Status.Success
    ):
        try:
            await self.send_single_cmd(
                cmd=Clusters.DeviceEnergyManagement.Commands.StartTimeAdjustClear(),  # StartTimeAdjustmentClear(),
                endpoint=endpoint,
                timedRequestTimeoutMs=timedRequestTimeoutMs,
            )
    ...
    async def send_cancel_request_command(
        self, endpoint: int = None, timedRequestTimeoutMs: int = 3000, expected_status: Status = Status.Success
    ):
        try:
            await self.send_single_cmd(
                cmd=Clusters.DeviceEnergyManagement.Commands.CancelRequest(),
                endpoint=endpoint,
                timedRequestTimeoutMs=timedRequestTimeoutMs,
            )
    ...
    async def send_pause_request_command(
        self,
        duration: int,
        cause: Clusters.Objects.DeviceEnergyManagement.Enums.AdjustmentCauseEnum,
        endpoint: int = None,
        timedRequestTimeoutMs: int = 3000,
        expected_status: Status = Status.Success,
    ):
        try:
            await self.send_single_cmd(
                cmd=Clusters.DeviceEnergyManagement.Commands.PauseRequest(duration=duration, cause=cause),
                endpoint=endpoint,
                timedRequestTimeoutMs=timedRequestTimeoutMs,
            )
    ...
    async def send_resume_request_command(
        self, endpoint: int = None, timedRequestTimeoutMs: int = 3000, expected_status: Status = Status.Success
    ):
        try:
            await self.send_single_cmd(
                cmd=Clusters.DeviceEnergyManagement.Commands.ResumeRequest(),
                endpoint=endpoint,
                timedRequestTimeoutMs=timedRequestTimeoutMs,
            )
    ...
    async def send_modify_forecast_request_command(
        self,
        forecastID: int,
        ...
        try:
            await self.send_single_cmd(
                cmd=Clusters.DeviceEnergyManagement.Commands.ModifyForecastRequest(
                    forecastID=forecastID, slotAdjustments=slotAdjustments, cause=cause
                ),
                endpoint=endpoint,
                timedRequestTimeoutMs=timedRequestTimeoutMs,
            )
    ...
    async def send_request_constraint_based_forecast(
        self,
        constraintList: list[Clusters.DeviceEnergyManagement.Structs.ConstraintsStruct],
        cause: Clusters.DeviceEnergyManagement.Enums.AdjustmentCauseEnum,
        endpoint: int = None,
        timedRequestTimeoutMs: int = 3000,
        expected_status: Status = Status.Success,
        ...
        try:
            await self.send_single_cmd(
                cmd=Clusters.DeviceEnergyManagement.Commands.RequestConstraintBasedForecast(
                    constraints=constraintList, cause=cause
                ),
                endpoint=endpoint,
                timedRequestTimeoutMs=timedRequestTimeoutMs,
            )
    ...
    def print_forecast(self, forecast):
        for index, slot in enumerate(forecast.slots):
            ...
            log.info(f"       ElapseSlotTime: {slot.elapsedSlotTime} RemainingSlotTime: {slot.remainingSlotTime}")
            log.info(
                f"       SlotIsPausable: {slot.slotIsPausable} MinPauseDuration: {slot.minPauseDuration} MaxPauseDuration: {slot.maxPauseDuration}"
            )
            log.info(f"       ManufacturerESAState: {slot.manufacturerESAState}")
            log.info(f"       NominalPower: {slot.nominalPower} MinPower: {slot.minPower} MaxPower: {slot.maxPower}")
            log.info(f"       MinPowerAdjustment: {slot.minPowerAdjustment} MaxPowerAdjustment: {slot.maxPowerAdjustment}")
            log.info(
