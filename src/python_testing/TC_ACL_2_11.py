...
class TC_ACL_2_11(MatterBaseTest):
    ...
    def steps_TC_ACL_2_11(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                4,
                "For each entry in ARL, iterate over each restriction and attempt access the restriction's ID on the Endpoint and Cluster in the ARL entry.",
            ...
            TestStep(
                7,
                "Wait for up to 1 hour. Follow instructions provided by device maker to remove all access restrictions",
    ...
    async def test_TC_ACL_2_11(self):
        ...
        asserts.assert_true(
            arl_data.have_arl,
            "ARL attribute must contain at least one restriction to run this test. Please follow manufacturer-specific steps to add access restrictions and re-run this test",
        )
        asserts.assert_true(
            arl_data.have_carl,
            "CommissioningARL attribute must contain at least one restriction to run this test. Please follow manufacturer-specific steps to add access restrictions and re-run this test",
        )

        self.step(2)
        await self.read_single_attribute_check_success(
            endpoint=0, cluster=Clusters.AccessControl, attribute=Clusters.AccessControl.Attributes.CommissioningARL
        )
        self.step(3)
        arl = await self.read_single_attribute_check_success(
            endpoint=0, cluster=Clusters.AccessControl, attribute=Clusters.AccessControl.Attributes.Arl
        )
        self.step(4)
                ...
                if restriction_type == AccessControl.Enums.AccessRestrictionTypeEnum.kAttributeAccessForbidden:
                    ...
                    if ID1 is NullValue:
                        for attr_id, attribute in ALL_ATTRIBUTES[C1].items():
                            if (
                                global_attribute_ids.attribute_id_type(attr_id)
                                != global_attribute_ids.AttributeIdType.kStandardGlobal
                            ):
                                await self.read_single_attribute_expect_error(
                                    cluster=cluster, attribute=attribute, error=Status.AccessRestricted, endpoint=E1
                                )
                    else:
                        ...
                        for attr_id, attribute in ALL_ATTRIBUTES[C1].items():
                            if (
                                ...
                                status = await self.write_single_attribute(
                                    attribute_value=attribute(), endpoint_id=E1, expect_success=False
                                )
        ...
        urgent = 1
        subscription_arru = await dev_ctrl.ReadEvent(
            dut_node_id,
            events=[(0, Clusters.AccessControl.Events.FabricRestrictionReviewUpdate, urgent)],
            reportInterval=(0, 30),
        ...
        set_of_endpoints = set(root_part_list[root_node_endpoint][Clusters.Descriptor][Clusters.Descriptor.Attributes.PartsList])
        for endpoint in set_of_endpoints:
            ...
            for server in server_list:
                ...
                data = await dev_ctrl.ReadAttribute(
                    dut_node_id,
                    [
                        (endpoint, cluster.Attributes.GeneratedCommandList),
                    ...
                    for cluster, attributes in clusters.items():
                        for attribute, value in attributes.items():
                            asserts.assert_false(
                                isinstance(value, ValueDecodeFailure)
                                and isinstance(value.Reason, InteractionModelError)
                                and value.Reason.status == Status.AccessRestricted,
                                "AccessRestricted is not allowed on Global attributes",
                            )

        self.step(6)
        response = await self.send_single_cmd(
            cmd=Clusters.AccessControl.Commands.ReviewFabricRestrictions([care_struct]), endpoint=0
        )
        asserts.assert_true(
            isinstance(response, Clusters.AccessControl.Commands.ReviewFabricRestrictionsResponse),
        ...
        arl = await self.read_single_attribute_check_success(
            node_id=self.dut_node_id, endpoint=0, cluster=cluster, attribute=attribute
        )
