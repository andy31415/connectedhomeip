...
class TC_SEAR_1_5(MatterBaseTest):
    ...
    async def send_cmd_skip_area_expect_response(self, step, skipped_area, expected_response):
        ...
        ret = await self.send_single_cmd(
            cmd=Clusters.Objects.ServiceArea.Commands.SkipArea(skippedArea=skipped_area), endpoint=self.endpoint
        )
    ...
    async def test_TC_SEAR_1_5(self):
        ...
        if self.check_pics("SEAR.S.M.INVALID_STATE_FOR_SKIP") and self.check_pics("SEAR.S.M.HAS_MANUAL_SKIP_STATE_CONTROL"):
            ...
            if self.is_ci:
                await self.send_single_cmd(
                    cmd=Clusters.Objects.ServiceArea.Commands.SelectAreas(newAreas=[7]), endpoint=self.endpoint
                )
            else:
                self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")

            await self.send_cmd_skip_area_expect_response(
                step=4, skipped_area=valid_area_id, expected_response=Clusters.ServiceArea.Enums.SkipAreaStatus.kInvalidInMode
            )

        if self.check_pics("SEAR.S.M.NO_SELAREA_FOR_SKIP") and self.check_pics("SEAR.S.M.HAS_MANUAL_SKIP_STATE_CONTROL"):
            test_step = (
                "Manually intervene to put the device in a state where the state would allow it to execute the SkipArea command, \
            ...
            await self.send_cmd_skip_area_expect_response(
                step=6, skipped_area=valid_area_id, expected_response=Clusters.ServiceArea.Enums.SkipAreaStatus.kInvalidAreaList
            )
        ...
        if self.check_pics("SEAR.S.M.VALID_STATE_FOR_SKIP") and self.check_pics("SEAR.S.M.HAS_MANUAL_SKIP_STATE_CONTROL"):
            ...
            if self.is_ci:
                self.write_to_app_pipe({"Name": "Reset"})
                await self.send_single_cmd(
                    cmd=Clusters.Objects.ServiceArea.Commands.SelectAreas(newAreas=[7, 1234567]), endpoint=self.endpoint
                )
                await self.send_single_cmd(cmd=Clusters.Objects.RvcRunMode.Commands.ChangeToMode(newMode=1), endpoint=self.endpoint)
            else:
                self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
        ...
        if self.check_pics("SEAR.S.A0003"):
            ...
            if old_current_area is not NullValue:
                await self.send_cmd_skip_area_expect_response(
                    step=13, skipped_area=old_current_area, expected_response=Clusters.ServiceArea.Enums.SkipAreaStatus.kSuccess
                )
                ...
                if self.check_pics("SEAR.S.A0005"):
                    ...
                    asserts.assert_true(
                        len(new_progress_list) > 0, f"len of Progress({len(new_progress_list)}) should not be zero)"
                    )
                    ...
                    for p in new_progress_list:
                        if p.areaID == old_current_area:
                            asserts.assert_true(
                                p.status == Clusters.ServiceArea.Enums.OperationalStatusEnum.kSkipped,
                    ...
                    if ret != "y":
                        for p in new_progress_list:
                            if p.areaID == new_current_area:
                                asserts.assert_true(
                                    p.status == Clusters.ServiceArea.Enums.OperationalStatusEnum.kOperating,
        ...
        await self.send_cmd_skip_area_expect_response(
            step=21, skipped_area=area_to_skip, expected_response=Clusters.ServiceArea.Enums.SkipAreaStatus.kSuccess
        )
