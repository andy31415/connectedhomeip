...
class TC_TLSCERT_Base(MatterBaseTest):
    ...
    def get_common_substeps(self, step_prefix: string = "1") -> list[TestStep]:
        return [
            ...
            TestStep(
                f"{step_prefix}.3",
                "CR1 sends RemoveRootCertificate command with CAID set to caidToClean[i] for all entries returned",
            ...
            TestStep(
                f"{step_prefix}.5",
                "CR1 sends RemoveClientCertificate command with CCDID set to ccdidToClean[i] for all entries returned.",
                test_plan_support.verify_success(),
            ),
        ]
    ...
    def pad_out_certificate(self) -> CertificateBuilder:
        ...
        for i in range(100):
            builder = builder.add_extension(
                UnrecognizedExtension(oid=ObjectIdentifier(f"1.1.{i}"), value=b"\x23" * 20), critical=False
            )
        return builder
