...
class TC_CNET_4_9(MatterBaseTest):
    ...
    async def test_TC_CNET_4_9(self):
        ...
        # Verify that DUT sends ArmFailSafeResponse command to the TH
        asserts.assert_true(
            matchers.is_type(result, Clusters.GeneralCommissioning.Commands.ArmFailSafeResponse),
        ...
        for ep in networks_dict:
            ...
            asserts.assert_equal(
                sum(connected_network_count.values()),
                1,
        ...
        asserts.assert_true(
            userwifi_netidx is not None, "There is not a NetworkID field equal to --wifi-ssid in Networks attribute list"
        )

        # Connected field value is of type bool and has the value true
        asserts.assert_true(
            is_valid_bool_value(networks[userwifi_netidx].connected), "Network Connected attribute field is not a boolean"
        )
        ...
        asserts.assert_true(
            matchers.is_type(result, Clusters.NetworkCommissioning.Commands.NetworkConfigResponse),
            "Unexpected value returned from RemoveNetwork",
        )
        asserts.assert_equal(
            result.networkingStatus,
        ...
        asserts.assert_equal(
            len(networks_after_removal),
            expected_num_networks,
        ...
        if len(networks_after_removal) == 0:
            asserts.assert_equal(
                last_networking_status, NullValue, "LastNetworkingStatus should be Null when Networks list is empty"
            )
        else:
            asserts.assert_equal(
                last_networking_status,
                Clusters.NetworkCommissioning.Enums.NetworkCommissioningStatusEnum.kSuccess,
                "LastNetworkingStatus should be Success when Networks list is not empty",
            )
        ...
        for network in networks:
            asserts.assert_equal(
                network.networkID,
                ssid.encode("utf-8"),
                f"Network ID: {network.networkID.decode('utf-8')} is not the hex representation of --wifi-ssid: {ssid}",
            )
        ...
        asserts.assert_true(
            userwifi_netidx is None, f"Networks should not contain an entry with the NetworkID for --wifi-ssid: {ssid}"
        )
        ...
        cmd = Clusters.NetworkCommissioning.Commands.AddOrUpdateWiFiNetwork(
            ssid=ssid.encode("utf-8"), credentials=credentials.encode("utf-8"), breadcrumb=4
        )
        result = await self.send_single_cmd(cmd=cmd)
