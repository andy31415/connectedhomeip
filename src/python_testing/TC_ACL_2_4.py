...
class TC_ACL_2_4(MatterBaseTest):
    ...
    def steps_TC_ACL_2_4(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                3,
                "TH1 reads DUT Endpoint 0 AccessControl cluster ACL attribute and saves as acl_original",
                "Result is SUCCESS, value is list of AccessControlEntryStruct containing 1 element with admin entry N1",
            ),
            TestStep(
                4,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute with 3 elements:\n"
            ...
            TestStep(
                6,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute with modified entries (N1 admin, N2 manage, N3 admin)",
                "Result is SUCCESS",
            ),
            TestStep(
                7,
            ...
            TestStep(
                10,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute with null subjects in the last 2 elements ",
                "Result is SUCCESS",
            ),
            TestStep(
                11,
            ...
            TestStep(
                16, "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute with ProxyView privilege", "Result is SUCCESS"
            ),
            TestStep(
                17,
                "TH1 reads DUT Endpoint 0 AccessControl cluster ACL attribute",
            ...
            TestStep(
                21,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute with 4 valid CATs as subjects in the last entry",
                "Result is SUCCESS",
            ),
            TestStep(
                22,
            ...
            TestStep(
                27,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute with a list of MAXENTRIES elements",
                "Result is SUCCESS",
            ),
            TestStep(
                28,
            ...
            TestStep(
                29,
                "TH1 resets DUT Endpoint 0 AccessControl cluster ACL attribute, writing it with acl_original",
                "Result is SUCCESS",
            ),
            TestStep(
                30,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing PASE auth mode",
            ...
            TestStep(
                32,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing invalid combination of Group auth mode with admin privilege",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                33,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing invalid privilege",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                34,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing invalid auth mode",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                35,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing invalid subject ID 0",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                36,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing max node ID",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                37,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing an invalid CAT as subject",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                38,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing invalid Group Node ID",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                39,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing empty target",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                40,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing invalid cluster ID",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                41,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing invalid endpoint ID",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                42,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing invalid device type",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                43,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing an invalid target entry containing both endpoint and device type fields in the same entry",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                44,
                "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute testing an invalid target entry containing endpoint, device type and cluster fields in the same entry",
                "Result is CONSTRAINT_ERROR.  Second invalid element is rejected",
            ),
            TestStep(
                45, "TH1 writes DUT Endpoint 0 AccessControl cluster ACL attribute value is acl_original", "Result is SUCCESS"
            ),
        ]
    ...
    async def test_TC_ACL_2_4(self):
        ...
        acl_original = await self.read_single_attribute_check_success(endpoint=0, cluster=acl_cluster, attribute=acl_attribute)
        ...
        new_acl = [
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                targets=[
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=11, endpoint=NullValue, deviceType=NullValue),
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=NullValue, endpoint=22, deviceType=NullValue),
                ],
                fabricIndex=f1,
            ),
            # Operate entry
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                targets=[
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=55, endpoint=NullValue, deviceType=NullValue),
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=NullValue, endpoint=66, deviceType=NullValue),
                ],
                fabricIndex=f1,
            ),
        ]

        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=new_acl))])
        ...
        self.step(5)
        read_acl = await self.read_single_attribute_check_success(endpoint=0, cluster=acl_cluster, attribute=acl_attribute)
        ...
        modified_acl = [
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                targets=[
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=44, endpoint=NullValue, deviceType=NullValue),
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=NullValue, endpoint=33, deviceType=NullValue),
                ],
                fabricIndex=f1,
            ),
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                targets=[
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=88, endpoint=NullValue, deviceType=NullValue),
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=NullValue, endpoint=77, deviceType=NullValue),
                ],
                fabricIndex=f1,
            ),
        ]

        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=modified_acl))])
        asserts.assert_equal(result[0].Status, Status.Success)

        # Step 7: Read and verify modified ACL
        self.step(7)
        read_modified_acl = await self.read_single_attribute_check_success(endpoint=0, cluster=acl_cluster, attribute=acl_attribute)
        ...
        updated_targets_acl = [
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                targets=[
                    ...
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=11, endpoint=22, deviceType=NullValue),
                    # Second target with cluster and deviceType
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=33, endpoint=NullValue, deviceType=44),
                ],
                fabricIndex=f1,
            ),
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                targets=[
                    # First target with cluster and endpoint
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=55, endpoint=66, deviceType=NullValue),
                    # Second target with cluster and deviceType
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=77, endpoint=NullValue, deviceType=88),
                ],
                fabricIndex=f1,
            ),
        ]

        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=updated_targets_acl))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write ACL with updated targets should succeed")

        # Step 9: Read and verify updated targets ACL
        self.step(9)
        read_updated_targets_acl = await self.read_single_attribute_check_success(
            endpoint=0, cluster=acl_cluster, attribute=acl_attribute
        )
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=null_subjects_acl))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write ACL with null subjects should succeed")
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=null_targets_acl))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write ACL with null targets should succeed")
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=two_element_acl))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write ACL with 2 elements should succeed")
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=proxy_view_acl))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write ACL with ProxyView privilege should succeed")
        ...
        max_subjects = await self.read_single_attribute_check_success(
            endpoint=0, cluster=acl_cluster, attribute=subjects_per_entry_attr
        )
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=max_subjects_acl))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write ACL with max subjects should succeed")
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=specific_subjects_acl))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write ACL with specific subjects should succeed")
        ...
        max_targets = await self.read_single_attribute_check_success(
            endpoint=0, cluster=acl_cluster, attribute=targets_per_entry_attr
        )
        ...
        for i in range(max_targets):
            ...
            if i % 3 == 0:
                ...
                target = Clusters.AccessControl.Structs.AccessControlTargetStruct(
                    cluster=i + 1, endpoint=NullValue, deviceType=NullValue
                )
            elif i % 3 == 1:
                # Target with endpoint only
                target = Clusters.AccessControl.Structs.AccessControlTargetStruct(
                    cluster=NullValue, endpoint=i + 1, deviceType=NullValue
                )
            else:
                # Target with deviceType only
                target = Clusters.AccessControl.Structs.AccessControlTargetStruct(
                    cluster=NullValue, endpoint=NullValue, deviceType=i + 1
                )
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=max_targets_acl))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write ACL with max targets should succeed")
        ...
        max_entries = await self.read_single_attribute_check_success(
            endpoint=0, cluster=acl_cluster, attribute=entries_per_fabric_attr
        )
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=max_entries_acl))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write ACL with max entries should succeed")
        ...
        asserts.assert_equal(len(read_max_entries_acl), max_entries, f"ACL should contain exactly {max_entries} entries")
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=admin_only_acl))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write admin-only ACL should succeed")

        # Verify reset was successful
        read_admin_only = await self.read_single_attribute_check_success(endpoint=0, cluster=acl_cluster, attribute=acl_attribute)
        asserts.assert_equal(len(read_admin_only), 1, "ACL should contain only admin entry after reset")
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=pase_acl))])
        # Should fail with CONSTRAINT_ERROR
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with PASE auth mode should fail with CONSTRAINT_ERROR"
        )
        ...
        asserts.assert_equal(len(read_after_pase_acl), 1, "ACL should contain only admin entry")
        admin_entry = read_after_pase_acl[0]
        asserts.assert_equal(
            admin_entry.privilege,
            Clusters.AccessControl.Enums.AccessControlEntryPrivilegeEnum.kAdminister,
            "Entry should have admin privilege",
        )
        asserts.assert_equal(admin_entry.subjects, [self.th1.nodeId], "Entry should have admin node ID")
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=group_admin_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with Group auth mode for admin should fail with CONSTRAINT_ERROR"
        )

        # Verify ACL still contains only admin entry after constraint error
        verify_acl = await self.read_single_attribute_check_success(endpoint=0, cluster=acl_cluster, attribute=acl_attribute)
        asserts.assert_equal(len(verify_acl), 1, "ACL should contain only admin entry after failed operation")
        asserts.assert_equal(
            verify_acl[0].privilege,
            Clusters.AccessControl.Enums.AccessControlEntryPrivilegeEnum.kAdminister,
            "Remaining entry must be admin",
        )
        asserts.assert_equal(verify_acl[0].subjects, [self.th1.nodeId], "Admin entry must have correct node ID")
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=invalid_privilege_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with invalid privilege should fail with CONSTRAINT_ERROR"
        )
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=invalid_auth_mode_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with invalid auth mode should fail with CONSTRAINT_ERROR"
        )
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=invalid_subject_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with invalid subject ID should fail with CONSTRAINT_ERROR"
        )
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=max_node_id_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with max node ID should fail with CONSTRAINT_ERROR"
        )
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=group_id_range_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with an invalid CAT as subject should fail with CONSTRAINT_ERROR"
        )
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=fabric_scoped_id_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with invalid Group Node ID should fail with CONSTRAINT_ERROR"
        )
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                targets=[
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(
                        cluster=NullValue, endpoint=NullValue, deviceType=NullValue
                    )
                ],
                fabricIndex=f1,
            ),
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=empty_target_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with empty target should fail with CONSTRAINT_ERROR"
        )
        ...
        invalid_cluster_acl = [
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                targets=[
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(
                        cluster=0xFFFFFFFF, endpoint=NullValue, deviceType=NullValue
                    )
                ],
                fabricIndex=f1,
            ),
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=invalid_cluster_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with invalid cluster ID should fail with CONSTRAINT_ERROR"
        )
        ...
        invalid_endpoint_acl = [
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                targets=[
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(
                        cluster=NullValue, endpoint=65535, deviceType=NullValue
                    )
                ],
                fabricIndex=f1,
            ),
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=invalid_endpoint_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with invalid endpoint ID should fail with CONSTRAINT_ERROR"
        )
        ...
        invalid_device_type_acl = [
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                targets=[
                    Clusters.AccessControl.Structs.AccessControlTargetStruct(
                        cluster=NullValue, endpoint=NullValue, deviceType=0xFFFFFFFF
                    )
                ],
                fabricIndex=f1,
            ),
        ...
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=invalid_device_type_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with invalid device type should fail with CONSTRAINT_ERROR"
        )
        ...
        endpoint_with_device_type_acl = [
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                authMode=Clusters.AccessControl.Enums.AccessControlEntryAuthModeEnum.kCase,
                subjects=NullValue,
                targets=[Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=NullValue, endpoint=22, deviceType=33)],
                fabricIndex=f1,
            ),
        ]

        result = await self.default_controller.WriteAttribute(
            self.dut_node_id, [(0, acl_attribute(value=endpoint_with_device_type_acl))]
        )
        asserts.assert_equal(
            result[0].Status,
        ...
        all_target_fields_acl = [
            ...
            Clusters.AccessControl.Structs.AccessControlEntryStruct(
                ...
                authMode=Clusters.AccessControl.Enums.AccessControlEntryAuthModeEnum.kCase,
                subjects=NullValue,
                targets=[Clusters.AccessControl.Structs.AccessControlTargetStruct(cluster=11, endpoint=22, deviceType=33)],
                fabricIndex=f1,
            ),
        ]

        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=all_target_fields_acl))])
        asserts.assert_equal(
            result[0].Status, Status.ConstraintError, "Write ACL with all target fields should fail with CONSTRAINT_ERROR"
        )
        ...
        self.step(45)
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(0, acl_attribute(value=acl_original))])
        asserts.assert_equal(result[0].Status, Status.Success, "Write admin-only ACL should succeed")
        log.info("Successfully reset ACL to admin-only entry")

        # Final verification that ACL contains only admin entry
        final_acl = await self.read_single_attribute_check_success(endpoint=0, cluster=acl_cluster, attribute=acl_attribute)
        asserts.assert_equal(len(final_acl), 1, "ACL should contain exactly one admin entry")
        asserts.assert_equal(
            final_acl[0].privilege,
            Clusters.AccessControl.Enums.AccessControlEntryPrivilegeEnum.kAdminister,
            "Remaining entry should have admin privilege",
        )
        asserts.assert_equal(final_acl[0].subjects, [self.th1.nodeId], "Entry should have admin node ID")
