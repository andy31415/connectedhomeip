...
class TC_ACL_2_8(MatterBaseTest):
    ...
    def _get_relevant_acl_events(
        self, all_events: list, expected_add_subject_node_id: int, expected_change_subject_node_ids: list
    ) -> tuple:
        """
        ...
        added_events = [
            ...
            for e in all_events
            if (
                e.Data.changeType == Clusters.AccessControl.Enums.ChangeTypeEnum.kAdded
                and e.Data.latestValue.subjects == [expected_add_subject_node_id]
            )
        ]
        ...
        changed_events = [
            ...
            for e in all_events
            if (
                e.Data.changeType == Clusters.AccessControl.Enums.ChangeTypeEnum.kChanged
                and set(e.Data.latestValue.subjects) == set(expected_change_subject_node_ids)
                and e.Header.EventNumber > added_event.Header.EventNumber
            )
        ]
        if changed_events:
        ...
        removed_events = [
            ...
            for e in all_events
            if (
                e.Data.changeType == Clusters.AccessControl.Enums.ChangeTypeEnum.kRemoved
                and e.Data.latestValue.subjects == [expected_add_subject_node_id]
                and e.Header.EventNumber > added_event.Header.EventNumber
            )
        ]
        asserts.assert_true(
            len(removed_events) > 0,
        ...
        added_new_events = [
            ...
            if (
                e.Data.changeType == Clusters.AccessControl.Enums.ChangeTypeEnum.kAdded
                and set(e.Data.latestValue.subjects) == set(expected_change_subject_node_ids)
                and e.Header.EventNumber > removed_event.Header.EventNumber
            )
        ]
        asserts.assert_true(
    ...
    def _verify_acl_event(self, event, admin_node_id, admin_passcode_id, change_type, subjects, fabric_index):
    ...
    def _validate_events(
        self,
        events,
        expected_fabric_index,
        expected_node_id,
        other_fabric_index,
        controller_name,
        is_filtered,
        force_legacy_encoding,
    ):
            ...
            if hasattr(event, "Data") and hasattr(event.Data, "fabricIndex"):
                ...
                if event.Data.fabricIndex == expected_fabric_index:
                    ...
                    if (
                        ...
                        and event.Data.adminNodeID == NullValue
                        and event.Data.adminPasscodeID == 0
                    ) or (
                        event.Data.changeType == Clusters.AccessControl.Enums.ChangeTypeEnum.kChanged
                        and event.Data.adminNodeID == expected_node_id
                        and event.Data.adminPasscodeID == NullValue
                    ):
    ...
    async def internal_test_TC_ACL_2_8(self, force_legacy_encoding: bool):
        ...
        f1 = await self.read_single_attribute_check_success(
            dev_ctrl=self.th1, endpoint=0, cluster=oc_cluster, attribute=cfi_attribute
        )
        ...
        acl_struct = Clusters.AccessControl.Structs.AccessControlEntryStruct(
            privilege=5, authMode=2, subjects=[self.th1.nodeId, 1111]
        )
        acl_list = [acl_struct]
        acl_attr = Clusters.AccessControl.Attributes.Acl
        result = await self.write_attribute_with_encoding_option(
            self.th1, self.dut_node_id, [(0, acl_attr(value=acl_list))], forceLegacyListEncoding=force_legacy_encoding
        )
        ...
        acl_struct_th2 = Clusters.AccessControl.Structs.AccessControlEntryStruct(
            privilege=5, authMode=2, subjects=[self.th2.nodeId, 2222]
        )
        acl_list = [acl_struct_th2]
        result = await self.write_attribute_with_encoding_option(
            self.th2, self.dut_node_id, [(0, acl_attr(value=acl_list))], forceLegacyListEncoding=force_legacy_encoding
        )
        asserts.assert_equal(result[0].Status, Status.Success, "Write should have succeeded")
        ...
        asserts.assert_equal(len(acl_list_filtered), 1, "Should have exactly one ACL entry when fabric filtered")
        ...
        acl_list_unfiltered = await self.read_single_attribute_check_success(
            dev_ctrl=self.th1, endpoint=0, cluster=ac_cluster, attribute=acl_attr, fabric_filtered=False
        )
        ...
        for entry in acl_list_unfiltered:
            if entry.fabricIndex == f2:
                ...
                asserts.assert_true(
                    entry.targets is None or entry.targets == NullValue, "Non-accessing fabric: targets should be empty"
                )
        ...
        # Read with fabric_filtered=False
        acl_list_unfiltered = await self.read_single_attribute_check_success(
            dev_ctrl=self.th2, endpoint=0, cluster=ac_cluster, attribute=acl_attr, fabric_filtered=False
        )
        ...
        events_filtered = await self.th1.ReadEvent(
            self.dut_node_id, [(0, Clusters.AccessControl.Events.AccessControlEntryChanged)], fabricFiltered=True
        )
        ...
        events_unfiltered = await self.th1.ReadEvent(
            self.dut_node_id, [(0, Clusters.AccessControl.Events.AccessControlEntryChanged)], fabricFiltered=False
        )
        ...
        self._verify_acl_event(added_event, NullValue, 0, Clusters.AccessControl.Enums.ChangeTypeEnum.kAdded, self.th1.nodeId, f1)
        relevant_events = [added_event]
        if force_legacy_encoding:
            removed_event, added_event2 = result[1], result[2]
            self._verify_acl_event(
                removed_event, self.th1.nodeId, NullValue, Clusters.AccessControl.Enums.ChangeTypeEnum.kRemoved, self.th1.nodeId, f1
            )
            self._verify_acl_event(
                added_event2,
                self.th1.nodeId,
        ...
        added_event = result[0]
        self._verify_acl_event(added_event, NullValue, 0, Clusters.AccessControl.Enums.ChangeTypeEnum.kAdded, self.th2.nodeId, f2)
        relevant_events = [added_event]
        if force_legacy_encoding:
            removed_event, added_event2 = result[1], result[2]
            self._verify_acl_event(
                removed_event, self.th2.nodeId, NullValue, Clusters.AccessControl.Enums.ChangeTypeEnum.kRemoved, self.th2.nodeId, f2
            )
            self._verify_acl_event(
                added_event2,
                self.th2.nodeId,
    ...
    def steps_TC_ACL_2_8(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                3,
                "TH1 puts DUT into commissioning mode, TH2 commissions DUT using admin node ID N2",
