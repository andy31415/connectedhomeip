...
class TC_OPCREDS_3_5(MatterBaseTest):
    ...
    def steps_TC_OPCREDS_3_5(self):
        return [
            TestStep(1, "TH0 adds TH1 over CASE", "Commissioning is successful", is_commissioning=True),
            TestStep(
                2,
            ...
            TestStep(
                7,
                """TH1 sends the UpdateNOC command to the Node Operational Credentials cluster with the following fields:
    ...
    async def test_TC_OPCREDS_3_5(self):
        ...
        nocs = await self.read_single_attribute_check_success(
            dev_ctrl=th1, node_id=self.dut_node_id, cluster=opcreds, attribute=opcreds.Attributes.NOCs, fabric_filtered=True
        )
        ...
        trusted_root_certificates = await self.read_single_attribute_check_success(
            dev_ctrl=th1, node_id=self.dut_node_id, cluster=opcreds, attribute=opcreds.Attributes.TrustedRootCertificates
        )
        asserts.assert_in(rcac_original, trusted_root_certificates, "RCAC should match one of the TrustedRootCertificates.")
        ...
        asserts.assert_not_equal(
            original_cert_chain.nocBytes, update1_cert_chain.nocBytes, "Generated matching NOC despite validity changes"
        )
        asserts.assert_not_equal(
            original_cert_chain.icacBytes, update1_cert_chain.icacBytes, "Generated matching ICAC despite validity changes"
        )
        ...
        asserts.assert_not_equal(
            update2_cert_chain.nocBytes, update1_cert_chain.nocBytes, "Generated matching NOC despite validity changes"
        )
        asserts.assert_not_equal(
            update2_cert_chain.icacBytes, update1_cert_chain.icacBytes, "Generated matching ICAC despite validity changes"
        )
        asserts.assert_not_equal(
            update2_cert_chain.nocBytes, original_cert_chain.nocBytes, "Generated matching NOC despite validity changes"
        )
        asserts.assert_not_equal(
            update2_cert_chain.icacBytes, original_cert_chain.icacBytes, "Generated matching ICAC despite validity changes"
        )
        ...
        self.step(19)
        fabric_idx = resp = await self.read_single_attribute_check_success(
            dev_ctrl=th1, node_id=self.dut_node_id, cluster=opcreds, attribute=opcreds.Attributes.CurrentFabricIndex
        )
