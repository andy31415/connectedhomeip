...
class TC_JFDS_2_3(MatterBaseTest):
    ...
    def steps_TC_JFDS_2_3(self) -> list[TestStep]:
        return [
            TestStep(
                "1",
                "TH reads GroupList attribute from DUT",
            ...
            TestStep(
                "2", "TH sends AddGroup command to DUT with GroupID=0x000A", "Verify that the DUT responds with Status as Success"
            ),
            TestStep(
                "3",
                "TH reads GroupList attribute from DUT",
            ...
            TestStep(
                "4",
                "TH sends UpdateGroup command to DUT with GroupID=0x000a and other values different from values used in step 2",
            ...
            TestStep(
                "6",
                "TH sends RemoveGroup command to DUT with GroupID=0x000a",
                "Verify that the DUT responds with Status as SUCCESS",
            ),
            TestStep("7", "TH reads GroupList attribute from DUT", "Verify that no entry with GroupID=0x000a exists in the list"),
            TestStep(
                "8",
                "TH sends AddGroup command to DUT with GroupCAT: 0xFFFF_0001 (Admin CAT)",
                "Verify that the DUT responds with Error with Status code CONSTRAINT_ERROR",
            ),
            TestStep(
                "9",
                "TH sends UpdateGroup command to DUT with GroupID=admin_cat_group_id",
                "Verify that the DUT responds with Status code CONSTRAINT_ERROR",
            ),
            TestStep(
                "10",
                "TH sends RemoveGroup command to DUT with GroupID=admin_cat_group_id",
                "Verify that the DUT responds with Status code CONSTRAINT_ERROR",
            ),
            TestStep(
                "11",
                "TH sends AddGroup command to DUT with GroupCAT: 0xFFFE_0001 (Anchor CAT)",
                "Verify that the DUT responds with Error with Status code CONSTRAINT_ERROR",
            ),
            TestStep(
                "12",
                "TH sends UpdateGroup command to DUT with GroupID=anchor_cat_group_id",
                "Verify that the DUT responds with Status code CONSTRAINT_ERROR",
            ),
            TestStep(
                "13",
                "TH sends RemoveGroup command to DUT with GroupID=anchor_cat_group_id",
                "Verify that the DUT responds with Status code CONSTRAINT_ERROR",
            ),
        ]
    ...
    async def test_TC_JFDS_2_3(self):
        ...
        asserts.assert_greater(len(groupList), num_entries, "A new entry was not added to GroupList")
        ...
        try:
            await self.send_single_cmd(
                cmd=step8_cmd, dev_ctrl=self.devCtrlEcoA, node_id=self.jfadmin_fabric_a_node_id, endpoint=jfds_endpoint
            )
        ...
        try:
            await self.send_single_cmd(
                cmd=step9_cmd, dev_ctrl=self.devCtrlEcoA, node_id=self.jfadmin_fabric_a_node_id, endpoint=jfds_endpoint
            )
            asserts.fail("Expected CONSTRAINT_ERROR but command succeeded")
        except InteractionModelError as e:
            asserts.assert_equal(e.status, Status.ConstraintError, f"Expected CONSTRAINT_ERROR but got {e.status}")
        ...
        try:
            await self.send_single_cmd(
                cmd=step11_cmd, dev_ctrl=self.devCtrlEcoA, node_id=self.jfadmin_fabric_a_node_id, endpoint=jfds_endpoint
            )
            asserts.fail("Expected CONSTRAINT_ERROR but command succeeded")
        except InteractionModelError as e:
            asserts.assert_equal(e.status, Status.ConstraintError, f"Expected CONSTRAINT_ERROR but got {e.status}")
