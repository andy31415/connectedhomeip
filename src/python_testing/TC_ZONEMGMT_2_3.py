...
class TC_ZONEMGMT_2_3(MatterBaseTest):
    ...
    def steps_TC_ZONEMGMT_2_3(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "2",
                "If DUT supports TwoDCartesianZone and User defined zones, TH sends CreateTwoDCartesianZone command with",
                " valid parameters",
                "Verify that the DUT response contains a new zoneId and the corresponding zone information matches.",
            ),
            TestStep(
                "2a",
                "TH sends UpdateTwoDCartesianZone command with ZoneID set to the created ZoneID in step 2 and zone field set to Zone1_Updated ",
                "Verify that the DUT responds with a Success status code and the updated zone information matches.",
            ),
            TestStep(
                "3",
                "TH sends UpdateTwoDCartesianZone command with ZoneID that does not exist in Zones attribute",
                "Verify that the DUT responds with a NotFound status code",
            ),
            TestStep(
                "4",
                "TH retrieves a zone whose ZoneSource is Mfg, if it exists and attempts to update that zone",
                "Verify that the DUT responds with a ConstraintError status code",
            ),
            TestStep(
                "5",
                "TH sends UpdateTwoDCartesianZone command with any X or Y value of zone vertices set to TwoDCartesianMax + 1",
            ...
            TestStep(
                "6a",
                "TH sends the UpdateTwoDCartesianZone command with the same zoneID  as step 2a(ZoneID1) and zone vertices as  in step 6",
                "Verify that the DUT responds with an AlreadyExists status code.",
            ),
            TestStep(
                "7",
                "TH sends the UpdateTwoDCartesianZone command with the zone vertices forming a self-intersecting polygonal shape",
                "Verify that the DUT responds with a DynamicConstraintError status code.",
            ),
            TestStep(
                "8",
                "TH reads Zones attribute and sends RemoveZone command for all reviously created zones",
    ...
    async def test_TC_ZONEMGMT_2_3(self):
        ...
        if self.twoDCartSupported and self.userDefinedSupported:
            ...
            zoneToCreate = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone1", use=enums.ZoneUseEnum.kMotion, vertices=zoneVertices, color="#00FFFF"
            )
            createTwoDCartesianCmd = commands.CreateTwoDCartesianZone(zone=zoneToCreate)
            ...
            asserts.assert_equal(type(cmdResponse), commands.CreateTwoDCartesianZoneResponse, "Incorrect response type")
            ...
            matchingZone = next((z for z in zonesBeforeCreate if z.zoneID == zoneID1), None)
            ...
            matchingZone = next((z for z in zonesAfterCreate if z.zoneID == zoneID1), None)
            asserts.assert_is_not_none(matchingZone, "fZone with {zoneID1} not found")
            asserts.assert_equal(
                matchingZone.zoneType, enums.ZoneTypeEnum.kTwoDCARTZone, "ZoneType of created Zone is not of type TwoDCartZone"
            )
            asserts.assert_equal(
                matchingZone.zoneSource, enums.ZoneSourceEnum.kUser, "ZoneSource of created Zone is not of type User"
            )
            asserts.assert_equal(
                matchingZone.twoDCartesianZone.vertices, zoneVertices, "ZoneVertices of created Zone are mismatched"
            )
            ...
            zoneToUpdate = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone1_Updated", use=enums.ZoneUseEnum.kMotion, vertices=updatedZoneVertices, color="#00FFFF"
            )
            updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=zoneID1, zone=zoneToUpdate)
            try:
            ...
            matchingZone = next((z for z in zonesAfterUpdate if z.zoneID == zoneID1), None)
            asserts.assert_is_not_none(matchingZone, "fZone with {zoneID1} not found")
            asserts.assert_equal(
                matchingZone.zoneType, enums.ZoneTypeEnum.kTwoDCARTZone, "ZoneType of updated Zone is not of type TwoDCartZone"
            )
            asserts.assert_equal(
                matchingZone.zoneSource, enums.ZoneSourceEnum.kUser, "ZoneSource of updated Zone is not of type User"
            )
            asserts.assert_equal(matchingZone.twoDCartesianZone.name, "Zone1_Updated", "Name of updated Zone does not match")
            asserts.assert_equal(
                matchingZone.twoDCartesianZone.vertices, updatedZoneVertices, "ZoneVertices of created Zone are mismatched"
            )
            ...
            try:
                ...
                zoneToUpdate = cluster.Structs.TwoDCartesianZoneStruct(
                    name="Zone_3", use=enums.ZoneUseEnum.kMotion, vertices=zoneVertices, color="#00FFFF"
                )
                updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=newZoneId, zone=zoneToUpdate)
                await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
            ...
            zones = await self.read_single_attribute_check_success(endpoint=endpoint, cluster=cluster, attribute=attr.Zones)
            log.info(f"Rx'd Zones: {zones}")
            matchingZone = next((z for z in zones if z.zoneSource == enums.ZoneSourceEnum.kMfg), None)
            if matchingZone is not None:
                ...
                try:
                    ...
                    updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=matchingZone.zoneID, zone=zoneToUpdate)
                    await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
            ...
            zoneToUpdate = cluster.Structs.TwoDCartesianZoneStruct(
                name="ZoneExceedsTwoDCartMax", use=enums.ZoneUseEnum.kMotion, vertices=zoneVertices, color="#00FFFF"
            )
            try:
                updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=zoneID1, zone=zoneToUpdate)
                await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
                ...
                asserts.assert_equal(
                    e.status, Status.DynamicConstraintError, "Unexpected error returned when trying to update zone"
                )
                pass
            ...
            zoneToCreate = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone2", use=enums.ZoneUseEnum.kMotion, vertices=zone2Vertices, color="#00FFFF"
            )
            createTwoDCartesianCmd = commands.CreateTwoDCartesianZone(zone=zoneToCreate)
            cmdResponse = await self.send_single_cmd(endpoint=endpoint, cmd=createTwoDCartesianCmd)
            log.info(f"Rx'd CreateTwoDCartesianZoneResponse : {cmdResponse}")
            ...
            matchingZone = next((z for z in zonesBeforeCreate if z.zoneID == zoneID2), None)
            ...
            matchingZone = next((z for z in zonesAfterCreate if z.zoneID == zoneID2), None)
            ...
            asserts.assert_equal(
                matchingZone.twoDCartesianZone.vertices, zone2Vertices, "ZoneVertices of created Zone are mismatched"
            )

            self.step("6a")
            # Attempt updating ZoneID1 with Zone2Vertices from ZoneID2
            zoneToUpdate = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone1_Updated", use=enums.ZoneUseEnum.kMotion, vertices=zone2Vertices, color="#00FFFF"
            )
            try:
                updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=zoneID1, zone=zoneToUpdate)
                await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
            ...
            zoneToUpdate1 = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone1", use=enums.ZoneUseEnum.kMotion, vertices=selfIntersectingVertices1, color="#00FFFF"
            )
            zoneToUpdate2 = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone1", use=enums.ZoneUseEnum.kMotion, vertices=selfIntersectingVertices2, color="#00FFFF"
            )
            zoneToUpdate3 = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone1", use=enums.ZoneUseEnum.kMotion, vertices=selfIntersectingVertices3, color="#00FFFF"
            )
            zoneToUpdate4 = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone1", use=enums.ZoneUseEnum.kMotion, vertices=selfIntersectingVertices4, color="#00FFFF"
            )
            zoneToUpdate5 = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone1", use=enums.ZoneUseEnum.kMotion, vertices=selfIntersectingVertices5, color="#00FFFF"
            )
            zoneToUpdate6 = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone1", use=enums.ZoneUseEnum.kMotion, vertices=selfIntersectingVertices6, color="#00FFFF"
            )
            zoneToUpdate7 = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone1", use=enums.ZoneUseEnum.kMotion, vertices=selfIntersectingVertices7, color="#00FFFF"
            )

            # Update and send the command with selfIntersectingVertices1
            updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=zoneID1, zone=zoneToUpdate1)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
                asserts.assert_equal(
                    e.status, Status.DynamicConstraintError, "Unexpected error returned when trying to create zone"
                )
                pass

            # Update and send the command with selfIntersectingVertices2
            updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=zoneID1, zone=zoneToUpdate2)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
            ...
            updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=zoneID1, zone=zoneToUpdate3)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
            ...
            updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=zoneID1, zone=zoneToUpdate4)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
            ...
            updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=zoneID1, zone=zoneToUpdate5)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
            ...
            updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=zoneID1, zone=zoneToUpdate6)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
            ...
            updateTwoDCartesianCmd = commands.UpdateTwoDCartesianZone(zoneID=zoneID1, zone=zoneToUpdate7)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=updateTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
