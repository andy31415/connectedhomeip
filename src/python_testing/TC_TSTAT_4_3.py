...
class TC_TSTAT_4_3(MatterBaseTest):
    ...
    async def send_add_thermostat_suggestion_command(
        self,
        endpoint: int = None,
        ...
        try:
            addThermostatSuggestionResponse = await self.send_single_cmd(
                cmd=cluster.Commands.AddThermostatSuggestion(preset_handle, effective_time, expiration_in_minutes),
                endpoint=endpoint,
            )
    ...
    async def send_remove_thermostat_suggestion_command(
        self, endpoint: int = None, uniqueID: int = None, expected_status: Status = Status.Success
    ):
        try:
            removeThermostatSuggestionResponse = await self.send_single_cmd(
                cmd=cluster.Commands.RemoveThermostatSuggestion(uniqueID), endpoint=endpoint
            )
            asserts.assert_equal(expected_status, Status.Success)
    ...
    def steps_TC_TSTAT_4_3(self) -> list[TestStep]:
        return [
            ...
            TestStep("2a", "TH reads the FeatureMap attribute.", "Verify that the TSUGGEST bit is set in the FeatureMap value."),
            TestStep(
                "2b",
            ...
            TestStep(
                "5",
                "TH picks a random preset handle that does not match any entry in the Presets attribute and calls the AddThermostatSuggestion command with the preset handle, the EffectiveTime set to the current UTC timestamp and ExpirationInMinutes is set to 30 minutes.",
            ...
            TestStep(
                "7b",
                "TH sets TemperatureSetpointHold to SetpointHoldOff after 2 seconds. TH reads the CurrentThermostatSuggestion, the ThermostatSuggestionNotFollowingReason and the ActivePresetHandle attributes.",
            ...
            TestStep(
                "9b",
                "TH calls the RemoveThermostatSuggestion command with the UniqueID field set to the UniqueID fields of the two entries in the ThermostatSuggestions attribute.",
    ...
    async def test_TC_TSTAT_4_3(self):
        ...
        asserts.assert_true(
            feature_map & cluster.Bitmaps.Feature.kThermostatSuggestions, "TSUGGEST bit is not set in the FeatureMap"
        )

        self.step("2b")
        # TH reads the Presets attribute and saves it in a SupportedPresets variable.
        supported_presets = await self.read_single_attribute_check_success(
        ...
        if self.pics_guard(self.check_pics("TIMESYNC.S.A0000")):
            # TH checks if the Thermostat does not have a valid UTC time.
            currentUTC = await self.read_single_attribute_check_success(
                endpoint=0, cluster=time_cluster, attribute=time_cluster.Attributes.UTCTime
            )
        ...
        if not has_valid_time:
            ...
            if len(possiblePresetHandles) > 0:
                ...
                # Verify that the AddThermostatSuggestion command returns INVALID_IN_STATE.
                await self.send_add_thermostat_suggestion_command(
                    endpoint=endpoint,
            ...
            if self.pics_guard(self.check_pics("TIMESYNC.S.C00.Rsp")):
                ...
                try:
                    ...
                    await self.send_single_cmd(
                        cmd=time_cluster.Commands.SetUTCTime(
                            UTCTime=get_epoch_utc_time() * 1_000_000,
                            granularity=time_cluster.Enums.GranularityEnum.kMillisecondsGranularity,
                        ),
                        endpoint=0,
                    )
                except InteractionModelError as e:
                ...
                asserts.assert_equal(
                    code,
                    0,
                    "Test failed because Thermostat has no UTCTime set and failed to set UTCTime. Thermostat suggestions feature needs UTCTime.",
                )
        else:
        ...
        possiblePresetHandles = [preset.presetHandle for preset in supported_presets if preset.presetHandle != activePresetHandle]
        asserts.assert_greater_equal(
            len(possiblePresetHandles),
            1,
        ...
        addThermostatSuggestionResponse = await self.send_add_thermostat_suggestion_command(
            endpoint=endpoint,
        ...
        currentThermostatSuggestion = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.CurrentThermostatSuggestion
        )
        thermostatSuggestionNotFollowingReason = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.ThermostatSuggestionNotFollowingReason
        )
        activePresetHandle = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.ActivePresetHandle
        )
        ...
        thermostatSuggestions = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.ThermostatSuggestions
        )
        asserts.assert_greater(
            len(thermostatSuggestions),
            0,
            "Expected at least one entry in ThermostatSuggestions after AddThermostatSuggestion command.",
        )
        asserts.assert_equal(
            thermostatSuggestions[0].uniqueID,
            addThermostatSuggestionResponse_uniqueID,
        ...
        asserts.assert_equal(
            currentThermostatSuggestion.expirationTime,
            expirationTime,
        ...
        asserts.assert_equal(
            thermostatSuggestionNotFollowingReason, NullValue, "ThermostatSuggestionNotFollowingReason should be set to null"
        )
        asserts.assert_equal(
            activePresetHandle,
            currentThermostatSuggestion.presetHandle,
        ...
        await self.send_remove_thermostat_suggestion_command(
            endpoint=endpoint, uniqueID=currentThermostatSuggestion.uniqueID, expected_status=Status.Success
        )
        ...
        if self.pics_guard(self.check_pics("TSTAT.S.A0023") and self.check_pics("TSTAT.S.A0024")):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.TemperatureSetpointHold(
                    cluster.Enums.TemperatureSetpointHoldEnum.kSetpointHoldOn
                ),
                endpoint_id=endpoint,
                expect_success=True,
            )
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.TemperatureSetpointHoldDuration(NullValue),
                endpoint_id=endpoint,
                expect_success=True,
            )
            ...
            asserts.assert_greater_equal(
                len(possiblePresetHandles),
                1,
                "Couldn't run test step 7a since all preset handles are also the ActivePresetHandle on this Thermostat",
            )
            presetHandle = possiblePresetHandles[0]
            currentUTC = get_epoch_utc_time()
            expirationInMinutes = 30
            ...
            temperatureSetpointHold = await self.read_single_attribute_check_success(
                endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.TemperatureSetpointHold
            )
            asserts.assert_equal(
            ...
            temperatureSetpointHoldDuration = await self.read_single_attribute_check_success(
                endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.TemperatureSetpointHoldDuration
            )
        ...
        if self.pics_guard(self.check_pics("TSTAT.S.A0023") and self.check_pics("TSTAT.S.A0024")):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.TemperatureSetpointHold(
                    cluster.Enums.TemperatureSetpointHoldEnum.kSetpointHoldOff
                ),
                endpoint_id=endpoint,
                expect_success=True,
            )
        ...
        asserts.assert_greater_equal(
            len(possiblePresetHandles),
            1,
            "Couldn't run test step 8a since all preset handles are also the ActivePresetHandle on this Thermostat",
        )
        presetHandle = possiblePresetHandles[0]
        currentUTC = get_epoch_utc_time()
        expirationInMinutes = 30
        ...
        await self.send_remove_thermostat_suggestion_command(
            endpoint=endpoint, uniqueID=random_uniqueID, expected_status=Status.NotFound
        )
        ...
        asserts.assert_greater_equal(
            len(possiblePresetHandles),
            1,
            "Couldn't run test step 9a since all preset handles are also the ActivePresetHandle on this Thermostat",
        )
        firstPresetHandle = possiblePresetHandles[0]
        firstCurrentUTC = get_epoch_utc_time()
        firstExpirationInMinutes = 35
        firstAddThermostatSuggestionResponse = await self.send_add_thermostat_suggestion_command(
            endpoint=endpoint,
        ...
        secondAddThermostatSuggestionResponse = await self.send_add_thermostat_suggestion_command(
            endpoint=endpoint,
        ...
        asserts.assert_is_not_none(
            firstAddThermostatSuggestionResponse,
        ...
        asserts.assert_is_not_none(
            secondAddThermostatSuggestionResponse,
        ...
        asserts.assert_in(
            firstAddThermostatSuggestionResponse_uniqueID, thermostatSuggestionUniqueIDs, "First suggestion's UniqueID not found"
        )
        asserts.assert_in(
            secondAddThermostatSuggestionResponse_uniqueID, thermostatSuggestionUniqueIDs, "Second suggestion's UniqueID not found"
        )
        ...
        try:
            asserts.assert_equal(
                currentThermostatSuggestion.uniqueID,
                thermostatSuggestions[0].uniqueID,
                "UniqueID in the entry for CurrentThermostatSuggestion does not match the UniqueID entry from First AddThermostatSuggestionResponse.",
            )
            thermSuggestionIndex = 0
        except AssertionError:
            asserts.assert_equal(
                currentThermostatSuggestion.uniqueID,
                thermostatSuggestions[1].uniqueID,
                "UniqueID in the entry for CurrentThermostatSuggestion does not match the UniqueID entry from First or Second AddThermostatSuggestionResponse.",
            )
            thermSuggestionIndex = 1

        asserts.assert_equal(
            currentThermostatSuggestion.presetHandle,
            thermostatSuggestions[thermSuggestionIndex].presetHandle,
        ...
        asserts.assert_equal(
            currentThermostatSuggestion.effectiveTime,
            thermostatSuggestions[thermSuggestionIndex].effectiveTime,
        ...
        asserts.assert_equal(
            currentThermostatSuggestion.expirationTime,
            thermostatSuggestions[thermSuggestionIndex].expirationTime,
        ...
        await self.send_remove_thermostat_suggestion_command(
            endpoint=endpoint, uniqueID=firstAddThermostatSuggestionResponse_uniqueID, expected_status=Status.Success
        )
        ...
        asserts.assert_equal(
            len(thermostatSuggestions), 1, "ThermostatSuggestions should have 1 entry after one suggestion has been removed."
        )

        await self.send_remove_thermostat_suggestion_command(
            endpoint=endpoint, uniqueID=secondAddThermostatSuggestionResponse_uniqueID, expected_status=Status.Success
        )
        ...
        asserts.assert_greater_equal(
            len(possiblePresetHandles),
            1,
            "Couldn't run test step 10 since all preset handles are also the ActivePresetHandle on this Thermostat",
        )
        presetHandle = possiblePresetHandles[0]
        currentUTC = int((datetime.now(timezone.utc) - datetime(2000, 1, 1, 0, 0, 0, 0, timezone.utc)).total_seconds()) + int(
        ...
        maxThermostatSuggestions = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.MaxThermostatSuggestions
        )
        ...
        asserts.assert_greater_equal(
            len(possiblePresetHandles),
            1,
            "Couldn't run test step 11 since all preset handles are also the ActivePresetHandle on this Thermostat",
        )
        presetHandle = possiblePresetHandles[0]
        currentUTC = get_epoch_utc_time()
        expirationInMinutes = 30
        ...
        for i in range(0, maxThermostatSuggestions + 1):
            if i < maxThermostatSuggestions:
                ...
                asserts.assert_equal(
                    len(thermostatSuggestions), i + 1, f"ThermostatSuggestions should have {i + 1} number of entries."
                )
            else:
