...
class TC_TSTAT_2_2(MatterBaseTest):
    ...
    def steps_TC_TSTAT_2_2(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "2b", "Test Harness Client then attempts Writes OccupiedCoolingSetpoint to a value below the MinCoolSetpointLimit"
            ),
    ...
    async def test_TC_TSTAT_2_2(self):
        ...
        ControlSequenceOfOperation = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.ControlSequenceOfOperation
        )
        ...
        if self.pics_guard(hasCoolingFeature):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.OccupiedCoolingSetpoint(
                    MinCoolSetpointLimitValue + ((MaxCoolSetpointLimitValue - MinCoolSetpointLimitValue) // 2)
                ),
        ...
        if self.pics_guard(hasCoolingFeature):
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.OccupiedCoolingSetpoint(MinCoolSetpointLimitValue - 1),
                endpoint_id=endpoint,
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.OccupiedCoolingSetpoint(MaxCoolSetpointLimitValue + 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
        ...
        if self.pics_guard(hasCoolingFeature):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.OccupiedCoolingSetpoint(MaxCoolSetpointLimitValue), endpoint_id=endpoint
            )
            ...
            if self.pics_guard(hasAutoModeFeature):
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.OccupiedCoolingSetpoint(
                        max(MinCoolSetpointLimitValue, (OccupiedHeatingSetpointValue + (MinSetpointDeadBandValue)))
                    ),
                    endpoint_id=endpoint,
                )
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.OccupiedCoolingSetpoint(MinCoolSetpointLimitValue), endpoint_id=endpoint
                )
        ...
        if self.pics_guard(hasHeatingFeature):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.OccupiedHeatingSetpoint(
                    MinHeatSetpointLimitValue + ((MaxHeatSetpointLimitValue - MinHeatSetpointLimitValue) // 2)
                ),
                endpoint_id=endpoint,
            )
        ...
        if self.pics_guard(hasHeatingFeature):
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.OccupiedHeatingSetpoint(MinHeatSetpointLimitValue - 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.OccupiedHeatingSetpoint(MaxHeatSetpointLimitValue + 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
        ...
        if self.pics_guard(hasHeatingFeature):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.OccupiedHeatingSetpoint(MinHeatSetpointLimitValue), endpoint_id=endpoint
            )
            ...
            if self.pics_guard(hasAutoModeFeature):
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.OccupiedHeatingSetpoint(upper_limit), endpoint_id=endpoint
                )

            else:
                # Test Harness Writes the limit of MaxHeatSetpointLimit to OccupiedHeatingSetpoint attribute If TSTAT.S.F05 is true
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.OccupiedHeatingSetpoint(MaxHeatSetpointLimitValue), endpoint_id=endpoint
                )
        ...
        if self.pics_guard(hasOccupancyFeature and hasCoolingFeature):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.UnoccupiedCoolingSetpoint(
                    MinCoolSetpointLimitValue + ((MaxCoolSetpointLimitValue - MinCoolSetpointLimitValue) // 2)
                ),
                endpoint_id=endpoint,
            )
        ...
        if self.pics_guard(hasOccupancyFeature and hasCoolingFeature):
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.UnoccupiedCoolingSetpoint(MinCoolSetpointLimitValue - 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.UnoccupiedCoolingSetpoint(MaxCoolSetpointLimitValue + 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
        ...
        if self.pics_guard(hasOccupancyFeature and hasCoolingFeature):
            if self.pics_guard(hasAutoModeFeature):
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.UnoccupiedCoolingSetpoint(LowerLimit), endpoint_id=endpoint
                )

            else:
                # Test Harness Writes the limit of MinCoolSetpointLimit to UnoccupiedCoolingSetpoint attribute
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.UnoccupiedCoolingSetpoint(MinCoolSetpointLimitValue), endpoint_id=endpoint
                )
            # Test Harness Writes the limit of MaxCoolSetpointLimit to UnoccupiedCoolingSetpoint attribute
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.UnoccupiedCoolingSetpoint(MaxCoolSetpointLimitValue), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasOccupancyFeature and hasHeatingFeature):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.UnoccupiedHeatingSetpoint(
                    MinHeatSetpointLimitValue + ((MaxHeatSetpointLimitValue - MinHeatSetpointLimitValue) // 2)
                ),
                endpoint_id=endpoint,
            )
        ...
        if self.pics_guard(hasOccupancyFeature and hasHeatingFeature):
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.UnoccupiedHeatingSetpoint(MinHeatSetpointLimitValue - 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.UnoccupiedHeatingSetpoint(MaxHeatSetpointLimitValue + 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
        ...
        if self.pics_guard(hasOccupancyFeature and hasHeatingFeature):
            if self.pics_guard(hasAutoModeFeature):
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.UnoccupiedHeatingSetpoint(
                        max(MaxHeatSetpointLimitValue, (UnoccupiedCoolingSetpointValue + MinSetpointDeadBandValue))
                    ),
                    endpoint_id=endpoint,
                )
                ...
                # Test Harness Writes the limit of MaxHeatSetpointLimit to UnoccupiedHeatingSetpoint attribute
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.UnoccupiedHeatingSetpoint(MaxHeatSetpointLimitValue), endpoint_id=endpoint
                )

            # Test Harness Writes the limit of MinHeatSetpointLimit to UnoccupiedHeatingSetpoint attribute
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.UnoccupiedHeatingSetpoint(MinHeatSetpointLimitValue), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasHeatingFeature and hasMinHeatSetpointLimitAttribute):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinHeatSetpointLimit(AbsMinHeatSetpointLimitValue + 1), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasHeatingFeature and hasMinHeatSetpointLimitAttribute):
            if self.pics_guard(hasAutoModeFeature):
                ...
                existingCoolMinSetpoint = await self.read_single_attribute_check_success(
                    endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.MinCoolSetpointLimit
                )
                status = await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MinHeatSetpointLimit(
                        (existingCoolMinSetpoint - MinSetpointDeadBandValue) + 1
                    ),
                    endpoint_id=endpoint,
                    expect_success=False,
                )
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinHeatSetpointLimit(AbsMinHeatSetpointLimitValue - 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinHeatSetpointLimit(AbsMaxHeatSetpointLimitValue + 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
        ...
        if self.pics_guard(hasHeatingFeature and hasMinHeatSetpointLimitAttribute):
            if self.pics_guard(hasAutoModeFeature):
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MinHeatSetpointLimit(
                        min(MaxHeatSetpointLimitValue, (MinCoolSetpointLimitValue - MinSetpointDeadBandValue))
                    ),
                    endpoint_id=endpoint,
                )
            else:
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MinHeatSetpointLimit(MaxHeatSetpointLimitValue), endpoint_id=endpoint
                )

            # Test Harness Writes the limit of AbsMinHeatSetpointLimit to MinHeatSetpointLimit attribute
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinHeatSetpointLimit(AbsMinHeatSetpointLimitValue), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasHeatingFeature and hasMaxHeatSetpointLimitAttribute):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.MaxHeatSetpointLimit(MinHeatSetpointLimitValue + 1), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasHeatingFeature and hasMinHeatSetpointLimitAttribute):
            if self.pics_guard(hasAutoModeFeature):
                ...
                status = await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MinHeatSetpointLimit(AbsMaxHeatSetpointLimitValue),
                    endpoint_id=endpoint,
                    expect_success=False,
                )
                asserts.assert_equal(status, Status.ConstraintError)
                ...
                # Test Harness Writes a value back that is different but valid for MaxHeatSetpointLimit attribute
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MaxHeatSetpointLimit(AbsMaxHeatSetpointLimitValue - 1), endpoint_id=endpoint
                )
        ...
        if self.pics_guard(hasHeatingFeature and hasMaxHeatSetpointLimitAttribute):
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MaxHeatSetpointLimit(AbsMinHeatSetpointLimitValue - 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MaxHeatSetpointLimit(MinHeatSetpointLimitValue - 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MaxHeatSetpointLimit(AbsMaxHeatSetpointLimitValue + 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
        ...
        if self.pics_guard(hasHeatingFeature and hasMaxHeatSetpointLimitAttribute):
            if self.pics_guard(hasAutoModeFeature):
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MaxHeatSetpointLimit(
                        min(AbsMaxHeatSetpointLimitValue, MaxCoolSetpointLimitValue - MinSetpointDeadBandValue)
                    ),
                    endpoint_id=endpoint,
                )
            else:
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MaxHeatSetpointLimit(AbsMaxHeatSetpointLimitValue), endpoint_id=endpoint
                )

            # Test Harness Writes the limit of MinHeatSetpointLimit to MaxHeatSetpointLimit attribute
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.MaxHeatSetpointLimit(MinHeatSetpointLimitValue), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasCoolingFeature and hasMinCoolSetpointLimitAttribute):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinCoolSetpointLimit(AbsMinCoolSetpointLimitValue + 1), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasCoolingFeature and hasMinCoolSetpointLimitAttribute):
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinCoolSetpointLimit(AbsMinCoolSetpointLimitValue - 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinCoolSetpointLimit(MaxCoolSetpointLimitValue + 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
        ...
        if self.pics_guard(hasCoolingFeature and hasMinCoolSetpointLimitAttribute):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinCoolSetpointLimit(MaxCoolSetpointLimitValue), endpoint_id=endpoint
            )
            ...
            if self.pics_guard(hasAutoModeFeature):
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MinCoolSetpointLimit(
                        max(AbsMinCoolSetpointLimitValue, MinHeatSetpointLimitValue + MinSetpointDeadBandValue)
                    ),
                    endpoint_id=endpoint,
                )
            else:
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MinCoolSetpointLimit(AbsMinCoolSetpointLimitValue), endpoint_id=endpoint
                )
        ...
        if self.pics_guard(hasCoolingFeature and hasMaxCoolSetpointLimitAttribute):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.MaxCoolSetpointLimit(AbsMaxCoolSetpointLimitValue - 1), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasCoolingFeature and hasMaxCoolSetpointLimitAttribute):
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MaxCoolSetpointLimit(MinCoolSetpointLimitValue - 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MaxCoolSetpointLimit(AbsMaxCoolSetpointLimitValue + 1),
                endpoint_id=endpoint,
                expect_success=False,
            )
            asserts.assert_equal(status, Status.ConstraintError)
        ...
        if self.pics_guard(hasCoolingFeature and hasMaxCoolSetpointLimitAttribute):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.MaxCoolSetpointLimit(AbsMaxCoolSetpointLimitValue), endpoint_id=endpoint
            )

            if self.pics_guard(hasAutoModeFeature):
                # Test Harness Writes the limit of MinCoolSetpointLimit to MaxCoolSetpointLimit attribute
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MaxCoolSetpointLimit(MinCoolSetpointLimitValue),
                    endpoint_id=endpoint,
                    expect_success=False,
                )
                ...
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MaxCoolSetpointLimit(
                        max(
                            MinCoolSetpointLimitValue,
                            min(AbsMaxCoolSetpointLimitValue, (MaxHeatSetpointLimitValue + MinSetpointDeadBandValue)),
                        )
                    ),
                    endpoint_id=endpoint,
                )
                ...
                # Test Harness Writes the limit of MinCoolSetpointLimit to MaxCoolSetpointLimit attribute
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MaxCoolSetpointLimit(MinCoolSetpointLimitValue), endpoint_id=endpoint
                )
        ...
        if self.pics_guard(hasHeatingFeature and hasMinHeatSetpointLimitAttribute):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinHeatSetpointLimit(MinHeatSetpointLimitValue), endpoint_id=endpoint
            )

            if self.pics_guard(hasAutoModeFeature):
                # Test Harness Writes MaxHeatSetpointLimit That meets the deadband of 2.5C
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MaxHeatSetpointLimit(MaxCoolSetpointLimitValue - MinSetpointDeadBandValue),
                    endpoint_id=endpoint,
                )
            else:
                # Test Harness Writes (sets back)default value of MaxHeatSetpointLimit
                await self.write_single_attribute(
                    attribute_value=cluster.Attributes.MaxHeatSetpointLimit(MaxHeatSetpointLimitValue), endpoint_id=endpoint
                )
        ...
        if self.pics_guard(hasAutoModeFeature and self.check_pics("TSTAT.S.M.MinSetpointDeadBandWritable")):
            ...
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinSetpointDeadBand(-1), endpoint_id=endpoint, expect_success=False
            )
            asserts.assert_equal(status, Status.ConstraintError)

            # Test Harness Writes the value above MinSetpointDeadBand
            status = await self.write_single_attribute(
                attribute_value=cluster.Attributes.MinSetpointDeadBand(128), endpoint_id=endpoint, expect_success=False
            )
            asserts.assert_equal(status, Status.ConstraintError)
        ...
        if self.pics_guard(hasCoolingFeature and not hasHeatingFeature):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.ControlSequenceOfOperation(1), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasHeatingFeature and not hasCoolingFeature):
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.ControlSequenceOfOperation(3), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasHeatingFeature and hasCoolingFeature):
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.ControlSequenceOfOperation(4), endpoint_id=endpoint
            )
        ...
        if self.pics_guard(hasCoolingFeature):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.OccupiedCoolingSetpoint(OccupiedCoolingSetpointValue), endpoint_id=endpoint
            )

        if self.pics_guard(hasHeatingFeature):
            # Sets OccupiedHeatingSetpoint to default value
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.OccupiedHeatingSetpoint(OccupiedHeatingSetpointValue), endpoint_id=endpoint
            )

            # Sends SetpointRaise Command Heat Only
            await self.send_single_cmd(
                cmd=Clusters.Objects.Thermostat.Commands.SetpointRaiseLower(
                    mode=Clusters.Objects.Thermostat.Enums.SetpointRaiseLowerModeEnum.kHeat, amount=-30
                ),
                endpoint=endpoint,
            )
        ...
        if self.pics_guard(hasCoolingFeature):
            ...
            await self.send_single_cmd(
                cmd=Clusters.Objects.Thermostat.Commands.SetpointRaiseLower(
                    mode=Clusters.Objects.Thermostat.Enums.SetpointRaiseLowerModeEnum.kCool, amount=-30
                ),
                endpoint=endpoint,
            )
        ...
        if self.pics_guard(hasHeatingFeature or hasCoolingFeature):
            ...
            await self.send_single_cmd(
                cmd=Clusters.Objects.Thermostat.Commands.SetpointRaiseLower(
                    mode=Clusters.Objects.Thermostat.Enums.SetpointRaiseLowerModeEnum.kBoth, amount=-30
                ),
                endpoint=endpoint,
            )
        ...
        if self.pics_guard(hasOccupancyFeature and hasCoolingFeature):
            ...
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.UnoccupiedCoolingSetpoint(UnoccupiedCoolingSetpointValue), endpoint_id=endpoint
            )

        if self.pics_guard(hasOccupancyFeature and hasHeatingFeature):
            # Restores UnoccupiedHeatingSetpoint to original value
            await self.write_single_attribute(
                attribute_value=cluster.Attributes.UnoccupiedHeatingSetpoint(UnoccupiedHeatingSetpointValue), endpoint_id=endpoint
            )
