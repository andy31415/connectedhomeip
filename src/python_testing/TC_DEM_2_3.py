...
class TC_DEM_2_3(MatterBaseTest, DEMTestBase):
    ...
    def steps_TC_DEM_2_3(self) -> list[TestStep]:
        return [
            ...
            TestStep("5b", "TH reads from the DUT the OptOutState", "Value has to be 0x01 (LocalOptOut)"),
            TestStep(
                "6",
                "TH sends command StartTimeAdjustRequest with RequestedStartTime=EarliestStartTime from Forecast, Cause=LocalOptimization",
            ...
            TestStep("6b", "TH reads from the DUT the Forecast", "Value has to be unchanged from step 3b"),
            TestStep(
                "7",
            ...
            TestStep("7b", "TH reads from the DUT the OptOutState", "Value has to be 0x00 (NoOptOut)"),
            TestStep(
                "8",
                "TH sends command StartTimeAdjustRequest with RequestedStartTime=EarliestStartTime from Forecast, Cause=LocalOptimization",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ...
            TestStep("9b", "TH reads from the DUT the OptOutState", "Value has to be 0x01 (LocalOptOut)"),
            TestStep(
                "9c",
                "TH reads from the DUT the Forecast",
                "Value has to include EarliestStartTime<=StartTime, LatestEndTime>=EndTime, and ForecastUpdateReason=Internal Optimization",
            ...
            TestStep(
                "10",
                "TH sends command StartTimeAdjustRequest with RequestedStartTime=StartTime+(LatestEndTime-EndTime) from Forecast, Cause=GridOptimization",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            ...
            TestStep("11", "TH sends command CancelRequest", "Verify DUT responds w/ status SUCCESS(0x00)"),
            ...
            TestStep(
                "12",
                "TH sends command StartTimeAdjustRequest with RequestedStartTime=EarliestStartTime-1 from Forecast, Cause=LocalOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep("12a", "TH reads from the DUT the ESAState", "Value has to be 0x01 (Online)"),
            TestStep(
                "12b", "TH reads from the DUT the Forecast", "Value has to include StartTime and EndTime unchanged from step 10b"
            ),
            TestStep(
                "13",
                "TH sends command StartTimeAdjustRequest with RequestedStartTime=StartTime+(LatestEndTime-EndTime)+1 from Forecast, Cause=LocalOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep("13a", "TH reads from the DUT the ESAState", "Value has to be 0x01 (Online)"),
            TestStep(
                "13b", "TH reads from the DUT the Forecast", "Value has to include StartTime and EndTime unchanged from step 10b"
            ),
            TestStep("14", "TH sends command CancelRequest", "Verify DUT responds w/ status INVALID_IN_STATE(0xcb)"),
            TestStep(
                "15",
    ...
    async def test_TC_DEM_2_3(self):
        ...
        self.step("6")
        await self.send_start_time_adjust_request_command(
            requestedStartTime=forecast.earliestStartTime,
        ...
        asserts.assert_equal(forecast, forecast2, f"Expected same forcast {forecast} to be == {forecast2}")
        ...
        self.step("10")
        await self.send_start_time_adjust_request_command(
            requestedStartTime=forecast4.startTime + forecast4.latestEndTime - forecast4.endTime,
        ...
        self.step("12")
        await self.send_start_time_adjust_request_command(
            requestedStartTime=forecast6.earliestStartTime - 1,
            cause=Clusters.DeviceEnergyManagement.Enums.AdjustmentCauseEnum.kLocalOptimization,
            expected_status=Status.ConstraintError,
        )
        self.step("12a")
        ...
        asserts.assert_equal(
            forecast6.endTime, forecast7.endTime, f"Expected old endTime {forecast6.endTime} to be == endTime {forecast7.endTime}"
        )

        self.step("13")
        await self.send_start_time_adjust_request_command(
            requestedStartTime=forecast7.startTime + (forecast7.latestEndTime - forecast7.endTime) + 1,
            cause=Clusters.DeviceEnergyManagement.Enums.AdjustmentCauseEnum.kLocalOptimization,
            expected_status=Status.ConstraintError,
        )
        self.step("13a")
        ...
        asserts.assert_equal(
            forecast7.endTime, forecast8.endTime, f"Expected old endTime {forecast7.endTime} to be == endTime {forecast8.endTime}"
        )
