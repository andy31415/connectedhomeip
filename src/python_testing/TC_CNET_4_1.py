...
class TC_CNET_4_1(MatterBaseTest):
    ...
    def steps_TC_CNET_4_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                5,
                "TH reads the Networks attribute list from the DUT on all available endpoints",
            ...
            TestStep(
                8,
                "TH reads the LastNetworkID attribute from the DUT. TH reads the Networks attribute from the DUT",
    ...
    async def test_TC_CNET_4_1(self):
        ...
        max_networks_count = await self.read_single_attribute_check_success(
            cluster=Clusters.NetworkCommissioning, attribute=Clusters.NetworkCommissioning.Attributes.MaxNetworks
        )
        ...
        supported_wifi_bands = await self.read_single_attribute_check_success(
            cluster=Clusters.NetworkCommissioning, attribute=Clusters.NetworkCommissioning.Attributes.SupportedWiFiBands
        )
        log.info(supported_wifi_bands)
        asserts.assert_greater_equal(
            len(supported_wifi_bands),
            1,
            "Verify that SupportedWiFiBands attribute value has 1 or more entries, all of which are in the range of WiFiBandEnum.",
        )
        matter_asserts.assert_list_element_type(
            supported_wifi_bands,
            Clusters.NetworkCommissioning.Enums.WiFiBandEnum,
            "Verify that SupportedWiFiBands attribute value has 1 or more entries, all of which are in the range of WiFiBandEnum.",
        )

        self.step(3)
        scan_max_time_seconds = await self.read_single_attribute_check_success(
            cluster=Clusters.NetworkCommissioning, attribute=Clusters.NetworkCommissioning.Attributes.ScanMaxTimeSeconds
        )
        ...
        matter_asserts.assert_int_in_range(
            connect_max_time_seconds, min_value=1, max_value=255, description="ConnectMaxTimeSeconds"
        )
        ...
        if current_cluster_connected:
            log.info("Current cluster is connected on the endpoint.")
            network = await self.read_single_attribute_check_success(
                cluster=Clusters.NetworkCommissioning, attribute=Clusters.NetworkCommissioning.Attributes.Networks
            )
            ...
            matter_asserts.assert_all(
                network,
                lambda x: isinstance(x.networkID, bytes) and 1 <= len(x.networkID) <= 32,
                "connected field is an instance of bool",
            )
            matter_asserts.assert_all(
                network, lambda x: isinstance(x.connected, bool), "NetworkID field is an octet string within a length range 1 to 32"
            )
        else:
        ...
        interface_enabled = await self.read_single_attribute_check_success(
            cluster=Clusters.NetworkCommissioning, attribute=Clusters.NetworkCommissioning.Attributes.InterfaceEnabled
        )
        ...
        last_networking_status = await self.read_single_attribute_check_success(
            cluster=Clusters.NetworkCommissioning, attribute=Clusters.NetworkCommissioning.Attributes.LastNetworkingStatus
        )
        ...
        last_network_id = await self.read_single_attribute_check_success(
            cluster=Clusters.NetworkCommissioning, attribute=Clusters.NetworkCommissioning.Attributes.LastNetworkID
        )
        ...
        asserts.assert_equal(
            matching_networks_count, 1, "Verify that LastNetworkID attribute matches the NetworkID count of the entries"
        )
        asserts.assert_in(
            last_network_id,
            network_ids_list,
        ...
        asserts.assert_true(
            isinstance(last_network_id, bytes) and 1 <= len(last_network_id) <= 32,
        ...
        last_connect_error_value = await self.read_single_attribute_check_success(
            cluster=Clusters.NetworkCommissioning, attribute=Clusters.NetworkCommissioning.Attributes.LastConnectErrorValue
        )
