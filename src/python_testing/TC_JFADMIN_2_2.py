...
class TC_JFADMIN_2_2(CADMINBaseTest):
    ...
    def steps_TC_JFADMIN_2_2(self) -> list[TestStep]:
        ...
        return [
            TestStep(
                "1",
                "DUT_AJF starts a commissioning process to commission TH_DEV1 on Fabric 1",
                "TH_DEV1 is commissioned by DUT_AJF on Fabric 1",
            ),
            TestStep(
                "2",
                "DUT_AJF starts a commissioning process to commission TH_DEV2 on Fabric 2",
                "TH_DEV2 is commissioned by DUT_AJF on Fabric 2",
            ),
            TestStep(
                "3",
                "Open a Commissioning Window on DUT_AJF using OpenJointCommissioningWindow command",
                "Verify DUT_AJF opens its Commissioning window to allow another commissioning",
            ),
            TestStep(
                "4",
                "DNS-SD records shows DUT_AJF advertising",
                "Verify that the DNS-SD advertisement has JF key with a value between 1 and 15",
            ),
            TestStep(
                "5",
                "TH_AAF2 starts a commissioning process to commission DUT_AJF on Fabric 2 using Joint Commissioning Method",
                "DUT_AJF is successfuly commissioned into Fabric 2 by TH_AAF2 using Joint Commissioning Method",
            ),
            TestStep(
                "6",
                "Read DUT_AJF NOC at FabricIndex equal to the Joint Fabric",
    ...
    async def test_TC_JFADMIN_2_2(self):
        ...
        self.fabric_a_server_app.start(expected_output="Updating services using commissioning mode 1", timeout=30)
        ...
        if self.is_pics_sdk_ci_only:
            ...
            self.fabric_b_admin.start(expected_output="Updating services using commissioning mode 1", timeout=30)
        else:
        ...
        self.fabric_b_ctrl.start(expected_output="CHIP task running", timeout=30)
        ...
        self.fabric_b_server_app.start(expected_output="Updating services using commissioning mode 1", timeout=30)
        ...
        log.info(
            "Waiting for transfer of ownership from the commissioner(controller) to the administrator and completion of commissioning"
        )
