...
class TC_ZONEMGMT_2_2(MatterBaseTest):
    ...
    def steps_TC_ZONEMGMT_2_2(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "3",
                "TH sends CreateTwoDCartesianZone command with valid non-duplicated Zone information _MaxUserDefinedZones_ + 1 times",
                "Verify that the DUT responds with a ResourceExhausted status code at the _MaxUserDefinedZones_ + 1 th command.",
            ),
            TestStep(
                "3a",
                "TH sends RemoveZone command for each ZoneID created in step 3 ",
                "Verify that the DUT responds with a Success status code.",
            ),
            TestStep(
                "4",
                "TH sends CreateTwoDCartesianZone command with valid Zone information and ZoneUse set to Focus",
            ...
            TestStep(
                "5",
                "TH sends CreateTwoDCartesianZone command with any X or Y value of zone vertices set to TwoDCartesianMax + 1",
                "Verify that the DUT responds with a DynamicConstraintError status code.",
            ),
            TestStep(
                "6",
                "TH sends the same CreateTwoDCartesianZone command with valid parameters 2 times",
                "Verify that the first DUT response contains a new zoneId and the second command returns AlreadyExists status code.",
            ),
            TestStep(
                "7",
                "TH sends the CreateTwoDCartesianZone command with the zone vertices forming a self-intersecting polygonal shape",
                "Verify that the DUT responds with a DynamicConstraintError status code.",
            ),
            TestStep(
                "8",
    ...
    async def test_TC_ZONEMGMT_2_2(self):
        ...
        if self.twoDCartSupported and self.userDefinedSupported:
            ...
            zoneToCreate = cluster.Structs.TwoDCartesianZoneStruct(
                name="Zone3", use=enums.ZoneUseEnum.kFocus, vertices=zoneVertices, color="#00FFFF"
            )
            ...
            createTwoDCartesianCmd = commands.CreateTwoDCartesianZone(zone=zoneToCreate1)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=createTwoDCartesianCmd)
            ...
            createTwoDCartesianCmd = commands.CreateTwoDCartesianZone(zone=zoneToCreate2)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=createTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
            ...
            createTwoDCartesianCmd = commands.CreateTwoDCartesianZone(zone=zoneToCreate3)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=createTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
            ...
            createTwoDCartesianCmd = commands.CreateTwoDCartesianZone(zone=zoneToCreate4)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=createTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
            ...
            createTwoDCartesianCmd = commands.CreateTwoDCartesianZone(zone=zoneToCreate5)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=createTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
            ...
            createTwoDCartesianCmd = commands.CreateTwoDCartesianZone(zone=zoneToCreate6)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=createTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
            ...
            createTwoDCartesianCmd = commands.CreateTwoDCartesianZone(zone=zoneToCreate7)
            try:
                await self.send_single_cmd(endpoint=endpoint, cmd=createTwoDCartesianCmd)
                asserts.fail("Unexpected success when expecting DYNAMIC_CONSTRAINT_ERROR due to self intersecting vertices")
            except InteractionModelError as e:
