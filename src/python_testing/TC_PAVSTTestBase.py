...
class PAVSTTestBase:
    ...
    async def allocate_one_pushav_transport(
        self,
        endpoint,
        triggerType=Clusters.PushAvStreamTransport.Enums.TransportTriggerTypeEnum.kContinuous,
        ...
        tlsEndPoint=1,
        expiryTime=DEFAULT_AV_TRANSPORT_EXPIRY_TIME_SEC,
    ):
        endpoint = self.get_endpoint()
        ...
        asserts.assert_equal(
            adoSupport, Clusters.CameraAvStreamManagement.Bitmaps.Feature.kAudio, "Audio Feature is not supported."
        )
        ...
        containerOptions = {
            ...
            "CMAFContainerOptions": {
                "CMAFInterface": cluster.Enums.CMAFInterfaceEnum.kInterface1,
                "chunkDuration": 4,
        ...
        try:
            ...
            if expected_cluster_status is not None:
                asserts.assert_true(e.clusterStatus == expected_cluster_status, "Unexpected error returned")
    ...
    async def check_and_delete_all_push_av_transports(self, endpoint, attribute):
        ...
        for config in transportConfigs:
            if config.connectionID != 0:
                try:
                    await self.send_single_cmd(
                        cmd=pvcluster.Commands.DeallocatePushTransport(connectionID=config.connectionID),
                        endpoint=endpoint,
                    )
                except InteractionModelError as e:
