...
class TC_ACL_2_7(MatterBaseTest):
    ...
    def _validate_events(self, events, expected_fabric_index, expected_node_id, other_fabric_index, controller_name, is_filtered):
        ...
        for event in events:
            ...
            if hasattr(event, "Data") and hasattr(event.Data, "changeType"):
                if expected_node_id == self.th1.nodeId:
                    ...
                    if (
                        event.Data.changeType == Clusters.AccessControl.Enums.ChangeTypeEnum.kAdded
                        and event.Data.adminNodeID == self.th1.nodeId
                        and isinstance(event.Data.adminPasscodeID, Nullable)
                        and event.Data.latestValue.data == D_OK_EMPTY
                        and event.Data.latestValue.fabricIndex == expected_fabric_index
                        and event.Data.fabricIndex == expected_fabric_index
                    ):
                ...
                if expected_node_id == self.th2.nodeId:
                    ...
                    if (
                        event.Data.changeType == Clusters.AccessControl.Enums.ChangeTypeEnum.kAdded
                        and event.Data.adminNodeID == self.th2.nodeId
                        and isinstance(event.Data.adminPasscodeID, Nullable)
                        and event.Data.latestValue.data == D_OK_SINGLE
                        and event.Data.latestValue.fabricIndex == expected_fabric_index
                        and event.Data.fabricIndex == expected_fabric_index
                    ):
                        found_valid_events += 1
        ...
        if is_filtered:
            ...
            asserts.assert_false(
                found_other_event,
                f"{controller_name} should not see any events from {other_controller}'s fabric when fabric filtered",
            )
    ...
    def steps_TC_ACL_2_7(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                3,
                "TH1 puts DUT into commissioning mode, TH2 commissions DUT using admin node ID",
            ...
            TestStep(
                6,
                "TH2 writes DUT Endpoint 0 AccessControl cluster Extension attribute, value is list of AccessControlExtensionStruct containing 1 element",
                "Result is SUCCESS",
            ),
            TestStep(
                7,
                "TH1 reads DUT Endpoint 0 AccessControl cluster Extension attribute with both fabricFiltered True and False",
                "Result is SUCCESS, value is list of AccessControlExtensionStruct containing 1 element when fabric filtered",
            ),
            TestStep(
                8,
                "TH2 reads DUT Endpoint 0 AccessControl cluster Extension attribute with both fabricFiltered True and False",
                "Result is SUCCESS, value is list of AccessControlExtensionStruct containing 1 element when fabric filtered",
            ),
            TestStep(
                9,
                "TH1 reads DUT Endpoint 0 AccessControl cluster AccessControlExtensionChanged event with both fabricFiltered True and False",
                "Result is SUCCESS, value is list of AccessControlExtensionChanged containing 1 element when fabric filtered",
            ),
            TestStep(
                10,
                "TH2 reads DUT Endpoint 0 AccessControl cluster AccessControlExtensionChanged event with both fabricFiltered True and False",
                "Result is SUCCESS, value is list of AccessControlExtensionChanged containing 1 element when fabric filtered",
            ),
            TestStep(
                11,
    ...
    async def test_TC_ACL_2_7(self):
        ...
        f2 = await self.read_single_attribute_check_success(
            dev_ctrl=self.th2, endpoint=0, cluster=oc_cluster, attribute=cfi_attribute
        )
        ...
        result = await self.th1.WriteAttribute(self.dut_node_id, [(0, extension_attr(value=extensions_list))])
        ...
        extension_th2 = Clusters.AccessControl.Structs.AccessControlExtensionStruct(data=D_OK_SINGLE)
        log.info(f"TH2 writing extension with data {D_OK_SINGLE.hex()}")

        extension_attr = Clusters.AccessControl.Attributes.Extension
        extensions_list = [extension_th2]
        result = await self.th2.WriteAttribute(self.dut_node_id, [(0, extension_attr(value=extensions_list))])
        log.info("Write result: %s", str(result))
        asserts.assert_equal(result[0].Status, Status.Success, "Write should have succeeded")
        ...
        asserts.assert_equal(endpoint_data[0].data, D_OK_EMPTY, "Extension data should match D_OK_EMPTY")
        asserts.assert_equal(endpoint_data[0].fabricIndex, f1, "FabricIndex should match F1")

        # Read with fabric_filtered=False
        result_unfiltered = await self.read_single_attribute_check_success(
            dev_ctrl=self.th1, endpoint=0, cluster=ac_cluster, attribute=extension_attr, fabric_filtered=False
        )
        ...
        asserts.assert_equal(endpoint_data_th2[0].data, b"", "Extension data should be empty")
        ...
        asserts.assert_equal(endpoint_data2[0].data, D_OK_SINGLE, "Extension data should match D_OK_SINGLE")
        ...
        result2_unfiltered = await self.read_single_attribute_check_success(
            dev_ctrl=self.th2, endpoint=0, cluster=ac_cluster, attribute=extension_attr, fabric_filtered=False
        )
        ...
        asserts.assert_equal(endpoint_data_th1[0].data, b"", "Extension data should be empty")
        ...
        events_response_filtered = await self.th1.ReadEvent(self.dut_node_id, events=[(0, acec_event)], fabricFiltered=True)
        self._validate_events(events_response_filtered, f1, self.th1.nodeId, f2, "TH1", True)

        # Read with fabricFiltered=False
        events_response_unfiltered = await self.th1.ReadEvent(self.dut_node_id, events=[(0, acec_event)], fabricFiltered=False)
        ...
        # Read with fabricFiltered=True
        events_response2_filtered = await self.th2.ReadEvent(self.dut_node_id, events=[(0, acec_event)], fabricFiltered=True)
        self._validate_events(events_response2_filtered, f2, self.th2.nodeId, f1, "TH2", True)

        # Read with fabricFiltered=False
        events_response2_unfiltered = await self.th2.ReadEvent(self.dut_node_id, events=[(0, acec_event)], fabricFiltered=False)
        ...
        asserts.assert_equal(result.statusCode, 0, "RemoveFabric command should have succeeded")
