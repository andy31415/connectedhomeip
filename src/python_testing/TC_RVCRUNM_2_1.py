...
class TC_RVCRUNM_2_1(MatterBaseTest):
    ...
    async def test_TC_RVCRUNM_2_1(self):
        asserts.assert_true(
            "PIXIT.RVCRUNM.MODE_CHANGE_OK" in self.matter_test_config.global_test_params,
        ...
        asserts.assert_true(
            "PIXIT.RVCRUNM.MODE_CHANGE_FAIL" in self.matter_test_config.global_test_params,
        ...
        if can_test_mode_failure and can_manually_control:
            ...
            if self.is_ci:
                ...
                self.wait_for_user_input(
                    prompt_msg="Manually put the device in a state from which it will FAIL to transition to mode %d, and press Enter when ready."
                    % (self.mode_fail)
                )
        ...
        if self.is_ci:
            ...
            self.wait_for_user_input(
                prompt_msg="Manually put the device in a state from which it will SUCCESSFULLY transition to mode %d, and press Enter when ready."
                % (self.mode_ok)
            )
