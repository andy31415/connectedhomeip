...
class TC_JFADMIN_2_1(MatterBaseTest):
    ...
    def steps_TC_JFADMIN_2_1(self) -> list[TestStep]:
        return [
            TestStep(
                "1",
                "DUT_AAF starts a commissioning process to commission TH_DEV1 on Fabric 1",
                "TH_DEV1 is commissioned by DUT_AAF on Fabric 1",
            ),
            TestStep(
                "2",
                "TH_AJF2 starts a commissioning process to commission TH_DEV2 on Fabric 2",
                "TH_DEV2 is commissioned by TH_AJF2 on Fabric 2",
            ),
            TestStep(
                "3",
                "Open a Commissioning Window on TH_AJF2 using OpenJointCommissioningWindow command",
                "Verify TH_AJF2 opens its Commissioning window to allow another commissioning",
            ),
            TestStep(
                "4",
                "DUT_AAF starts a commissioning process to commission TH_AJF2 on Fabric 1 using Joint Commissioning Method",
    ...
    async def test_TC_JFADMIN_2_1(self):
        ...
        self.fabric_a_server_app.start(expected_output="Server initialization complete", timeout=20)
        ...
        response = await _devCtrlEcoA.ReadAttribute(
            nodeId=1, attributes=[(0, Clusters.OperationalCredentials.Attributes.NOCs)], returnClusterObject=True
        )
        ...
        response = await _devCtrlEcoA.ReadAttribute(
            nodeId=2, attributes=[(0, Clusters.OperationalCredentials.Attributes.Fabrics)], returnClusterObject=True
        )
        ...
        response = await _devCtrlEcoA.ReadAttribute(
            nodeId=2, attributes=[(0, Clusters.AccessControl.Attributes.Acl)], returnClusterObject=True
        )
        asserts.assert_equal(
        ...
        self.fabric_b_admin.start(expected_output="Server initialization complete", timeout=20)
        ...
        self.fabric_b_ctrl.start(expected_output="CHIP task running", timeout=20)
        ...
        self.ecoBCtrlStorage = {
            ...
            "repl-config": {"caList": {"1": [{"fabricId": 1, "vendorId": self.jfctrl_fabric_b_vid}]}},
        }
        # Extract CATs to be provided to the Python Controller later
        ...
        self.fabric_b_server_app.start(expected_output="Server initialization complete", timeout=20)
        ...
        _certAuthorityManagerB = CertificateAuthority.CertificateAuthorityManager(
            chipStack=self.matter_stack._chip_stack, persistentStorage=_fabric_b_persistent_storage
        )
        _certAuthorityManagerB.LoadAuthoritiesFromStorage()
        _devCtrlEcoB = (
            _certAuthorityManagerB.activeCaList[0]
            .adminList[0]
            .NewController(
                nodeId=201, paaTrustStorePath=str(self.matter_test_config.paa_trust_store_path), catTags=[int(self.ecoBCATs, 16)]
            )
        )

        response = await _devCtrlEcoB.ReadAttribute(
            nodeId=11, attributes=[(0, Clusters.OperationalCredentials.Attributes.NOCs)], returnClusterObject=True
        )
        # Search Administrator CAT (FFFF0001) and Anchor CAT (FFFD0001) in JF-Admin NOC on Ecoystem A
        noc_tlv_data = matter.tlv.TLVReader(response[0][Clusters.OperationalCredentials].NOCs[0].noc).get()
        _admin_cat_found = False
        ...
        response = await _devCtrlEcoB.ReadAttribute(
            nodeId=22, attributes=[(0, Clusters.OperationalCredentials.Attributes.Fabrics)], returnClusterObject=True
        )
        asserts.assert_equal(
        ...
        response = await _devCtrlEcoB.ReadAttribute(
            nodeId=22, attributes=[(0, Clusters.AccessControl.Attributes.Acl)], returnClusterObject=True
        )
        asserts.assert_equal(
        ...
        response = await _devCtrlEcoB.ReadAttribute(
            nodeId=11, attributes=[(0, Clusters.OperationalCredentials.Attributes.Fabrics)], returnClusterObject=True
        )
        asserts.assert_equal(
            self.jfctrl_fabric_b_vid,
            response[0][Clusters.OperationalCredentials].fabrics[0].vendorID,
        ...
        try:
            response = await _devCtrlEcoB.OpenJointCommissioningWindow(
                nodeId=11, endpointId=1, timeout=400, iteration=random.randint(1000, 100000), discriminator=discriminator
            )
        ...
        rcac_data = matter.tlv.TLVReader(
            fabric_a_trusted_roots[0][Clusters.OperationalCredentials].trustedRootCertificates[1]
        ).get()
        icac_data = matter.tlv.TLVReader(fabric_b_nocs[0][Clusters.OperationalCredentials].NOCs[2].icac).get()

        log.info(
            "Verify the chain of trust between Ecosystem A and Ecosystem B. Issuer of ICAC for Ecosystem B should be the RCAC from Ecosystem A"
        )
        asserts.assert_equal(
