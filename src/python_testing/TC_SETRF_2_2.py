...
class TC_SETRF_2_2(CommodityTariffTestBaseHelper):
    ...
    def steps_TC_SETRF_2_2(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "6",
                "TH sends command GetTariffComponent command with TariffComponentID set to tariffComponentID1.",
                """
            ...
            TestStep(
                "7",
                "TH sends command GetTariffComponent command with TariffComponentID set to an uint32 value not equal any from tariffComponentIDs.",
            ...
            TestStep(
                "9",
                "TH sends command GetDayEntry command with DayEntryID set to dayEntryID1.",
                """
            ...
            TestStep(
                "10",
                "TH sends command GetDayEntry command with DayEntryID set to an uint32 value not equal any from dayEntryIDs.",
                "Verify that the DUT response contains status NOT_FOUND(0x8b).",
            ),
            TestStep(
                "11",
    ...
    async def test_TC_SETRF_2_2(self):
        ...
        try:
            ...
            result: cluster.Commands.GetTariffComponentResponse = await self.send_single_cmd(
                cmd=command, endpoint=endpoint, timedRequestTimeoutMs=3000
            )
        except InteractionModelError as err:
            asserts.fail(f"Unexpected error returned: {err.status}")

        asserts.assert_true(
            isinstance(result, cluster.Commands.GetTariffComponentResponse), "Command must be of type GetTariffComponentResponse."
        )
        ...
        asserts.assert_is_instance(
            result.tariffComponent, cluster.Structs.TariffComponentStruct, "TariffComponent must be a TariffComponentStruct."
        )
        await self.checkTariffComponentStruct(endpoint=endpoint, cluster=cluster, struct=result.tariffComponent)
        asserts.assert_equal(
            result.tariffComponent,
            tariffComponentStruct1,
        ...
        try:
            command = Clusters.CommodityTariff.Commands.GetTariffComponent(tariffComponentID=absentTariffComponentID)
            result: cluster.Commands.GetTariffComponentResponse = await self.send_single_cmd(
                cmd=command, endpoint=endpoint, timedRequestTimeoutMs=3000
            )
            ...
            asserts.assert_equal(err.status, Status.NotFound, "Unexpected error returned")
        ...
        try:
            ...
            result: cluster.Commands.GetDayEntryResponse = await self.send_single_cmd(
                cmd=command, endpoint=endpoint, timedRequestTimeoutMs=3000
            )
        except InteractionModelError as err:
            asserts.fail(f"Unexpected error returned: {err.status}")
        ...
        asserts.assert_true(
            isinstance(result, cluster.Commands.GetDayEntryResponse), "Command must be of type GetDayEntryResponse."
        )
        asserts.assert_is_instance(result.dayEntry, cluster.Structs.DayEntryStruct, "DayEntry must be a DayEntryStruct.")
        await self.checkDayEntryStruct(endpoint=endpoint, cluster=cluster, struct=result.dayEntry)
        asserts.assert_equal(
            result.dayEntry, dayEntryStruct1, "DayEntry field must be equal to the first DayEntryStruct in DayEntries attribute."
        )
