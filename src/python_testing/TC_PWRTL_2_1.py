...
class TC_PWRTL_2_1(MatterBaseTest):
    ...
    async def test_TC_PWRTL_2_1(self):
        ...
        attribute_list = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=powertop_cluster, attribute=powertop_attr_list
        )
        ...
        if avail_endpoints_attr_id in attribute_list:
            available_endpoints = await self.read_single_attribute_check_success(
                endpoint=endpoint, cluster=Clusters.Objects.PowerTopology, attribute=attributes.AvailableEndpoints
            )
            ...
            if available_endpoints == []:
                ...
                asserts.assert_less_equal(
                    len(available_endpoints), 20, "AvailableEndpoints length %d must be less than 21!" % len(available_endpoints)
                )
        ...
        if act_endpoints_attr_id in attribute_list:
            active_endpoints = await self.read_single_attribute_check_success(
                endpoint=endpoint, cluster=Clusters.Objects.PowerTopology, attribute=attributes.ActiveEndpoints
            )
            log.info("ActiveEndpoints: %s" % (active_endpoints))
            asserts.assert_less_equal(
                len(active_endpoints), 20, "ActiveEndpoints length %d must be less than 21!" % len(active_endpoints)
            )

            if available_endpoints == []:
                # Verify that ActiveEndpoints is a subset of AvailableEndpoints
                asserts.assert_true(
                    set(active_endpoints).issubset(set(available_endpoints)),
