...
class TC_SwitchTests(MatterBaseTest):
    ...
    def _send_multi_press_named_pipe_command(
        self, endpoint_id: int, number_of_presses: int, pressed_position: int, feature_map: uint, multi_press_max: uint
    ):
        command_dict = {
            "Name": "SimulateMultiPress",
            "EndpointId": endpoint_id,
            "ButtonId": pressed_position,
            "MultiPressPressedTimeMillis": 500,
            "MultiPressReleasedTimeMillis": 500,
            "MultiPressNumPresses": number_of_presses,
    ...
    def _send_long_press_named_pipe_command(self, endpoint_id: int, pressed_position: int, feature_map: int):
        command_dict = {
            ...
            "EndpointId": endpoint_id,
            "ButtonId": pressed_position,
            "LongPressDelayMillis": int(1000 * (SIMULATED_LONG_PRESS_LENGTH_SECONDS - 0.5)),
            "LongPressDurationMillis": int(1000 * SIMULATED_LONG_PRESS_LENGTH_SECONDS),
            "FeatureMap": feature_map,
        }
        self.write_to_app_pipe(command_dict)
    ...
    def _ask_for_multi_press_short_long(self, endpoint_id: int, pressed_position: int, feature_map: uint, multi_press_max: uint):
        if not self._use_button_simulator():
            ...
            self._send_multi_press_named_pipe_command(
                endpoint_id,
                number_of_presses=2,
                pressed_position=pressed_position,
    ...
    def _ask_for_multi_press(
        ...
        if not self._use_button_simulator():
            ...
            self._send_multi_press_named_pipe_command(
                endpoint_id, number_of_presses, pressed_position, feature_map, multi_press_max
            )
    ...
    async def _ask_for_release(self):
        ...
        if not self._use_button_simulator():
            self.wait_for_user_input(prompt_msg="Release the button.")
        else:
    ...
    def _await_sequence_of_events(
        self, event_queue: queue.Queue, endpoint_id: int, sequence: list[ClusterObjects.ClusterEvent], timeout_sec: float
    ):
    ...
    def _expect_no_events_for_cluster(
        self, event_queue: queue.Queue, endpoint_id: int, expected_cluster: ClusterObjects.Cluster, timeout_sec: float
    ):
        start_time = time.time()
        elapsed = 0.0
        time_remaining = timeout_sec

        log.info(f"Waiting {timeout_sec:.1f} seconds for no more events for cluster {expected_cluster} on endpoint {endpoint_id}")
        while time_remaining > 0:
            try:
                item: EventReadResult = event_queue.get(block=True, timeout=time_remaining)
                event_data = item.Data
                ...
                if (
                    item.Header.EndpointId == endpoint_id
                    and item.Header.ClusterId == event_data.cluster_id
                    and item.Header.ClusterId == expected_cluster.id
                ):
    ...
    def _received_event(
        self, event_listener: EventSubscriptionHandler, target_event: ClusterObjects.ClusterEvent, timeout_s: int
    ) -> bool:
        """
    ...
    def steps_TC_SWTCH_2_2(self):
        return [
            ...
            TestStep(
                9, "Wait 10 seconds for event reports stable.Verify that last SwitchLatched event received is for NewPosition 0."
            ),
            TestStep(
                10,
                "TH reads the CurrentPosition attribute from the DUT",
    ...
    async def test_TC_SWTCH_2_2(self):
        ...
        num_positions = await self.read_single_attribute_check_success(
            cluster=cluster, attribute=cluster.Attributes.NumberOfPositions
        )
        ...
        for expected_switch_position in range(1, num_positions):
            ...
            asserts.assert_equal(
                data, cluster.Events.SwitchLatched(newPosition=expected_switch_position), "Did not get expected switch position"
            )
            ...
            if expected_switch_position == 1:  # Indicate step 7 only once
                self.step(6)
            button_val = await self.read_single_attribute_check_success(
                cluster=cluster, attribute=cluster.Attributes.CurrentPosition
            )
            ...
            attrib_listener.await_sequence_of_reports(
                attribute=cluster.Attributes.CurrentPosition,
                sequence=[expected_switch_position],
        ...
        asserts.assert_equal(
            last_event_data,
            cluster.Events.SwitchLatched(newPosition=expected_switch_position),
            "Did not get expected switch position",
        )
        ...
        expected_final_value = [
            AttributeValue(endpoint_id, attribute=cluster.Attributes.CurrentPosition, value=expected_switch_position)
        ]
    ...
    def steps_TC_SWTCH_2_3(self):
        return [
            ...
            TestStep(
                5,
                "Operator operates switch (keep it pressed, and wait at least 5 seconds)",
            ...
            TestStep(
                "8a",
                "If the DUT implements the MSR feature and does not implement the MSL feature, verify that the TH receives ShortRelease event with NewPosition set to 0 on the DUT",
                "Event received",
            ),
            TestStep(
                "8b",
                "If the DUT implements the MSR feature and the MSL feature, verify that the TH receives LongRelease event with NewPosition set to 0 on the DUT",
                "Event received",
            ),
            TestStep(
                "8c",
                "If the DUT implements the AS feature, verify that the TH does not receive ShortRelease event on the DUT",
                "No event received",
            ),
    ...
    async def test_TC_SWTCH_2_3(self):
        ...
        if has_msr_feature and not has_msl_feature:
            asserts.assert_true(
                self._received_event(event_listener, cluster.Events.ShortRelease, 10), "Did not receive short release"
            )
        else:
        ...
        if has_msr_feature and has_msl_feature:
            asserts.assert_true(
                self._received_event(event_listener, cluster.Events.LongRelease, 10), "Did not receive long release"
            )
    ...
    def steps_TC_SWTCH_2_4(self):
        return [
            ...
            TestStep(
                5,
                "Operator operates switch (keep pressed for long time, e.g. 5 seconds) on the DUT, then release it",
                """
    ...
    async def test_TC_SWTCH_2_4(self):
        ...
        attrib_listener.await_sequence_of_reports(
            attribute=cluster.Attributes.CurrentPosition,
            sequence=[switch_pressed_position, 0],
            timeout_sec=post_prompt_settle_delay_seconds,
        )
        ...
        self._await_sequence_of_events(
        ...
        if not has_msl_feature and not has_as_feature and not has_msr_feature and not has_msm_feature:
            self._expect_no_events_for_cluster(
                event_queue=event_listener.event_queue, endpoint_id=endpoint_id, expected_cluster=cluster, timeout_sec=10.0
            )
    ...
    def steps_TC_SWTCH_2_5(self):
        return [
            ...
            TestStep(
                "4a",
                "Operator operates switch (press briefly) associated with position 1 on the DUT then release switch from DUT",
                """
            ...
            TestStep(
                "5a",
                "Operator repeat step 4a 2 times quickly",
                """
            ...
            TestStep(
                "6b",
                "Operator repeat step 4a 3 times quickly",
                """
                         * Verify that the TH receives InitialPress event with NewPosition set to 1 from the DUT
            ...
            TestStep(
                "6c",
                "Operator does not operate switch on the DUT for 5 seconds",
    ...
    async def test_TC_SWTCH_2_5(self):
        ...
        def test_multi_press_sequence(starting_step: str, count: int, short_long: bool = False):
            ...
            if short_long:
                self._ask_for_multi_press_short_long(
                    endpoint_id, pressed_position, feature_map=feature_map, multi_press_max=multi_press_max
                )
            else:
                self._ask_for_multi_press(
                    endpoint_id,
                    number_of_presses=count,
                    pressed_position=pressed_position,
                    feature_map=feature_map,
                    multi_press_max=multi_press_max,
                )
            for pos_idx in range(count):
                ...
                if pos_idx > 0:
                    ...
                    asserts.assert_equal(
                        event.currentNumberOfPressesCounted,
                        pos_idx + 1,
        ...
        self.step("6a")
        multi_press_max = await self.read_single_attribute_check_success(
            cluster=cluster, attribute=cluster.Attributes.MultiPressMax
        )
    ...
    def steps_TC_SWTCH_2_6(self):
        return [
            ...
            TestStep(
                "6a",
                "Operator repeat step 4a MultiPressMax + 1(see 2c of TC-SWTCH-2.1) times quickly",
                """
            ...
            TestStep(
                "7a",
                "If the switch cluster does not implement the MomentarySwitchLongPress (MSL) feature, skip the remaining steps",
            ),
