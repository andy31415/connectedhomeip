...
class TC_CGEN_2_2(MatterBaseTest):
    ...
    async def set_failsafe_timer(self, dev_ctrl, node_id, expiration_time_seconds):
        ...
        In CI/Cert environments, this function helps bypass long waits by resetting the failsafe timer to failsafe_expiration_seconds,
        ...
        Args:
            ...
            failsafe_expiration_seconds: Configure the failsafe timer using PIXIT.CGEN.FailsafeExpiryLengthSeconds.
        ...
        log.info(f"Failsafe timer expired after: {elapsed_time:.2f} seconds.")
    ...
    async def run_steps_3_to_5(self, failsafe_duration: int, is_first_run: bool):
        ...
        asserts.assert_true(
            debug_text == "" or len(debug_text) <= 512, "debugText must be empty or have a maximum length of 512 characters."
        )
        if is_first_run:
        ...
        breadcrumb_info = await self.read_single_attribute_check_success(
            cluster=self.cluster_cgen, attribute=self.cluster_cgen.Attributes.Breadcrumb
        )
        asserts.assert_equal(breadcrumb_info, 1, "The Breadcrumb attribute is not 1")
        if is_first_run:
        ...
        if is_first_run:
            log.info("Step #5: Completed - CSR generated, certificates issued, and new trusted root certificate added.")
        else:
            log.info(
                "Step #10 - Repeated Step #5: Completed - CSR generated, certificates issued, and new trusted root certificate added."
            )
    ...
    async def run_steps_8_to_9(self, trusted_root_list_original_size: int, is_first_run: bool):
        ...
        asserts.assert_equal(
            trusted_root_list_original_size_after_wait,
        ...
        if is_first_run:
            ...
            log.info(f"Step #12 - Repeated Step #9: After waiting for failsafe timeout the Breadcrumb attribute: {breadcrumb_info}")
    ...
    def steps_TC_CGEN_2_2(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                2,
                """TH1 reads the BasicCommissioningInfo attribute
                     and saves the MaxCumulativeFailsafeSeconds as maxFailsafe.""",
            ),
            TestStep(
                "2a",
                """The failsafe timer is set based on PIXIT.CGEN.FailsafeExpiryLengthSeconds if provided;
            ...
            TestStep(
                3,
                """TH1 sends ArmFailSafe command to the DUT with ExpiryLengthSeconds field set to PIXIT.CGEN.FailsafeExpiryLengthSeconds
                     and the Breadcrumb value as 1.""",
            ),
            TestStep(4, "TH1 reads the Breadcrumb attribute."),
            TestStep(
                5,
                """TH1 generates a new TrustedRootCertificate that is different from the previously commissioned TrustedRootCertificate for TH1.
            ...
            TestStep(
                7,
                """TH1 sends an ArmFailSafe command to the DUT with ExpiryLengthSeconds field set to 1 and the Breadcrumb value as 2.
            ...
            TestStep(
                16,
                """TH2 obtains or generates a TrustedRootCertificate that is different from the previously commissioned TrustedRootCertificate for TH1.
            ...
            TestStep(
                21,
                """TH2 starts commissioning the DUT. It performs all steps up to establishing a CASE connection,
            ...
            TestStep(
                23, "TH1 reads the NOCs attribute from the Node Operational Credentials cluster using a non-fabric-filtered read."
            ),
            TestStep(
                24,
                "TH1 reads the Fabrics attribute from the Node Operational Credentials cluster using a non-fabric-filtered read.",
            ...
            TestStep(
                30,
                """TH1 obtains or generates a new TrustedRootCertificate that is different from the previously commissioned TrustedRootCertificate for TH1.
                     TH1 sends an AddTrustedRootCertificate command to the Node Operational Credentials cluster to install this new certificate.""",
            ),
            ...
            TestStep(
                35,
                """TH2 sends ArmFailSafe command to the DUT with ExpiryLengthSeconds field set to PIXIT.CGEN.FailsafeExpiryLengthSeconds
                     and the Breadcrumb value as 1.""",
            ),
            ...
            TestStep(
                43,
                """TH1 waits until the current wall time clock is Tstart + maxFailsafe. maxFailsafe is the maximum amount of time a failsafe can be armed for,
    ...
    async def test_TC_CGEN_2_2(self):
        ...
        basic_commissioning_info = await self.read_single_attribute_check_success(
            cluster=self.cluster_cgen, attribute=self.cluster_cgen.Attributes.BasicCommissioningInfo
        )
        ...
        # Default value for failsafe_expiration_seconds is 20 seconds if not specified.

        failsafe_expiration_seconds = self.user_params.get("PIXIT.CGEN.FailsafeExpiryLengthSeconds", self.default_failsafe)
        ...
        asserts.assert_in(
            new_root_cert, trusted_root_list_original_updated, "New root certificate was not added to the trusted root list."
        )
        ...
        resp = await self.set_failsafe_timer(dev_ctrl=self.default_controller, node_id=self.dut_node_id, expiration_time_seconds=1)
        ...
        asserts.assert_equal(
            resp.errorCode,
            Clusters.GeneralCommissioning.Enums.CommissioningErrorEnum.kBusyWithOtherAdmin,
            "Failure status returned from arm failsafe",
        )
        ...
        log.info("Step #15 - TH2 successfully establish PASE session completed")
        ...
        log.info("Step #21 - CommissioningComplete skipped to avoid premature completion.")

        self.step(22)
        log.info("Step #22 - TH1 Waiting for PASE session to stabilize...")
        resp = await TH2.FindOrEstablishPASESession(setupCode=setup_qr_code, nodeId=newNodeId)
        log.info("Step #22 - TH2 successfully establish PASE session completed")
        ...
        try:
            ...
            log.info("Step #22 - ArmFailSafe did not fail, device response before closing is acceptable.")
        ...
        asserts.assert_equal(nocs_updated_size, nocs_original_size, "The nocs list size should match the original nocs list size.")
        ...
        asserts.assert_equal(
            fabrics_updated_size, fabrics_original_size, "The fabrics list size should match the original fabrics list size."
        )
        ...
        try:
            ...
            asserts.assert_in("FailsafeRequired (0xca)", str(e), f"Expected FailsafeRequired error, but got {str(e)}")
        ...
        asserts.assert_equal(
            fabrics_updated_size,
            fabrics_original_size + 1,
        ...
        trusted_root_list_original_size_updated = len(trusted_root_list_original_updated)
        log.info(f"Step #40: The updated size of the num_trusted_roots_original list is {trusted_root_list_original_size_updated}")
        # Verify that the trusted root list size is numTrustedRootsOriginal + 2
        asserts.assert_equal(
            trusted_root_list_original_size_updated,
            trusted_root_list_original_size + 2,
        ...
        if self.is_pics_sdk_ci_only:
            ...
            resp = await self.set_failsafe_timer(
                dev_ctrl=self.default_controller, node_id=self.dut_node_id, expiration_time_seconds=failsafe_expiration_seconds
            )
            log.info(
        ...
        fabric_idx = await self.read_single_attribute_check_success(
            ...
            node_id=newNodeId + 1,
            cluster=self.cluster_opcreds,
            attribute=self.cluster_opcreds.Attributes.CurrentFabricIndex,
        )
        ...
        trusted_root_list_original_size_updated = len(trusted_root_list_original_updated)
        log.info(f"Step #44: he updated size of the num_trusted_roots_original list: {trusted_root_list_original_size_updated}")
