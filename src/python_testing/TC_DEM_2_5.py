...
class TC_DEM_2_5(MatterBaseTest, DEMTestBase):
    ...
    def steps_TC_DEM_2_5(self) -> list[TestStep]:
        ...
        return [
            ...
            TestStep(
                "4",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.DEM.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.DEM.TEST_EVENT_TRIGGER for Forecast Adjustment Test Event",
            ...
            TestStep(
                "5",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep(
                "6",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID+1, SlotAdjustments[0].{SlotIndex=0, NominalPower=Forecast.Slots[0].MinPowerAdjustment, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status FAILURE(0x01)",
            ),
            TestStep(
                "7",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=len(Forecast.Slots), NominalPower=Forecast.Slots[0].MinPowerAdjustment, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status FAILURE(0x01)",
            ),
            TestStep(
                "8",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, NominalPower=Forecast.Slots[0].MinPowerAdjustment-1, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep(
                "9",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, NominalPower=Forecast.Slots[0].MaxPowerAdjustment+1, Duration=Forecast.Slots[0].MinDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep(
                "10",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, NominalPower=Forecast.Slots[0].MinPowerAdjustment, Duration=Forecast.Slots[0].MaxDurationAdjustment+1}, Cause=GridOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep(
                "11",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, NominalPower=Forecast.Slots[0].MaxPowerAdjustment, Duration=Forecast.Slots[0].MinDurationAdjustment-1}, Cause=GridOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep(
                "12",
            ...
            TestStep("13a", "TH reads from the DUT the ESAState", "Value has to be 0x01 (Online)"),
            TestStep("13b", "TH reads from the DUT the OptOutState", "Value has to be 0x02 (LocalOptOut)"),
            TestStep(
                "14",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, NominalPower=Forecast.Slots[0].MinPowerAdjustment, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=LocalOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep(
                "15",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, NominalPower=Forecast.Slots[0].MinPowerAdjustment, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep("15a", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=GridOptimization"),
            TestStep("16", "TH sends command CancelRequest", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep("16a", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=InternalOptimization"),
            TestStep(
                "17",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, NominalPower=Forecast.Slots[0].MinPowerAdjustment, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep("17a", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=GridOptimization"),
            TestStep(
                "18",
            ...
            TestStep("18a", "TH reads from the DUT the OptOutState", "Value has to be 0x03 (OptOut)"),
            TestStep(
                "18b", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=Internal Optimization"
            ),
            TestStep(
                "19",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, NominalPower=Forecast.Slots[0].MinPowerAdjustment, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
            ...
            TestStep("20a", "TH reads from the DUT the OptOutState", "Value has to be 0x00 (NoOptOut)"),
            TestStep(
                "21",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, NominalPower=Forecast.Slots[0].MaxPowerAdjustment, Duration=Forecast.Slots[0].MinDurationAdjustment}, Cause=LocalOptimization",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep("21a", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=LocalOptimization"),
            TestStep("22", "TH sends command CancelRequest", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep("22a", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=InternalOptimization"),
            TestStep(
                "23",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.DEM.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.DEM.TEST_EVENT_TRIGGER for Forecast Adjustment Test Event Next Slot",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep(
                "24",
            ...
            TestStep("25", "TH sends command CancelRequest", "Verify DUT responds w/ status INVALID_IN_STATE(0xcb)"),
            TestStep(
                "26",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.DEM.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.DEM.TEST_EVENT_TRIGGER for Forecast Adjustment Test Event Clear",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
        ]
