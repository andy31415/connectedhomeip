...
class TC_CC_6_5(MatterBaseTest):
    ...
    def steps_TC_CC_6_5(self) -> list[TestStep]:
        return [
            ...
            TestStep("0b", "TH sends On command to DUT", "Verify DUT responds with a successful (value 0x00) status response."),
            TestStep(
                "0c",
                "TH reads ColorTemperatureMireds attribute from DUT.",
                "Verify that the DUT response contains the ColorTemperatureMireds attribute value.",
            ),
            TestStep(
                "0d",
                "TH reads ColorTempPhysicalMinMireds attribute from DUT.",
                "Verify that the DUT response contains the ColorTempPhysicalMinMireds attribute value.",
            ),
            TestStep(
                "0e",
                "TH reads ColorTempPhysicalMaxMireds attribute from DUT.",
            ...
            TestStep(
                "5a",
                "TH reads ColorMode attribute from DUT.",
            ...
            TestStep(
                "5b",
                "TH reads EnhancedColorMode attribute from DUT.",
    ...
    async def test_TC_CC_6_5(self):
        ...
        await self.send_single_cmd(endpoint=self.endpoint, cmd=Clusters.OnOff.Commands.On(), node_id=self.dut_node_id)
        log.info("Lights On!")
        ...
        asserts.assert_true(
            matter_asserts.is_valid_uint_value(startup_color_temp_mireds, 16) or startup_color_temp_mireds is NullValue,
        ...
        self.step("2a")
        if (
            (colortemp_physical_max_mireds - colortemp_physical_min_mireds) // 2 + colortemp_physical_min_mireds
        ) == color_temp_mireds:
            startup_color_temperature_mireds2a = int(
                (colortemp_physical_max_mireds - colortemp_physical_min_mireds) // 4 + colortemp_physical_min_mireds
            )
        else:
            startup_color_temperature_mireds2a = int(
                (colortemp_physical_max_mireds - colortemp_physical_min_mireds) // 2 + colortemp_physical_min_mireds
        ...
        await self.write_single_attribute(
            cc_attributes.StartUpColorTemperatureMireds(startup_color_temperature_mireds2a), self.endpoint, expect_success=True
        )
        ...
        asserts.assert_equal(
            startup_color_temp_mireds, startup_color_temperature_mireds2a, "Startup color temperature should match target value"
        )
        ...
        asserts.assert_equal(
            startup_color_temp_mireds,
            startup_color_temperature_mireds2a,
        ...
        asserts.assert_true(isinstance(color_temperature_mireds, int), "Color temperature value should be an integer")
        asserts.assert_greater_equal(
        ...
        asserts.assert_equal(
            color_temperature_mireds, startup_color_temp_mireds, "Post cycle color temperature should match target value"
        )

        self.step("5a")
        color_mode = await self.read_single_attribute_check_success(
            cc_cluster, cc_attributes.ColorMode, dev_ctrl=self.TH1, endpoint=self.endpoint
        )
        ...
        self.step("5b")
        enhanced_color_mode = await self.read_single_attribute_check_success(
            cc_cluster, cc_attributes.EnhancedColorMode, dev_ctrl=self.TH1, endpoint=self.endpoint
        )
