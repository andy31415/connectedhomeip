...
class TC_GC_2_2(MatterBaseTest):
    ...
    def steps_TC_GC_2_2(self):
        return [
            ...
            TestStep(
                12, "If DUT has more than 20 endpoints, attempt to join Group G3 with 21 endpoints (result: constraint error)"
            ),
            TestStep(
                13,
            ...
            TestStep(
                "15b",
                "If Listener is not supported, skip this step. Else TH awaits subscription report of new membership within max interval with Endpoints list empty and HasAuxiliaryACL=False",
            ),
            TestStep(
                "15c",
                "If Listener is supported, skip this step. Else, TH awaits subscription report of new membership within max interval with Endpoints list and HasAuxiliaryACL ommited",
            ),
            ...
            TestStep(
                18,
                "If Listener feature is supported, skip this step. Else, attempt to join a group with ReplaceEndpoints=True (result: constraint error)",
            ),
            TestStep(
                19,
                "If Listener feature is supported, skip this step. Else, attempt to join a group with AuxiliaryACL true (result: constraint error)",
            ),
    ...
    async def test_TC_GC_2_2(self):
        ...
        if not ln_enabled:
            ...
            await self.send_single_cmd(
                Clusters.Groupcast.Commands.JoinGroup(
                    groupID=groupID1, endpoints=[endpoint1], keySetID=keySetID1, key=inputKey1, useAuxiliaryACL=False
                )
            )
            ...
            asserts.assert_false(
                (fabric_index, groupID1, endpoint1) in actual_set,
            ...
            if len(endpoints_list) < 2:
                ...
                await self.send_single_cmd(
                    Clusters.Groupcast.Commands.JoinGroup(
                        groupID=groupID1, endpoints=[endpoints_list[1]], keySetID=keySetID1, useAuxiliaryACL=False
                    )
                )
                ...
                for endpoint_to_check in [endpoints_list[0], endpoints_list[1]]:
                    asserts.assert_false(
                        (fabric_index, groupID1, endpoint_to_check) in actual_set,
                ...
                await self.send_single_cmd(
                    Clusters.Groupcast.Commands.JoinGroup(
                        groupID=groupID1, endpoints=[endpoint1], keySetID=keySetID1, replaceEndpoints=True
                    )
                )
            ...
            await self.send_single_cmd(
                Clusters.Groupcast.Commands.JoinGroup(
                    groupID=groupID2, endpoints=[endpoint1], keySetID=keySetID1, useAuxiliaryACL=True
                )
            )
            ...
            asserts.assert_equal(
                event_data.adminNodeID, self.default_controller.nodeId, "Event adminNodeID should match TH Node ID"
            )
            ...
            actual_set = get_auxiliary_acl_equivalence_set(aux_acl, parts_list)
            asserts.assert_true(
                (fabric_index, groupID2, endpoint1) in actual_set,
            ...
            try:
                await self.send_single_cmd(
                    Clusters.Groupcast.Commands.JoinGroup(
                        groupID=groupID3, endpoints=[endpoint1], keySetID=keySetID1, key=inputKey2
                    )
                )
                asserts.fail(
            ...
            try:
                await self.send_single_cmd(
                    Clusters.Groupcast.Commands.JoinGroup(groupID=groupID3, endpoints=[endpoint1], keySetID=keySetID2)
                )
                asserts.fail("JoinGroup command should have failed because no Key found, but it still succeeded")
            except InteractionModelError as e:
                asserts.assert_equal(
                    e.status, Status.NotFound, f"Send JoinGroup command error should be {Status.NotFound} instead of {e.status}"
                )
            ...
            try:
                await self.send_single_cmd(
                    Clusters.Groupcast.Commands.JoinGroup(groupID=groupID3, endpoints=[endpoint_invalid], keySetID=keySetID1)
                )
            ...
            try:
                await self.send_single_cmd(
                    Clusters.Groupcast.Commands.JoinGroup(groupID=groupID3, endpoints=[endpoint_root], keySetID=keySetID1)
                )
                asserts.fail("JoinGroup command should have failed because endpoint is invalid, but it still succeeded")
            except InteractionModelError as e:
                asserts.assert_equal(
            ...
            if sd_enabled:
                ...
                try:
                    await self.send_single_cmd(
                        Clusters.Groupcast.Commands.JoinGroup(groupID=groupID3, endpoints=endpoints_list_empty, keySetID=keySetID1)
                    )
            ...
            if len(endpoints_list) > 20:
                ...
                try:
                    await self.send_single_cmd(
                        Clusters.Groupcast.Commands.JoinGroup(
                            groupID=groupID3, endpoints=endpoints_list_exceeds_DUT_endpoints, keySetID=keySetID2
                        )
                    )
                    asserts.fail(
        ...
        if not sd_enabled:
            ...
            await self.send_single_cmd(
                Clusters.Groupcast.Commands.JoinGroup(
                    groupID=groupID4, endpoints=endpoints_list_empty, keySetID=keySetID2, key=inputKey2
                )
            )
            ...
            if ln_enabled:
                ...
                self.skip_step("15b")
                self.step("15c")
                membership_matcher = generate_membership_entry_matcher(group_id=groupID4, key_set_id=keySetID2, endpoints=None)
                membership_sub.await_all_expected_report_matches(expected_matchers=[membership_matcher], timeout_sec=60)
            ...
            groupID5 = 5
            await self.send_single_cmd(
                Clusters.Groupcast.Commands.JoinGroup(groupID=groupID5, endpoints=endpoints_list_empty, keySetID=keySetID2)
            )
            ...
            if ln_enabled:
                ...
                self.skip_step("16b")
                self.step("16c")
                membership_matcher = generate_membership_entry_matcher(group_id=groupID5, key_set_id=keySetID2, endpoints=None)
                membership_sub.await_all_expected_report_matches(expected_matchers=[membership_matcher], timeout_sec=60)
            ...
            try:
                await self.send_single_cmd(
                    Clusters.Groupcast.Commands.JoinGroup(
                        groupID=groupID5, endpoints=endpoints_list_empty, keySetID=keySetID2, key=inputKey3
                    )
                )
                asserts.fail(
                    "JoinGroup command should have failed because Group with keySetID already exists and does not match key, but it still succeeded"
            ...
            try:
                await self.send_single_cmd(
                    Clusters.Groupcast.Commands.JoinGroup(groupID=groupID5, endpoints=endpoints_list_empty, keySetID=keySetID3)
                )
                asserts.fail("JoinGroup command should have failed because no Key found, but it still succeeded")
            except InteractionModelError as e:
                asserts.assert_equal(
            ...
            if ln_enabled:
                ...
                try:
                    await self.send_single_cmd(
                        Clusters.Groupcast.Commands.JoinGroup(
                            groupID=groupID5, endpoints=endpoints_list_empty, keySetID=keySetID2, replaceEndpoints=True
                        )
                    )
                    asserts.fail(
                        "JoinGroup command should have failed because listener cannot do JoinGroup commands, but it still succeeded"
                    )
                except InteractionModelError as e:
                    asserts.assert_equal(
                        e.status,
            ...
            if ln_enabled:
                ...
                try:
                    await self.send_single_cmd(
                        Clusters.Groupcast.Commands.JoinGroup(
                            groupID=groupID5, endpoints=endpoints_list_empty, keySetID=keySetID2, useAuxiliaryACL=True
                        )
                    )
                    asserts.fail(
                        "JoinGroup command should have failed because listener cannot do JoinGroup commands, but it still succeeded"
        ...
        try:
            await self.send_single_cmd(
                Clusters.Groupcast.Commands.JoinGroup(groupID=groupID0, endpoints=valid_endpoints, keySetID=1)
            )
        ...
        try:
            await self.send_single_cmd(
                Clusters.Groupcast.Commands.JoinGroup(
                    groupID=groupID6, endpoints=valid_endpoints, keySetID=keySetID4, key=inputKeyLong
                )
            )
