...
class TC_COMMTR_3_1(CommodityMeteringTestBaseHelper):
    ...
    def steps_TC_COMMTR_3_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "2",
                """Set up a subscription to the Commodity Metering cluster attributes:
                     - MinIntervalFloor: 0
            ...
            TestStep(
                "5",
                "TH reads MeteredQuantityTimestamp attribute.",
                """
            ...
            TestStep(
                "9",
                "TH awaits a MaximumMeteredQuantities attribute with 10s timeout.",
                """
            ...
            TestStep(
                "10",
                "TH awaits a MeteredQuantity attribute with 10s timeout.",
                """
            ...
            TestStep(
                "11",
                "TH awaits a MeteredQuantityTimestamp attribute with 10s timeout.",
                """
            ...
            TestStep(
                "12",
                "TH awaits a TariffUnit attribute with 10s timeout.",
                """
            ...
            TestStep(
                "13",
                """TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.COMMTR.TEST_EVENT_TRIGGER_KEY and
    ...
    async def test_TC_COMMTR_3_1(self):
        ...
        MaxMeteredQuantitiesValue = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.MaximumMeteredQuantities
        )
        ...
        MeteredQuantityValue = await self.read_single_attribute_check_success(
        ...
        MeteredQuantityTimestampValue = await self.read_single_attribute_check_success(
        ...
        await self.verify_reporting(
            ...
            cluster.Attributes.MaximumMeteredQuantities,
            "MaximumMeteredQuantities",
            MaxMeteredQuantitiesValue,
        )

        self.step("10")
        await self.check_metered_quantity_attribute(
            endpoint, subscription_handler.attribute_reports[cluster.Attributes.MeteredQuantity][0].value
        )
        await self.verify_reporting(
            subscription_handler.attribute_reports, cluster.Attributes.MeteredQuantity, "MeteredQuantity", MeteredQuantityValue
        )
        ...
        await self.verify_reporting(
            subscription_handler.attribute_reports,
            cluster.Attributes.MeteredQuantityTimestamp,
            "MeteredQuantityTimestamp",
            MeteredQuantityTimestampValue,
        )
