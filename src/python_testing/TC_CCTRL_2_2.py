...
class TC_CCTRL_2_2(MatterBaseTest):
    ...
    async def test_TC_CCTRL_2_2(self):
        ...
        await self.default_controller.FindOrEstablishPASESession(
            setupCode=params.commissioningParameters.setupQRCode, nodeId=pase_nodeid
        )
        try:
        ...
        cmd = Clusters.AdministratorCommissioning.Commands.RevokeCommissioning()
        await self.send_single_cmd(
            cmd, dev_ctrl=self.TH_server_controller, node_id=self.server_nodeid, timedRequestTimeoutMs=5000, endpoint=0
        )
