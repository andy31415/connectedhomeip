...
class TC_DD_1_16_17(MatterBaseTest):
    ...
    def steps_TC_DD_1_16(self):
        return [
            TestStep(1, "TH parses the QR code"),
            TestStep(
                2,
                "If the Custom Flow field is set to 0, this device uses standard flow. Verify that the DUT is advertising as Commissionable",
                "Device is advertising as commissionable",
            ),
            TestStep(
                3,
                "If the Custom Flow field is set to 1, this device uses user-intent flow. Verify that the DUT is NOT advertising as Commissionable",
    ...
    async def test_TC_DD_1_16(self):
        self.step(1)
        asserts.assert_true(self.matter_test_config.qr_code_content, "This test needs to be run with the qr setup code.")
        ...
        if parsed.commissioning_flow == 0:
            ...
            await self.ensure_advertising(
                filter_type=matter.discovery.FilterType.LONG_DISCRIMINATOR, filter=parsed.long_discriminator
            )
        else:
        ...
        if parsed.commissioning_flow == 1:
            responses = await self.default_controller.DiscoverCommissionableNodes(
                filterType=matter.discovery.FilterType.LONG_DISCRIMINATOR, filter=parsed.long_discriminator
            )
    ...
    def steps_TC_DD_1_17(self):
        return [
            TestStep(1, "TH parses the manual code"),
            TestStep(
                2,
                "If the VID_PID_PRESENT field is set to 0, this device uses standard flow. Verify that the DUT is advertising as Commissionable",
                "Device is advertising as commissionable",
            ),
            TestStep(
                3,
                "If the device uses custom flow or user-intent commissioning, ask the tester to place the device into commissionable mode",
            ),
            TestStep(
                4,
                "If the device uses custom flow or user-intent commissioning, verify that the DUT is advertising as commissionable",
                "Device is advertising as commissionable",
            ),
        ]
    ...
    async def ensure_advertising(self, filter_type: matter.discovery.FilterType, filter: int):
        responses = await self.default_controller.DiscoverCommissionableNodes(
            filterType=filter_type, filter=filter, stopOnFirst=True
        )
    ...
    async def test_TC_DD_1_17(self):
        self.step(1)
        asserts.assert_true(self.matter_test_config.manual_code, "This test needs to be run with the manual setup code.")
        ...
        if standard_flow:
            ...
            await self.ensure_advertising(
                filter_type=matter.discovery.FilterType.SHORT_DISCRIMINATOR, filter=parsed.short_discriminator
            )
        else:
            self.mark_current_step_skipped()
