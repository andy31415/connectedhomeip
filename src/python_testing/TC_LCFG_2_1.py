...
class Test_TC_LCFG_2_1(MatterBaseTest):
    ...
    async def test_TC_LCFG_2_1(self):
        ...
        asserts.assert_false(
            self.has_repeated_values(initial_supported_locales), "SupportedLocales attribute should not have repeated values"
        )

        # Verify maximun number of elements in the SupportedLocales list is 32
        asserts.assert_less_equal(
            len(initial_supported_locales),
            max_length_list,
        ...
        asserts.assert_true(
            self.supported_locales_has_active_locale(initial_supported_locales, initial_active_locale),
            "ActiveLocale attribute value should be present in the SupportedLocales attribute list",
        )
        ...
        result = await self.write_single_attribute(
            attribute_value=Clusters.LocalizationConfiguration.Attributes.ActiveLocale(value_not_present_in_supported_locales),
        ...
        if filtered_supported_locales:
            ...
            log.info(
                "SupportedLocales attribute has only one element and is the same value as ActiveLocale. Skipping remaining test steps."
            )
            self.mark_all_remaining_steps_skipped(5)
            return

        result = await self.write_single_attribute(
            attribute_value=Clusters.LocalizationConfiguration.Attributes.ActiveLocale(value_present_in_supported_locales),
            endpoint_id=endpoint,
            expect_success=True,
        )
