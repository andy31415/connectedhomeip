...
class TC_WebRTCP_2_5(MatterBaseTest, WEBRTCPTestBase):
    ...
    def steps_TC_WebRTCP_2_5(self) -> list[TestStep]:
        ...
        return [
            ...
            TestStep(
                5,
                "Send SolicitOffer with valid parameters, audio stream ID is Null, video stream ID is for the allocated stream => expect INVALID_IN_STATE",
            ),
            TestStep(6, "Allocate Audio Stream"),
            TestStep(
                7,
                "Send SolicitOffer with valid parameters, audio stream ID is the allocated stream ID, video stream ID is invalid => expect DYNAMIC_CONSTRAINT_ERROR",
            ),
            TestStep(
                8,
                "Send SolicitOffer with a supported stream usage that doesn't match the allocated streams, audio stream ID is the allocated stream ID, video stream ID is the allocated stream => expect SolictOfferResponse",
            ),
            TestStep(
                9,
                "Send SolicitOffer with an unsupported stream usage, audio stream ID is the allocated stream ID, video stream ID is the allocated stream => expect DYNAMIC_CONSTRAINT_ERROR",
            ),
            TestStep(
                10,
                "Send SolicitOffer with a supported stream usage that doesn't match the allocated streams, audio stream ID is null, video stream ID is null => expect SolicitOfferResponse and Ids that match the allocated streams",
            ),
    ...
    async def test_TC_WebRTCP_2_5(self):
        ...
        if myStreamUsage is not None:
            ...
            try:
                ...
                asserts.assert_equal(
                    resp.videoStreamID, videoStreamID, "VideoStreamID in the response does not match that allocated"
                )
                asserts.assert_equal(
                    resp.audioStreamID, audioStreamID, "AudioStreamID in the response does not match that allocated"
                )
            except InteractionModelError as e:
                asserts.assert_equal(e.status, Status.Success, "Unexpected error on an expected success scenario")
        else:
