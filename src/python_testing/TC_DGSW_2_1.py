...
class TC_DGSW_2_1(MatterBaseTest):
    ...
    def steps_TC_DGSW_2_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                2,
                "Read the ThreadMetrics attribute",
            ...
            TestStep(
                3,
                "Read the CurrentHeapFree attribute",
    ...
    async def test_TC_DGSW_2_1(self):
        ...
        if attr_condition:
            thread_metrics_list = await self.read_dgsw_attribute_expect_success(
            ...
            for metric in thread_metrics_list:
                ...
                if metric.stackFreeCurrent is not None:
                    ...
                    if stackSize is not None:
                        asserts.assert_less(metric.stackFreeCurrent, stackSize, "StackFreeCurrent is not less than StackSize.")
                else:
                ...
                if metric.stackFreeMinimum is not None:
                    ...
                    if metric.stackFreeCurrent is not None:
                        asserts.assert_less_equal(
                            metric.stackFreeMinimum,
                            metric.stackFreeCurrent,
        ...
        if attr_condition:
            current_heap_free_attr = await self.read_dgsw_attribute_expect_success(
                endpoint=endpoint, attribute=attributes.CurrentHeapFree
            )
        ...
        if attr_condition:
            current_heap_used_attr = await self.read_dgsw_attribute_expect_success(
        ...
        if attr_condition:
            current_heap_high_watermark_attr = await self.read_dgsw_attribute_expect_success(
