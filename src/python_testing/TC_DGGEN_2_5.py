...
class TC_DGGEN_2_5(MatterBaseTest):
    ...
    def steps_TC_DGGEN_2_5(self):
        return [
            TestStep(
                1, "Ensure DUT is commissioned in TH1’s fabric.", "TH1 is able to communicate with DUT.", is_commissioning=True
            ),
            TestStep(
                2,
    ...
    async def test_TC_DGGEN_2_5(self):
        ...
        asserts.assert_less_equal(
            initialCurrentSubscriptionsForFabric,
            initialCurrentSubscriptions,
        ...
        asserts.assert_greater_equal(
            initialTotalSubscriptionsEstablished,
            initialCurrentSubscriptions,
        ...
        await rebootCountCallBack.start(
            dev_ctrl=self.th1,
        ...
        await rebootCountCallBack2.start(
            dev_ctrl=self.th1,
            node_id=self.dut_node_id,
            endpoint=endpoint,
            fabric_filtered=False,
        ...
        await rebootCountCallBack3.start(
            dev_ctrl=self.th2,
            node_id=self.dut_node_id,
            endpoint=endpoint,
            fabric_filtered=False,
        ...
        if await self.attribute_guard(endpoint=endpoint, attribute=cluster.Attributes.DeviceLoadStatus):
            ...
            asserts.assert_greater_equal(
                finalTotalInteractionModelMessageSent,
                initialTotalInteractionModelMessageSent + expectedIncreaseInteractionModeMessagesSent,
            ...
            asserts.assert_greater_equal(
                finalTotalInteractionModelMessagesReceived,
                initialTotalInteractionModelMessagesReceived + expectedIncreaseInteractionModeMessagesReceived,
            ...
            asserts.assert_equal(
                ...
                initialCurrentSubscriptions + expectedIncreaseCurrentSubscriptions,
            ...
            asserts.assert_equal(
                finalCurrentSubscriptionsForFabric,
                initialCurrentSubscriptionsForFabric + expectedIncreaseSubscriptionsForFabric,
            ...
            asserts.assert_equal(
                finalTotalSubscriptionsEstablished,
                initialTotalSubscriptionsEstablished + expectedIncreaseSubscriptionsEstablished,
        ...
        th2FabricIndex = await self.read_single_attribute(
            dev_ctrl=self.th2,
            node_id=self.dut_node_id,
            endpoint=endpoint,
        ...
        response = await self.th1.SendCommand(
            nodeId=self.dut_node_id,
            endpoint=endpoint,
            payload=Clusters.OperationalCredentials.Commands.RemoveFabric(fabricIndex=th2FabricIndex),
        )
        ...
        asserts.assert_equal(
            response.statusCode, successfulResponse, f"Response is {response.statusCode} and it should be {successfulResponse}"
        )
