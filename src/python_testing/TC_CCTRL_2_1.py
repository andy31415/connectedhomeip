...
class TC_CCTRL_2_1(MatterBaseTest):
    def steps_TC_CCTRL_2_1(self) -> list[TestStep]:
        return [
            TestStep(1, "Read MCORE.FS PICS code", is_commissioning=True),
    ...
    async def test_TC_CCTRL_2_1(self):
        ...
        self.step(2)
        supported_device_categories = await self.read_single_attribute_check_success(
            cluster=Clusters.CommissionerControl, attribute=Clusters.CommissionerControl.Attributes.SupportedDeviceCategories
        )
        is_fabric_sync_bit_set = bool(
            supported_device_categories & Clusters.CommissionerControl.Bitmaps.SupportedDeviceCategoryBitmap.kFabricSynchronization
        )
        asserts.assert_equal(
            is_fabric_sync_bit_set, is_fabric_sync_pics_enabled, "Mismatch between PICS MCORE.FS value and what attribute indicates"
        )
