...
kUatInstructionDependentBitMask = (
    uat.kCustomInstruction
    | uat.kActuateSensorSeconds
    | uat.kActuateSensorTimes
    | uat.kActuateSensorLightsBlink
    | uat.kResetButtonLightsBlink
    | uat.kResetButtonSeconds
    | uat.kResetButtonTimes
    | uat.kSetupButtonSeconds
    | uat.kSetupButtonLightsBlink
    | uat.kSetupButtonTimes
    | uat.kAppDefinedButton
)
...
class TC_ICDM_2_1(MatterBaseTest):
    ...
    def steps_TC_ICDM_2_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(5, "TH reads from the DUT the ClientsSupportedPerFabric attribute."),
            ...
            TestStep(8, "TH reads from the DUT the UserActiveModeTriggerHint attribute."),
            TestStep(9, "TH reads from the DUT the UserActiveModeTriggerInstruction attribute"),
    ...
    async def test_TC_ICDM_2_1(self):
        ...
        if self.check_pics("ICDM.S.A0002"):
            activeModeThreshold = await self._read_icdm_attribute_expect_success(attributes.ActiveModeThreshold)
            # Verify ActiveModeThreshold is not bigger than uint16
            asserts.assert_true(
                self.is_valid_uint16_value(activeModeThreshold), "ActiveModeThreshold attribute does not fit in a uint16."
            )

            if featureMap > 0 and features.kLongIdleTimeSupport in features(featureMap):
                asserts.assert_greater_equal(activeModeThreshold, 5000, "Minimum ActiveModeThreshold is 5s for a LIT ICD.")
        ...
        if self.check_pics("ICDM.S.A0001"):
            activeModeDuration = await self._read_icdm_attribute_expect_success(attributes.ActiveModeDuration)
            # Verify ActiveModeDuration is not bigger than uint32
            asserts.assert_true(
                self.is_valid_uint32_value(activeModeDuration), "ActiveModeDuration attribute does not fit in a uint32"
            )
        else:
        ...
        if self.check_pics("ICDM.S.A0000"):
            idleModeDuration = await self._read_icdm_attribute_expect_success(attributes.IdleModeDuration)
        ...
        if self.pics_guard(self.check_pics("ICDM.S.A0005")):
            ...
            asserts.assert_true(
                self.is_valid_uint16_value(clientsSupportedPerFabric),
        ...
        if self.pics_guard(self.check_pics("ICDM.S.A0003")):
            ...
            asserts.assert_true(isinstance(registeredClients, list), "RegisteredClients is not a list.")
        ...
        if self.pics_guard(self.check_pics("ICDM.S.A0004")):
            ...
            asserts.assert_true(self.is_valid_uint32_value(icdCounter), "ActiveModeDuration attribute does not fit in a uint32")

        # Validate UserActiveModeTriggerHint
        self.step(8)
        if self.pics_guard(self.check_pics("ICDM.S.A0006")):
            userActiveModeTriggerHint = await self._read_icdm_attribute_expect_success(attributes.UserActiveModeTriggerHint)

            # Verify that it is a bitmap32 - Only the first 16 bits are used
            asserts.assert_true(
                0 <= userActiveModeTriggerHint <= kMaxUserActiveModeBitmap,
            ...
            uatHintInstructionDepedentBitmap = uat(userActiveModeTriggerHint) & kUatInstructionDependentBitMask

            asserts.assert_less_equal(
                self.set_bits_count(uatHintInstructionDepedentBitmap),
                1,
                "UserActiveModeTriggerHint has more than 1 bit that is dependent on the UserActiveModeTriggerInstruction",
            )
        ...
        if self.check_pics("ICDM.S.A0007"):
            ...
            try:
                encodedUATInstruction = userActiveModeTriggerInstruction.encode("utf-8")
            except Exception:
                asserts.assert_true(False, "UserActiveModeTriggerInstruction is not encoded in the correct format (utf-8).")

            # Verify byte length of the UserActiveModeTirggerInstruction
            asserts.assert_less_equal(
                len(encodedUATInstruction),
                kMaxUserActiveModeTriggerInstructionByteLength,
                "UserActiveModeTriggerInstruction is longuer than the maximum allowed length (128).",
            )
            ...
            if uatHintInstructionDepedentBitmap > 0 and uatHintInstructionDepedentBitmap in kUatNumberInstructionBitMask:
                ...
                asserts.assert_true(
                    (re.search(r"^(?!0)[0-9]*$", userActiveModeTriggerInstruction) is not None),
            ...
            if uatHintInstructionDepedentBitmap > 0 and uatHintInstructionDepedentBitmap in kUatColorInstructionBitMask:
                ...
                asserts.assert_true(
                    pattern.match(userActiveModeTriggerInstruction),
                    "UserActiveModeTriggerInstruction is not in the correct format for the associated UserActiveModeTriggerHint",
                )
        elif self.check_pics("ICDM.S.A0006"):
            # Check if the UserActiveModeTriggerInstruction was required
            asserts.assert_false(
                uatHintInstructionDepedentBitmap in kUatInstructionMandatoryBitMask,
        ...
        if self.pics_guard(self.check_pics("ICDM.S.A0008")):
            ...
            asserts.assert_true(self.is_valid_uint8_value(operatingMode), "OperatingMode does not fit in an enum8")

            asserts.assert_less(operatingMode, modes.kUnknownEnumValue, "OperatingMode can only have 0 and 1 as valid values")
        self.step(11)
        if self.pics_guard(self.check_pics("ICDM.S.A0009")):
            ...
            asserts.assert_true(
                self.is_valid_uint32_value(maximumCheckInBackOff), "MaximumCheckInBackOff attribute is not a valid uint32."
            )
            asserts.assert_greater_equal(
            ...
            asserts.assert_less_equal(
                maximumCheckInBackOff, 64800, "MaximumCheckInBackOff attribute is greater than maximum value (64800)."
            )
