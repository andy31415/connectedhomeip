...
class TC_RVCCLEANM_2_1(MatterBaseTest):
    ...
    async def send_clean_change_to_mode_cmd(self, newMode) -> Clusters.Objects.RvcCleanMode.Commands.ChangeToModeResponse:
        ...
        asserts.assert_true(
            matchers.is_type(ret, Clusters.Objects.RvcCleanMode.Commands.ChangeToModeResponse),
    ...
    async def send_run_change_to_mode_cmd(self, newMode) -> Clusters.Objects.RvcRunMode.Commands.ChangeToModeResponse:
        ...
        asserts.assert_true(
            matchers.is_type(ret, Clusters.Objects.RvcRunMode.Commands.ChangeToModeResponse),
    ...
    async def test_TC_RVCCLEANM_2_1(self):
        asserts.assert_true(
            "PIXIT.RVCCLEANM.MODE_CHANGE_OK" in self.matter_test_config.global_test_params,
        ...
        asserts.assert_true(
            "PIXIT.RVCCLEANM.MODE_CHANGE_FAIL" in self.matter_test_config.global_test_params,
        ...
        if can_test_mode_failure and can_manually_control:
            asserts.assert_true(
                self.mode_fail in modes, "The MODE_CHANGE_FAIL PIXIT value (%d) is not a supported mode" % (self.mode_fail)
            )
            self.print_step(
                5, "Manually put the device in a state from which it will FAIL to transition to mode %d" % (self.mode_fail)
            )
            if self.is_ci:
                ...
                self.wait_for_user_input(
                    prompt_msg="Manually put the device in a state from which it will FAIL to transition to mode %d, and press Enter when done"
                    % (self.mode_fail)
                )

            self.print_step(6, "Read CurrentMode attribute")
            old_current_mode = await self.read_mod_attribute_expect_success(
                endpoint=self.endpoint, attribute=attributes.CurrentMode
            )
            ...
            is_err_code = (
                (st == CommonCodes.GENERIC_FAILURE.value)
                or (st == CommonCodes.INVALID_IN_MODE.value)
                or (st in rvcCleanCodes)
                or is_mfg_code
            )
            asserts.assert_true(
        ...
        self.print_step(
            9, "Manually put the device in a state from which it will SUCCESSFULLY transition to mode %d" % (self.mode_ok)
        )
        if self.is_ci:
            ...
            self.wait_for_user_input(
                prompt_msg="Manually put the device in a state from which it will SUCCESSFULLY transition to mode %d, and press Enter when done"
                % (self.mode_ok)
            )
        ...
        asserts.assert_true(
            ret.status == CommonCodes.SUCCESS.value,
        ...
        asserts.assert_true(
            current_mode == self.mode_ok, "CurrentMode doesn't match the argument of the successful ChangeToMode command!"
        )
        ...
        asserts.assert_true(
            ret.status == CommonCodes.UNSUPPORTED_MODE.value,
