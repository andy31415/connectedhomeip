...
class TC_CADMIN(CADMINBaseTest):
    async def combined_commission_val_steps(self, commission_type: str):
        ...
        if commission_type == "ECM":
            params = await self.th1.OpenCommissioningWindow(
                nodeId=self.dut_node_id, timeout=self.max_window_duration, iteration=1000, discriminator=1234, option=1
            )
        ...
        self.step("3b")
        if commission_type == "ECM":
            service = await self.wait_for_correct_cm_value(expected_cm_value=2, expected_discriminator=1234)
            ...
            service = await self.wait_for_correct_cm_value(
                expected_cm_value=1, expected_discriminator=setupPayloadInfo[0].filter_value
            )
            log.info(f"Successfully found service with CM={service.txt.get('CM')}, D={service.txt.get('D')}")
        ...
        th1_cam_rcac = TLVReader(self.certificate_authority_manager.activeCaList[0]._persistentStorage.GetSdkKey("f/1/r")).get()[
            "Any"
        ][9]
        ...
        if commission_type == "ECM":
            ...
            params, window_status_accumulator = await self.open_commissioning_window_with_subscription_monitoring(
                th=self.th2, timeout=180, node_id=self.dut_node_id, discriminator=0, iteration=1000
            )
            ...
            params2 = await self.th2.OpenCommissioningWindow(
                nodeId=self.dut_node_id,
            ...
            try:
                await self.th1.CommissionOnNetwork(
                    nodeId=self.dut_node_id,
                    setupPinCode=params2.setupPinCode,
                    filterType=ChipDeviceCtrl.DiscoveryFilterType.LONG_DISCRIMINATOR,
                ...
                asserts.assert_equal(e.err, 0x0000007E, "Expected to return Trying to add NOC for fabric that already exists")
            """
            expected error:
    ...
    def steps_TC_CADMIN_1_3(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "3a",
                "TH_CR1 opens a commissioning window on DUT_CE using a commissioning timeout of max_window_duration using ECM",
            ...
            TestStep(
                4,
                "TH creates a controller (TH_CR2) on a new fabric and commissions DUT_CE using that controller. TH_CR2 should commission the device using a different NodeID than TH_CR1.",
            ...
            TestStep(
                12,
                "TH_CR1 sends an RevokeCommissioning command to the DUT to cleanup step 11",
    ...
    def steps_TC_CADMIN_1_4(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "3a",
                "TH_CR1 opens a commissioning window on DUT_CE using a commissioning timeout of max_window_duration using BCM",
                "DUT_CE opens its Commissioning window to allow a second commissioning.",
            ),
    ...
    @run_if_endpoint_matches(
        has_feature(cluster=Clusters.AdministratorCommissioning, feature=Clusters.AdministratorCommissioning.Bitmaps.Feature.kBasic)
    )
