...
def main():
    ...
    test_runner = MockTestRunner(Path(__file__).parent / "../TC_SC_7_1.py", "TC_SC_7_1", "test_TC_SC_7_1", 0)
    failures = []
    ...
    test_config = MatterTestConfig(
        qr_code_content=[qr_2222_20202021, qr_3333_20202024], global_test_params={"post_cert_test": True}
    )
    test_runner.set_test_config(test_config)
    ok = test_runner.run_test_with_mock_read(read_trusted_root(False))
    if ok:
    ...
    test_config = MatterTestConfig(
        manual_code=[manual_2222_20202021, manual_3333_20202024], global_test_params={"post_cert_test": True}
    )
    test_runner.set_test_config(test_config)
    ok = test_runner.run_test_with_mock_read(read_trusted_root(False))
    if ok:
    ...
    print(f"Test of TC-SC-7.1: test response incorrect: {len(failures)}")
    for f in failures:
        print(f)
