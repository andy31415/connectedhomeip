...
class TestPicsHelpers(MatterBaseTest):
    def test_pics_generation(self):
        ...
        wildcard.attributes = {
            0: {desc: {desc.Attributes.DeviceTypeList: [desc.Structs.DeviceTypeStruct(deviceType=0x16, revision=1)]}}
        }
        ...
        wildcard.tlvAttributes[1][lock.id][lock.Attributes.AttributeList.attribute_id] = [
            lock.Attributes.DoorState.attribute_id,
            lock.Attributes.LockType.attribute_id,
        ]
        wildcard.tlvAttributes[1][lock.id][lock.Attributes.AcceptedCommandList.attribute_id] = [
            lock.Commands.GetUser.command_id,
            lock.Commands.GetCredentialStatus.command_id,
        ]
        wildcard.tlvAttributes[1][lock.id][lock.Attributes.GeneratedCommandList.attribute_id] = [
            lock.Commands.GetUserResponse.command_id,
            lock.Commands.GetCredentialStatusResponse.command_id,
        ]
        ...
        def check_expected_pics(pics_list: dict[int, list[str]]):
            ...
            asserts.assert_equal(
                set(pics_list[0]),
                set(desc_expected + opcreds_expected + cadmin_expected + ["IDM.S", "MCORE.ROLE.COMMISSIONEE"]),
                "Incorrect PICS list on EP0",
            )
        ...
        wildcard.tlvAttributes[1][lock.id][lock.Attributes.AttributeList.attribute_id].extend(
            [
                lock.Attributes.AttributeList.attribute_id,
                lock.Attributes.AcceptedCommandList.attribute_id,
                lock.Attributes.GeneratedCommandList.attribute_id,
                lock.Attributes.FeatureMap.attribute_id,
                lock.Attributes.ClusterRevision.attribute_id,
            ]
        )
