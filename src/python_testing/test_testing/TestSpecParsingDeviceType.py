...
class TestSpecParsingDeviceType(DeviceConformanceTests):
    ...
    def test_device_type_clusters(self):
        ...
        asserts.assert_equal(
            len(device_type[self.device_type_id].server_clusters), len(self.clusters), "Unexpected number of clusters"
        )
        for id, name in self.clusters.items():
            ...
            asserts.assert_equal(
                str(device_type[self.device_type_id].server_clusters[id].conformance), "M", "Incorrect cluster conformance"
            )
    ...
    def test_feature_overrides_good(self):
        ...
        asserts.assert_in(
            groups_id, device_type[self.device_type_id].server_clusters, "Groups Cluster not found in device type parse"
        )
        asserts.assert_in(
            Clusters.Identify.id,
            device_type[self.device_type_id].server_clusters,
            "Identify Cluster not found in device type parse",
        )
        asserts.assert_in(
            group_name_mask,
            device_type[self.device_type_id].server_clusters[groups_id].feature_overrides,
            "GroupName override not found in groups cluster",
        )
        asserts.assert_equal(
            str(device_type[self.device_type_id].server_clusters[groups_id].feature_overrides[group_name_mask]),
            "M",
            "Improper conformance override for GroupName feature",
        )
    ...
    def test_feature_overrides_bad(self):
        ...
        asserts.assert_in(
            identify_id, device_type[self.device_type_id].server_clusters, "Identify Cluster not found in device type parse"
        )
        asserts.assert_not_in(
            group_name_mask,
            device_type[self.device_type_id].server_clusters[groups_id].feature_overrides,
            "GroupName override found in groups cluster",
        )
        asserts.assert_not_in(
            group_name_mask,
            device_type[self.device_type_id].server_clusters[identify_id].feature_overrides,
            "GroupName override found in identify cluster",
        )
    ...
    def test_attribute_overrides_good(self):
        ...
        asserts.assert_in(
            name_support_attr,
            device_type[self.device_type_id].server_clusters[groups_id].attribute_overrides,
            "NameSupport override not found in groups cluster",
        )
        asserts.assert_equal(
            str(device_type[self.device_type_id].server_clusters[groups_id].attribute_overrides[name_support_attr]),
            "M",
            "Improper conformance override",
        )
    ...
    def test_attribute_overrides_bad(self):
        ...
        asserts.assert_not_in(
            name_support_attr,
            device_type[self.device_type_id].server_clusters[groups_id].attribute_overrides,
            "NameSupport override found in groups cluster",
        )
        asserts.assert_not_in(
            name_support_attr,
            device_type[self.device_type_id].server_clusters[identify_id].attribute_overrides,
            "NameSupport override not found in identify cluster",
        )
    ...
    def test_command_overrides_good(self):
        ...
        asserts.assert_in(
            add_group_cmd,
            device_type[self.device_type_id].server_clusters[groups_id].command_overrides,
            "AddGroup override not found in groups cluster",
        )
        asserts.assert_equal(
            str(device_type[self.device_type_id].server_clusters[groups_id].command_overrides[add_group_cmd]),
            "M",
            "Improper conformance override",
        )
        asserts.assert_equal(len(self.problems), 0, "Found problems when parsing good attribute override")
    ...
    def test_command_overrides_bad(self):
        ...
        asserts.assert_not_in(
            add_group_cmd,
            device_type[self.device_type_id].server_clusters[identify_id].command_overrides,
            "NameSupport override not found in identify cluster",
        )
    ...
    def test_overrides_real(self):
        ...
        asserts.assert_in(
            trigger_effect,
            self.xml_device_types[on_off_light_device_type].server_clusters[identify].command_overrides.keys(),
            "Did not find expected command override for TriggerEffect",
        )
        asserts.assert_equal(
            str(self.xml_device_types[on_off_light_device_type].server_clusters[identify].command_overrides[trigger_effect]),
            "M",
            "Unexpected command override for TriggerEffect",
        )
        onoff = Clusters.OnOff.id
        lighting = Clusters.OnOff.Bitmaps.Feature.kLighting
        asserts.assert_in(
            lighting,
            self.xml_device_types[on_off_light_device_type].server_clusters[onoff].feature_overrides.keys(),
            "Did not find expected feature override for Lighting",
        )
        asserts.assert_equal(
            str(self.xml_device_types[on_off_light_device_type].server_clusters[onoff].feature_overrides[lighting]),
            "M",
            "Unexpected feature override for Lighting",
        )
        asserts.assert_equal(
            len(self.xml_device_types[on_off_light_device_type].server_clusters[onoff].attribute_overrides),
            0,
            "Unexpected attribute override in on off device type",
        )
        ...
        asserts.assert_in(
            remaining_time,
            self.xml_device_types[color_temp_device_type].server_clusters[color_control].attribute_overrides.keys(),
            "Did not find expected attribute override for RemainingTime",
        )
        asserts.assert_equal(
            str(self.xml_device_types[color_temp_device_type].server_clusters[color_control].attribute_overrides[remaining_time]),
            "M",
            "Unexpected attribute override for RemainingTime",
        )
    ...
    def create_test(self, server_list: list[uint], no_descriptor: bool = False, bad_device_id: bool = False):
        ...
        if no_descriptor:
            ...
            attrs = create_minimal_dt(
                xml_device_types=self.xml_device_types,
                xml_clusters=self.xml_clusters,
                device_type_id=0x302,
                is_tlv_endpoint=False,
                server_override=server_list,
            )
            attrs_tlv = create_minimal_dt(
                xml_device_types=self.xml_device_types,
                xml_clusters=self.xml_clusters,
                device_type_id=0x302,
                is_tlv_endpoint=True,
                server_override=server_list,
            )
    ...
    def test_spec_files(self):
        ...
        asserts.assert_greater(
            len(set(one_five.keys()) - set(one_four_two.keys())), 0, "1.5 does not contain any device types not in 1.4.2"
        )
        asserts.assert_greater(
            len(set(one_four_two.keys()) - set(one_three.keys())), 0, "Master dir does not contain any device types not in 1.3"
        )
        asserts.assert_greater(
            len(set(one_four_two.keys()) - set(one_four.keys())), 0, "Master dir does not contain any device types not in 1.4"
        )
        asserts.assert_greater(
            len(set(one_four_two.keys()) - set(one_three.keys())), 0, "1.4 dir does not contain any device types not in 1.3"
        )
        asserts.assert_greater(
            len(set(one_four_two.keys()) - set(one_two.keys())), 0, "Master does not contain any device types not in 1.2"
        )
        ...
        asserts.assert_equal(
            set(one_two.keys() - set(one_three.keys())), {0x0300}, "1.2 contains unexpected device types not in 1.3"
        )
        asserts.assert_equal(
            len(one_four.keys()), len(one_four_one.keys()), "Number of device types in 1.4 and 1.4.1 does not match"
        )
        asserts.assert_equal(
            set(one_three.keys()) - set(one_four.keys()),
            set(),
            "There are some 1.3 device types that are unexpectedly not included in the 1.4 spec",
        )
        asserts.assert_equal(
            set(one_four.keys()) - set(one_four_two.keys()),
            set(),
            "There are some 1.4 device types that are unexpectedly not included in the 1.4.2 spec",
        )
        asserts.assert_equal(
            set(one_three.keys()) - set(one_four_two.keys()),
            set(),
            "There are some 1.3 device types that are unexpectedly not included in the 1.4.2 spec",
        )
        asserts.assert_equal(
            set(one_four_two.keys()) - set(one_five.keys()),
            set(),
            "There are some 1.4.2 device types that are unexpectedly not included in the 1.5 spec",
        )
        asserts.assert_equal(
            set(one_five.keys()) - set(one_five_one.keys()),
            set(),
            "There are some 1.5 device types that are unexpectedly not included in the 1.5.1 spec",
        )
        asserts.assert_equal(
            set(one_five_one.keys()) - set(one_six.keys()),
            set(),
            "There are some 1.5.1 device types that are unexpectedly not included in the 1.6 spec",
        )
    ...
    def test_superset_parsing_released_spec(self):
        ...
        expected_light_superset = {
            self.dt_ids["extendedcolorlight"],
            self.dt_ids["colortemperaturelight"],
            self.dt_ids["dimmablelight"],
            self.dt_ids["onofflight"],
        }
        expected_dimmer_switch = {self.dt_ids["colordimmerswitch"], self.dt_ids["dimmerswitch"], self.dt_ids["onofflightswitch"]}
    ...
    def _build_superset_tree(self) -> dict[int, XmlDeviceType]:
        ...
        # 6 - utility endpoint
        one = XmlDeviceType(
            "one", 1, [], [], "simple", "endpoint", {1: ""}, superset_of_device_type_name="two", superset_of_device_type_id=2
        )
        two = XmlDeviceType(
            "two", 1, [], [], "simple", "endpoint", {1: ""}, superset_of_device_type_name="three", superset_of_device_type_id=3
        )
        three = XmlDeviceType(
            "three", 1, [], [], "simple", "endpoint", {1: ""}, superset_of_device_type_name=None, superset_of_device_type_id=0
        )
        four = XmlDeviceType(
            "four", 1, [], [], "simple", "endpoint", {1: ""}, superset_of_device_type_name="three", superset_of_device_type_id=3
        )
        five = XmlDeviceType(
            "five", 1, [], [], "simple", "endpoint", {1: ""}, superset_of_device_type_name=None, superset_of_device_type_id=0
        )
        six = XmlDeviceType(
            "six", 1, [], [], "utility", "endpoint", {1: ""}, superset_of_device_type_name=None, superset_of_device_type_id=0
        )
    ...
    def test_supersets_on_endpoints_mocks(self):
        ...
        self.endpoints[1][desc][desc.Attributes.DeviceTypeList] = [
            desc.Structs.DeviceTypeStruct(1, 1),
            desc.Structs.DeviceTypeStruct(2, 1),
            desc.Structs.DeviceTypeStruct(3, 1),
        ]
        problems = self.check_all_application_device_types_superset()
        ...
        self.endpoints[1][desc][desc.Attributes.DeviceTypeList] = [
            ...
            desc.Structs.DeviceTypeStruct(2, 1),
            desc.Structs.DeviceTypeStruct(3, 1),
            desc.Structs.DeviceTypeStruct(6, 1),
        ]
        problems = self.check_all_application_device_types_superset()
        ...
        self.endpoints[1][desc][desc.Attributes.DeviceTypeList] = [
            desc.Structs.DeviceTypeStruct(5, 1),
            desc.Structs.DeviceTypeStruct(6, 1),
        ]
        problems = self.check_all_application_device_types_superset()
        ...
        self.endpoints[1][desc][desc.Attributes.DeviceTypeList] = [
            desc.Structs.DeviceTypeStruct(1, 1),
            desc.Structs.DeviceTypeStruct(2, 1),
            desc.Structs.DeviceTypeStruct(4, 1),
        ]
        problems = self.check_all_application_device_types_superset()
