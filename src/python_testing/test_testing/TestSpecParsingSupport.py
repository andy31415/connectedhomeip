...
from matter.testing.spec_parsing import (
    ...
    XmlCluster,
    add_cluster_data_from_xml,
    build_xml_clusters,
    check_clusters_for_unknown_commands,
...
def single_attribute_cluster_xml(read_access: str, write_access: str, write_supported: str, invoke_access: str):
    ...
    id_table = f'<clusterIds><clusterId id="{CLUSTER_ID}" name="{CLUSTER_NAME}"/></clusterIds>'
    ...
    return f"{xml_cluster}{revision_table}{id_table}{classification}{attribute}{command}</cluster>"
...
class TestSpecParsingSupport(MatterBaseTest):
    ...
    def test_build_xml_override(self):
        ...
        asserts.assert_greater(
            len(set(one_four_two_xml_clusters.keys()) - set(one_two_clusters.keys())),
            0,
            "1.2.2 dir does not contain any clusters not in 1.3",
        )
        asserts.assert_greater(
            len(set(one_four_two_xml_clusters.keys()) - set(one_three_clusters.keys())),
            0,
            "1.4.2 dir does not contain any clusters not in 1.3",
        )
        asserts.assert_equal(
            len(set(one_four_two_xml_clusters.keys()) - set(one_four_clusters.keys())), 0, "1.4.2 contains clusters not in 1.4"
        )
        asserts.assert_greater(
            len(set(one_four_clusters.keys()) - set(one_three_clusters.keys())),
            0,
            "1.4 dir does not contain any clusters not in 1.3",
        )
        asserts.assert_equal(
            len(one_four_clusters.keys()), len(one_four_one_clusters.keys()), "1.4 and 1.4.1 do not contain the same clusters"
        )
        asserts.assert_greater(
            len(set(one_five_xml_clusters.keys()) - set(one_four_two_xml_clusters.keys())),
            0,
            "1.5 dir does not contain any clusters not in 1.4.2",
        )
        asserts.assert_greater_equal(
            len(set(one_five_one_xml_clusters.keys()) - set(one_five_xml_clusters.keys())), 0, "1.5.1 has fewer clusters than 1.5"
        )
        asserts.assert_greater_equal(
            len(set(one_six_xml_clusters.keys()) - set(one_five_one_xml_clusters.keys())), 0, "1.6 has fewer clusters than 1.5.1"
        )
        ...
        asserts.assert_equal(
            set(one_two_clusters.keys()) - set(one_three_clusters.keys()),
            one_two_removed,
            "There are some 1.3 clusters that are unexpectedly not included in the 1.4 spec",
        )
        # only the pulse width modulation cluster was removed post 1.3
        one_four_removed = {Clusters.PulseWidthModulation.id}
        asserts.assert_equal(
            set(one_three_clusters.keys()) - set(one_four_clusters.keys()),
            one_four_removed,
            "There are some 1.3 clusters that are unexpectedly not included in the 1.4 spec",
        )
        # Ballast and all the proxy clusters are being removed in 1.4.2
        one_four_two_removed = {
        ...
        asserts.assert_equal(
            set(one_four_clusters.keys()) - set(one_four_two_xml_clusters.keys()),
            one_four_two_removed,
            "There are some 1.4 clusters that are unexpectedly not included in the 1.4.2 spec",
        )
        asserts.assert_equal(
            set(one_three_clusters.keys()) - set(one_four_two_xml_clusters.keys()),
            one_four_removed.union(one_four_two_removed),
            "There are some 1.3 clusters that are unexpectedly not included in the 1.4.2 spec",
        )
        asserts.assert_equal(
            set(one_four_two_xml_clusters.keys()) - set(one_five_xml_clusters.keys()),
            set(),
    ...
    def test_spec_parsing_access(self):
        ...
        for read in strs:
            for write in strs:
                ...
                asserts.assert_true(ATTRIBUTE_ID in xml_cluster.attributes, "Did not find test attribute in XmlCluster.attributes")
                asserts.assert_equal(
                    xml_cluster.attributes[ATTRIBUTE_ID].read_access, get_access_enum_from_string(read), "Unexpected read access"
                )
                asserts.assert_equal(
                    xml_cluster.attributes[ATTRIBUTE_ID].write_access, get_access_enum_from_string(write), "Unexpected write access"
                )
        for invoke in strs:
            ...
            asserts.assert_true(
                COMMAND_ID in xml_cluster.accepted_commands, "Did not find test command in XmlCluster.accepted_commands"
            )
            asserts.assert_equal(
                xml_cluster.accepted_commands[COMMAND_ID].privilege,
                get_access_enum_from_string(invoke),
                "Unexpected invoke privilege",
            )
    ...
    def test_write_optional(self):
        for write_support in ["true", "optional"]:
            ...
            asserts.assert_true(ATTRIBUTE_ID in xml_cluster.attributes, "Did not find test attribute in XmlCluster.attributes")
            asserts.assert_equal(
                xml_cluster.attributes[ATTRIBUTE_ID].write_optional, write_support == "optional", "Unexpected write_optional value"
            )
    ...
    def test_derived_clusters(self):
        ...
        expected_global_attrs = [
            GlobalAttributeIds.FEATURE_MAP_ID,
            ...
            GlobalAttributeIds.CLUSTER_REVISION_ID,
        ]
        ...
        asserts.assert_equal(
            set(clusters[0xFFFF].attributes.keys()), set([0, 2, 3] + expected_global_attrs), "Unexpected attribute list"
        )
        ...
        asserts.assert_equal(
            set(pure_base_clusters["Test Base"].attributes.keys()),
            set([0, 1, 2, 3] + expected_global_attrs),
            "Unexpected attribute list",
        )
        asserts.assert_equal(set(pure_base_clusters["Test Base"].accepted_commands.keys()), {0}, "Unexpected accepted commands")
        asserts.assert_equal(set(pure_base_clusters["Test Base"].generated_commands.keys()), {1}, "Unexpected generated commands")
        asserts.assert_equal(
            str(pure_base_clusters["Test Base"].accepted_commands[0].conformance),
            "M",
            "Unexpected conformance on base accepted command",
        )
        asserts.assert_equal(
            str(pure_base_clusters["Test Base"].generated_commands[1].conformance),
            "M",
            "Unexpected conformance on base generated command",
        )

        asserts.assert_equal(
            len(pure_base_clusters["Test Base"].unknown_commands), 0, "Unexpected number of unknown commands in base"
        )
        ...
        asserts.assert_equal(
            set(clusters[0xFFFF].attributes.keys()), set([0, 1, 2, 3] + expected_global_attrs), "Unexpected attribute list"
        )
        ...
        asserts.assert_true(set(clusters[0xFFFF].accepted_commands.keys()), {0}, "Unexpected accepted command list after merge")
        asserts.assert_true(set(clusters[0xFFFF].generated_commands.keys()), {1}, "Unexpected generated command list after merge")
        asserts.assert_equal(
            str(clusters[0xFFFF].accepted_commands[0].conformance), "X", "Unexpected conformance on accepted commands"
        )
        asserts.assert_equal(
            str(clusters[0xFFFF].generated_commands[1].conformance), "X", "Unexpected conformance on generated commands"
        )
    ...
    def test_atomic_thermostat(self):
        ...
        asserts.assert_in(
            "Atomic Request",
            one_four_two_xml_clusters[Clusters.Thermostat.id].command_map,
        ...
        asserts.assert_in(
            request_id,
            one_four_two_xml_clusters[Clusters.Thermostat.id].accepted_commands.keys(),
            "Atomic request not found in thermostat accepted command list",
        )

        asserts.assert_in(
            "Atomic Response",
            one_four_clusters[Clusters.Thermostat.id].command_map,
        ...
        asserts.assert_in(
            response_id,
            one_four_clusters[Clusters.Thermostat.id].generated_commands.keys(),
            "Atomic response not found in thermostat generated command list",
        )

        asserts.assert_not_in(
            "Atomic Request",
            one_three_clusters[Clusters.Thermostat.id].command_map,
            "Atomic request found on thermostat command map for 1.3",
        )
        asserts.assert_not_in(
            request_id,
            one_three_clusters[Clusters.Thermostat.id].accepted_commands.keys(),
            "Atomic request found in thermostat accepted command list for 1.3",
        )
        asserts.assert_not_in(
            "Atomic Response",
            one_three_clusters[Clusters.Thermostat.id].command_map,
            "Atomic response found on thermostat command map for 1.3",
        )
        asserts.assert_not_in(
            response_id,
            one_three_clusters[Clusters.Thermostat.id].generated_commands.keys(),
