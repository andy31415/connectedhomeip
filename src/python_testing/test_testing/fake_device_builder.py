...
def _get_field_by_label(
    cl_object: Clusters.ClusterObjects.ClusterObject, label: str
) -> Optional[Clusters.ClusterObjects.ClusterObjectFieldDescriptor]:
...
def create_minimal_cluster(
    ...
    cluster_id: int,
    is_tlv_endpoint: bool = True,
    additional_features: list[uint] = [],
    additional_attributes: list[uint] = [],
    additional_commands: list[uint] = [],
) -> dict[int, Any]:
    attrs = {}
    mandatory_features = [
        mask
        for mask, f in xml_clusters[cluster_id].features.items()
    ...
    mandatory_attributes = [
        id for id, a in xml_clusters[cluster_id].attributes.items() if a.conformance(info).decision == ConformanceDecision.MANDATORY
    ]
    mandatory_attributes.extend(additional_attributes)

    mandatory_accepted_commands = [
        id
        for id, c in xml_clusters[cluster_id].accepted_commands.items()
        if c.conformance(info).decision == ConformanceDecision.MANDATORY
    ]
    mandatory_accepted_commands.extend(additional_commands)

    mandatory_generated_commands = [
        id
        for id, c in xml_clusters[cluster_id].generated_commands.items()
        if c.conformance(info).decision == ConformanceDecision.MANDATORY
    ]
...
def create_minimal_dt(
    xml_clusters: dict[uint, XmlCluster],
    xml_device_types: dict[uint, XmlDeviceType],
    device_type_id: int,
    is_tlv_endpoint: bool = True,
    server_override: Optional[list[uint]] = None,
    apply_dt_element_overrides: bool = True,
    ...
    """
    endpoint = {}
    mandatory_servers = [id for id, c in xml_device_types[device_type_id].server_clusters.items() if _is_mandatory(c.conformance)]
    if server_override:
        required_servers = server_override
    else:
        required_servers = mandatory_servers

    required_clients = [id for id, c in xml_device_types[device_type_id].client_clusters.items() if _is_mandatory(c.conformance)]
    ...
    for s in required_servers:
        ...
        if apply_dt_element_overrides and s in mandatory_servers:
            additional_features = [
                id
                for id, conformance in xml_device_types[device_type_id].server_clusters[s].feature_overrides.items()
                if _is_mandatory(conformance, revision=device_type_revision)
            ]
            additional_attributes = [
                id
                for id, conformance in xml_device_types[device_type_id].server_clusters[s].attribute_overrides.items()
                if _is_mandatory(conformance, revision=device_type_revision)
            ]
            additional_commands = [
                id
                for id, conformance in xml_device_types[device_type_id].server_clusters[s].command_overrides.items()
                if _is_mandatory(conformance, revision=device_type_revision)
            ]
        endpoint[s if is_tlv_endpoint else Clusters.ClusterObjects.ALL_CLUSTERS[s]] = create_minimal_cluster(
            xml_clusters,
            s,
            is_tlv_endpoint,
    ...
    attribute_values = [
        ...
        (xml_clusters[Clusters.Descriptor.id].revision, xml_clusters[Clusters.Descriptor.id].revision),  # ClusterRevision
        (
            [
                {
