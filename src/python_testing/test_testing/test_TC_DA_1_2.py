...
def run_single_test(
    dac_provider: str, product_id: int, factory_reset: bool = False, additional_script_args: Optional[str] = None
) -> int:
    reset = ""
    if factory_reset:
        reset = " --factory-reset"
    ...
    app_args = (
        "--discriminator 1234 --KVS kvs1 --product-id " + str(product_id) + " --vendor-id 65521 --dac_provider " + dac_provider
    )
