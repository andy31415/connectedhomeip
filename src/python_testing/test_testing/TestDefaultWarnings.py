...
class TestDefaultChecker(MatterBaseTest):
    ...
    def test_calendar_is_not_default_check(self):
        ...
        def run_check(
            calendar_type: Clusters.TimeFormatLocalization.Enums.CalendarTypeEnum, set_override: bool, expect_problem: bool
        ):
            self.test.user_params = {DefaultChecker.FLAG_DEFAULT_CALENDAR_FORMAT: set_override}
            self.test.problems = []
            self.test.endpoints = {
                0: {Clusters.TimeFormatLocalization: {Clusters.TimeFormatLocalization.Attributes.ActiveCalendarType: calendar_type}}
            }
        ...
        self.test.check_default_calendar_format()
        asserts.assert_equal(
            len(self.test.problems), 0, "Unexpected problem when testing device with no TimeFormatLocalization cluster"
        )
        ...
        self.test.check_default_calendar_format()
        asserts.assert_equal(
            len(self.test.problems), 0, "Unexpected problem when testing device with no TimeFormatLocalization cluster and override"
        )
        # Should have marked this as skipped
        asserts.assert_equal(self.skipped, 1, "Some override tests did not mark steps skipped")
        ...
        self.test.check_default_calendar_format()
        asserts.assert_equal(
            len(self.test.problems), 0, "Unexpected problem when testing device with no ActiveCalendarType attribute"
        )
        # Should have marked this as skipped
        ...
        self.test.check_default_calendar_format()
        asserts.assert_equal(
            len(self.test.problems), 0, "Unexpected problem when testing device with no ActiveCalendarType cluster and override"
        )
        # Should have marked this as skipped
        asserts.assert_equal(self.skipped, 1, "Override test did not mark steps skipped")
    ...
    def _cluster_presence_test(self, cluster: Clusters.ClusterObjects.Cluster, fn: Callable, flag: str):
        def test_endpoint(endpoint: int):
            ...
            self.test.endpoints = {
                endpoint: {cluster: {}, Clusters.Descriptor: {Clusters.Descriptor.Attributes.ServerList: [cluster.id]}}
            }
            fn()
            asserts.assert_equal(
                len(self.test.problems),
            ...
            asserts.assert_equal(
                len(self.test.problems),
                0,
                f"Unexpected problem notification when testing with {cluster} on endpoint {endpoint} and override {flag} set",
            )
    ...
    def test_unit_testing_is_not_present_check(self):
        self._cluster_presence_test(
            Clusters.UnitTesting, self.test.check_unit_testing_cluster_presence, DefaultChecker.FLAG_UNIT_TESTING
        )

    def test_fault_injection_is_not_present_check(self):
        self._cluster_presence_test(
            Clusters.FaultInjection, self.test.check_fault_injection_cluster_presence, DefaultChecker.FLAG_FAULT_INJECTION
        )

    def test_sample_mei_is_not_present_check(self):
        self._cluster_presence_test(Clusters.SampleMei, self.test.check_sample_mei_cluster_presence, DefaultChecker.FLAG_SAMPLE_MEI)
    ...
    def test_fixed_label_is_not_empty_check(self):
        ...
        self.test.endpoints = {0: {}}
        self.test.check_fixed_label_cluster_empty()
        asserts.assert_equal(len(self.test.problems), 0, "Unexpected problem when testing device with no FixedLabel cluster")
        # Should have marked this as skipped
        asserts.assert_equal(self.skipped, 1, "Some override tests did not mark steps skipped")
    ...
    def test_fixed_label_is_not_default_check(self):
        ...
        self.test.check_fixed_label_cluster_defaults()
        asserts.assert_equal(
            len(self.test.problems), 0, "Unexpected problem when testing device with no FixedLabel cluster and override"
        )
        # Should have marked this as skipped
        asserts.assert_equal(self.skipped, 1, "Some override tests did not mark steps skipped")
