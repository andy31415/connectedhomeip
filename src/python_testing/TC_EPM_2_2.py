...
class TC_EPM_2_2(MatterBaseTest, EnergyReportingBaseTestHelper):
    ...
    def steps_TC_EPM_2_2(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "6",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.EPM.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.EPM.TEST_EVENT_TRIGGER for Stop Fake Readings Test Event.",
            ),
        ]
    ...
    async def test_TC_EPM_2_2(self):
        ...
        asserts.assert_not_equal(
            active_power, active_power2, f"Expected ActivePower readings to have changed {active_power}, {active_power2}"
        )
