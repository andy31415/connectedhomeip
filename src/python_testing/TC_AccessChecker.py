...
def step_number_with_privilege(
    step: int, substep: str, privilege: Clusters.AccessControl.Enums.AccessControlEntryPrivilegeEnum
) -> str:
    return f"{step}{substep}_{privilege.name}"


def operation_allowed(
    spec_requires: Clusters.AccessControl.Enums.AccessControlEntryPrivilegeEnum,
    acl_set_to: Optional[Clusters.AccessControl.Enums.AccessControlEntryPrivilegeEnum],
) -> bool:
...
def checkable_attributes(cluster_id, cluster, xml_cluster) -> list[uint]:
    ...
    def is_known_cluster_attribute(attribute_id) -> bool:
        ...
        return (
            is_standard_attribute_id(attribute_id)
            and attribute_id in xml_cluster.attributes
            and attribute_id in Clusters.ClusterObjects.ALL_ATTRIBUTES[cluster_id]
        )
...
def checkable_commands(cluster_id, cluster, xml_cluster) -> list[uint]:
    ...
    def is_known_cluster_cmd(command_id) -> bool:
        ...
        return (
            is_standard_command_id(command_id)
            and command_id in xml_cluster.accepted_commands
            and command_id in Clusters.ClusterObjects.ALL_ACCEPTED_COMMANDS[cluster_id]
        )
...
class AccessChecker(BasicCompositionTests):
    ...
    async def setup_class(self):
        ...
        self.default_acl = await self.read_single_attribute_check_success(
            endpoint=0, cluster=Clusters.AccessControl, attribute=acl_attr
        )
        self._record_errors()
    ...
    async def teardown_test(self):
        await self.default_controller.WriteAttribute(
            self.dut_node_id, attributes=[(0, Clusters.AccessControl.Attributes.Acl(self.default_acl))]
        )
    ...
    async def _setup_acl(self, privilege: Optional[Clusters.AccessControl.Enums.AccessControlEntryPrivilegeEnum]):
        ...
        new_entry = Clusters.AccessControl.Structs.AccessControlEntryStruct(
            ...
            subjects=[self.TH2_nodeid],
        )
        new_acl.append(new_entry)
        await self.default_controller.WriteAttribute(
            self.dut_node_id, attributes=[(0, Clusters.AccessControl.Attributes.Acl(new_acl))]
        )
        ...
        for endpoint_id, endpoint in self.endpoints_tlv.items():
            ...
            for cluster_id, device_cluster_data in endpoint.items():
                ...
                if GlobalAttributeIds.ATTRIBUTE_LIST_ID not in device_cluster_data:
                    ...
                    self.record_error(
                        test_name="Access Checker", location=location, problem="Cluster does not have the AttributeList attribute"
                    )
                else:
                ...
                if GlobalAttributeIds.ACCEPTED_COMMAND_LIST_ID not in device_cluster_data:
                    ...
                    cmds[cluster_id].update(
                        {
                            id
                            for id in device_cluster_data[GlobalAttributeIds.ACCEPTED_COMMAND_LIST_ID]
                            if is_standard_command_id(id)
                        }
                    )
        ...
        for cluster_id in all_clusters:
            ...
            for attribute_id in attrs[cluster_id]:
                ...
                if attribute_id not in xml_cluster.attributes:
                    self.record_error(
                        test_name="Access Checker", location=location, problem="Cluster attribute not found in spec XML"
                    )
                    self.success = False
                    continue
                if attribute_id not in Clusters.ClusterObjects.ALL_ATTRIBUTES[cluster_id]:
                    self.record_error(test_name="Access Checker", location=location, problem="Unknown attribute")
                    self.success = False
                    continue
            ...
            for command_id in cmds[cluster_id]:
                ...
                if command_id not in xml_cluster.accepted_commands:
                    self.record_error(
                        test_name="Access Checker", location=location, problem="Cluster command not found in spec XML"
                    )
                    self.success = False
                    continue
                if command_id not in Clusters.ClusterObjects.ALL_ACCEPTED_COMMANDS[cluster_id]:
                    self._record_error(test_name="Access Checker", location=location, problem="Unknown command")
                    self.success = False
                    continue

    async def _maybe_run_command_access_test_for_cluster_privilege(
        self,
        endpoint_id,
        cluster_id,
        device_cluster_data,
        xml_cluster: XmlCluster,
    ...
    async def _run_read_access_test_for_cluster_privilege(
        self,
        endpoint_id,
        cluster_id,
        device_cluster_data,
        ...
        for attribute_id in checkable_attributes(cluster_id, device_cluster_data, xml_cluster):
            ...
            if operation_allowed(spec_requires, privilege):
                ret = await self.read_single_attribute_check_success(
                    ...
                    test_name=f"Read access Checker - {privilege}",
                )
                if ret is None:
                    self.success = False
            else:
                ret = await self.read_single_attribute_expect_error(
                    dev_ctrl=self.TH2,
                    endpoint=endpoint_id,
                    cluster=cluster_class,
                    attribute=attribute,
    ...
    async def _subscribe_single_attribute_check_success(
        self, endpoint_id, cluster_id, attribute_id, attribute, cluster_class, privilege, test_name
    ):
    ...
    async def _subscribe_single_attribute_expect_error(
        self, endpoint_id, cluster_id, attribute_id, attribute, cluster_class, privilege, test_name
    ):
    ...
    async def _run_subscribe_access_test_for_cluster_privilege(
        self,
        endpoint_id,
        cluster_id,
        device_cluster_data,
    ...
    async def _run_write_access_test_for_cluster_privilege(
        self,
        endpoint_id,
        cluster_id,
        cluster,
        ...
        for attribute_id in checkable_attributes(cluster_id, cluster, xml_cluster):
            ...
            if (
                ...
                and privilege == Clusters.AccessControl.Enums.AccessControlEntryPrivilegeEnum.kAdminister
            ):
                log.info(
                    "Skipping ACL attribute check for admin privilege as this is known to be writeable and is being used for this test"
                )
                continue
            ...
            if spec_requires == Clusters.AccessControl.Enums.AccessControlEntryPrivilegeEnum.kUnknownEnumValue:
                ...
                if resp[0].Status == Status.UnsupportedAccess:
                    self.record_error(
                        test_name=test_name, location=location, problem="Unexpected UnsupportedAccess writing attribute"
                    )
                    self.success = False
            else:
                if resp[0].Status != Status.UnsupportedAccess:
    ...
    async def run_access_test(self, test_type: AccessTestType):
        ...
        for privilege in privilege_enum:
            ...
            for endpoint_id, endpoint in self.endpoints_tlv.items():
                for cluster_id, device_cluster_data in endpoint.items():
                    if (
                        not is_standard_cluster_id(cluster_id)
                        or cluster_id not in self.xml_clusters
                        or cluster_id not in Clusters.ClusterObjects.ALL_ATTRIBUTES
                    ):
                        # These cases have already been recorded by the _record_errors function
                        continue
                    xml_cluster = self.xml_clusters[cluster_id]
                    if test_type == AccessTestType.READ:
                        await self._run_read_access_test_for_cluster_privilege(
                            endpoint_id, cluster_id, device_cluster_data, xml_cluster, privilege
                        )
                    elif test_type == AccessTestType.WRITE:
                        await self._run_write_access_test_for_cluster_privilege(
                            endpoint_id, cluster_id, device_cluster_data, xml_cluster, privilege, wildcard_read
                        )
                    elif test_type == AccessTestType.INVOKE:
                        await self._maybe_run_command_access_test_for_cluster_privilege(
                            endpoint_id, cluster_id, device_cluster_data, xml_cluster, privilege
                        )
                    elif test_type == AccessTestType.SUBSCRIBE:
                        await self._run_subscribe_access_test_for_cluster_privilege(
                            endpoint_id, cluster_id, device_cluster_data, xml_cluster, privilege
                        )
                    else:
    ...
    def steps_TC_ACE_2_1(self):
        ...
        for p in privilege_enum:
            steps.append(
                TestStep(
                    step_number_with_privilege(3, "a", p), "TH_commissioner gives TH_second_commissioner the specified privilege"
                )
            )
            steps.append(
                TestStep(
                    step_number_with_privilege(3, "b", p),
    ...
    def steps_TC_ACE_2_2(self):
        ...
        for p in privilege_enum:
            steps.append(
                TestStep(
                    step_number_with_privilege(5, "a", p), "TH_commissioner gives TH_second_commissioner the specified privilege"
                )
            )
            steps.append(
                TestStep(
                    step_number_with_privilege(5, "b", p),
    ...
    def steps_TC_ACE_2_3(self):
        ...
        for p in privilege_enum:
            steps.append(
                TestStep(
                    step_number_with_privilege(3, "a", p), "TH_commissioner gives TH_second_controller the specified privilege"
                )
            )
            steps.append(
                TestStep(
