...
class TC_DRLK_2_9(MatterBaseTest, DRLK_COMMON):
    def steps_TC_DRLK_2_9(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "1", "TH reads NumberOfTotalUsersSupported attribute.", "Verify that TH is able to read the attribute successfully."
            ),
            TestStep("2a", "TH sends SetUser Command to DUT.", "Verify that the DUT sends SUCCESS response"),
            TestStep(
                "2b",
                "TH reads MinPINCodeLength attribute .",
                "Verify that TH is able to read the attribute successfully and value is within range.",
            ),
            TestStep(
                "2c",
                "TH reads MaxPINCodeLength attribute.",
                "Verify that TH is able to read the attribute successfully and value is within range.",
            ),
            TestStep("2d", "TH reads MinRFIDCodeLength attribute.", "Verify that TH is able to read the attribute successfully."),
            TestStep(
                "2e",
                "TH reads MaxRFIDCodeLength attribute.",
                "Verify that TH is able to read the attribute successfully and value is within range.",
            ),
            TestStep(
                "2f",
                "TH sends SetCredential Command to DUT.",
                "Verify that the DUT responds with SetCredentialResponse command with Status SUCCESS.",
            ),
            TestStep(
                "3",
                "TH sends GetCredentialStatus Command .",
            ...
            TestStep("8", "TH sends ClearCredential Command to DUT.", "Verify that the DUT sends SUCCESS response."),
            TestStep(
                "9a",
                "TH sends GetCredentialStatus command to DUT ",
                "Verify that the DUT responds with  GetCredentialStatusResponse Command.",
            ),
            TestStep("9b", "TH sends ClearUser Command to DUT.", "Verify that the DUT sends SUCCESS response"),
            TestStep("10", "TH sends SetUser Command to DUT.", "Verify that the DUT sends SUCCESS response."),
            TestStep(
                "11",
                "TH sends SetCredential Command to DUT.",
                "Verify that the DUT responds with SetCredentialResponse command with Status SUCCESS.",
            ),
            TestStep("12a", "TH sends ClearCredential Command to DUT.", " Verify that the DUT sends SUCCESS response."),
            TestStep(
                "13",
                " TH sends GetCredentialStatus Command.",
            ...
            TestStep(
                "14a",
                "TH sends ClearCredential Command to DUT",
                "if {PICS_SF_ALIRO} verify that the DUT sends a SUCCESS response else DUT sends an INVALID_COMMAND response.",
            ),
            TestStep(
                "14b",
                "TH sends ClearCredential Command to DUT with Invalid Credential Type.",
                "Verify that the DUT sends an INVALID_COMMAND.",
            ),
            TestStep(
                "14c", "TH sends ClearUser Command to DUT to clear all the users.", "Verify that the DUT sends SUCCESS response."
            ),
            TestStep(
                "14d",
                "TH reads NumberOfPINUsersSupported attribute.",
                "Verify that TH is able to read the attribute successfully and value is within range.",
            ),
            TestStep(
                "15a",
                "TH reads NumberOfCredentialsSupportedPerUser attribute from DUT.",
                "Verify that TH is able to read the attribute successfully and value is within range.",
            ),
            TestStep("15b", "TH sends SetUser Command to DUT.", "Verify that the DUT sends SUCCESS response."),
            TestStep(
                "15c",
                "TH sends SetCredential Command to DUT.",
            ...
            TestStep(
                "15d",
                " TH sends SetCredential Command  with CredentialIndex as 'max-num-credential-user'.",
                "Verify that the DUT sends SetCredentialResponse command with Status RESOURCE_EXHAUSTION.",
            ),
            TestStep(
                "15e",
                "TH sends ClearCredential Command to DUT to clear all the credentials of PIN type.",
                "Verify that the DUT sends SUCCESS response.",
            ),
            TestStep(
                "15f",
                "TH sends ClearUser Command to DUT with UserIndex as 0xFFFE to clear all the users.",
                "Verify that the DUT sends SUCCESS response.",
            ),
            TestStep("16", " TH sends SetUser Command to DUT.", "Verify that the DUT sends SUCCESS response"),
            TestStep(
                "17",
                "TH sends SetCredential Command to DUT with CredentialType.RFID.",
                "Verify that the DUT sends SetCredentialResponse command  with Status SUCCESS.",
            ),
            TestStep(
                "18",
                "TH sends SetCredential Command to DUT with CredentialType.PIN.",
                "Verify that the DUT sends SetCredentialResponse command  with Status SUCCESS.",
            ),
            TestStep("19", "TH sends GetUser Command to DUT.", "Verify that the DUT responds with GetUserResponse Command."),
            TestStep(
                "20",
                "TH sends SetCredential Command to DUT to modify the CredentialData.",
                "Verify that the DUT sends SetCredentialResponse command  with Status SUCCESS.",
            ),
            TestStep("21", "TH sends GetUser Command to DUT.", "Verify that the DUT responds with GetUserResponse Command."),
            TestStep(
                "22",
                "TH sends ClearCredential Command to DUT to clear all the credentials.",
                "Verify that the DUT sends SUCCESS response.",
            ),
            TestStep(
                "23", "TH sends ClearUser Command to DUT to clear all the users.", "Verify that the DUT sends SUCCESS response."
            ),
            TestStep("24", "TH sends SetUser Command to DUT.", "Verify that the DUT sends SUCCESS response."),
            TestStep("25", "TH sends ClearAliroReaderConfig Command to DUT.", "Verify that the DUT sends SUCCESS response."),
            TestStep(
                "26",
                ",TH sends SetAliroReaderConfig Command to DUT without GroupResolvingKey.",
                "Verify that DUT sends success response.",
            ),
            TestStep(
                "27",
                ",TH sends SetAliroReaderConfig Command to DUT with GroupResolvingKey.",
                "Verify that DUT sends success response.",
            ),
            TestStep(
                "28",
                "TH sends SetCredential Command CredentialType as AliroEvictableEndpointKey.",
            ...
            TestStep("30", "TH sends GetUser Command to DUT.", "Verify that the DUT sends SUCCESS response."),
            TestStep(
                "31",
                "TH sends SetCredential Command to modify PIN CredentialType",
                "Verify that the DUT responds with SetCredentialResponse command with status success ",
            ),
            TestStep("32", "TH sends GetUser Command to DUT.", "Verify that the DUT sends SUCCESS response."),
            TestStep(
                "33",
                "TH sends SetCredential Command to modify AliroEvictableEndpointKey CredentialType",
                "Verify that the DUT responds with SetCredentialResponse command with status success ",
            ),
            TestStep("34", "TH sends GetUser Command to DUT.", "Verify that the DUT sends SUCCESS response."),
            TestStep("35", "TH sends ClearUser Command to DUT.", "Verify that the DUT sends SUCCESS response."),
            TestStep(
                "36",
                "TH sends ClearCredential Command to DUT to clear all the credentials.",
                "Verify that the DUT sends SUCCESS response.",
            ),
            TestStep("37", "TH sends ClearAliroReaderConfig Command to DUT.", "Verify that the DUT sends SUCCESS response."),
        ]
    ...
    async def read_attributes_from_dut(self, endpoint, cluster, attribute, expected_status: Status = Status.Success):
        try:
            ...
            log.error(e)
            asserts.assert_equal(expected_status, Status.Success, f"Error reading attributes, response={attribute_value}")
    ...
    async def get_user(self, userindex, username, useruniqueid, credentiallist, expected_status: Status = Status.Success):
        try:
            response = await self.send_single_cmd(
                cmd=Clusters.DoorLock.Commands.GetUser(userIndex=userindex),
                endpoint=self.app_cluster_endpoint,
                timedRequestTimeoutMs=1000,
            )

            asserts.assert_true(
                matchers.is_type(response, Clusters.DoorLock.Commands.GetUserResponse), "Unexpected return type for GetUserResponse"
            )
            asserts.assert_true(
                response.userIndex == userindex,
                "Error when executing GetUserResponse command, userIndex={}".format(str(response.userIndex)),
            )
            asserts.assert_true(
                response.userName == username,
                "Error when executing GetUserResponse command, userName={}".format(str(response.userName)),
            )
            asserts.assert_true(
                response.userUniqueID == useruniqueid,
                "Error when executing GetUserResponse command, userUniqueID={}".format(str(response.userUniqueID)),
            )
            ...
            asserts.assert_equal(
                ...
                "Error mismatch in expected credential from GetUserResponse command = {}".format(str(credentiallist)),
            )
    ...
    async def get_credentials_status(
        self, credentialIndex: int, credentialType: drlkcluster.Enums.CredentialTypeEnum, credential_exists, next_credential_index
    ):
        try:
            credentials_struct = drlkcluster.Structs.CredentialStruct(
                credentialIndex=credentialIndex, credentialType=credentialType
            )
            response = await self.send_single_cmd(
                endpoint=self.app_cluster_endpoint,
                timedRequestTimeoutMs=1000,
                cmd=drlkcluster.Commands.GetCredentialStatus(credential=credentials_struct),
            )
            asserts.assert_true(
                matchers.is_type(response, Clusters.DoorLock.Commands.GetCredentialStatusResponse),
                "Unexpected return type for GetCredentialStatus",
            )
            asserts.assert_true(
                response.credentialExists == credential_exists,
                "Error when executing GetCredentialStatus command, credentialExists={}".format(str(response.credentialExists)),
            )
            if not credential_exists:
                asserts.assert_true(
                    response.userIndex == NullValue,
                    "Error when executing GetCredentialStatus command, userIndex={}".format(str(response.userIndex)),
                )
            asserts.assert_true(
                response.nextCredentialIndex == next_credential_index,
    ...
    async def set_credential_cmd(
        self,
        credential_enum: drlkcluster.Enums.CredentialTypeEnum,
        statuscode,
        credentialIndex,
        operationType,
        userIndex,
        ...
        credentials = drlkcluster.Structs.CredentialStruct(credentialType=credential_enum, credentialIndex=credentialIndex)
        try:
            log.info("Credential Data is %s" % (credentialData))
            response = await self.send_single_cmd(
                cmd=drlkcluster.Commands.SetCredential(
            ...
            asserts.assert_true(
                matchers.is_type(response, drlkcluster.Commands.SetCredentialResponse), "Unexpected return type for SetCredential"
            )
            asserts.assert_equal(response.userIndex, NullValue)
            if statuscode != custom_status_code:
                asserts.assert_true(
                    response.status == statuscode, "Error sending SetCredential command, status={}".format(str(response.status))
                )
            else:
                asserts.assert_true(
                    response.status == 2 or response.status == 3,
    ...
    async def send_clear_aliro_reader_config_cmd(self, expected_status: Status = Status.Success):
        try:
            await self.send_single_cmd(
                cmd=Clusters.DoorLock.Commands.ClearAliroReaderConfig(),
                endpoint=self.app_cluster_endpoint,
                timedRequestTimeoutMs=1000,
            )
            asserts.assert_equal(expected_status, Status.Success)
    ...
    async def send_set_aliro_reader_config_cmd(self, use_group_resolving_key: bool, expected_status: Status = Status.Success):
        try:
            ...
            if use_group_resolving_key is False:
                pics_check = self.pics_guard(
                    self.check_pics("DRLK.S.F0d") and not self.check_pics("DRLK.S.F0e") and self.check_pics("DRLK.S.C28.Rsp")
                )
            else:
            ...
            if not use_group_resolving_key and pics_check:
                await self.send_single_cmd(
                    cmd=Clusters.DoorLock.Commands.SetAliroReaderConfig(
                        signingKey=signingKey, verificationKey=verificationKey, groupIdentifier=groupIdentifier
                    ),
                    endpoint=self.app_cluster_endpoint,
                    timedRequestTimeoutMs=1000,
                )
    ...
    async def test_TC_DRLK_2_9(self):
        ...
        if self.pics_guard(self.check_pics("DRLK.S.F08") and self.check_pics("DRLK.S.A0011")):
            self.numberoftotaluserssupported = await self.read_attributes_from_dut(
                endpoint=self.app_cluster_endpoint,
                cluster=drlkcluster,
                attribute=Clusters.DoorLock.Attributes.NumberOfTotalUsersSupported,
            )
            asserts.assert_in(
                self.numberoftotaluserssupported, range(0, 65534), "NumberOfTotalUsersSupported value is out of range"
            )
        self.step("2a")
        ...
        if self.pics_guard(self.check_pics("DRLK.S.F08") and self.check_pics("DRLK.S.F00")):
            self.minpincodelength = await self.read_attributes_from_dut(
                endpoint=self.app_cluster_endpoint, cluster=drlkcluster, attribute=Clusters.DoorLock.Attributes.MinPINCodeLength
            )
            asserts.assert_in(self.minpincodelength, range(0, 255), "MinPINCodeLength value is out of range")
        self.step("2c")
        if self.pics_guard(self.check_pics("DRLK.S.F08") and self.check_pics("DRLK.S.F00")):
            self.maxpincodelength = await self.read_attributes_from_dut(
                endpoint=self.app_cluster_endpoint, cluster=drlkcluster, attribute=Clusters.DoorLock.Attributes.MaxPINCodeLength
            )
            asserts.assert_in(self.maxpincodelength, range(0, 255), "MaxPINCodeLength value is out of range")
        self.step("2d")
        if self.pics_guard(self.check_pics("DRLK.S.F01")):
            self.minrfidcodelength = await self.read_attributes_from_dut(
                endpoint=self.app_cluster_endpoint, cluster=drlkcluster, attribute=Clusters.DoorLock.Attributes.MinRFIDCodeLength
            )
            asserts.assert_in(self.minrfidcodelength, range(0, 255), "MinRFIDCodeLength value is out of range")
        self.step("2e")
        if self.pics_guard(self.check_pics("DRLK.S.F01")):
            self.maxrfidcodelength = await self.read_attributes_from_dut(
                endpoint=self.app_cluster_endpoint, cluster=drlkcluster, attribute=Clusters.DoorLock.Attributes.MaxRFIDCodeLength
            )
            asserts.assert_in(self.maxrfidcodelength, range(0, 255), "MaxRFIDCodeLength value is out of range")
        self.step("2f")
        await self.generate_code()
        if self.pics_guard(
            self.check_pics("DRLK.S.F00")
            and self.check_pics("DRLK.S.F08")
            and self.check_pics("DRLK.S.C22.Rsp")
            and self.check_pics("DRLK.S.C23.Tx")
        ):
            await self.set_credential_cmd(
                credentialData=self.pin_code,
                operationType=drlkcluster.Enums.DataOperationTypeEnum.kAdd,
                credential_enum=drlkcluster.Enums.CredentialTypeEnum.kPin,
                credentialIndex=credentialIndex_1,
                userIndex=userIndex_1,
        ...
        if self.pics_guard(
            self.check_pics("DRLK.S.F00")
            and self.check_pics("DRLK.S.F08")
            and self.check_pics("DRLK.S.C24.Rsp")
            and self.check_pics("DRLK.S.C25.Tx")
        ):
            await self.get_credentials_status(
                credentialIndex=credentialIndex_1,
                credentialType=drlkcluster.Enums.CredentialTypeEnum.kPin,
                credential_exists=True,
                next_credential_index=NullValue,
            )
        self.step("4")
        if self.pics_guard(
            self.check_pics("DRLK.S.F00")
            ...
            await self.set_credential_cmd(
                credentialData=self.inavlid_pincode,
                operationType=drlkcluster.Enums.DataOperationTypeEnum.kAdd,
                credential_enum=drlkcluster.Enums.CredentialTypeEnum.kPin,
                credentialIndex=credentialIndex_2,
                userIndex=NullValue,
                userStatus=invalid_user_status,
        ...
        if self.pics_guard(
            ...
            await self.set_credential_cmd(
                credentialData=self.pin_code2,
                operationType=drlkcluster.Enums.DataOperationTypeEnum.kModify,
                credential_enum=drlkcluster.Enums.CredentialTypeEnum.kPin,
                credentialIndex=credentialIndex_3,
                userIndex=userIndex_1,
                userStatus=NullValue,
                userType=NullValue,
                statuscode=Status.InvalidCommand,
        ...
        if self.pics_guard(self.check_pics("DRLK.S.F00") and self.check_pics("DRLK.S.F08") and self.check_pics("DRLK.S.C26.Rsp")):
            credentials = drlkcluster.Structs.CredentialStruct(
                credentialIndex=1, credentialType=drlkcluster.Enums.CredentialTypeEnum.kPin
            )
        ...
        if self.pics_guard(self.check_pics("DRLK.S.F00") and self.check_pics("DRLK.S.F08") and self.check_pics("DRLK.S.C26.Rsp")):
            credentials = drlkcluster.Structs.CredentialStruct(
                credentialIndex=0xFFFE, credentialType=drlkcluster.Enums.CredentialTypeEnum.kPin
            )
            await self.clear_credentials_cmd(credential=credentials)
        ...
        if self.pics_guard(self.check_pics("DRLK.S.F08") and self.check_pics("DRLK.S.C26.Rsp")):
            feature_map = await self.read_attributes_from_dut(
                endpoint=self.app_cluster_endpoint, cluster=drlkcluster, attribute=Clusters.DoorLock.Attributes.FeatureMap
            )
            aliro_enabled = feature_map & Clusters.DoorLock.Bitmaps.Feature.kAliroProvisioning
            credentials = drlkcluster.Structs.CredentialStruct(
                credentialIndex=credentialIndex_1, credentialType=drlkcluster.Enums.CredentialTypeEnum.kAliroNonEvictableEndpointKey
            )
            if aliro_enabled:
                await self.clear_credentials_cmd(credential=credentials)
            else:
                try:
                    await self.send_single_cmd(
                        cmd=Clusters.DoorLock.Commands.ClearCredential(credential=credentials),
                        endpoint=self.app_cluster_endpoint,
                        timedRequestTimeoutMs=1000,
                    )
        ...
        if self.pics_guard(self.check_pics("DRLK.S.F08") and self.check_pics("DRLK.S.C26.Rsp")):
            credentials = drlkcluster.Structs.CredentialStruct(
                credentialIndex=credentialIndex_1, credentialType=invalid_credential_type
            )
            try:
                await self.send_single_cmd(
                    cmd=Clusters.DoorLock.Commands.ClearCredential(credential=credentials),
                ...
                asserts.assert_equal(
                    e.status, Status.InvalidCommand, "Unexpected error sending ClearCredential Command  with invalid CredentialTpe"
                )
        ...
        if self.pics_guard(self.check_pics("DRLK.S.F00") and self.check_pics("DRLK.S.A0012")):
            num_pin_users_supported = await self.read_attributes_from_dut(
                endpoint=self.app_cluster_endpoint,
                cluster=drlkcluster,
                attribute=Clusters.DoorLock.Attributes.NumberOfPINUsersSupported,
            )
            asserts.assert_in(num_pin_users_supported, range(0, 65534), "NumberOfPINUsersSupported value is out of range")
        self.step("15a")
        if self.pics_guard(self.check_pics("DRLK.S.F00") and self.check_pics("DRLK.S.F08") and self.check_pics("DRLK.S.A001c")):
            numberofcredentialsupportedperuser = await self.read_attributes_from_dut(
                endpoint=self.app_cluster_endpoint,
                cluster=drlkcluster,
                attribute=Clusters.DoorLock.Attributes.NumberOfCredentialsSupportedPerUser,
            )
            asserts.assert_in(
                numberofcredentialsupportedperuser, range(0, 255), "NumberOfCredentialsSupportedPerUser value is out of range"
            )
        self.step("15b")
        if self.pics_guard(self.check_pics("DRLK.S.F08") and self.check_pics("DRLK.S.C1a.Rsp")):
            try:
            ...
            if numberofcredentialsupportedperuser < num_pin_users_supported:
                ...
                while 1:
                    ...
                    if start_credential_index <= (numberofcredentialsupportedperuser):
                        nextCredentialIndex = await self.set_credential_cmd(
                            credentialData=uniquePincode,
                            operationType=drlkcluster.Enums.DataOperationTypeEnum.kAdd,
                            credential_enum=drlkcluster.Enums.CredentialTypeEnum.kPin,
                        ...
                        start_credential_index += 1
                        asserts.assert_true(
                            nextCredentialIndex == start_credential_index,
        ...
        self.step("17")
        if self.pics_guard(
            self.check_pics("DRLK.S.F01") and self.check_pics("DRLK.S.C22.Rsp") and self.check_pics("DRLK.S.C23.Tx")
        ):
            await self.set_credential_cmd(
                credentialData=self.rfid_tag,
                operationType=drlkcluster.Enums.DataOperationTypeEnum.kAdd,
        ...
        self.step("18")
        if self.pics_guard(
            self.check_pics("DRLK.S.F00") and self.check_pics("DRLK.S.C22.Rsp") and self.check_pics("DRLK.S.C23.Tx")
        ):
            await self.set_credential_cmd(
                credentialData=self.pin_code,
                operationType=drlkcluster.Enums.DataOperationTypeEnum.kAdd,
        ...
        if self.pics_guard(self.check_pics("DRLK.S.F00") and self.check_pics("DRLK.S.F01") and self.check_pics("DRLK.S.C1b.Rsp")):
            ...
            credentiallist = [
                drlkcluster.Structs.CredentialStruct(
                    credentialIndex=credentialIndex_1, credentialType=drlkcluster.Enums.CredentialTypeEnum.kRfid
                ),
                drlkcluster.Structs.CredentialStruct(
                    credentialIndex=credentialIndex_1, credentialType=drlkcluster.Enums.CredentialTypeEnum.kPin
                ),
            ]
        ...
        self.step("28")
        if self.pics_guard(
            self.check_pics("DRLK.S.F0d") and self.check_pics("DRLK.S.C22.Rsp") and self.check_pics("DRLK.S.C23.Tx")
        ):
            await self.set_credential_cmd(
                credentialData=aliroevictableendpointkey1,
                operationType=drlkcluster.Enums.DataOperationTypeEnum.kAdd,
        ...
        if self.pics_guard(
            ...
            await self.set_credential_cmd(
                credentialData=aliroevictableendpointkey2,
                operationType=drlkcluster.Enums.DataOperationTypeEnum.kModify,
                credential_enum=drlkcluster.Enums.CredentialTypeEnum.kAliroEvictableEndpointKey,
                credentialIndex=credentialIndex_1,
                userIndex=userIndex_1,
