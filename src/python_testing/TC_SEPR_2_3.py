...
class TC_SEPR_2_3(CommodityPriceTestBaseHelper, MatterBaseTest):
    ...
    def steps_TC_SEPR_2_3(self) -> list[TestStep]:
        ...
        return [
            ...
            TestStep(
                "1a", "Create CASE session connection via TCP if the DUT claims to support TCP", "TCP connection established OK"
            ),
            TestStep(
            ...
            TestStep(
                "4",
                "TH sends command GetDetailedForecastRequest with Details=CommodityPriceDetailBitmap.Description set to False and Components set to True.",
                """Verify that the DUT response contains GetDetailedForecastResponse with a list of CommodityPriceStruct entries (it may be empty) and shall have not more than 56 entries.
                        For each entry in the list:
            ...
            TestStep(
                "5",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.SEPR.TESTEVENT_TRIGGERKEY and EventTrigger field set to PIXIT.SEPR.TESTEVENTTRIGGER for Forecast Update Test Event",
            ...
            TestStep(
                "8",
                "TH sends command GetDetailedForecastRequest with Details=CommodityPriceDetailBitmap.Description set to True and Components set to True.",
                """Verify that the DUT response contains GetDetailedForecastResponse with a list of CommodityPriceStruct entries (it may be empty) and shall have not more than 56 entries.
                        For each entry in the list:
                        - except for the first list entry, verify that the PeriodStart is greater than the PeriodEnd of the list entry.
                        - verify that the Price is null or Money type.
    ...
    async def test_TC_SEPR_2_3(self):
        ...
        if tcp_support:
            ...
            self.check_CommodityPriceForecast(cluster=cluster, priceForecast=val.priceForecast, details=details)
        ...
        val = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.PriceForecast
        )

        self.check_CommodityPriceForecast(
            cluster=cluster, priceForecast=val, details=Clusters.CommodityPrice.Bitmaps.CommodityPriceDetailBitmap(0)
        )
        ...
        if tcp_support:
            ...
            # and Components set to True.
            details = (
                cluster.Bitmaps.CommodityPriceDetailBitmap.kComponents | cluster.Bitmaps.CommodityPriceDetailBitmap.kDescription
            )
