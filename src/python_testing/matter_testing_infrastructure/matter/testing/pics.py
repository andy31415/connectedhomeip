...
def generate_device_element_pics_from_device_wildcard(
    wildcard: AsyncReadTransaction.ReadResponse, xml_clusters: dict[uint, XmlCluster]
) -> tuple[dict[int, list[str]], list[ProblemNotice]]:
            ...
            if cluster_id not in xml_clusters:
                ...
                problems.append(
                    ProblemNotice(
                        ...
                        problem="Unknown standard cluster on device",
                    )
                )
                continue
    ...
    ep0_device_type_list = (
        wildcard.attributes.get(0, {}).get(Clusters.Descriptor, {}).get(Clusters.Descriptor.Attributes.DeviceTypeList, [])
    )
