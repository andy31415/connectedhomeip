...
def _has_cluster(
    wildcard: Clusters.Attribute.AsyncReadTransaction.ReadResponse, endpoint: int, cluster: ClusterObjects.ClusterObjectDescriptor
) -> bool:
...
def _has_attribute(
    wildcard: Clusters.Attribute.AsyncReadTransaction.ReadResponse,
    endpoint: int,
...
def _has_command(
    wildcard: Clusters.Attribute.AsyncReadTransaction.ReadResponse, endpoint: int, command: ClusterObjects.ClusterCommand
) -> bool:
...
def run_on_singleton_matching_endpoint(accept_function: EndpointCheckFunction):
    ...
    def run_on_singleton_matching_endpoint_internal(body):
        ...
        def matching_runner(self: "MatterBaseTest", *args, **kwargs):
            ...
            if not matching:
                LOGGER.info("Test is not applicable to any endpoint - skipping test")
            ...
            try:
                ...
                LOGGER.info(f"Running test on endpoint {self.matter_test_config.endpoint}")
    ...
    def run_if_endpoint_matches_internal(body):
        ...
        def per_endpoint_runner(test_instance, *args, **kwargs):
            ...
            if not should_run_test:
                LOGGER.info("Test is not applicable to this endpoint - skipping test")
                asserts.skip("Endpoint does not match test requirements")
                return
            LOGGER.info(f"Running test on endpoint {test_instance.matter_test_config.endpoint}")
            timeout = getattr(test_instance.matter_test_config, "timeout", None) or test_instance.default_timeout
