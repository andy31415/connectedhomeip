...
class TC_ICDM_3_1(MatterBaseTest):
    ...
    def steps_TC_ICDM_3_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                1,
                "TH reads from the DUT the FeatureMap. If the CIP feature is not supported on the cluster, skip all remaining steps",
            ),
            TestStep("2a", "TH reads TestEventTriggersEnabled attribute from General Diagnostics Cluster."),
            TestStep(
                "2b",
                "TH sends TestEventTrigger command to General Diagnostics Cluster on Endpoint 0 with EnableKey field set to PIXIT.ICDM.S.TEST_EVENT_TRIGGER_KEY and EventTrigger field set to PIXIT.ICDM.S.TEST_EVENT_TRIGGER for the Active Mode requirement.",
            ),
            TestStep(
                3,
                "TH reads from the DUT the RegisteredClients attribute. If not empty, TH sends command UnregisterClient to clear all clients in RegisteredClients.",
            ),
            ...
            TestStep(
                6,
                "TH sends RegisterClient command. - CheckInNodeID: registering client’s node ID - MonitoredSubject: MonitoredSubID - Key: shared secret between the client and the ICD - ClientType : Ephemeral(1).",
            ),
            TestStep(7, "TH reads from the DUT the RegisteredClients attribute."),
            TestStep(
                8,
                "If len(RegisteredClients) is less than ClientsSupportedPerFabric, TH repeats RegisterClient command with different CheckInNodeID(s) and the Permanent(0) ClientType until the number of entries in RegisteredClients equals ClientsSupportedPerFabric.",
            ),
            ...
            TestStep(
                14, "Repeat Step 12-13 with the rest of CheckInNodeIDs from the list of RegisteredClients from Step 8, if any."
            ),
    ...
    async def test_TC_ICDM_3_1(self):
        ...
        try:
            ...
            self.step(6)
            try:
                response = await self._send_single_icdm_command(
            ...
            asserts.assert_greater_equal(
                response.ICDCounter, icdCounter, "The ICDCounter in the response does not match the read ICDCounter."
            )
            ...
            asserts.assert_equal(
                registeredClients[0].monitoredSubject,
                kStep2MonitoredSubjectStep2,
                "The read attribute does not match the registered value.",
            )
            asserts.assert_equal(
                registeredClients[0].clientType,
                clientTypeEnum.kEphemeral,
                "The read attribute does not match the registered value.",
            )
            ...
            with asserts.assert_raises(InteractionModelError) as e:
                await self._send_single_icdm_command(
                    commands.RegisterClient(
                        checkInNodeID=0xFFFF, monitoredSubject=0xFFFF, key=os.urandom(16), clientType=clientTypeEnum.kPermanent
                    )
                )
