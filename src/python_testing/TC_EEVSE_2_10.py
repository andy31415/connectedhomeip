...
class TC_EEVSE_2_10(MatterBaseTest, EEVSEBaseTestHelper):
    ...
    def steps_TC_EEVSE_2_10(self) -> list[TestStep]:
        return [
            ...
            TestStep("3a", "After a few seconds TH reads from the DUT the State", "Value has to be 0x00 (NotPluggedIn)"),
            ...
            TestStep("4a", "TH reads from the DUT the State", "Value has to be 0x01 (PluggedInNoDemand)"),
            TestStep(
                "5",
                "TH sends command EnableCharging with ChargingEnabledUntil=15 seconds in the future, MinimumChargeCurrent=6000, MaximumChargeCurrent=60000. Store the ChargingEnabledUntil into Matter EPOCH in UTC as ChargingEnabledUntilEpochTime, MinimumChargeCurrent as MinimumChargeCurrent and MaximumChargeCurrent as MaximumChargeCurrent",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep("6a", "TH reads from the DUT the SupplyState", "Value has to be 0x01 (ChargingEnabled)"),
            TestStep(
                "7",
                "TH sends command EnableDischarging with DischargingEnabledUntil=5 seconds in the future, MaximumDischargeCurrent=32000. Store the DischargingEnabledUntil into Matter EPOCH in UTC as DischargingEnabledUntilEpochTime, MaximumDischargeCurrent as MaximumDischargeCurrent",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep("7a", "TH reads from the DUT the SupplyState", "Value has to be 0x05 (Enabled)"),
            TestStep(
                "7b",
            ...
            TestStep(
                "7c", "TH reads from the DUT the MinimumChargeCurrent", "Value has to be equal to the value of MinimumChargeCurrent"
            ),
            TestStep("7d", "TH reads from the DUT the CircuitCapacity", "Store the value as CircuitCapacity"),
            TestStep(
                "7e",
            ...
            TestStep("9a", "TH reads from the DUT the SupplyState", "Value has to be 0x01 (ChargingEnabled)"),
            TestStep(
                "9b",
                "TH reads from the DUT the ChargingEnabledUntil",
                "Value has to be equal to the value of ChargingEnabledUntilEpochTime",
            ...
            TestStep(
                "9c", "TH reads from the DUT the MinimumChargeCurrent", "Value has to be equal to the value of MinimumChargeCurrent"
            ),
            TestStep(
                "9d",
                "TH reads from the DUT the MaximumChargeCurrent",
            ...
            TestStep("9e", "TH reads from the DUT the DischargingEnabledUntil", "Value has to be 0"),
            TestStep("9f", "TH reads from the DUT the MaximumDischargeCurrent", "Value has to be 0"),
            TestStep("10", "Wait 10 seconds"),
            TestStep("10a", "TH reads from the DUT the SupplyState", "Value has to be 0x00 (Disabled)"),
            TestStep("10b", "TH reads from the DUT the ChargingEnabledUntil", "Value has to be 0"),
            TestStep("10c", "TH reads from the DUT the MinimumChargeCurrent", "Value has to be 0"),
            TestStep("10d", "TH reads from the DUT the MaximumChargeCurrent", "Value has to be 0"),
            TestStep(
                "11",
                "TH sends command EnableDischarging with DischargingEnabledUntil=null, maximumDischargeCurrent=12000. Store MaximumDischargeCurrent as MaximumDischargeCurrent",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep("11a", "TH reads from the DUT the SupplyState", "Value has to be 0x02 (DischargingEnabled)"),
            TestStep("11b", "TH reads from the DUT the ChargingEnabledUntil", "Value has to be 0"),
            TestStep("11c", "TH reads from the DUT the MinimumChargeCurrent", "Value has to be 0"),
            TestStep("11d", "TH reads from the DUT the MaximumChargeCurrent", "Value has to be 0"),
            TestStep("11e", "TH reads from the DUT the DischargingEnabledUntil", "Value has to be null"),
            TestStep(
                "11f",
                "TH reads from the DUT the MaximumDischargeCurrent",
            ...
            TestStep("12a", "TH reads from the DUT the SupplyState", "Value has to be 0x00 (Disabled)"),
            TestStep("12b", "TH reads from the DUT the ChargingEnabledUntil", "Value has to be 0"),
            TestStep("12c", "TH reads from the DUT the MinimumChargeCurrent", "Value has to be 0"),
            TestStep("12d", "TH reads from the DUT the MaximumChargeCurrent", "Value has to be 0"),
            TestStep("12e", "TH reads from the DUT the DischargingEnabledUntil", "Value has to be 0"),
            TestStep("12f", "TH reads from the DUT the MaximumDischargeCurrent", "Value has to be 0"),
            TestStep(
                "13",
    ...
    async def test_TC_EEVSE_2_10(self):
        ...
        await events_callback.start(self.default_controller, self.dut_node_id, endpoint=endpoint)
        ...
        event_data = events_callback.wait_for_event_report(cluster.Events.EVConnected)
        ...
        charging_end_epoch_time = int((utc_time_charging_end - datetime(2000, 1, 1, 0, 0, 0, 0, timezone.utc)).total_seconds())
        await self.send_enable_charge_command(
            charge_until=charging_end_epoch_time, min_charge=min_charge_current, max_charge=max_charge_current
        )
        ...
        utc_time_discharging_end = datetime.now(tz=timezone.utc) + timedelta(seconds=discharging_duration)
        discharging_end_epoch_time = int(
            (utc_time_discharging_end - datetime(2000, 1, 1, 0, 0, 0, 0, timezone.utc)).total_seconds()
        )
        maximum_discharge_current = 32000
        await self.send_enable_discharge_command(
            discharge_until=discharging_end_epoch_time, max_discharge=maximum_discharge_current
        )
        ...
        circuit_capacity = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.CircuitCapacity
        )
        asserts.assert_true(circuit_capacity is not NullValue, "CircuitCapacity must not be NullValue")
        asserts.assert_true(isinstance(circuit_capacity, int), "CircuitCapacity must be of type int")
        ...
        asserts.assert_equal(
            maximum_current,
            expected_maximum_charge_current,
            f"MaximumCurrent in EnergyTransferStarted event should be {expected_maximum_charge_current}, but was {maximum_current}",
        )
        asserts.assert_equal(
            maximum_discharge_current_event,
            expected_maximum_discharge_current,
        ...
        event_data = events_callback.wait_for_event_report(cluster.Events.EVNotDetected)
