...
class TC_SU_2_5(SoftwareUpdateBaseTest):
    ...
    async def setup_test(self):
        ...
        if not self.expected_software_version:
            asserts.fail(
                "Missing OTA image software version. Specify using --int-arg ota_image_expected_version:<ota_image_expected_version>"
            )
        ...
        await self.set_default_ota_providers_list(
            controller=self.controller, provider_node_id=self.provider_node_id, requestor_node_id=self.requestor_node_id, endpoint=0
        )
        super().setup_test()
    ...
    def steps_TC_SU_2_5(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                1,
                'OTA-P/TH sends the ApplyUpdateResponse Command to the DUT. Action field is set to "Proceed", DelayedActionTime is set to 0.',
            ...
            TestStep(
                2,
                'OTA-P/TH sends the ApplyUpdateResponse Command to the DUT. Action field is set to "Proceed", DelayedActionTime is set to 3 minutes.',
            ...
            TestStep(
                3,
                'OTA-P/TH sends the ApplyUpdateResponse Command to the DUT. Action field is set to "AwaitNextAction", DelayedActionTime is set to 1 minute.',
            ...
            TestStep(
                4,
                'OTA-P/TH sends the ApplyUpdateResponse Command to the DUT. Action field is set to "AwaitNextAction", DelayedActionTime is set to 3 minutes. On the subsequent ApplyUpdateRequest command, TH/OTA-P sends the ApplyUpdateResponse back to DUT. Action field is set to "Proceed".',
            ...
            TestStep(
                ...
                "Verify that the DUT clears its previously downloaded software image, and resets the UpdateState Attribute to Idle."
    ...
    async def test_TC_SU_2_5(self):
        ...
        asserts.assert_equal(
            update_state, Clusters.OtaSoftwareUpdateRequestor.Enums.UpdateStateEnum.kIdle, "Update state should be idle"
        )
        self.terminate_provider()
        ...
        software_version_attr_handler = AttributeSubscriptionHandler(
            expected_cluster=Clusters.BasicInformation, expected_attribute=Clusters.BasicInformation.Attributes.SoftwareVersion
        )
        ...
        await software_version_attr_handler.start(
            dev_ctrl=self.controller,
            node_id=self.requestor_node_id,
            endpoint=0,
            fabric_filtered=False,
        ...
        software_version_match = AttributeMatcher.from_callable(
            f"Sofware Version should be: {current_sw_version}", lambda report: report.value == current_sw_version
        )
        ...
        update_state_attr_handler.await_all_expected_report_matches([update_state_match], timeout_sec=5)
        ...
        await download_progress_attr_handler.start(
            dev_ctrl=self.controller,
            node_id=self.requestor_node_id,
            endpoint=0,
            fabric_filtered=False,
        ...
        asserts.assert_equal(
            update_state, Clusters.OtaSoftwareUpdateRequestor.Enums.UpdateStateEnum.kIdle, "Update state is not idle"
        )
        ota_file_data = self.get_downloaded_ota_image_info()
        logger.info(f"Downloaded ota image data {str(ota_file_data)}")
