...
class CommodityPriceTestBaseHelper:
    ...
    def check_CommodityPriceForecast(
        self,
        cluster: Clusters.CommodityPrice,
        priceForecast: list,
        details: Clusters.CommodityPrice.Bitmaps.CommodityPriceDetailBitmap = Clusters.CommodityPrice.Bitmaps.CommodityPriceDetailBitmap(
            0
        ),
    ):
        ...
        matter_asserts.assert_list_element_type(
            priceForecast,
            cluster.Structs.CommodityPriceStruct,
            "PriceForecast list must contain CommodityPriceStruct elements",
            allow_empty=True,
        )

        # According to spec we must not have more than 56 entries
        asserts.assert_less_equal(len(priceForecast), self.kMaxForecastEntries, "PriceForecast list must be less than 56 entries")
        for item in priceForecast:
            # The other aspects of this verification are handled by the helper
            self.check_CommodityPriceStruct(
                cluster=cluster, struct=item, details=details, now_time_must_be_within_period=False
            )  # Do not check time limits for forecast

    def check_CommodityPriceStruct(
        self,
        cluster: Clusters.CommodityPrice,
        ...
        if now_time_must_be_within_period:  # Only check time limits when dealing with current price (not list of Forecast)
            ...
            asserts.assert_less_equal(struct.periodStart, now_time_epoch_s, "PeriodStart must not be in the past")
            ...
            if struct.periodEnd is not NullValue:
                ...
                asserts.assert_greater_equal(struct.periodEnd, now_time_epoch_s, "PeriodEnd must be in the future")
        ...
        if details & cluster.Bitmaps.CommodityPriceDetailBitmap.kDescription:
            if struct.description is not None:
                ...
                asserts.assert_less_equal(
                    len(struct.description),
                    self.kMaxDescriptionLength,
        ...
        log.info(
            f"PRICE: from: {self.convert_epoch_s_to_time(struct.periodStart, tz=None)} to {self.convert_epoch_s_to_time(struct.periodEnd, tz=None)} : Price: {struct.price} / PriceLevel: {struct.priceLevel} / Description: {struct.description}"
        )
        ...
        if details & cluster.Bitmaps.CommodityPriceDetailBitmap.kComponents:
            if struct.components is not None:
                ...
                asserts.assert_less_equal(
                    len(struct.components),
                    self.kMaxComponentsPerPrice,
    ...
    def check_CommodityPriceComponentStruct(self, struct: Clusters.CommodityPrice.Structs.CommodityPriceComponentStruct):
        matter_asserts.assert_valid_int64(struct.price, "Price")
    ...
    async def send_get_detailed_price_request(
        self,
        endpoint=None,
        details: Clusters.CommodityPrice.Bitmaps = Clusters.CommodityPrice.Bitmaps.CommodityPriceDetailBitmap(0),
        ...
        return await self.send_single_cmd(
            cmd=Clusters.CommodityPrice.Commands.GetDetailedPriceRequest(details=details),
            endpoint=endpoint,
            timedRequestTimeoutMs=timedRequestTimeoutMs,
        )

    async def send_get_detailed_forecast_request(
        self,
        endpoint=None,
        details: Clusters.CommodityPrice.Bitmaps = Clusters.CommodityPrice.Bitmaps.CommodityPriceDetailBitmap(0),
        timedRequestTimeoutMs: int = 3000,
        ...
        """If endpoint is None then it falls through to use the matter test config value"""
        return await self.send_single_cmd(
            cmd=Clusters.CommodityPrice.Commands.GetDetailedForecastRequest(details=details),
            endpoint=endpoint,
            timedRequestTimeoutMs=timedRequestTimeoutMs,
        )
