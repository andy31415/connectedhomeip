...
class TC_DA_1_9(MatterBaseTest):
    def setup_class(self):
        ...
        if self.app_path is None or not os.path.exists(self.app_path):
            asserts.fail(
                "--string-arg app_path:<app_path> is required for this test, please provide the path to the app (eg: all-clusters-app)"
            )

        if self.dac_provider_base_path is None or not os.path.exists(self.dac_provider_base_path):
            asserts.fail(
                "--string-arg dac_provider_base_path:<dac_provider_base_path> is required for this test, please provide the path to the dac provider test vectors, it can be found in Matter SDK at: credentials/test/revoked-attestation-certificates/dac-provider-test-vectors"
            )

        if self.revocation_set_base_path is None or not os.path.exists(self.revocation_set_base_path):
            asserts.fail(
                "--string-arg revocation_set_base_path:<revocation_set_base_path> is required for this test, please provide the path to the revocation set, it can be found in Matter SDK at: credentials/test/revoked-attestation-certificates/revocation-sets"
            )
    ...
    def steps_TC_DA_1_9(self) -> list[TestStep]:
        return [
            TestStep(
                1,
                "Test commissioning with revoked DAC",
            ...
            TestStep(
                2,
                "Test commissioning with revoked PAI",
                "(DUT)Commissioner warns about commissioning the non-genuine device, Or Commissioning fails with device appropriate attestation error",
            ),
            TestStep(
                3,
                "Test commissioning with both DAC and PAI revoked",
                "(DUT)Commissioner warns about commissioning the non-genuine device, Or Commissioning fails with device appropriate attestation error",
            ),
            TestStep(
                4,
                "Test commissioning with revoked DAC using delegated CRL signer",
                "(DUT)Commissioner warns about commissioning the non-genuine device, Or Commissioning fails with device appropriate attestation error",
            ),
            TestStep(
                5,
                "Test commissioning with revoked PAI using delegated CRL signer",
                "(DUT)Commissioner warns about commissioning the non-genuine device, Or Commissioning fails with device appropriate attestation error",
            ),
            TestStep(
                6,
                "Test commissioning with both DAC and PAI revoked using delegated CRL signer",
                "(DUT)Commissioner warns about commissioning the non-genuine device, Or Commissioning fails with device appropriate attestation error",
            ),
            TestStep(7, "Test commissioning with valid DAC and PAI", "Commissioning succeeds without any attestation errors"),
        ]
        ...
        for idx, test_case in enumerate(test_vectors):
            ...
            with open(app_log_file_name, "w") as app_log_file:
                ...
                app_process = await asyncio.create_subprocess_exec(
                    self.app_path, *app_args.split(), stdout=app_log_file, stderr=app_log_file
                )
