...
class TC_PAVST_2_3(MatterBaseTest, PAVSTTestBase, PAVSTIUtils):
    ...
    def steps_TC_PAVST_2_3(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                15,
                "If the zone management cluster is present on this endpoint, TH sends the AllocatePushTransport command with an Null Zone within MotionZones.",
            ...
            TestStep(
                19,
                "TH sends the AllocatePushTransport command with a VideoStreamID not present in aAllocatedVideoStreams.",
                "DUT responds with Status Code InvalidStream.",
            ),
            TestStep(
                20,
                "TH sends the AllocatePushTransport command with a AudioStreamID not present in aAllocatedAudioStreams.",
                "DUT responds with Status Code InvalidStream.",
            ),
            TestStep(
                21,
                "TH sends the AllocatePushTransport command with both VideoStreamID and AudioStreamID absent.",
                "DUT responds with Status Code InvalidCommand.",
            ),
            TestStep(
                22,
                "TH sends the AllocatePushTransport command with both VideoStreamID and AudioStreamID set to None..",
            ...
            TestStep(
                29,
                "TH deallocates transport allocated in step 5 using DeallocatePushTransport command successfully. TH sends the AllocatePushTransport command with all valid parameters. The TriggerType = Motion and MotionZones and MotionSensitivity fields are present and None.",
                "DUT responds with AllocatePushTransportResponse containing the allocated ConnectionID.",
            ),
            TestStep(
                30,
            ...
            TestStep(
                33,
                "TH sends the AllocatePushTransport command with ContainerType = CMAF and CMAFContainerOptions having a segment that is not an exact multiplier of the framerate.",
                "DUT responds with InvalidOptions.",
            ),
            TestStep(
                34,
                "TH sends the AllocatePushTransport command with an value for StreamUsage not in SupportedStreamUsages.",
    ...
    async def test_TC_PAVST_2_3(self):
        ...
        asserts.assert_equal(
            current_connections[0].transportStatus,
            pvcluster.Enums.TransportStatusEnum.kInactive,
            "TransportStatus must be Inactive",
        )
        ...
        asserts.assert_equal(status, pvcluster.Enums.StatusCodeEnum.kInvalidURL, "DUT must respond with Status Code InvalidURL.")
        ...
        asserts.assert_equal(
            status, pvcluster.Enums.StatusCodeEnum.kInvalidStream, "DUT must  responds with Status Code InvalidStream."
        )
        ...
        status = await self.allocate_one_pushav_transport(
            endpoint,
            videoStream_ID=Nullable(),
            audioStream_ID=Nullable(),
            tlsEndPoint=self.tlsEndpointId,
            url=f"https://{host_ip}:1234/streams/{uploadStreamId}/",
        )
        ...
        status = await self.allocate_one_pushav_transport(
            ...
            stream_Usage=Clusters.Globals.Enums.StreamUsageEnum.kRecording,
            tlsEndPoint=self.tlsEndpointId,
            container_Options=containerOptions,
            url=f"https://{host_ip}:1234/streams/{uploadStreamId}/",
        )
        asserts.assert_equal(
            status, pvcluster.Enums.StatusCodeEnum.kInvalidOptions, "DUT must responds with Status Code InvalidOptions."
        )
        ...
        asserts.assert_equal(
            status, pvcluster.Enums.StatusCodeEnum.kInvalidStreamUsage, "DUT must  responds with Status code InvalidStreamUsage"
        )
