...
class TC_LVL_9_1(MatterBaseTest):
    ...
    def steps_TC_LVL_9_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "1a",
                "If the Groupcast cluster is enabled on the RootNode endpoint, the TH sends Groupcast JoinGroup command with GroupID 1, Endpoints set with the EndpointId where LevelControl cluster is enabled and KeySetID 0x01a1 to DUT. Otherwise, TH sends AddGroup command to DUT, with the GroupID field set to 1.",
            ),
            ...
            TestStep(
                "4",
                "TH sends a AddScene command to DUT with the GroupID field set to 1, the SceneID field set to 0x02, the TransitionTime field set to 0 and the ExtensionFieldSetStructs set to: '[{ ClusterID: 0x0008, AttributeValueList: [{ AttributeID: 0x0000, ValueUnsigned8: 0x64 }]}]'",
            ),
            TestStep(
                "5a",
            ...
            TestStep(
                "6a",
                "TH sends a RecallScene command to DUT with the GroupID field set to 1, the SceneID field set to 0x01 and the TransitionTime set to 0.",
            ),
            ...
            Clusters.ScenesManagement.Commands.AddScene(
                ...
                [
                    Clusters.ScenesManagement.Structs.ExtensionFieldSetStruct(
                        ...
                        attributeValueList=[
                            Clusters.ScenesManagement.Structs.AttributeValuePairStruct(attributeID=0x0000, valueUnsigned8=0x64)
                        ],
                    )
                ],
            ),
