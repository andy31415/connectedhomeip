...
class TC_DEM_2_6(MatterBaseTest, DEMTestBase):
    ...
    def steps_TC_DEM_2_6(self) -> list[TestStep]:
        ...
        return [
            ...
            TestStep(
                "5",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID+1, SlotAdjustments[0].{SlotIndex=0, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status FAILURE(0x01)",
            ),
            TestStep(
                "6",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=4, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status FAILURE(0x01)",
            ),
            TestStep(
                "7",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, Duration=Forecast.Slots[0].MaxDurationAdjustment+1}, Cause=GridOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep(
                "8",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, Duration=Forecast.Slots[0].MinDurationAdjustment-1}, Cause=GridOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep(
                "9",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, Duration=Forecast.Slots[0].MaxDurationAdjustment}, SlotAdjustments[1].{SlotIndex=4, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status FAILURE(0x01)",
            ),
            TestStep(
                "10",
            ...
            TestStep("10a", "TH reads from the DUT the ESAState", "Value has to be 0x01 (Online)"),
            TestStep("10b", "TH reads from the DUT the OptOutState", "Value has to be 0x02 (LocalOptOut)"),
            TestStep(
                "11",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=LocalOptimization",
                "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)",
            ),
            TestStep(
                "12",
            ...
            TestStep("12a", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=GridOptimization"),
            TestStep("13", "TH sends command CancelRequest", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep("13a", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=InternalOptimization"),
            TestStep(
                "14",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ...
            TestStep("15a", "TH reads from the DUT the OptOutState", "Value has to be 0x03 (OptOut)"),
            TestStep(
                "15b", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=Internal Optimization"
            ),
            TestStep(
                "16",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, Duration=Forecast.Slots[0].MaxDurationAdjustment}, Cause=GridOptimization",
            ...
            TestStep("17a", "TH reads from the DUT the OptOutState", "Value has to be 0x00 (NoOptOut)"),
            TestStep(
                "18",
                "TH sends command ModifyForecastRequest with ForecastID=Forecast.ForecastID, SlotAdjustments[0].{SlotIndex=0, Duration=Forecast.Slots[0].MinDurationAdjustment}, Cause=LocalOptimization",
                "Verify DUT responds w/ status SUCCESS(0x00)",
            ),
            TestStep("18a", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=LocalOptimization"),
            TestStep("19", "TH sends command CancelRequest", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep("19a", "TH reads from the DUT the Forecast", "Value has to include ForecastUpdateReason=InternalOptimization"),
            TestStep(
                "20",
