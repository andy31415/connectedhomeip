...
{
    MATTER_TRACE_SCOPE("EstablishPASEConnection", Tracing::kMetricPASESession);

    [[maybe_unused]] Tracing::MetricKey operationState = Tracing::kMetricPASESessionBLE;
    ...
    if (params.GetPeerAddress().GetTransportType() == Transport::Type::kBle)
    {
        ...
        else if (params.HasDiscriminator())
        {
            ...
            SuccessOrExit(err = mSystemState->BleLayer()->NewBleConnectionByDiscriminator(discriminator, this, OnDiscoveredDeviceOverBleSuccess, OnDiscoveredDeviceOverBleError));
            ExitNow(CHIP_NO_ERROR);
        }
        else
        {
...
#endif
    operationState = Tracing::kMetricPASESessionPair;
