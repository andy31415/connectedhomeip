...
class NetworkCommissioningTests:
    ...
    async def must_verify_breadcrumb(self):
        res = await self._devCtrl.ReadAttribute(
            nodeId=self._nodeid, attributes=[(0, Clusters.GeneralCommissioning.Attributes.Breadcrumb)], returnClusterObject=True
        )
    ...
    def log_interface_basic_info(self, values):
        ...
        logger.info(f"ScanNetworks should take no more than {values.scanMaxTimeSeconds} seconds.")
        logger.info(f"ConnectNetwork should take no more than {values.connectMaxTimeSeconds} seconds.")
        logger.info(f"The feature map of this endpoint is {values.featureMap}.")

    async def readLastNetworkingStateAttributes(self, endpointId):
        res = await self._devCtrl.ReadAttribute(
            nodeId=self._nodeid,
            attributes=[
                (endpointId, Clusters.NetworkCommissioning.Attributes.LastConnectErrorValue),
    ...
    async def test_negative(self, endpointId):
        logger.info(f"Running negative test cases for NetworkCommissioning cluster on endpoint {endpointId}")

        try:
            logger.info("1. Send ConnectNetwork command with a illegal network id")
            req = Clusters.NetworkCommissioning.Commands.ConnectNetwork(networkID=b"0" * 254, breadcrumb=0)
    ...
    async def test_wifi(self, endpointId):
        ...
        res = await self._devCtrl.ReadAttribute(
            nodeId=self._nodeid,
            attributes=[
                (endpointId, Clusters.NetworkCommissioning.Attributes.ConnectMaxTimeSeconds),
                (endpointId, Clusters.NetworkCommissioning.Attributes.ScanMaxTimeSeconds),
        ...
        self.log_interface_basic_info(res[endpointId][Clusters.NetworkCommissioning])
        ...
        if (res.lastNetworkID != NullValue) or (res.lastNetworkingStatus != NullValue) or (res.lastConnectErrorValue != NullValue):
            raise AssertionError("LastNetworkID, LastNetworkingStatus and LastConnectErrorValue should be Null")
        ...
        res = await self._devCtrl.SendCommand(
            nodeId=self._nodeid, endpoint=endpointId, payload=req, interactionTimeoutMs=interactionTimeoutMs
        )
        ...
        if res.networkIndex != 0:
            raise AssertionError(f"Unexpected result: {res.networkIndex} (should be 0)")
        await self.must_verify_breadcrumb()
        ...
        if len(networkList) != 1:
            raise AssertionError(f"Unexpected result: expect 1 networks, but {len(networkList)} networks received")
        if networkList[0].networkID != TEST_WIFI_SSID.encode():
            raise AssertionError(f"Unexpected result: first network ID should be 'TestSSID' got {networkList[0].networkID}")
        ...
        if not networkList[0].connected:
            raise AssertionError("Unexpected result: network is not marked as connected")
    ...
    async def test_thread(self, endpointId):
        ...
        logger.info(f"Got network list: {networkList}")
        if len(networkList) != 1:
            raise AssertionError(f"Unexpected result: expect 1 networks, but {len(networkList.Data.value)} networks received")
    ...
    async def Test(self):
        clusters = await self._devCtrl.ReadAttribute(
            nodeId=self._nodeid, attributes=[(Clusters.Descriptor.Attributes.ServerList)], returnClusterObject=True
        )
        if Clusters.NetworkCommissioning.id not in clusters[0][Clusters.Descriptor].serverList:
            logger.info("Network commissioning cluster is not enabled on this device.")
            return
        endpoints = await self._devCtrl.ReadAttribute(
            nodeId=self._nodeid, attributes=[(Clusters.NetworkCommissioning.Attributes.FeatureMap)], returnClusterObject=True
        )
        logger.info(endpoints)
        for endpoint, obj in endpoints.items():
            clus = obj[Clusters.NetworkCommissioning]
            if clus.featureMap == WIFI_NETWORK_FEATURE_MAP:
                logger.info(f"Endpoint {endpoint} is configured as WiFi network, run WiFi commissioning test.")
                await self.test_negative(endpoint)
                await self.test_wifi(endpoint)
            elif clus.featureMap == THREAD_NETWORK_FEATURE_MAP:
                logger.info(f"Endpoint {endpoint} is configured as Thread network, run Thread commissioning test.")
                await self.test_negative(endpoint)
                await self.test_thread(endpoint)
            else:
                logger.info(f"Skip endpoint {endpoint} with featureMap {clus.featureMap}")
