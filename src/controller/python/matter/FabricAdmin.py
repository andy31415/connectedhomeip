...
class FabricAdmin:
    ...
    def __init__(self, certificateAuthority: CertificateAuthority.CertificateAuthority, vendorId: int, fabricId: int = 1):
        ...
        if vendorId is None or vendorId == 0:
            raise ValueError(f"Invalid VendorID ({vendorId}) provided!")

        if fabricId is None or fabricId == 0:
            raise ValueError(f"Invalid FabricId ({fabricId}) provided!")
    ...
    def NewController(
        ...
        catTags: List[int] = [],
        keypair: Optional[p256keypair.P256Keypair] = None,
        ...
        if not (self._isActive):
            raise RuntimeError("FabricAdmin object was previously shutdown and is no longer valid!")
