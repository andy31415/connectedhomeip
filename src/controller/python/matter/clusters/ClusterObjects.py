    ...
    def _PutSingleElementToTLV(self, tag, val, elementType, writer: tlv.TLVWriter, debugPath: str = "?"):
        if issubclass(elementType, ClusterObject):
            if not isinstance(val, dict):
                raise ValueError(f"Field {debugPath}.{self.Label} expected a struct, but got {type(val)}")
            elementType.descriptor.DictToTLVWithWriter(f"{debugPath}.{self.Label}", tag, val, writer)
            return

        try:
            val = elementType(val)
        except Exception:
            raise ValueError(f"Field {debugPath}.{self.Label} expected {elementType}, but got {type(val)}")
        writer.put(tag, val)

    def PutFieldToTLV(self, tag, val, writer: tlv.TLVWriter, debugPath: str = "?"):
        if val == NullValue:
            if GetUnionUnderlyingType(self.Type, Nullable) is None:
                raise ValueError(f"Field {debugPath}.{self.Label} was not nullable, but got a null")

            writer.put(tag, None)
        elif val is None:
            if GetUnionUnderlyingType(self.Type, type(None)) is None:
                raise ValueError(f"Field {debugPath}.{self.Label} was not optional, but encountered None")
        else:
            #
            ...
            for i, v in enumerate(val):
                self._PutSingleElementToTLV(None, v, elementType, writer, debugPath + f"[{i}]")
            writer.endContainer()
    ...
    def _ConvertNonArray(self, debugPath: str, elementType, value: Any) -> Any:
        if not issubclass(elementType, ClusterObject):
            ...
            if not isinstance(value, elementType):
                raise ValueError(f"Failed to decode field {debugPath}, expected type {elementType}, got {type(value)}")
            return value
    ...
    def TagDictToLabelDict(self, debugPath: str, tlvData: Dict[int, Any]) -> Dict[str, Any]:
        ...
        for tag, value in tlvData.items():
            ...
            if typing.get_origin(descriptor.Type) == typing.Union:
                ...
                if realType is None:
                    raise ValueError(f"Field {debugPath}.{descriptor.Label} has no valid underlying data model type")
            ...
            if typing.get_origin(valueType) == list:
                ...
                ret[descriptor.Label] = [
                    self._ConvertNonArray(f"{debugPath}[{i}]", listElementType, v) for i, v in enumerate(value)
                ]
                continue
            ret[descriptor.Label] = self._ConvertNonArray(f"{debugPath}.{descriptor.Label}", valueType, value)
        return ret
...
class ClusterAttributeDescriptor:
    ...
    def ToTLV(cls, tag: Union[int, None], value):
        ...
        cls.attribute_type.PutFieldToTLV(tag, asdict(wrapped_value)["Value"], writer, "")
    ...
    def FromTLV(cls, tlvBuffer: bytes):
        ...
        return obj_class.FromDict(
            obj_class.descriptor.TagDictToLabelDict("", {0: tlv.TLVReader(tlvBuffer).get().get("Any", {})})
        ).Value
                ...
                (
                    ...
                    field(
                        default=ClusterObjectDescriptor(
                            Fields=[ClusterObjectFieldDescriptor(Label="Value", Tag=0, Type=cls.attribute_type.Type)]
                        )
                    ),
                ),
            ],
