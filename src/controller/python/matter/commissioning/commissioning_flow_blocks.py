...
class CommissioningFlowBlocks:
    ...
    async def arm_failsafe(self, node_id: int, duration_seconds: int = 180):
        response = await self._devCtrl.SendCommand(
            node_id,
            commissioning.ROOT_ENDPOINT_ID,
            Clusters.GeneralCommissioning.Commands.ArmFailSafe(expiryLengthSeconds=duration_seconds),
        )
    ...
    async def operational_credentials_commissioning(self, parameter: commissioning.Parameters, node_id: int):
        ...
        device_info = (
            await self._devCtrl.ReadAttribute(
                node_id,
                [
        ...
        )[commissioning.ROOT_ENDPOINT_ID][Clusters.BasicInformation]
        ...
        try:
            attestation_elements = await self._devCtrl.SendCommand(
                node_id,
                commissioning.ROOT_ENDPOINT_ID,
                Clusters.OperationalCredentials.Commands.AttestationRequest(attestationNonce=attestation_nonce),
            )
        ...
        try:
            dac = await self._devCtrl.SendCommand(
                node_id,
                commissioning.ROOT_ENDPOINT_ID,
                Clusters.OperationalCredentials.Commands.CertificateChainRequest(certificateType=1),
            )
        except Exception as ex:
        ...
        try:
            pai = await self._devCtrl.SendCommand(
                node_id,
                commissioning.ROOT_ENDPOINT_ID,
                Clusters.OperationalCredentials.Commands.CertificateChainRequest(certificateType=2),
            )
        except Exception as ex:
        ...
        try:
            csr = await self._devCtrl.SendCommand(
                node_id, commissioning.ROOT_ENDPOINT_ID, Clusters.OperationalCredentials.Commands.CSRRequest(CSRNonce=csr_nonce)
            )
        except Exception as ex:
        ...
        try:
            response = await self._devCtrl.SendCommand(
                node_id,
                commissioning.ROOT_ENDPOINT_ID,
                Clusters.OperationalCredentials.Commands.AddTrustedRootCertificate(rootCACertificate=commissionee_credentials.rcac),
            )
        except Exception as ex:
        ...
        try:
            ...
            root_public_key = x509_rcac.public_key().public_bytes(
        ...
        response = await self._devCtrl.SendCommand(
            node_id,
            commissioning.ROOT_ENDPOINT_ID,
            Clusters.OperationalCredentials.Commands.AddNOC(
        ...
        response = await self._devCtrl.SendCommand(
            node_id,
            commissioning.ROOT_ENDPOINT_ID,
            Clusters.OperationalCredentials.Commands.UpdateFabricLabel(label=parameter.fabric_label),
        )
        if response.statusCode != 0:
            raise commissioning.CommissionFailure(repr(response))
    ...
    async def network_commissioning_thread(self, parameter: commissioning.Parameters, node_id: int):
        ...
        response = await self._devCtrl.SendCommand(
            nodeId=node_id,
            endpoint=commissioning.ROOT_ENDPOINT_ID,
            payload=Clusters.NetworkCommissioning.Commands.AddOrUpdateThreadNetwork(
        ...
        network_list = (
            await self._devCtrl.ReadAttribute(
                nodeId=node_id,
                attributes=[(commissioning.ROOT_ENDPOINT_ID, Clusters.NetworkCommissioning.Attributes.Networks)],
                returnClusterObject=True,
            )
        )[commissioning.ROOT_ENDPOINT_ID][Clusters.NetworkCommissioning].networks
        ...
        response = await self._devCtrl.SendCommand(
            nodeId=node_id,
            endpoint=commissioning.ROOT_ENDPOINT_ID,
            payload=Clusters.NetworkCommissioning.Commands.ConnectNetwork(networkID=network_id),
            interactionTimeoutMs=self._devCtrl.ComputeRoundTripTimeout(node_id, upperLayerProcessingTimeoutMs=30000),
        )
        if response.networkingStatus != Clusters.NetworkCommissioning.Enums.NetworkCommissioningStatusEnum.kSuccess:
    ...
    async def network_commissioning_wifi(self, parameter: commissioning.Parameters, node_id: int):
        ...
        response = await self._devCtrl.SendCommand(
            nodeId=node_id,
            endpoint=commissioning.ROOT_ENDPOINT_ID,
            payload=Clusters.NetworkCommissioning.Commands.AddOrUpdateWiFiNetwork(
                ssid=parameter.wifi_credentials.ssid, credentials=parameter.wifi_credentials.passphrase
            ),
        )
        if response.networkingStatus != Clusters.NetworkCommissioning.Enums.NetworkCommissioningStatusEnum.kSuccess:
            raise commissioning.CommissionFailure(f"Unexpected result for adding network: {response.networkingStatus}")
    ...
    async def network_commissioning(self, parameter: commissioning.Parameters, node_id: int):
        clusters = await self._devCtrl.ReadAttribute(
            nodeId=node_id, attributes=[(Clusters.Descriptor.Attributes.ServerList)], returnClusterObject=True
        )
        if Clusters.NetworkCommissioning.id not in clusters[commissioning.ROOT_ENDPOINT_ID][Clusters.Descriptor].serverList:
            self._logger.info(f"Network commissioning cluster {commissioning.ROOT_ENDPOINT_ID} is not enabled on this device.")
            return None

        network_commissioning_cluster_state = (
            await self._devCtrl.ReadAttribute(
                nodeId=node_id,
                attributes=[(commissioning.ROOT_ENDPOINT_ID, Clusters.NetworkCommissioning)],
                returnClusterObject=True,
            )
        ...
        if network_commissioning_cluster_state.networks:
            for networks in network_commissioning_cluster_state.networks:
                if networks.connected:
                    self._logger.info(f"Device already connected to {networks.networkID.hex()} skip network commissioning")
                    return None

        if parameter.commissionee_info.is_wifi_device:
            if (
                network_commissioning_cluster_state.featureMap
                != commissioning.NetworkCommissioningFeatureMap.WIFI_NETWORK_FEATURE_MAP
            ):
        ...
        if parameter.commissionee_info.is_thread_device:
            if (
                network_commissioning_cluster_state.featureMap
                != commissioning.NetworkCommissioningFeatureMap.THREAD_NETWORK_FEATURE_MAP
            ):
    ...
    async def complete_commission(self, node_id: int):
        response = await self._devCtrl.SendCommand(
            node_id, commissioning.ROOT_ENDPOINT_ID, Clusters.GeneralCommissioning.Commands.CommissioningComplete()
        )
        if response.errorCode != 0:
            raise commissioning.CommissionFailure(repr(response))
