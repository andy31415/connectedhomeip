...
def ParseGnList(value):
    ...
    if value.startswith("[") and value.endswith("]"):
        gn_list = value.strip("[]").replace('"', "").replace(" ", "").split(",")
        if not gn_list[0]:
            return []
        return gn_list
    return None
...
def GetAllDependentJars(deps_configs_data):
    ...
    while configs_to_process:
        ...
        child_configs = LoadBuildConfigs(deps_config["deps_info"]["deps_configs"])
...
def main(argv):
    ...
    parser.add_option("--deps-configs", help="GN-list of dependent build_config files")
