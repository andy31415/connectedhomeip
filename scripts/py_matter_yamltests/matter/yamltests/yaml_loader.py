...
from .errors import (
    TestStepArgumentsValueError,
    TestStepError,
    TestStepGroupEndPointError,
    TestStepGroupResponseError,
    TestStepInvalidTypeError,
    TestStepKeyError,
    TestStepNodeIdAndGroupIdError,
    TestStepResponseVariableError,
    TestStepSaveAsNameError,
...
yaml_tree = SchemaTree(schema=_TOP_LEVEL_SCHEMA, children={"tests": _test_step_tree, "config": _config_tree, "CI": _ci_tree})
...
class YamlLoader:
    ...
    def __check_test_step(self, config: dict, content):
        ...
        self.__rule_step_with_verification_should_be_disabled_or_interactive(content)
    ...
    def __check_test_step_response(self, content):
        ...
        if "values" in content:
            ...
            values = content.get("values")
            [self.__check_test_step_response_value(x, allow_name_key=True) for x in values]
        else:
