...
class TestYamlParser(unittest.TestCase):
    ...
    def test_config_override(self):
        config_override = {"nodeId": 12345, "cluster": "TestOverride", "endpoint": 4}
        parser_config = TestParserConfig(None, self._definitions, config_override)
        yaml_parser = TestParser(simple_test_yaml, parser_config)
        for idx, test_step in enumerate(yaml_parser.tests):
    ...
    def test_config_read_response_wrong_code(self):
        parser_config = TestParserConfig(None, self._definitions)
        self.assertRaises(TestStepEnumError, TestParser, enum_value_read_response_wrong_code_yaml, parser_config)

    def test_config_read_response_wrong_name(self):
        parser_config = TestParserConfig(None, self._definitions)
        self.assertRaises(TestStepEnumError, TestParser, enum_value_read_response_wrong_name_yaml, parser_config)

    def test_config_read_response_wrong_code_specified(self):
        parser_config = TestParserConfig(None, self._definitions)
        self.assertRaises(
            TestStepEnumSpecifierWrongError, TestParser, enum_value_read_response_wrong_code_specified_yaml, parser_config
        )
    ...
    def test_revision_step_injection(self):
        ...
        self.assertEqual(steps[1].label, "Read ClusterRevision for conditions (EP: 1, CL: Test)")
        self.assertEqual(steps[1].command, "readAttribute")
        self.assertEqual(steps[1].attribute, "ClusterRevision")
        self.assertEqual(steps[1].responses[0]["values"][0]["saveAs"], build_revision_var_name(endpoint=1, cluster="Test"))
    ...
    def test_revision_step_multi_injection(self):
        parser_config = TestParserConfig(None, self._definitions)
        yaml_parser = TestParser(min_max_revision_multi_injection_yaml, parser_config)
        ...
        self.assertEqual(steps[0].label, "Read ClusterRevision for conditions (EP: 1, CL: Test)")
        self.assertEqual(steps[0].endpoint, 1)
        self.assertEqual(steps[0].responses[0]["values"][0]["saveAs"], build_revision_var_name(endpoint=1, cluster="Test"))
        ...
        self.assertEqual(steps[2].label, "Read ClusterRevision for conditions (EP: 2, CL: Test)")
        self.assertEqual(steps[2].endpoint, 2)
        self.assertEqual(steps[2].responses[0]["values"][0]["saveAs"], build_revision_var_name(endpoint=2, cluster="Test"))
