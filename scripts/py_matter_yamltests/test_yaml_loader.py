...
from matter.yamltests.errors import (
    TestStepGroupResponseError,
    TestStepInvalidTypeError,
    TestStepKeyError,
    TestStepNodeIdAndGroupIdError,
    TestStepValueAndValuesError,
...
class TestYamlLoader(unittest.TestCase):
    ...
    def test_key_name_wrong_values(self):
        ...
        [self.assertRaises(TestStepInvalidTypeError, load, f"{key}: {x}") for x in values]
    ...
    def test_key_tests_step_dict_keys(self):
        ...
        for key in keys:
            _, _, _, _, tests = load(content.format(key=key, value=valid_value))
            self.assertEqual(tests, [{"command": "writeAttribute", key: {"value": True}}])
    ...
    def test_key_tests_step_verification_key(self):
        ...
        self.assertEqual(tests, [{"verification": "Test Sentence", "disabled": True}])
    ...
    def test_key_tests_step_rule_step_with_verification_should_be_disabled_or_interactive(self):
        ...
        _, _, _, _, tests = load(content)
        self.assertEqual(tests, [{"label": "A Test Name", "verification": "A verification sentence", "disabled": True}])
        ...
        _, _, _, _, tests = load(content)
        self.assertEqual(tests, [{"label": "A Test Name", "verification": "A verification sentence", "command": "UserPrompt"}])
