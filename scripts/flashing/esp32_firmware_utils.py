    ...
    "configuration": {
        ...
        "parttool": {
            ...
            "command": [
                ...
                {"optional": "port"},
                {"optional": "baud"},
                {"option": "partition", "result": ["--partition-table-file", "{partition}"], "expand": True},
                (),
            ],
...
class Flasher(firmware_utils.Flasher):
    ...
    def _postprocess_argv(self):
        if self.option.sdkconfig:
            namespace_defaults(self.option, self.read_sdkconfig(self.option.sdkconfig))
    ...
    FLASH_ARGUMENTS = [
        ...
        {"optional": "flash_freq"},
        {"optional": "flash_size"},
        {"match": "{compress}", "test": [(True, "-z"), ("y", "-z")], "default": "-u"},
    ]
    ...
    def flash(self, image, address=0):
        ...
        if not isinstance(image, list):
            image = [address, image]
        return self.run_tool("esptool", ["write_flash", self.FLASH_ARGUMENTS, image], name="Flash")
    ...
    PARTITION_INFO = {
        ...
        "factory": ["--partition-type", "app", "--partition-subtype", "factory"],
    }

    def get_partition_info(self, item, info, options=None):
        """Run parttool to get partition information."""
        return self.run_tool(
            "parttool", ["get_partition_info", self.PARTITION_INFO[item], "--info", info], options or {}, capture_output=True
        ).stdout.strip()

    def make_wrapper(self, argv):
        self.parser.add_argument("--use-parttool", metavar="FILENAME", help="partition tool to configure flashing script")
        self.parser.add_argument("--use-partition-file", metavar="FILENAME", help="partition file to configure flashing script")
        self.parser.add_argument("--use-sdkconfig", metavar="FILENAME", help="sdkconfig to configure flashing script")
        ...
        if parttool and partfile:
            ...
            if args.application and args.application_offset is None:
                args.application_offset = self.get_partition_info(
                    "application", "offset", {"parttool": parttool, "partition": partfile}
                )
