...
def print_check_revision_warning_message(current_revision, recommended_revision):
    ...
    longest_message_len = max(
        [
            len(current_revision_message),
            len(recommended_revision_message),
            len(allowed_message),
            len(update_message),
    ...
    print_messages(
        [
            ...
            fmt.format(current_revision_message),
            fmt.format(recommended_revision_message),
            fmt.format(""),
            fmt.format(allowed_message),
...
def main():
    try:
        ...
        parser.add_argument(
            "-c",
            "--check",
            help="Check if your current nRF Connect SDK revision is the same as recommended one.",
            action="store_true",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "-s",
            "--shallow",
            help="Fetch only specific commits (without the history) when updating nRF Connect SDK.",
            action="store_true",
        )
        ...
        if args.check:
            ...
            if current_sha != recommended_sha:
                print_check_revision_warning_message(current_sha, recommended_sha)
                sys.exit(1)
