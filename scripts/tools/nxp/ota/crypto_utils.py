...
class rijndael:
    def __init__(self, key, block_size=16):
        ...
        tk = []
        for i in range(0, KC):
            tk.append((key[i * 4] << 24) | (key[i * 4 + 1] << 16) | (key[i * 4 + 2] << 8) | key[i * 4 + 3])
...
def aParsePassKeyString(sPassKey):
    ...
    return struct.pack(">LLLL", lstu32Passkey[0], lstu32Passkey[1], lstu32Passkey[2], lstu32Passkey[3])
...
def encryptData(sSrcData, sPassKey, aPassIv):
    ...
    sPassString = (
        "0x" + sKeyString[:8] + "," + "0x" + sKeyString[8:16] + "," + "0x" + sKeyString[16:24] + "," + "0x" + sKeyString[24:32]
    )
    aPassKey = aParsePassKeyString(sPassString)

    sIvString = aPassIv.strip()
    sPassString = (
        "0x" + sIvString[:8] + "," + "0x" + sIvString[8:16] + "," + "0x" + sIvString[16:24] + "," + "0x" + sIvString[24:32]
    )
