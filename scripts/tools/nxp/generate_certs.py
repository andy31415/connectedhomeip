...
def gen_test_certs(
    ...
    if generate_cd:
        ...
        cmd = [
            ...
            CD_PATH,
            "--out",
            output + "/Chip-Test-CD-" + f"{vendor_id:X}" + "-" + f"{product_id:X}" + ".der",
            "--format-version",
            "1",
            "--vendor-id",
            hex(vendor_id),
    ...
    if (paa_cert_path is None) or (paa_key_path is None):
        ...
        cmd = [
            ...
            device_name,
            "--out",
            new_certificates["PAA_CERT"] + ".pem",
            "--out-key",
            new_certificates["PAA_KEY"] + ".pem",
            "--valid-from",
            valid_from,
            "--lifetime",
            lifetime,
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "This tool requires a path to chip-cert executable. "
            "By default you can find chip-cert in connectedhomeip/src/tools/chip-cert directory "
            "and build it there."
        ),
    )
    parser.add_argument("-o", "--output", type=str, required=True, help="Output path to store certificates, e.g. /path/to/my/dir")
    parser.add_argument(
        "--vendor_id", type=allow_any_int, required=True, help="[int | hex int] Provide Vendor Identification Number"
    )
    parser.add_argument(
        "--product_id", type=allow_any_int, required=True, help="[int | hex int] Provide Product Identification Number"
    )
    parser.add_argument("--vendor_name", type=str, required=True, help="[string] provide human-readable vendor name")
    parser.add_argument("--product_name", type=str, required=True, help="[string] provide human-readable product name")
    parser.add_argument(
        "--gen_cd",
        action="store_true",
        default=False,
        help=("Generate a new Certificate Declaration in .der format according to used Vendor ID and Product ID."),
    )
    parser.add_argument(
        "--cd_type",
        type=int,
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "[int] Provides the primary device type implemented by the node. "
    ...
    parser.add_argument(
        ...
        help=(
            "The start date for the certificate's validity period in"
            "<YYYY>-<MM>-<DD> [ <HH>:<MM>:<SS> ] format. Default to 2023-01-01 00:00:00"
        ),
    )
    parser.add_argument(
        "--lifetime", type=str, default="7305", help=("The lifetime for the new certificate, in whole days. Default to 7305 days.")
    )
