...
def populate_factory_data(args, spake2p_params):
    ...
    if not args.dac_in_secure_cert:
        ...
        if args.ds_peripheral:
            ...
            priv_key = tlv_priv_key_t(
                key_type=tlv_priv_key_type_t.ESP_SECURE_CERT_ECDSA_PERIPHERAL_KEY, key_path=args.dac_key, key_pass=None
            )
            # priv_key_len is in bits
            priv_key.priv_key_len = 256
            priv_key.efuse_key_id = args.efuse_key_id
            generate_partition_ds(
                priv_key=priv_key,
                device_cert=args.dac_cert,
                ca_cert=args.pai_cert,
                idf_target=args.target,
                op_file=secure_cert_partition_file_path,
            )
        else:
            priv_key = tlv_priv_key_t(
                key_type=tlv_priv_key_type_t.ESP_SECURE_CERT_DEFAULT_FORMAT_KEY, key_path=args.dac_key, key_pass=None
            )
    ...
    if args.supported_modes is not None:
        ...
        for ep in dictionary:
            _sz = {"type": "data", "encoding": "u32", "value": len(dictionary[ep])}
            FACTORY_DATA.update({"sm-sz/{:x}".format(int(ep)): _sz})
            for i in range(len(dictionary[ep])):
                item = dictionary[ep][i]
                _label = {"type": "data", "encoding": "string", "value": item["Label"]}
                _mode = {"type": "data", "encoding": "u32", "value": item["Mode"]}
                _st_sz = {"type": "data", "encoding": "u32", "value": len(item["Semantic_Tag"])}
                ...
                for j in range(len(item["Semantic_Tag"])):
                    ...
                    _value = {"type": "data", "encoding": "u32", "value": entry["value"]}
                    _mfg_code = {"type": "data", "encoding": "u32", "value": entry["mfgCode"]}
...
def generate_nvs_csv(output_dir, out_csv_filename):
    ...
    log.info("Generated the factory partition csv file : '%s'", os.path.abspath(os.path.join(output_dir, out_csv_filename)))


def generate_nvs_bin(encrypt, size, csv_filename, bin_filename, output_dir):
    nvs_args = SimpleNamespace(version=2, outdir=output_dir, input=csv_filename, output=bin_filename, size=hex(size))
    if encrypt:
...
def print_flashing_help(encrypt, output_dir, bin_filename):
    ...
    log.info("esptool.py -p (PORT) write_flash (FACTORY_PARTITION_ADDR) '%s'", os.path.join(os.getcwd(), output_dir, bin_filename))
    if encrypt:
        log.info("Run below command to flash '%s'", NVS_KEY_PARTITION_BIN)
        log.info(
            "esptool.py -p (PORT) write_flash --encrypt (NVS_KEY_PARTITION_ADDR) '%s'",
...
def get_args():
    ...
    parser.add_argument("-p", "--passcode", type=any_base_int, help="The setup passcode for pairing, range: 0x01-0x5F5E0FE")
    parser.add_argument("-d", "--discriminator", type=any_base_int, help="The discriminator for pairing, range: 0x00-0x0FFF")
    ...
    parser.add_argument(
        "-ds", "--ds-peripheral", action="store_true", help="Use DS Peripheral in generating secure cert partition."
    )
    parser.add_argument(
        "--efuse-key-id",
        type=int,
    ...
    parser.add_argument(
        "--rd-id-uid",
        help=(
            "128-bit unique identifier for generating rotating device identifier, "
    ...
    parser.add_argument(
        "--product-finish", type=str, choices=product_finish_choices, help="Product finishes choices for product appearance"
    )

    product_color_choices = [color.name for color in Product_Color_Enum]
    parser.add_argument(
        "--product-color", type=str, choices=product_color_choices, help="Product colors choices for product appearance"
    )
    ...
    parser.add_argument("-s", "--size", type=any_base_int, default=0x6000, help="The size of the partition.bin, default: 0x6000")
    parser.add_argument("--target", default="esp32", help="The platform type of device. eg: one of esp32, esp32c3, etc.")
    parser.add_argument("-e", "--encrypt", action="store_true", help="Encrypt the factory parititon NVS binary")
    parser.add_argument("--no-bin", action="store_false", dest="generate_bin", help="Do not generate the factory partition binary")
