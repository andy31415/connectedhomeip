...
def get_args():
    ...
    mandatory_arguments.add_argument(
        ...
        type=allow_any_int,
        required=True,
        help=("[int | hex] Default PASE session passcode. (This is mandatory to generate QR Code)."),
    )
    mandatory_arguments.add_argument(
        "-o", "--output", type=str, required=True, help="Output path to store .json file, e.g. my_dir/output.json"
    )
...
def generate_qrcode_data(args):
    if no_onboarding_modules is False:
        ...
        print(
            "Please install it with all dependencies: pip3 install -r ./scripts/setup/requirements.bouffalolab.txt from the Matter root directory"
        )
