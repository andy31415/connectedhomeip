...
def convert_ini_to_json(ini_dir: str, json_path: str):
    ...
    for path in ini_file_paths:
        ...
        if exists(full_path):
            ...
            create_repl_config_from_init(ini_file=full_path, json_dict=python_json_store, replace_suffix=str(counter))
        counter = counter + 1
...
def create_repl_config_from_init(ini_file: str, json_dict: typing.Dict, replace_suffix: str):
    ...
    json_dict["repl-config"]["caList"][replace_suffix] = [{"fabricId": int(replace_suffix), "vendorId": 0xFFF1}]
