...
QUERY_CONFIG = {
    Config.group_map("query"): {"group": "output"},
    "report.increases": {
...
def argsplit(metavar: str, value: str) -> Tuple[Optional[Tuple], Dict]:
    ...
    if len(names) < len(values):
        ...
        return (None, {})
    if len(names) > len(values):
        log.error("Missing '%s' for '%s'", ",".join(names[len(values) :]), metavar)
        return (None, {})
    return (values, dict(zip(names, values)))


def postprocess_canned_sql_option(config: Config, key: str, info: Mapping) -> None:
...
def postprocess_df_time(_config: Config, df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["time"] = df["time"].map(lambda t: datetime.datetime.utcfromtimestamp(t).isoformat())
    return df
...
def postprocess_df_changes(config: Config, df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["change"] = df.apply(lambda row: row.commit_size - row.parent_size, axis=1)
    df["% change"] = df.apply(lambda row: SizeDatabase.percent_change(row.parent_size, row.commit_size), axis=1)
...
def make_build_sizes_query(config: Config, thing_id: str, sections: List[str]) -> Tuple[List[str], str]:
...
def query_build_sizes(config: Config, db: SizeDatabase, build: str) -> Optional[pd.DataFrame]:
...
def main(argv):
    ...
    try:
        ...
        memdf.report.write_dfs(config, dfs, hierify=config["hierify"], title=True, floatfmt="5.1f")
