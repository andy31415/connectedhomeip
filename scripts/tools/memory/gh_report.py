...
class SizeContext:
    ...
    def __init__(self, config: Config):
        ...
        self.db = memdf.sizedb.SizeDatabase(db_file, not config["database.readonly"])
        self.db.open()
    ...
    def add_sizes_from_github(self):
            ...
            for report in group_reports.values():
                if self.should_report(report.event):
                    if report.parent not in group_reports:
                        ...
                        continue
                    if artifact_limit and len(required_artifact_ids) >= artifact_limit:
                        continue
        ...
        for i in required_artifact_ids:
            ...
            if blob:
                try:
                    self.db.add_sizes_from_zipfile(io.BytesIO(blob), {"artifact": i})
                except Exception:
    ...
    def get_newest_commit(self, pr: int) -> str:
        ...
        commits = sorted(
            self.gh.get_commits_for_pr(pr), key=lambda c: f"{c.commit.committer.date}{c.commit.author.date}", reverse=True
        )
    ...
    def post_change_report(self, df: pd.DataFrame) -> bool:
        ...
        existing_comment, existing_comment_format = self.get_existing_comment(pr, df.attrs["title"])
        ...
        if not existing_comment:
            ...
            if commit != latest:
                log.info("SCS: PR #%s: not commenting for stale '%s'; newest is '%s'", pr, commit, latest)
    ...
    def report_matching_commits(self) -> Dict[str, pd.DataFrame]:
        ...
        comment_enabled = self.config["github.comment"] or self.config["github.dryrun-comment"]
        ...
        for event, pr, commit, parent in commits.fetchall():
            ...
            df.attrs = {
                ...
                "title": (f"PR #{pr}: " if pr else "") + f"Size comparison from {parent} to {commit}",
            ...
            if event == "pull_request" and comment_enabled and (comment_limit == 0 or comment_limit > comment_count):
...
class V1Comment:
    ...
    @staticmethod
    def write_df(config: Config, df: pd.DataFrame, out: memdf.report.OutputOption):
        memdf.report.write_df(config, df, out, "pipe", hierify=True, title=False, floatfmt="5.1f")
    ...
    def merge(df: pd.DataFrame, comment) -> pd.DataFrame:
        ...
        attrs = df.attrs
        df = pd.concat([df, pd.DataFrame(data=rows, columns=cols).astype(df.dtypes)], ignore_index=True)
        df.attrs = attrs
        return df.sort_values(by=["platform", "target", "config", "section"]).drop_duplicates()
...
def main(argv):
    ...
    try:
        ...
        memdf.report.write_dfs(config, dfs, hierify=True, title=True, floatfmt="5.1f")
