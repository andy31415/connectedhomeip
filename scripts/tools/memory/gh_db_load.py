        ...
        if gh := Gh(config):
            ...
            for a in gh.get_size_artifacts(label=config["github.label"]):
                if events and a.event not in events:
                    ...
                    continue
                cur = db.execute("SELECT id FROM build WHERE artifact = ?", (a.id,))
                if cur.fetchone():
                ...
                if blob:
                    log.info(
                        "Adding artifact id=%d commit='%s' pr='%s' event='%s' group='%s'",
                        a.id,
                        a.commit[:12],
                        a.pr,
                        a.event,
                    ...
                    db.add_sizes_from_zipfile(io.BytesIO(blob), {"artifact": a.id})
                    db.commit()
                    artifacts_added += 1
