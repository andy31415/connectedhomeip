...
def read_sources(config: Config, filename: str) -> SymbolSourceDF:
    ...
    process = memdf.util.subprocess.run_tool_pipe(config, ["nm", "-l", filename])
    ...
    while line := text.readline():
        ...
        rows.append([address, match.group("kind"), match.group("symbol"), source])
...
def decode_segment_flags(sflags: str) -> int:
    ...
    return (int("R" in sflags) << 2) | (int("W" in sflags) << 1) | (int("E" in sflags))
...
def read_file(config: Config, filename: str, method: str = None) -> DFs:
    ...
    process = memdf.util.subprocess.run_tool_pipe(config, ["readelf", "--wide", "--segments", "--sections", "--symbols", filename])
    if not process or not process.stdout:
        return SegmentDF()
    ...
    sections["segment"] = sections["section"].apply(lambda s: section_to_segment.get(s, memdf.name.UNKNOWN))

    # Add section name column to symbols.
    section_map = {str(k): v for k, v in sections["section"].items()}
    section_map.update({"0": memdf.name.UNDEF, "UND": memdf.name.UNDEF, "ABS": memdf.name.ABS})
    if "shndx" in symbols.columns:
        symbols["section"] = symbols["shndx"].apply(lambda s: section_map.get(s, memdf.name.UNKNOWN))
    ...
    return {SegmentDF.name: segments, SectionDF.name: sections, SymbolDF.name: symbols}
