...
def read_segments(config: Config, ef: ELFFile) -> SegmentDF:
    ...
    for segment in ef.iter_segments():
        rows.append([segment["p_type"], segment["p_vaddr"], segment["p_paddr"], segment["p_memsz"], segment["p_flags"]])
...
def read_symbols(config: Config, ef: ELFFile, sections: SectionDF) -> SymbolDF:
    ...
    section_map.update({0: memdf.name.UNDEF, "SHN_UNDEF": memdf.name.UNDEF, "SHN_ABS": memdf.name.ABS})
    ...
    for section_id, section in enumerate(ef.iter_sections()):
        ...
        for symbol_id, symbol in enumerate(section.iter_symbols()):
            st_type = elftools.elf.descriptions.describe_symbol_type(symbol["st_info"]["type"])
            st_bind = elftools.elf.descriptions.describe_symbol_bind(symbol["st_info"]["bind"])
            ...
            rows.append(
                [
                    ...
                    section_map.get(st_shndx, memdf.name.UNKNOWN),  # column: 'section'
                    st_type,  # column: 'type'
                    st_bind,  # column: 'bind'
                ]
            )
...
def read_file(config: Config, filename: str, method: str = None) -> DFs:
    ...
    with open(filename, "rb") as fp:
        ...
        if config["args.need_cu"]:
            ...
            symbols["cu"] = symbols["address"].apply(lambda a: m.get(aranges.cu_offset_at_addr(a), "")).astype("string")
