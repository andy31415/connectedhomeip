...
ChangeInfo = collections.namedtuple(
    "ChangeInfo", ["columns", "rows", "things", "builds", "stale_builds", "artifacts", "stale_artifacts"]
)
...
class SizeDatabase(memdf.util.sqlite.Database):
    ...
    def add_sizes_from_json(self, s: Union[bytes, str], origin: Dict):
        ...
        for frame in ["section", "region"]:
            for i in r["frames"].get(frame, []):
                r["sizes"].append({"name": i[frame], "size": i["size"], "kind": frame})
        self.add_sizes(**r)
    ...
    def select_thing_id(self, platform: str, config: str, target: str) -> Optional[str]:
        cur = self.execute("SELECT id FROM thing WHERE platform=? AND config=? AND target=?", (platform, config, target))
        row = cur.fetchone()
    ...
    def select_changes(self, parent: str, commit: str) -> ChangeInfo:
        ...
        keep = ("platform", "target", "config", "kind", "name", "parent_size", "commit_size")
        things: set[int] = set()
        ...
        for row in cur.fetchall():
            ...
            if previous is not None and row["thing"] == previous["thing"] and row["name"] == previous["name"]:
        ...
        return ChangeInfo(
            ("platform", "target", "config", "kind", "section", parent[:8], commit[:8], "change", "% change"),
            rows,
            things,
            builds,
            stale_builds,
    ...
    def set_commented(self, build_ids: Iterable[int]):
        ...
        for build_id in build_ids:
            self.execute("UPDATE build SET commented = 1 WHERE id = ?", (build_id,))
        self.commit()
