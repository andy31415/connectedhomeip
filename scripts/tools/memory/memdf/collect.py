...
def fill_holes(config: Config, symbols: SymbolDF, sections: SectionDF) -> DFs:
    ...
    def filler(name, address, size, previous, current) -> List:
        row = [
            ...
            address,  # address
            size,  # size
            (previous.section if previous else current.section if current else memdf.name.UNDEF),  # section
            (previous.file if previous else current.file if current else ""),  # file
        ]
        if need_cu:
            row.append(previous.cu if previous else current.cu if current else "")
        if need_input:
            row.append(previous.input if previous else current.input if current else "")
        row.append("NOTYPE")  # type
        row.append("LOCAL")  # bind
        return row

    def fill_gap(previous, current, from_address, to_address) -> Tuple[str, List]:
        """Add a row for a unaccounted gap or unused space."""
        size = to_address - from_address
        if previous is None or previous.symbol in start_unused or current.symbol in end_unused:
            use = "unused"
    ...
    def fill_overlap(previous, current, from_address, to_address) -> Tuple[str, List]:
        """Add a row for overlap."""
        size = to_address - from_address
        return ("overlap", filler(memdf.name.overlap(from_address, -size), from_address, size, previous, current))
    ...
    new_symbols: Dict[str, List[list]] = {"gap": [], "unused": [], "overlap": []}
    section_range = None
    previous_symbol = None
    current_address = 0
    iterable_symbols = symbols.loc[(symbols.type != "SECTION") & (symbols.type != "FILE") & symbols.section.isin(section_to_range)]
    ...
    for symbol in iterable_symbols.itertuples():
        if not previous_symbol or symbol.section != previous_symbol.section:
            ...
            symbol_address_section = start_to_section.get(section_starts[bisect.bisect_right(section_starts, symbol.address) - 1])
            ...
            if previous_symbol and section_range:
                ...
                if current_address < section_range[-1] + 1:
                    use, row = fill_gap(previous_symbol, previous_symbol, current_address, section_range[-1] + 1)
        ...
        if section_range:
            if current_address < symbol.address:
                use, row = fill_gap(previous_symbol, symbol, current_address, symbol.address)
                new_symbols[use].append(row)
            elif current_address > symbol.address:
                use, row = fill_overlap(previous_symbol, symbol, current_address, symbol.address)
                new_symbols[use].append(row)
...
def postprocess_collected(config: Config, dfs: DFs) -> None:
    ...
    for c in [SymbolDF, SectionDF]:
        if c.name in dfs:
            dfs[c.name] = memdf.select.select_configured(config, dfs[c.name], memdf.select.COLLECTED_CHOICES)

    # Account for space not used by any symbol, or by multiple symbols.
    if SymbolDF.name in dfs and SectionDF.name in dfs and config.get("args.fill_holes", True):
    ...
    for c in [SymbolDF, SectionDF]:
        if c.name in dfs:
            for column in memdf.select.SYNTHETIC_CHOICES:
                dfs[c.name] = memdf.select.synthesize_column(config, dfs[c.name], column)
                dfs[c.name] = memdf.select.select_configured_column(config, dfs[c.name], column)
...
def collect_files(config: Config, files: Optional[List[str]] = None, method: Optional[str] = None) -> DFs:
...
def parse_args(config_desc: Mapping, argv: Sequence[str]) -> Config:
    ...
    config = Config().init({**memdf.util.config.CONFIG, **CONFIG, **config_desc})
