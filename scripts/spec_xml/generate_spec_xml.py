...
@click.option("--output-dir", required=True, help="Path to output xml files")
@click.option("--dry-run", default=False, is_flag=True, help="Flag for dry run")
@click.option("--include-in-progress", type=click.Choice(["All", "None", "Current"]), default="All")
@click.option(
    "--skip-scrape",
...
def dump_versions(scraper, spec_root, output_dir):
    ...
    version_cmd = "version"
    out = subprocess.run([scraper, version_cmd], capture_output=True, encoding="utf8")
    version = out.stdout
...
def dump_ids_from_data_model_dirs():
    ...
    for dir in data_model_dirs:
        ...
        def device_type_support_str(d):
            ...
            dt_server_mandatory = [
                id
                for id, requirement in d.server_clusters.items()
            ...
            dt_client_mandatory = [
                id
                for id, requirement in d.client_clusters.items()
                if requirement.conformance(EMPTY_CLUSTER_GLOBAL_ATTRIBUTES).decision == ConformanceDecision.MANDATORY
            ]
            ...
            if server_provisional or client_provisional:
                log.info(
                    "Found provisional mandatory clusters server:'%s' client: '%s' in device type '%s' for revision '%s'",
                    server_provisional,
                    client_provisional,
                    d.name,
                    dir,
