...
def load_module_info() -> None:
    ...
    for name, module in config.items():
        if name != "DEFAULT":
            ...
            assert not (excluded_platforms - ALL_PLATFORMS), "Submodule excluded on platforms not contained in ALL_PLATFORMS"
...
def make_chip_root_safe_directory() -> None:
    ...
    config = subprocess.run(
        ["git", "config", "--global", "--null", "--get-all", "safe.directory"], stdout=subprocess.PIPE, text=True
    )
    existing = []
...
def main():
    ...
    parser.add_argument(
        "--platform", nargs="+", choices=ALL_PLATFORMS, default=[], help="Process submodules for specific platforms only"
    )
    parser.add_argument("--force", action="store_true", help="Perform action despite of warnings")
    parser.add_argument("--deinit-unmatched", action="store_true", help="Deinitialize submodules for non-matching platforms")
    ...
    unmatched_modules = [m for m in modules if not module_matches_platforms(m, selected_platforms) and module_initialized(m)]
