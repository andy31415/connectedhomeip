...
class RealtekBuilder(Builder):
    ...
    def generate(self):
        ...
        cmd = "arm-none-eabi-gcc -D BUILD_BANK=0 -E -P -x c {ot_src_dir}/src/rtl87x2g/{board_name}/app.ld -o {ot_src_dir}/src/rtl87x2g/{board_name}/app.ld.gen".format(
            ot_src_dir=self.ot_src_dir, board_name=self.board.BoardName
        )
        ...
        cmd = "cmake -GNinja -DOT_COMPILE_WARNING_AS_ERROR=ON {build_flags} -DPython3_EXECUTABLE=$(which python3) {example_folder} -B{out_folder}".format(
            build_flags=self.CmakeBuildFlags(), example_folder=self.rtk_matter_dir, out_folder=self.output_dir
        )
    ...
    def build_outputs(self):
        if self.os_env == RtkOsUsed.FREERTOS:
            yield BuilderOutput(os.path.join(self.output_dir, "bin", self.app.TargetName), self.app.AppNamePrefix)
            if self.options.enable_link_map_file:
                yield BuilderOutput(os.path.join(self.output_dir, "map.map"), self.app.AppNamePrefix + ".map")
        else:
            yield BuilderOutput(os.path.join(self.output_dir, "zephyr", "zephyr.elf"), self.app.AppNamePrefix + ".elf")
            yield BuilderOutput(os.path.join(self.output_dir, "zephyr", "zephyr.bin"), self.app.AppNamePrefix + ".bin")
            if self.options.enable_link_map_file:
                yield BuilderOutput(os.path.join(self.output_dir, "zephyr", "zephyr.map"), self.app.AppNamePrefix + ".map")
