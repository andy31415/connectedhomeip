...
class Esp32Builder(Builder):
    ...
    def _IdfEnvExecute(self, cmd, title=None):
        ...
        self._Execute(["bash", "-c", "source $IDF_PATH/export.sh; source scripts/activate.sh; %s" % cmd], title=title)
    ...
    def generate(self):
        ...
        cmake_args = ["-C", self.ExamplePath, "-B", shlex.quote(self.output_dir)] + cmake_flags
    ...
    def _build(self):
        ...
        cmd = "\nexport SDKCONFIG_DEFAULTS={defaults}\nidf.py -C {example_path} -B {out} build".format(
            defaults=shlex.quote(defaults_out), example_path=self.ExamplePath, out=shlex.quote(self.output_dir)
        )
