...
class NrfConnectBuilder(Builder):
    ...
    def _check_ncs_version(self):
        ...
        try:
            ...
            log.error("To update $ZEPHYR_BASE run: python3 scripts/setup/nrfconnect/update_ncs.py --update --shallow")
    ...
    def generate(self):
        if not os.path.exists(self.output_dir):
            ...
            cmd += "west build --cmake-only -d {outdir} -b {board} --sysbuild {sourcedir}{build_flags}\n".format(
                ...
                sourcedir=shlex.quote(os.path.join(self.root, self.app.AppPath(), "nrfconnect")),
                build_flags=self._get_build_flags(),
            )
            self._Execute(["bash", "-c", cmd.strip()], title="Generating " + self.identifier)
        ...
        if self.app == NrfApp.UNIT_TESTS:
            ...
            self._Execute(
                [
                    ...
                    "--output-on-failure",
                    "--test-dir",
                    os.path.join(self.output_dir, "nrfconnect"),
                    "--no-tests=error",
                ],
    ...
    def _bundle(self):
        ...
        self._Execute(
            ["ninja", "-C", os.path.join(self.output_dir, "nrfconnect"), "flashing_script"],
            title="Generating flashable files of " + self.identifier,
        )

    def build_outputs(self):
        yield BuilderOutput(
            os.path.join(self.output_dir, "nrfconnect", "zephyr", "zephyr.elf"), "%s.elf" % self.app.AppNamePrefix()
        )
        if self.options.enable_link_map_file:
            yield BuilderOutput(
                os.path.join(self.output_dir, "nrfconnect", "zephyr", "zephyr.map"), "%s.map" % self.app.AppNamePrefix()
            )
