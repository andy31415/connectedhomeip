...
class IMXBuilder(GnBuilder):
    def __init__(self, root, runner, app: IMXApp, release: bool = False, trusty: bool = False, ele: bool = False):
        super(IMXBuilder, self).__init__(root=os.path.join(root, "examples", app.ExamplePath()), runner=runner)
    ...
    def GnBuildArgs(self):
        ...
        args.extend(
            [
                ...
                'target_cc="%s/sysroots/x86_64-pokysdk-linux/usr/bin/%s/%s"'
                % (self.SysRootPath("IMX_SDK_ROOT"), cross_compile, cc),
                'target_cxx="%s/sysroots/x86_64-pokysdk-linux/usr/bin/%s/%s"'
                % (self.SysRootPath("IMX_SDK_ROOT"), cross_compile, cxx),
                'target_ar="%s/sysroots/x86_64-pokysdk-linux/usr/bin/%s/%s-ar"'
