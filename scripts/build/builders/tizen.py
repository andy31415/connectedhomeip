...
class TizenApp(Enum):
    ALL_CLUSTERS = App(
        "chip-all-clusters-app", "examples/all-clusters-app/tizen", ("chip-all-clusters-app", "chip-all-clusters-app.map")
    )
    LIGHT = App("chip-lighting-app", "examples/lighting-app/tizen", ("chip-lighting-app", "chip-lighting-app.map"))

    CHIP_TOOL = Tool("chip-tool", "examples/chip-tool", ("chip-tool", "chip-tool.map"))

    TESTS = TestDriver("tests", "src/test_driver/tizen")
...
class TizenBuilder(GnBuilder):
    def __init__(
        ...
        super(TizenBuilder, self).__init__(root=os.path.join(root, app.value.source), runner=runner)
        ...
        if self.app.is_tpk:
            with contextlib.suppress(FileNotFoundError):
                ...
                self.app.parse_manifest(os.path.join(self.root, "tizen-manifest.xml"))
    ...
    def PreBuildCommand(self):
        if self.app == TizenApp.TESTS and self.use_coverage:
            ...
            self._Execute(
                self.lcov_args() + ["--initial", "--output-file", os.path.join(self.coverage_dir, "lcov_base.info")],
    ...
    def PostBuildCommand(self):
        if self.app == TizenApp.TESTS and self.use_coverage:
            self._Execute(
                self.lcov_args() + ["--output-file", os.path.join(self.coverage_dir, "lcov_test.info")], title="Update coverage"
            )
    ...
    def GnBuildArgs(self):
        ...
        if env not in os.environ:
            raise Exception("Environment %s missing, cannot build Tizen target" % env)
    ...
    def bundle_outputs(self):
        if not self.app.is_tpk:
            return
        source = os.path.join(self.output_dir, self.app.package_name, "out", self.app.package)
