...
class TestBuildTarget(unittest.TestCase):
    ...
    def test_fixed_targets(self):
        ...
        self.assertEqual(t.HumanString(), "fake-{bar,foo}-{1,2,3}-{1,2,3}")
    ...
    def test_modifiers(self):
        ...
        self.assertEqual(t.HumanString(), "fake-{bar,foo}-{one,two}[-m1][-m2][-x1][-y1]")
    ...
    def test_completion_strings(self):
        ...
        self.assertEqual(t.CompletionStrings("fake-"), ["fake-foo-", "fake-bar-"])
        self.assertEqual(t.CompletionStrings("fake-f"), ["fake-foo-"])

        # The last fixed target - completions should not be suffixed with "-".
        self.assertEqual(t.CompletionStrings("fake-foo-"), ["fake-foo-one", "fake-foo-two"])

        # Modifiers should be added to the last fixed target.
        self.assertEqual(
            t.CompletionStrings("fake-foo-two"), ["fake-foo-two", "fake-foo-two-m1", "fake-foo-two-m2", "fake-foo-two-extra"]
        )
        self.assertEqual(t.CompletionStrings("fake-foo-two-m"), ["fake-foo-two-m1", "fake-foo-two-m2"])

        # After adding a modifier, completions should respect exclusion rules.
        self.assertEqual(t.CompletionStrings("fake-foo-one-m1"), ["fake-foo-one-m1", "fake-foo-one-m1-extra"])
        self.assertEqual(t.CompletionStrings("fake-bar-one-m1"), ["fake-bar-one-m1"])
