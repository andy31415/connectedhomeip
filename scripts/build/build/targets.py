...
def BuildHostTestRunnerTarget():
    ...
    target.AppendFixedTargets([TargetPart("efr32-test-runner", app=HostApp.EFR32_TEST_RUNNER)])
...
def BuildHostFakeTarget():
    ...
    target.AppendModifier("pw-fuzztest", fuzzing_type=HostFuzzingType.PW_FUZZTEST).OnlyIfRe("-clang").ExceptIfRe(
        "-(libfuzzer|ossfuzz|asan)"
    )
...
def BuildHostTarget():
    ...
    app_parts = [
        ...
        TargetPart("ota-requestor", app=HostApp.OTA_REQUESTOR, enable_ble=False, enable_wifipaf=False),
        TargetPart("simulated-app1", app=HostApp.SIMULATED_APP1, enable_ble=False, enable_wifipaf=False),
        TargetPart("simulated-app2", app=HostApp.SIMULATED_APP2, enable_ble=False, enable_wifipaf=False),
    ...
    target.AppendModifier(
        "nodeps", enable_ble=False, enable_wifi=False, enable_thread=False, crypto_library=HostCryptoLibrary.MBEDTLS, use_clang=True
    ).ExceptIfRe("-(clang|noble|boringssl|mbedtls)")
...
def BuildEfr32Target():
    ...
    target.AppendModifier("heap-monitoring", enable_heap_monitoring=True)
    target.AppendModifier("no-openthread-cli", enable_openthread_cli=False)
    target.AppendModifier("show-qr-code", show_qr_code=True).ExceptIfRe("-low-power")
    ...
    target.AppendModifier("additional-data-advertising", enable_additional_data_advertising=True)
    target.AppendModifier("use-ot-lib", enable_ot_lib=True).ExceptIfRe("-(wifi|use-ot-coap-lib)")
    target.AppendModifier("use-ot-coap-lib", enable_ot_coap_lib=True).ExceptIfRe("-(wifi|use-ot-lib)")
...
def BuildNrfNativeTarget():
    ...
    target.AppendFixedTargets(
        [
            TargetPart("native-sim-tests", board=NrfBoard.NATIVE_SIM, app=NrfApp.UNIT_TESTS),
        ]
    )
...
def BuildAndroidTarget():
    ...
    target.AppendFixedTargets(
        [
            ...
            TargetPart("virtual-device-app", app=AndroidApp.VIRTUAL_DEVICE_APP),
        ]
    )
...
def BuildInfineonTarget():
    ...
    # board
    target.AppendFixedTargets([TargetPart("psoc6", board=InfineonBoard.PSOC6BOARD)])
...
def BuildNxpTarget():
    ...
    target.AppendModifier(name="gn", build_system=NxpBuildSystem.GN).OnlyIfRe("thermostat|contact-sensor").OnlyIfRe(
        "rw61x|mcxw72"
    ).OnlyIfRe("freertos")
...
def BuildCC13x4Target():
    ...
    # board
    target.AppendFixedTargets([TargetPart("cc13x4_26x4", board=TIBoard.LP_EM_CC1354P10_6)])
...
def BuildTizenTarget():
    ...
    target.AppendModifier("ubsan", use_ubsan=True)
    target.AppendModifier("coverage", use_coverage=True).OnlyIfRe("-tests")
...
def BuildBouffalolabTarget():
    ...
    target.AppendFixedTargets(
        [
            TargetPart("BL602DK", board=BouffalolabBoard.BL602DK, module_type="BL602"),
            ...
            TargetPart("BL706DK", board=BouffalolabBoard.BL706DK, module_type="BL706C-22"),
            TargetPart("BL602-NIGHT-LIGHT", board=BouffalolabBoard.BL602_NIGHT_LIGHT, module_type="BL602", enable_resetCnt=True),
            TargetPart(
                "BL706-NIGHT-LIGHT", board=BouffalolabBoard.BL706_NIGHT_LIGHT, module_type="BL706C-22", enable_resetCnt=True
            ),
        ]
    )
