...
class AppProcessManager:
    ...
    def start(self):
        ...
        self.app_process = Subprocess(
            self.config.app,
            *shlex.split(self.config.app_args),
...
@click.option("--script-args", type=str, default="", help="Script arguments, can use placeholders like {SCRIPT_BASE_NAME}.")
@click.option("--script-gdb/--no-script-gdb", default=None, help="Run script through gdb")
@click.option(
    "--quiet/--no-quiet",
    default=None,
...
@click.option(
    "--ip-packet-capture-dir",
    type=click.Path(file_okay=False, writable=True, path_type=pathlib.Path),
    default=pathlib.Path.cwd() / "out/ip_packet_captures",
    help="Storage for capture files.",
)
@click.option("--app-filter", type=str, default=None, help="Run only for the specified app(s). Comma separated.")
def main(
    app: str,
    factory_reset: bool,
    factory_reset_app_only: bool,
    app_args: str,
    app_ready_pattern: str,
    app_stdin_pipe: str,
    script: str,
    script_args: str,
    script_gdb: bool,
    ...
    load_from_env,
    run,
    ip_packet_capture: bool,
    ip_packet_capture_dir: pathlib.Path,
    app_filter,
):
    if load_from_env:
    ...
    for run in runs:
        ...
        main_impl(
            run.app,
            run.factory_reset,
            run.factory_reset_app_only,
            run.app_args or "",
            run.app_ready_pattern,
            run.app_stdin_pipe,
            run.py_script_path,
            run.script_args or "",
            run.script_gdb,
            ip_packet_capture,
            ip_packet_capture_dir,
            run.quiet,
            run.run,
        )
...
def main_impl(
    ...
    if app:
        ...
        restart_monitor_thread = threading.Thread(
            ...
            args=(app_manager_ref, app_manager_lock, app_config, restart_flag_file),
            daemon=True,
        )
    ...
    if script_gdb:
        ...
        #
        script_command = (
            "gdb -batch -return-child-result -q -ex run -ex thread|apply|all|bt --args python3".split() + script_command
        )
    else:
    ...
    test_script_process = Subprocess(
        final_script_command[0],
        *final_script_command[1:],
...
def monitor_app_restart_requests(app_manager_ref, app_manager_lock, config: TestRunConfig, restart_flag_file):
    while True:
