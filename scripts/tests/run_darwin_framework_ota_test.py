...
class DarwinToolRunner:
    ...
    def start(self):
        self.process, self.outpipe, errpipe = self.runner.RunSubprocess(
            self.application, name="DARWIN-TOOL", wait=False, stdin=PIPE
        )
    ...
    def waitForMessage(self, message):
        ...
        ready, self.lastLogIndex = self.outpipe.CapturedLogContains(message, self.lastLogIndex)
        while not ready:
...
@main.command("run", help="Execute the test")
@click.option("--darwin-framework-tool", help="what darwin-framework-tool to use")
@click.option("--ota-requestor-app", help="what ota requestor app to use")
@click.option("--ota-data-file", required=True, help="The file to use to store our OTA data.  This file does not need to exist.")
@click.option(
    "--ota-image-file",
    required=True,
...
@click.option(
    "--ota-candidate-file", required=True, help="The file to use for our OTA candidate JSON.  This file does not need to exist."
)
@click.pass_context
def cmd_run(
    context, darwin_framework_tool, ota_requestor_app, ota_data_file, ota_image_file, ota_destination_file, ota_candidate_file
):
    ...
    if ota_requestor_app is not None:
        ota_requestor_app = SubprocessInfo(
            kind=SubprocessKind.APP, path=Path(ota_requestor_app), args=("--otaDownloadPath", ota_destination_file)
        )
    ...
    try:
        ...
        darwin_tool = InteractiveDarwinTool(
            runner, prompt, darwin_framework_tool.with_args("interactive", "start", "--additional-prompt", prompt)
        )
        darwin_tool.start()
        ...
        darwin_tool.sendCommand(
            "otasoftwareupdaterequestor announce-otaprovider %s 0 0 0 %s 0" % (commissionerNodeId, TEST_NODE_ID)
        )
