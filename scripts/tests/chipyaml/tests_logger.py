...
class TestColoredLogPrinter:
    def __init__(self, log_format="[{module}] {message}"):
        ...
        self.__log_styles = {"Info": "green", "Error": "bright_red", "Debug": "blue", "Others": "white"}
...
class RunnerStrings:
    ...
    test_harness_setup_device_connection_failure = (
        "\t\t**** Test Setup: Device Connection Failure [deviceId={deviceId}. Error {message}]"
    )
    log = "\t\t{message}"
...
class TestRunnerLogger(TestRunnerHooks):
    def __init__(
        self, show_adapter_logs: bool = False, show_adapter_logs_on_error: bool = True, use_test_harness_log_format: bool = False
    ):
    ...
    def test_stop(self, duration: int):
        ...
        print(
            self.__strings.test_stop.format(state=state, successes=successes, errors=errors, warnings=warnings, duration=duration)
        )
    ...
    def __print_failure(self, expected_response, received_response):
        ...
        received_response = self.__prepare_data_for_printing(received_response)
