...
_ALIASES = {
    "*": {
        "commands": {
            ...
            "WriteById": {
                "alias": "write-by-id",
                "arguments": {"ClusterId": "cluster-ids", "AttributeId": "attribute-ids", "Value": "attribute-values"},
            },
            "SubscribeById": {
...
class Encoder:
    ...
    def __init__(self, specifications):
        ...
        is_darwin_framework_tool = os.path.basename(sys.argv[0]) == "darwinframeworktool.py"
    ...
    def encode(self, request):
        ...
        base64_arguments = base64.b64encode((f"{{ {arguments} }}").encode("utf-8")).decode("utf-8")
    ...
    def __get_command_name(self, request):
        command_name = self.__get_alias(request.cluster, request.command) or self.__format_command_name(request.command)
        ...
        if request.is_attribute:
            ...
            if command_name == "write":
                attribute = self.__specs.get_attribute_by_name(request.cluster, request.attribute)
    ...
    def __get_arguments(self, request):
        ...
        arguments = self.__maybe_add(arguments, request.min_interval, "min-interval")
        arguments = self.__maybe_add(arguments, request.max_interval, "max-interval")
        arguments = self.__maybe_add(arguments, request.keep_subscriptions, "keepSubscriptions")
        arguments = self.__maybe_add(arguments, request.timed_interaction_timeout_ms, "timedInteractionTimeoutMs")
        arguments = self.__maybe_add(arguments, request.timeout, "timeout")
        arguments = self.__maybe_add(arguments, request.event_number, "event-min")
        arguments = self.__maybe_add(arguments, request.busy_wait_ms, "busyWaitForMs")
        arguments = self.__maybe_add(arguments, request.identity, "commissioner-name")
        return self.__maybe_add(arguments, request.fabric_filtered, "fabric-filtered")
    ...
    def __maybe_add_destination(self, rv, request):
        ...
        if request.group_id:
            destination_argument_value = hex(0xFFFFFFFFFFFF0000 | int(request.group_id))
    ...
    def __maybe_add_endpoint(self, rv, request):
        ...
        if (
            (request.is_attribute and request.command != "writeAttribute")
    ...
    def __maybe_add_command_arguments(self, rv, request):
        ...
        for entry in request.arguments["values"]:
            ...
            value = self.__encode_value(request.command, entry.get("name"), entry["value"])
            if rv:
                rv += ", "
            rv += f'"{name}":{value}'
    ...
    def __get_argument_name(self, request, entry):
        ...
        return (
            self.__get_alias("*", command_name, argument_name)
            or self.__get_alias(cluster_name, command_name, argument_name)
            or argument_name
        )
    ...
    def __encode_octet_strings(self, value):
        if isinstance(value, list):
            ...
            value = {key: self.__encode_octet_strings(value[key]) for key in value}
    ...
    def __lower_camel_case_member_fields(self, value):
        if isinstance(value, list):
            value = [self.__lower_camel_case_member_fields(entry) for entry in value]
        elif isinstance(value, dict):
            value = {self.__to_lower_camel_case(key): self.__lower_camel_case_member_fields(value[key]) for key in value}
        return value
    ...
    def __format_command_name(self, name):
        ...
        return (
            re.sub(r"([a-z])([A-Z])", r"\1-\2", name).replace(" ", "-").replace(":", "-").replace("/", "").replace("_", "-").lower()
        )
