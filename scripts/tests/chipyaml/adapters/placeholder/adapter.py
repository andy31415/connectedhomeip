...
class PlaceholderDecoder:
    ...
    def decode(self, payload):
        ...
        for key, value in json_response.items():
            if key == _CLUSTER_ID:
                decoded_response[_CLUSTER] = self.__definitions.get_cluster_name(value)
            elif key == _ENDPOINT_ID:
                decoded_response[_ENDPOINT] = value
            elif key == _COMMAND_ID:
                clusterId = json_response[_CLUSTER_ID]
                decoded_response[_COMMAND] = self.__definitions.get_response_name(clusterId, value)
                ...
                clusterId = json_response[_CLUSTER_ID]
                decoded_response[_ATTRIBUTE] = self.__definitions.get_attribute_name(clusterId, value)
            elif key == _WAIT_TYPE:
