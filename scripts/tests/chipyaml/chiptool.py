...
def chiptool_runner_options(f):
    f = click.option(
        ...
        default=None,
        help="Path to a websocket server that will be executed to forward parsed command. Most likely you want to use chiptool.",
    )(f)
    f = click.option(
        "--server_name",
        type=str,
        default="chip-tool",
        help="If server_path is not provided, we use this argument to seach various directories within the SDK for a binary that matches this name.",
    )(f)
    f = click.option(
        "--server_arguments", type=str, default="interactive server", help="Arguments to pass to the websocket server at launch."
    )(f)
    f = click.option(
        "--show_adapter_logs", type=bool, default=False, show_default=True, help="Show additional logs provided by the adapter."
    )(f)
    f = click.option("--trace_file", type=click.Path(), default=None, help="Optional file path to save the tracing output to.")(f)
    f = click.option("--trace_decode", type=bool, default=True, help="Decode the tracing ouput to a human readable format.")(f)
    f = click.option(
        "--delay-in-ms", "--delayInMs", type=int, default=0, show_default=True, help="Add a delay between each test suite steps."
    )(f)
    f = click.option(
        "--continueOnFailure",
        type=bool,
        default=False,
        show_default=True,
        help="Do not stop running the test suite on first error.",
    )(f)
    f = click.option(
    ...
    return click.option(
        ...
        type=click.Path(),
        show_default=True,
        default=_DEFAULT_EXTENSIONS_DIR,
