...
_CLUSTER_XML_DIRECTORY_PATH = os.path.abspath(os.path.join(_DEFAULT_CHIP_ROOT, "src/app/zap-templates/zcl/data-model/"))
...
async def execute_test(yaml, runner):
    ...
    for test_step in yaml.tests:
        ...
        if not test_step.is_revision_condition_passed:
            ...
            try:
                var_name = build_revision_var_name(test_step.endpoint, test_step.cluster)
                current_val = test_step.get_runtime_variable(var_name)
            except (ValueError, IndexError, KeyError):
                current_val = "unknown"

            log.warning(
                "Step '%s' skipped due to ClusterRevision range not matching (val=%s, min=%s, max=%s)",
                test_step.label,
                current_val,
                test_step.min_revision,
                test_step.max_revision,
            )
            continue
...
@click.command()
@click.option("--setup-code", default=None, help="setup-code")
@click.option("--yaml-path", default=None, help="yaml-path")
@click.option("--node-id", default=0x12344321, help="Node ID to use when commissioning device")
@click.option("--pics-file", default=None, help="Optional PICS file")
@asyncio_executor
