...
DEFAULT_ALL_CLUSTERS = os.path.join(
    DEFAULT_CHIP_ROOT, "out", "linux-x64-all-clusters-ipv6only-no-ble-no-wifi-tsan-clang-test", "chip-all-clusters-app"
)
...
class TestDriver:
    ...
    def run_test_section(
        self, app_args: str, script_args: str, factory_reset_all: bool = False, factory_reset_app: bool = False
    ) -> int:
        # quotes are required here
        cmd = self.get_base_run_python_cmd(self.run_python_test_path, self.app_path, app_args, self.script_path, script_args)
