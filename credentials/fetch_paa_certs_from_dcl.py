...
MATTER_CERT_CA_SUBJECT = (
    "MFIxDDAKBgNVBAoMA0NTQTEsMCoGA1UEAwwjTWF0dGVyIENlcnRpZmljYXRpb24gYW5kIFRlc3RpbmcgQ0ExFDASBgorBgEEAYKifAIBDARDNUEw"
)
...
def write_cert(certificate, subject):
    filename = "dcld_mirror_" + re.sub("[^a-zA-Z0-9_-]", "", re.sub("[=, ]", "_", subject))
...
def get_cert_from_rest(rest_node_url, subject, subject_key_id):
    response = requests.get(f"{rest_node_url}/dcl/pki/certificates/{subject}/{subject_key_id}").json()["approvedCertificates"][
        "certs"
    ][0]
...
def fetch_cd_signing_certs(store_path):
    ...
    cd_signer_ids = requests.get(
        f"{rest_node_url}/dcl/pki/child-certificates/{MATTER_CERT_CA_SUBJECT}/{MATTER_CERT_CA_SUBJECT_KEY_ID}"
    ).json()["childCertificates"]["certIds"]
